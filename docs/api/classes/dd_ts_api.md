...
- POST JSON with `dd_api: "dd_ts_api"` and `action` set to one of the methods below. The registered actions are thin wrappers over `src/core/ts_object/ts_api.ts`, which owns permission gating and the response envelopes.
...
- **Returns:** `result` is the child-reordering outcome (the `relation_children` sibling-ordering write, `src/core/relations/`), or `false` on error; `msg` explains the result.
