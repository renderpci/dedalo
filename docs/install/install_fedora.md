...
### 2. Web Server and PHP Installation (RHEL/Alma/Rocky)
...
sudo dnf groupinstall "Development Tools" -y
sudo dnf install libxml2-devel libzip-devel openssl-devel bzip2-devel zlib-devel curl-devel libpng-devel libjpeg-turbo-devel freetype-devel libwebp-devel oniguruma-devel -y
...
# Create .pgpass file for database access
echo "*:*:*:dedalo_user:YourSecurePassword" > ~/.pgpass
chmod 600 ~/.pgpass
```
...
# Ensure module is enabled in /etc/php.d/
