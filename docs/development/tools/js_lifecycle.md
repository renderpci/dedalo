...
The client-side contract of a Dédalo tool. Base: `tools/tool_common/js/tool_common.js`. Reference implementation: `tools/tool_dev_template/js/`.
...
A tool is opened by `open_tool({tool_context, caller, open_as})` (bound automatically to the tool buttons that `common::get_tools()` placed in the element context).
...
const response = await self.tool_request({
	action  : 'my_method',          // PHP static method, must be in API_ACTIONS
	options : { section_tipo: '...', section_id: '...', my_param: 1 },
	background : false,             // true = detached CLI run (needs BACKGROUND_RUNNABLE)
