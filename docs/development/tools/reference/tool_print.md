...
**Server (PHP).** `tools/tool_print/class.tool_print.php` extends `tool_common` with an **empty** `API_ACTIONS` (UI-only; printing is a browser action). All persistence rides the generic **core data API** — the same path `tool_export` presets use — so no bespoke tool endpoint and no bespoke table. A phase-2 `generate_pdf` method is sketched (gated `record` / `min_level 1`) for server-side PDF; it is not yet active.
...
`tool_print` exposes **no** API actions in the MVP:

| `API_ACTIONS` | Form | Notes |
| --- | --- | --- |
| *(none)* | — | UI-only. Printing happens in the browser; saving/loading templates uses the generic core data API on `dd25`/`dd625`. |

Phase-2 `generate_pdf(object $options)` (planned) would read the `dd625` blob server-side and the box values via the export atoms contract, gated `record` / `min_level 1`.
