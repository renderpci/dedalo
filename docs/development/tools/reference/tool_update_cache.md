...
`get_component_list` enumerates the caller section's components through the shared `buildSectionElementsContext` (the "simple" element context, `src/core/resolve/section_elements_context.ts`), keeps only the `component` entries, and annotates each with its `regenerate_options` — the v6 descriptor array (`[{name:'delete_normalized_files', type:'boolean', default:false}]`) for media models, `null` for everything else. The result feeds the client's checkbox list.
...
4. Iterates the rows. For each selected component tipo it resolves the model and branches:
   ...
   - **media** — **repairs the media** via the shared kernel `refreshMediaItems` (`src/core/media/repair.ts`, called with `regenerate: true`), with **v6 `regenerate_component` semantics** (component_media_common :2614): builds only the derivatives that are **MISSING** — an existing file is never re-encoded; the image thumb is re-created **always**, FROM the default-quality file (v6 create_thumb — no original needed); the image SVG envelope is created when absent and its embedded raster path is FIXED when it drifted; `delete_normalized_files` (the per-component regenerate option — honored since WC-043) first moves the default quality's normalized files to `deleted/<bulk id>/`, guarded on a locally present original. A missing `original_file_name` is restored from the section's `properties.target_filename` component (v6 :2670). Then the disk is re-scanned and the handler persists the fresh `files_info` per stored item. The persist is the established files_info write-back (per-key jsonb via `updateMatrixKeyData`, **no Time Machine entry** — files_info is a filesystem-derived cache; `src/core/media/tools/files_info_persist.ts` documents the discipline), and `refreshStoredFilesInfo` preserves the sibling keys (`original_*`, `lib_data`). `component_av` derivative rebuild is an async transcode and is *not* enqueued here (that is tool_media_versions' job) — its `files_info` refresh still applies. A record whose original is absent on the box gets the honest result: derivatives no-op. **Shrinks are HELD** (`holdShrink: true`, counted in `media_held`): when the rescan finds FEWER existing files than the stored index claims — the signature of a partial local media copy, not of a stale index — the stored `files_info` is kept, never wiped; shrinking an index requires the ops script's explicit `--allow-shrink` adjudication. Derivative-build failures are collected into `media_errors`; the files_info refresh still lands.
5. Returns `{ result, msg, errors, regenerated, records, processed, stopped, bulk_process_id, media_errors, media_held }` (see the response section below).
...
!!! note "Regenerate options, bulk process, and Time Machine (v6 parity)"
    `get_component_list` annotates media components with the v6 option-descriptor array (`[{name:'delete_normalized_files', type:'boolean', default:false}]` — the client ITERATES this shape to build the options panel; any other shape renders it empty). `update_cache` honors the per-component `regenerate_options.delete_normalized_files`. Each run mints one **dd800 bulk-process record** (`Update cache | <section> | <components>`; id returned as `bulk_process_id`) — deleted normalized files land under `deleted/<id>/`. The generic re-saves run with **`saveTm: false`** (v6 disabled Time Machine for the whole run — a regenerate re-writes the same value, so per-row TM versions would be pure bloat).
...
Response: `{ result, msg, errors, regenerated, records, processed, stopped, bulk_process_id, media_errors, media_held }` — `regenerated` is the number of component writes (media and non-media), `records` the number of matched records, `processed` how many were fully processed (`< records` only after a stop), `stopped` whether the run ended by cooperative cancellation, `media_errors` the count of media derivative rebuilds that failed (each detailed in `errors`; the files_info refresh still landed for them), `media_held` the count of stored media indexes kept because the rescan found fewer files than stored (shrink guard).
