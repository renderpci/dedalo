...
**Server.** `tools/tool_cataloging/class.tool_cataloging.php` is a near-empty subclass of `tool_common`. Its only declared member is:

```php
public const API_ACTIONS = []; // SEC-024 (§9.2): UI-only tool. No remotely callable methods.
```

An empty-but-present `API_ACTIONS` satisfies the SEC-024 dispatch requirement while exposing **nothing** to `dd_tools_api::tool_request`. The class overrides no lifecycle hooks (`is_available` / `on_register` / `on_remove`) and inherits all registry/config/context plumbing from `tool_common`. All persistence happens through other, already-gated endpoints (see *Actions & options*).
...
| `API_ACTIONS` | Value |
| --- | --- |
| `tool_cataloging` | `[]` (empty — UI-only, nothing dispatchable on this class) |
...
`tools/tool_cataloging/register.json` is a **legacy v6** file (a raw record dump with `components`/`relations` keys); `tools_register` auto-converts it at registration (the `components` key triggers the v6 converter). Essentials it carries:
...
- [Creating new tools](../creating_tools.md) · [Server contract](../server_contract.md) — the tool model, `API_ACTIONS` (including the empty-array UI-only case), gates and lifecycle this page builds on.
...
- Subsystems the client drives: the thesaurus/ontology tree (`core/ts_object/`, `core/api/v1/common/class.dd_ts_api.php`, the `ts_add_child_tool_cataloging` event), `core/component_portal/` (locator storage), and the `section` mosaic view.
- Source: `tools/tool_cataloging/class.tool_cataloging.php`, `tools/tool_cataloging/register.json`, `tools/tool_cataloging/js/{index,tool_cataloging,render_tool_cataloging,view_tool_cataloging_mosaic}.js`, `tools/tool_cataloging/css/tool_cataloging.less`.
