...
**Server (PHP).** `tools/tool_ontology/class.tool_ontology.php` is a thin, privileged wrapper around the core ontology engine. It declares a **list-form** `API_ACTIONS` with a single entry, `set_records_in_dd_ontology`, and enforces developer-only access with an imperative gate (`assert_developer()`, calling `security::is_developer(logged_user_id())`, throwing `permission_exception` otherwise) as the very first statement of the method. The list form is used (rather than a map-form `developer` gate) because the developer check is performed inside the method body.

`set_records_in_dd_ontology(object $options)` reads `section_tipo` (required — refuses with an error if missing) and `section_id` (optional), then builds a `search_query_object` (SQO) in one of two modes:

- **Edit mode** (`section_id` present): a single-record SQO — `limit: 1`, `offset: 0`, `filter_by_locators` set to the one `{section_tipo, section_id}` locator.
- **List mode** (`section_id` absent): it reloads the user's current list SQO from the session at `$_SESSION['dedalo']['config']['sqo'][ section::build_sqo_id(section_tipo) ]`, clones it, then strips ordering and removes the limit/offset (`set_order([])`, `set_limit(0)`, `set_offset(0)`) so the whole result set is processed. If no session SQO is found it returns an error (`Not sqo_session found from id: …`).

It then delegates the real work to `ontology::set_records_in_dd_ontology($sqo)` (`core/ontology/class.ontology.php`). That core method runs the search, and for each matched record: if it is a `matrix_ontology_main` (TLD root, `ontology35`) record it deletes the TLD's `dd_ontology` nodes when the TLD is inactive or upserts the root node when active; otherwise it upserts the node via `insert_dd_ontology_record()`. It uses **partial-success** semantics (some-ok/some-failed → `result: true` with a "Partial success" message and the failing records enumerated in `errors`) and returns `{result, msg, errors, total, processed_count}`. Back in the tool, on success the method unsets `$_SESSION['dedalo']['config']['active_elements']` (consumed by `dd_ts_api::get_children_data()`) and returns the core response shaped to the tool contract `{result, msg, errors}`.
...
`tool_ontology` exposes exactly one remotely callable action. There is **no** `BACKGROUND_RUNNABLE` (the sync runs inline within the request).

| `API_ACTIONS` (list form) | Permission gate | Reads from `options` | Returns |
| --- | --- | --- | --- |
| `set_records_in_dd_ontology` | Imperative, inside the method: `assert_developer()` → `security::is_developer(logged_user_id())`; throws `permission_exception` otherwise. (Listed in list form, not map form, because the gate is in the body.) | `section_tipo` (**required**); `section_id` (optional — its presence selects edit vs. list mode). `mode` is sent by the client but the server infers edit/list from `section_id`. | `{ result: bool, msg: string, errors: string[] }`. On success, `result`/`msg`/`errors` are forwarded from `ontology::set_records_in_dd_ontology()` (which additionally carries `total` and `processed_count`, and may report *Partial success*). |
...
> Lifecycle hooks: `tool_ontology` does **not** override `is_available`, `on_register` or `on_remove` — surfacing is governed by `affected_models` / `affected_tipos` and the developer-only authorization, not by an `is_available` hook. (As always, these hooks must never appear in `API_ACTIONS`.)
...
- `dd1350` **affected_tipos** — `["ontology16","ontology17","ontology18","ontology19","ontology35","/^[a-z]+0$/"]`. The trailing regex matches every per-project **TLD root tipo** (e.g. `dmm0`, `oh0`), so the tool surfaces on any project's ontology root as well as on the generic ontology component tipos. (This is the `affected_tipos`-restriction path in `common::get_tools()`: only matching tipos pass.)
...
- [Server contract](../server_contract.md) — the PHP class contract, `API_ACTIONS` list vs. map form, and the lifecycle-hook rule.
- [Security](../security.md) — what the framework enforces; here the developer gate is imperative (inside the method), so the action is listed in **list form**.
