...
**Server (PHP).** `tools/tool_subtitles/class.tool_subtitles.php` is intentionally empty. It extends `tool_common` and declares an **empty** `API_ACTIONS` constant — the SEC-024 §9.2 marker for a UI-only tool with no remotely callable methods:
...
There is no `BACKGROUND_RUNNABLE`, no `is_available()` / `on_register()` / `on_remove()` override, and no business method. Nothing in this tool is dispatchable through `dd_tools_api` / `tool_request`; a request that named this class would be refused at the `API_ACTIONS` allowlist gate. The PHP class exists only to (a) be a valid, registerable `tool_common` subclass whose name matches its directory, and (b) carry the registry record (version, label, localized UI labels). All behavior is client-side, and any data writes happen through the **components** the tool hosts (the text-area save, the json save), not through the tool.
...
| --- | --- | --- |
| `[]` (empty) | — | UI-only tool (SEC-024 §9.2). No method is dispatchable through `dd_tools_api`; a `tool_request` naming this class is refused at the allowlist gate. There is **no** `BACKGROUND_RUNNABLE` and no lifecycle-hook override. |
...
`tools/tool_subtitles/register.json` is a **legacy v6** file (a raw record dump with `components` / `relations` keys), auto-converted at registration by `tools_register` (the `components` key triggers the v6 converter). The essentials it carries:
...
**Where it appears.** Surfacing is element-driven through `common::get_tools()`: the button attaches to a **`component_text_area`** whose tipo is `rsc36`, and renders **inline on that component** (`show_in_component: true`) rather than in the section inspector (`show_in_inspector: false`). Because `require_translatable` is set, it only shows on translatable instances. Profile authorization applies as for any tool (superusers see it; otherwise it must be granted on the profile's Tools).
...
- [Server contract](../server_contract.md) — the PHP class contract; note that a UI-only tool still declares `API_ACTIONS = []` and that lifecycle hooks are never listed.
