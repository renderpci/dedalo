...
!!! warning "Sample, not a production tool"
    The class header says it plainly: *"This tool is intended to be used as a base build for new tools. Do not use as a production tool."* Treat `tool_qr` as a worked example of a **UI-only** tool with a `ddo_map`-driven render, not as a supported feature.
...
**Server.** There is essentially nothing on the server. `tools/tool_qr/class.tool_qr.php` extends `tool_common`, declares an **empty** `API_ACTIONS = []`, and adds no methods. All behavior is client-side; the tool never calls `tool_request`. The empty constant is deliberate (annotated *SEC-024 (§9.2): UI-only tool. No remotely callable methods.*) — it satisfies the framework's "every tool must declare `API_ACTIONS`" rule while exposing no remote surface.
...
`API_ACTIONS = []`. Any attempt to call a method on this tool through `dd_tools_api` is refused at dispatch by the `API_ACTIONS` allowlist (there is nothing in it).
...
`tools/tool_qr/register.json` is a **legacy v6** file (a raw record dump with `components`/`relations` keys); it is auto-converted at registration by `tools_register` (the `components` key triggers the v6 converter). The essentials it carries:
...
- [Server contract](../server_contract.md) · [Security](../security.md) — why a UI-only tool still declares `API_ACTIONS` (here, empty).
