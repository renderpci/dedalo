...
The change log lives in the dd15 `matrix_time_machine` table, accessed server-side through `tm_record` (`core/tm_record/class.tm_record.php`). Each TM row carries the element's full data snapshot plus metadata (when `dd559`, who `dd578`, what component `dd577`, section_tipo/section_id, and an optional `bulk_process_id`).
...
**Server** (`class.tool_time_machine.php`):

- `apply_value()` reads the TM snapshot for `matrix_id` via `tm_record::get_instance($matrix_id)->get_element_data()`, then branches on the target's model:
  - **section** — loads the `section_record`, `set_data()` (overwrite) + `save()`; on success it clears the section's session SQO, calls `restore_deleted_section_media_files()`, logs a `RECOVER SECTION` activity, and **deletes** the consumed TM row.
  - **component_** — loads the component in `list` mode and `set_data()` + `save()`. Dataframe-bearing components are handled first: each dataframe ddo's slice of the snapshot is filtered out by `from_component_tipo` and pushed via `set_time_machine_data()` (which does not itself create new TM rows — the main component's save records the revert). Relation components and `component_iri` get their own snapshot-slice filtering. Logs a `RECOVER COMPONENT` activity.
- `bulk_revert_process()` searches `tm_record` for every row sharing the given `bulk_process_id`, creates a **new** bulk-process section record (so the revert is itself reversible) and labels it, then for each affected `(section_tipo, tipo, section_id)` finds the row that immediately *precedes* the bulk change and restores that component to its pre-run value (an empty array when the bulk change was the component's only history). Each restored component is tagged with the new `bulk_process_id` before save and logged.

## Actions & options

`API_ACTIONS` is declared in **map form**, so `dd_tools_api`/`tool_security` enforce the gate before dispatch. The imperative `security::assert_*` calls inside the methods stay as defense-in-depth (they also cover the CLI/background path).
...
| --- | --- | --- |
| `apply_value` | map: `tipo` @ level 2 (write). For a section restore `tipo === section_tipo`, so it is equivalent to the section gate. Plus imperative per-record `assert_record_in_user_scope` on `section_id`. | `section_tipo`, `section_id`, `tipo`, `lang`, `matrix_id`, `caller_dataframe` |
| `bulk_revert_process` | map: `section` @ level 2 (write). Plus imperative **per-row** `assert_tipo_permission` + `assert_record_in_user_scope` for every record in the bulk set (rows the caller cannot write are skipped with a `permissions_denied:…` error, not aborting the whole run). | `section_tipo`, `section_id`, `tipo`, `lang`, `bulk_process_id`, `bulk_revert_process_label` |
...
Both methods return the standard `{result, msg, errors}`; `apply_value` on a section restore also returns `restore_deleted_section_media_files`. Neither action is listed in `BACKGROUND_RUNNABLE` — they run inline (the client gives `bulk_revert_process` a 180 s timeout).

There is also a lifecycle hook (never in `API_ACTIONS`):
...
`component_relation_children` has no time-machine data, so the tool hides itself there.

## How it is registered & surfaced

`tools/tool_time_machine/register.json` is a **legacy v6** file (raw record dump with `components`/`relations` keys); it is auto-converted at registration by `tools_register` (the `components` key triggers the v6 converter). The essentials it carries:
...
Surfacing (in `common::get_tools()`): the tool attaches to **record elements** — both components and section records — and uses its `is_available` hook as the last word, hiding only on `component_relation_children`. The in-code comment at `common::get_tools()` notes this hook "replaces the previous hardcoded `tool_diffusion` / `tool_time_machine` cases" — the per-element availability is now declared by the tool rather than special-cased in core. (Note: this shipped v6 register dump does not include an explicit `dd1330` affected_models relation; surfacing is element/availability-driven, and the bulk-revert UI is further restricted to global admins client-side via `page_globals.is_global_admin`.) One related core policy lives in `get_tools()`: on the dd15 time-machine section itself only `tool_export` is allowed.
...
- Source: `tools/tool_time_machine/class.tool_time_machine.php`, `tools/tool_time_machine/js/{tool_time_machine,render_tool_time_machine}.js`, `core/tm_record/class.tm_record.php`, `core/services/service_time_machine/`.
