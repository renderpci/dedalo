...
!!! warning "PHP-era audit — historical context, superseded by the TS rewrite"
    Everything below (executive summary, findings F1–F10, roadmap A1–C5) describes the **PHP** tools subsystem as it stood in June 2026, before the PHP→TypeScript/Bun rewrite. It is kept as the design history that shaped the current model: the end state this audit's roadmap reached (constants file, v7-native `register.json` + schema, enforced allowlist, declarative permission gates, the production-shaped template, the scaffolder, the docs set) is exactly what the TS engine re-implements — just with different mechanics. Concretely: there is no `class.tool_x.php` / `class.tool_common.php` / `class.dd_tools_api.php` in the TS tree at all (they never existed here — this repo has zero PHP files under `tools/`); the reflection-based checks this audit worries about (F3, F6, A4, A5) do not exist as a *runtime* concept in TS, because a tool's `apiActions` is a plain object of typed handlers, validated once at load time (`src/core/tools/loader.ts`), not reflected per request. For the current (TS) design, see [Tools subsystem (TS/Bun engine)](../../../engineering/TOOLS_SPEC.md), [Server contract](server_contract.md), [Security](security.md) and [register.json reference](register_json.md). The vanilla-JS **client** findings (F8, C2) still apply almost verbatim — `tool_common.js` was copied as-is and lives at `src/core/tools/client/js/tool_common.js`.
