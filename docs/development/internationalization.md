...
`core/common/class.lang.php` is the resolver between codes, records and names. The lookup path is `lang::resolve()` → `lang::resolve_multiple()` → a PostgreSQL search over the `matrix_langs` matrix table (GIN index), cached per request. Useful entry points:

| Method | Returns |
| --- | --- |
| `lang::resolve_multiple(['spa','eng'])` | per-code object of multilingual names `{lg-spa:{lg-eng:"Spanish",…}}` |
| `lang::get_section_id_from_code('lg-spa')` | the `lg1` `section_id` of that language record |
| `lang::get_lang_locator_from_code('lg-spa')` | a [locator](../core/locator.md) into `lg1` for that language |
| `lang::get_code_from_locator($locator)` | the `lg-xxx` code for a stored language locator |
...
| `DEDALO_APPLICATION_LANG` | interface | The **current** interface language, computed per request via `fix_cascade_config_var()` (GET → POST → session → default). Labels resolve to this. |
...
| `DEDALO_DATA_LANG` | data | The **current** data language the cataloguer is editing/reading; also computed per request via the cascade. Translatable components instantiate in this language by default. |
...
!!! note "Server API for per-language data"
    On the server, a component reads its current-language items with `get_data_lang($lang)` and writes a different language's items with `set_data_lang($data, $lang)` on a separate instance bound to that language — the pattern [`tool_lang`](#plane-3-the-translation-workflow-tool_lang--tool_lang_multi) uses to translate (read `source_lang`, write `target_lang`).
...
At build time the node label is resolved to the **current interface language** with `ontology_node::get_term_by_tipo($tipo, DEDALO_APPLICATION_LANG, …)`, and the resolved string is stamped into the component [context](../core/glossary.md#context) as `label`. If a node has no term in `DEDALO_APPLICATION_LANG`, resolution falls back to `DEDALO_STRUCTURE_LANG` (the master authoring language, `lg-spa`) and then to any available language, so a label never comes back empty.
...
// interface can be offered in the same three (or more):
define('DEDALO_APPLICATION_LANGS', [
    'lg-spa' => 'Castellano',
    'lg-eng' => 'English',
    'lg-cat' => 'Català'
]);
define('DEDALO_DATA_LANG_DEFAULT', 'lg-spa');         // primary content language
define('DEDALO_APPLICATION_LANGS_DEFAULT', 'lg-eng'); // default UI language
```
