...
!!! warning "A service is *not* an ontology node"
    Unlike a component, section or area, a service has **no ontology node, no `tipo`, no `properties`, no record data and no JSONB column**. You do **not** create or edit an ontology node, and there is **no** `section_record_data::$column_map` entry. A service is pure code: it is instanced *by* a caller and acts on the caller's behalf. This is the opposite of the "ontology-only" path some extensions take — a service is "code-only".

Services are **almost entirely client-side** (ES6 modules under `core/services/<service>/js/`). They have **no PHP model class** in the normal case: their server side is whatever *generic* API action they already call (`dd_utils_api`, `dd_core_api`, the read API). The only PHP file in the tree, `core/services/service_subtitles/class.service_subtitle.php`, is vestigial — see the warning in the [services reference](../../core/system/services.md#service_subtitles). Do not add a bespoke server endpoint for a service.

## 1. Create the directory

There is **no scaffolder** for services (the CLI generator exists only for tools). Copy an existing sibling that matches the shape you need and rename every `service_<old>` occurrence — directory, file names, the exported identifier, the CSS/LESS basename. Good starting points:
...
The file nomenclature mirrors [components](../../core/components/index.md):

``` text
core/services/
└── service_myhelper/
    ├── css/service_myhelper.less     # compiled to .css
...
- **No bespoke endpoint.** Call the existing generic actions — e.g. `service_upload` uses `dd_utils_api::get_system_info` / `upload` / `join_chunked_files_uploaded`; `service_autocomplete` builds an SQO and runs the read API through the caller. Permission enforcement is the caller's and the API action's job (`dd_utils_api::upload` asserts write permission on the target section when a `tipo` is supplied).
...
**2. The service does its work and publishes an event.** `upload_file()` slices the file into chunks, uploads them (each chunk a `POST` carrying the per-session CSRF token, with retry), then calls `dd_utils_api::join_chunked_files_uploaded` server-side and publishes `upload_file_done_<caller.id>`.

**3. The host reacts via the event** — it never calls a service-specific endpoint; it routes the result through its **own** generic action. In `tool_dev_template` this is `file_upload_handler`, which fires `tool_dev_template::handle_upload_file` (write gate: `tipo`, level 2):
...
- **Adding an ontology node.** Don't. Services are not in the ontology — no `tipo`, no `$column_map` entry, no permission row. If you find yourself wanting a node, you probably want a [component](../../core/components/index.md) or [tool](../tools/creating_tools.md) instead.
- **Inventing a server endpoint.** Call the existing generic actions and let the caller / the action enforce permissions. A bespoke service endpoint is an anti-pattern (and the one PHP forwarder that exists, `service_subtitles`, is dead code — see the [reference warning](../../core/system/services.md#service_subtitles)).
