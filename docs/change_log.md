...
## [Unreleased] - A relation search that cannot be read is refused instead of returning everything

### Fixed
- **A relation search with an unreadable value now fails with an error instead
  of quietly matching every record.** Relation fields (selects, portals,
