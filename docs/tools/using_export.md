...
4. For a relation or hierarchy column, drill into it to reach the related field you want (for example *Mint → name*). Columns of a portal or autocomplete whose target section is hierarchical (it has a parent component) additionally show a per-column **parents** checkbox: turn it on to also emit the ancestor chain of each linked term (nearest parent first, ` > ` separated) as a sibling *parents* column. Parents columns are produced by the **standard** (`value`) and **breakdown** (`grid_value`) formats. In the standard format the *parents* column sits right after its column, laid out exactly like its column: the same separators in the same places, with one chain for each term, in the same order (a term without parents leaves an empty slot). If one linked record shows several ` | `-separated values, its chain is repeated once for each value, so the two columns always split into the same number of pieces. The **Dédalo (Raw)** format exports stored data only, so the checkbox is disabled there, with a *(not in Raw)* note; your choice is kept and comes back when you switch format.
...
| Parents (per column) | On eligible relation columns (portal / autocomplete pointing at a hierarchical section), adds a sibling column with the target's ancestor chain. Off by default; set on each column in the active list — there is no global switch. Standard (`value`) and breakdown (`grid_value`) formats; disabled in Dédalo (Raw). |
