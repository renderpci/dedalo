...
| NDJSON | The lossless export: every line of the export protocol exactly as the server produced it, for programs rather than people. |
| Media | A ZIP of the media files the exported records point at. A dialog asks for the **quality** of each media type in the export (for example the image and the PDF quality separately). A file goes into the ZIP only if you can read that component on that very record. The ZIP includes an `info.txt` that lists what was archived and what was left out, and why. |
