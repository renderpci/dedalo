...
!!! warning "TS gap: read-only on the TS server"
    `update_data_version.get_value` is ported and reports the real state
    (`current_version_in_db` from the shared `matrix_updates` table,
    `dedalo_version` = this engine's own `TS_DATA_VERSION`; `update_version`
    and the pending `updates` list are always `null`, since the migration
    catalog is PHP-owned) — so the "Update data" panel below renders and shows
    an honest version snapshot on the TS server. But the migration EXECUTE
    itself refuses (`engine_denied`, `src/core/resolve/widget_request.ts`):
    data migrations must run exactly once, on the engine that owns the
    migration catalog. Run this step from a PHP install; a version mismatch
    between the two engines' `matrix_updates` reads will surface through the
    panel's version triples.
