...
!!! warning "TS gap: this whole flow is PHP-install-only"
    The "Update code" maintenance panel and the manual directory-swap
    procedure below both operate on a PHP `/dedalo` code tree. On the TS/Bun
    server, `update_code.update_code` and
    `update_code.build_version_from_git_master` are explicit, permanent
    refusals (`engine_denied` — "it downloads and REPLACES the PHP code
    tree" / "it packages the PHP code tree from its git checkout"; see
    `src/core/resolve/widget_request.ts`). Updating the TS server means pulling
    the `src/`/`tools/`/`client/` tree from its own repository and restarting
    the Bun process — there is no in-app updater, and none is planned, since a
    TS-only deployment manages its own deployment pipeline. The rest of this
    page applies to a PHP install (standalone or coexisting with a TS server
    over the same database).

!!! info "Migration from Dédalo v5"
    If you are migrating from an older release, the v6 (PHP) → v7 (TypeScript) migration guide is planned and out of scope for this first TS documentation.
