...
- **Unknown entries at the tree root** — any top-level file or directory that the release does not ship and the engine does not account for would vanish into the backup. Move it out (or delete it) first. Operating-system metadata is exempt (`.DS_Store`, `Thumbs.db`, `desktop.ini`): it holds nothing, no release ships it, and a tree browsed from a macOS or Windows file manager grows it again by itself — so it rides into the backup with the old tree instead of refusing the update.
...
- **A containerised (image) deployment** — when the code tree lives inside a container image, a swap lands in the container's writable layer and is discarded on the next recreation. Such installs update by replacing the image, not through this panel. The shipped image marks itself for this (`/etc/dedalo/image_tree`); a bind-mounted checkout inside a container, or any container that does not bake the code in, is fine.
