...
!!! warning "TS gap: the flag exists, the ontology swap does not"
    The TS server can **store** the `recovery_mode` flag
    (`set_recovery_mode` in the `check_config` widget → `<private>/ts_state.json`,
    `src/core/resolve/server_state.ts`), but nothing on the TS side reads that
    flag to actually swap the served ontology to a `dd_ontology_recovery`
    table — the flag is set/reported, not yet acted on. The two mechanisms
    this page depends on to *use* recovery mode are explicitly refused by
    design on the TS server (`src/core/resolve/widget_request.ts`):
    `build_database_version.build_recovery_version_file` ("it writes recovery
    files into the PHP tree") and
    `build_database_version.restore_dd_ontology_recovery_from_file` ("it
    replaces the shared `dd_ontology` from a PHP-tree recovery file"). In a
    coexisting install, drive recovery mode from the PHP server — it owns the
    ontology-recovery machinery.
