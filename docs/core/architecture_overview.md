...
1. **The work system** — the back office where curators and researchers create,
   edit and relate records. Its server is PHP; its client is HTML, CSS and
   JavaScript. It talks to PostgreSQL, where all data lives as JSON in the
   `matrix` tables. The work API is the **Dédalo API** (see [RQO](rqo.md)).
...
Both systems have a server half (PHP) and a client half (HTML/CSS/JS). This
document is almost entirely about the **work system**, because that is where the
ontology, sections, components and the request lifecycle live. Diffusion is the
downstream consumer.
...
left, the *work system* contains a client (HTML/CSS/JS) and a server (PHP) that
exchange Request Query Objects over the Dédalo API; the server reads and writes
PostgreSQL, where data lives as JSON in the `matrix` tables. On the right, the
*diffusion system* contains portals/websites and a PHP+Bun server exchanging
data over the diffusion API; that server reads and writes a classic SQL
database with conventional columns. A dotted arrow runs from the work server to
the diffusion server labelled "publish marked data" — diffusion is downstream.
Below everything, an "Ontology (active schema)" node sends dotted arrows to both
servers: it drives the work system and maps the output formats for diffusion.
...
> (`section_record_data::$column_map`) so PostgreSQL can index every data shape.
...
the matrix tables). Dédalo reads the ontology at execution time and builds the
PHP and JS objects dynamically, so editing a node changes behavior with no code
change. Changing a `component_select` into a `component_autocomplete` is a node
...
2. **Models** — the PHP/JS classes (`section`, `component_input_text`,
   `component_portal`, …) chosen by each node's `model`.
...
flowchart LR
    ...
    O --> B["Server build:<br/>section / component get_instance()"]
...
where `get_instance()` constructs a section or component object. That build emits
two things: a `context` (the description — label, model, properties, css,
request_config, permissions, tools) and a `data` payload (values only — an array
of `{id, value}` items, with `lang` when translatable). Both are packed into a
single `ddo`/datum of shape `{context, data}` and sent as the JSON API response.
The client render layer (the shared `ui.component` builders) consumes that datum
...
A component **never** touches the database. It reads with `get_data()` /
`get_value()` and writes with `set_data()`, but persistence is delegated to its
section: `save()` builds a save path and calls
`$section_record->save_component_data(...)`. The section is the only thing that
reads and writes the matrix. Components are instantiated through the factory
`component_common::get_instance()` (never `new`); `tipo` and `section_tipo` are
mandatory and the `model` is force-corrected from the ontology. Modes are `edit`
(read/write a real record), `list` (read-only listing), `search` (build SQO
filters; saves blocked) and `tm` (Time Machine read; saves blocked).
...
nodes. When a section is built, it gathers its component children, instantiates
each, and assembles their contexts and data into the section's response. Most
components are **literal** — they own their value (input_text, number, date,
media). Others are **related** — they store [locators](locator.md) instead of
literals and resolve their displayed value from a *target* record.
...
(via `ts_object::get_term_by_locator()` or by instantiating the named target
components and collecting their `get_value()`), so the value is always live: edit
...
server instantiates the section/component, builds the context and data, and
returns a ddo; the client renders the DOM.

```mermaid
sequenceDiagram
    participant C as Client (JS)
    participant API as Dédalo API<br/>(json/index.php → dd_manager)
    participant SRV as Section / Component<br/>(get_instance)
    participant ONT as Ontology + matrix (PostgreSQL)
    C->>API: RQO { source, action, dd_api, sqo, show/search/choose }
    API->>API: sanitize + dispatch (dd_api::action)
    API->>SRV: get_instance(tipo, section_tipo, section_id, mode, lang)
...
layout maps (what to return). The API sanitizes and dispatches the call to the
right API class/method. That handler calls `get_instance()` to build the section
or component. The instance reads its ontology node (model, properties, relations)
and the record's `datos` from PostgreSQL, resolving locators if it is a related
component. It assembles a `context` (the description) and `data` (the values)
into a `ddo` of shape `{context, data}`, which travels back through the API as a
JSON response. Finally the client renders the DOM using the shared
`ui.component` builders. See [RQO](rqo.md) for the request format and
[dd_object](dd_object.md) for the ddo.
...
- [Sections](sections/index.md) — the table abstraction in depth.
