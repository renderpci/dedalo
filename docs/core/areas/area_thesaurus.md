...
!!! note "PHP class → TS modules"
    The TS/Bun server has no `area_thesaurus` object (`engineering/AREA_SPEC.md`
    §5, phases A–E, done). The boot payload (`get_hierarchy_sections()` +
    typologies, the shape this page calls "the data item") is
    `readAreaHierarchyData` in `src/core/area/tree.ts`, byte-parity gated for
    BOTH thesaurus and ontology areas (`area_hierarchy_differential.test.ts` —
    37 thesaurus + 80 ontology entries). The per-hierarchy read-permission
    filter and the context `section_tipo` override + `thesaurus_mode` stamp
    (§ below) are the `'tree'` behavior branch of `dispatchAreaRead`
    (`readTreeArea`, `src/core/area/read.ts`). Search-with-path-resolution
    (`search_thesaurus`/`get_hierarchy_terms_sqo`) is `searchThesaurus`/
    `getHierarchyTermsSqo` in `src/core/ts_object/search.ts`
    (`ts_search_differential.test.ts`). The tree ROWS themselves — node
    assembly, expand/move/add/order mutations — are a shared subsystem with
    `area_ontology`, ported in `src/core/ts_object/` and documented in
    [Thesaurus / TS tree](../thesaurus/index.md#ts-rewrite-status), not
    duplicated here.
...
2. **data** (`$options->get_data` and `permissions > 0`):
   ...
   - **re-filters by permission per hierarchy**: drops any whose
     ...
     list — TS: the same filter runs in `readTreeArea` (`src/core/area/read.ts`),
     with the `area_ontology` bypass keyed on `principal.isGlobalAdmin`, but TS
     gates entry to `area_ontology` at all behind a **superuser-only** check
     upstream of this filter (see
     [area_ontology](area_ontology.md#ts-rewrite-status)) — PHP's admin bypass
     here is reachable by any global admin, TS's is not reachable by anyone but
     the superuser;
...
The TS equivalent — no instance, one call against the tipo:

```ts
import { readAreaHierarchyData } from './src/core/area/tree.ts';

// which active hierarchies show up as trees, for the thesaurus area?
const item = await readAreaHierarchyData('area_thesaurus', 'dd100', 'lg-spa', false);
// → { tipo, value: [ { section_tipo, target_section_tipo, children_tipo, root_terms, ... } ], typologies }
```

Permission filtering, the context `section_tipo`/`thesaurus_mode` stamps and
the `ts_search` attachment are the caller's job — `readTreeArea`
(`src/core/area/read.ts`) does all three around this call, mirroring
`area_thesaurus_json.php`'s controller shape exactly.
...
| --- | --- | --- | --- |
| `get_hierarchy_section_tipo()` | | The hierarchy definition section tipo — `DEDALO_HIERARCHY_SECTION_TIPO` (`hierarchy1`). Overridden by `area_ontology` to return `ontology35`. | a parameter (`areaModel: 'area_thesaurus' \| 'area_ontology'`) to `readAreaHierarchyData`, not a method to override — `src/core/area/tree.ts` |
| `get_main_table()` | | The main matrix table of the current hierarchy — `hierarchy::$main_table` (`matrix_hierarchy_main`). Overridden by `area_ontology` (`matrix_ontology_main`). | resolved inline in `tree.ts` from the same `areaModel` switch |
| `get_hierarchy_typologies()` | | All typology record ids, via `section::get_ar_all_section_records_unfiltered($typologies_section_tipo)`. | folded into `readAreaHierarchyData`'s typology dedup pass |
| `get_hierarchy_sections(?$hierarchy_types_filter=null, ?$hierarchy_sections_filter=null, $terms_are_model=false)` | | The core resolver: turn each active hierarchy/ontology element into a tree-section item (skipping not-active, no-typology, no-root-term, and filtered ones). Picks `hierarchy` vs `ontology` by `get_called_class()`. | `readAreaHierarchyData(areaModel, areaTipo, lang, termsAreModel)`, `src/core/area/tree.ts` — byte-parity gated (`area_hierarchy_differential.test.ts`); the per-hierarchy permission/active/children-tipo/root-terms filtering happens one layer up, in `readTreeArea` (`area/read.ts`) |

### Names & order (cached)

| method | static? | purpose | TS |
| --- | --- | --- | --- |
| `get_typology_data($section_id)` | | The typology locator of a hierarchy record (reads `DEDALO_HIERARCHY_TYPOLOGY_TIPO`, `hierarchy9`, a `component_select`). Returns the first locator or `null`. | inlined in the SQL `tree.ts` runs per hierarchy row |
| `get_typology_name($typology_section_id)` | | Resolve a typology's label (with fallback), memoized in `$typology_names_cache`. | inlined in `tree.ts`'s typology assembly (no separate per-process cache — the whole boot payload is one gated read) |
| `get_typology_order($typology_section_id)` | | Resolve a typology's order int (reads `DEDALO_HIERARCHY_TYPES_ORDER`, `hierarchy106`), memoized in `$typology_order_values_cache`. | same |
| `get_hierarchy_name($hierarchy_section_id)` | | Resolve a hierarchy term's name (reads `DEDALO_HIERARCHY_TERM_TIPO`, `hierarchy5`, with fallback), memoized in `$hierarchy_names_cache`. | same — `target_section_name` in the emitted hierarchy item |

### Search (with path resolution)

| method | static? | purpose | TS |
| --- | --- | --- | --- |
| `search_thesaurus($search_query_object)` | | Run the SQO via `search::get_instance()`, then for every hit walk its recursive ancestors (`component_relation_parent::get_parents_recursive()`, memoized per call) and build the `ts_object` data for the whole branch (root + each parent + that parent's children), so the client can rebuild and hilite the path. Returns `{result, msg, errors, total, found}`. | `searchThesaurus(sqo, principal)`, `src/core/ts_object/search.ts` — differential-gated (`ts_search_differential.test.ts`); dedups nodes by `` `${tipo}:${id}` `` in insertion order, same as PHP |
| `get_hierarchy_terms_sqo($hierarchy_terms)` | | Build a custom-filter SQO that matches a set of `{section_tipo, section_id}` terms (filters on `hierarchy22` id + section_tipo column, `$or` of `$and` groups, `limit 100`). Used to seed a search scoped to chosen hierarchy nodes. | `getHierarchyTermsSqo(hierarchyTerms)`, same file — reproduces the PHP shared-`$path`-object mutation quirk verbatim (every emitted group's `path.section_tipo` ends up equal to the LAST node's) rather than "fixing" it, since the live PHP JSON is the wire contract |
...
!!! note "How `search_thesaurus` builds the path"
    ...
    their order from `hierarchy::get_main_order($tld)`. TS: `searchThesaurus`
    calls `getParentsRecursive` (`src/core/relations/parent.ts`) and
    `fetchNodeInfo` (`src/core/ts_object/node_repository.ts`) for the same
    batched read; root order comes from `getMainOrder`
    (`src/core/ts_object/search.ts`), reading the ontology-main record (NOT
    `matrix_hierarchy_main`) exactly as PHP does — a thesaurus TLD with no
    ontology-main record resolves to a `null` root order, same as PHP. TS
    diverges deliberately on child order: **children get a positional order
    (index+1), not the stored order value** — a preserved PHP quirk, not a bug
    to fix (see [Thesaurus / TS tree](../thesaurus/index.md#ts-rewrite-status)).
...
- **[TS tree (`ts_object` / `dd_ts_api`)](../ontology/ts_object.md)** — the area is
  ...
  In the Bun rewrite this whole stack is ported and wired — see
  [Thesaurus / TS tree](../thesaurus/index.md#ts-rewrite-status) for the
  `src/core/ts_object/` module map and gates.
