...
!!! note "PHP class → TS modules"
    In the TypeScript/Bun server `area_admin` is not a class — it is one entry
    (`'area_admin' → 'dashboard'`) in the behavior map in
    `src/core/concepts/area.ts`, resolved by the single generic dashboard
    resolver `readDashboardArea` (`src/core/area/read.ts`) that every
    dashboard-behavior area shares. There is nothing "thin-subclass" left to
    explain in TS: the PHP pattern of an empty class existing only so the
    ontology `model` resolves to *something* has no TS analogue — a model
    string maps straight to a behavior, with no class to instantiate at all
    (`engineering/AREA_SPEC.md`, phases A–E, all done).
...
!!! note "TS: no instance, and model IS validated"
    `readDashboardArea` (`src/core/area/read.ts`) takes the tipo straight from
    the RQO — no factory call. Unlike PHP, the TS read dispatcher validates
    that the client-supplied `source.model` actually matches the tipo's
    resolved ontology model before dispatching, refusing loudly on a mismatch
    (`engineering/AREA_SPEC.md` §9 — a deliberate strengthening; see
    [area_ontology](area_ontology.md#ts-rewrite-status) for the fixture that
    proved PHP's looser behavior).
...
Every method in this section is inherited, and every one has a TS mapping
already documented once: construction/identity, JSON output and the dashboard/
metrics methods are on [area_common → Public API](area_common.md#public-api);
the menu walk (`get_areas`/`get_config_areas`/
`get_ar_children_areas_recursive`) is on
[area → Public API](area.md#public-api). This page does not repeat those
mappings — `area_admin` runs through the exact same generic dashboard resolver
(`readDashboardArea`, `src/core/area/read.ts`) as every other thin area.
...
- **`menu`** — built from `area::get_areas()`; the Administration area is one of
  its top-level nodes. TS: `src/core/resolve/menu.ts` walks the ontology
  directly against `concepts/area.ts`'s `MENU_ROOT_MODEL_ORDER`; see
  [Menu](../ui/menu.md).

## Examples

### Resolve and read the dashboard
...
The TS equivalent — no instance, the tipo drives both calls directly
