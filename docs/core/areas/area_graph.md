...
!!! danger "TS rewrite status: excluded — dead, dropped by user decision"
    `area_graph` is **not part of the TypeScript/Bun rewrite**
    (`engineering/AREA_SPEC.md` §1, decision dated 2026-07-03: "dead/deprecated,
    dropped from the rewrite by user decision"). It has no behavior resolver
    in `concepts/area.ts`'s `AREA_MODELS`/`areaBehaviorOf()` map, and a `read`
    addressed at it is **refused loudly** by `dispatchAreaRead`
    (`src/core/area/read.ts`) rather than silently dashboarded or tree-rendered
    — this is a deliberate, ledgered gap, not an oversight. The one place
    `area_graph` still appears in TS is `MENU_ROOT_MODEL_ORDER`
    (`src/core/concepts/area.ts`), which keeps its slot in the fixed root order
    purely so the live menu's root-node emission stays byte-identical; that is
    wire-order parity only; it carries no behavior. Everything below this note
    documents the PHP subsystem as it exists on the live server today — read it
    as historical/PHP-oracle reference, not as something to port.
