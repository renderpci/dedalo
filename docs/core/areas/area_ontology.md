...
!!! danger "TS rewrite status: superuser-only, fail-closed — a deliberate divergence from PHP"
    PHP has **no hard gate** on this area: ordinary ACL applies, plus a
    *positive* global-admin bypass in the shared controller (§ below) — any
    dd774 self-keyed profile with area-level read could, in principle, open
    it. The TypeScript/Bun rewrite makes this **superuser-only** by explicit
    user direction (2026-07-03, "only root can access"): `dispatchAreaRead`
    (`src/core/area/read.ts`) checks `principal.userId === SUPERUSER_ID`
    *before* touching the ontology, for both the tipo-shaped model check and
    the tree-area read itself, and refuses with a plain 403 otherwise. The
    same gate must remove `area_ontology` from every non-superuser menu (the
    menu walk's dd5 hiding) and from `readTreeArea`'s global-admin bypass path
    (§ below) — a global admin who is not the superuser gets nothing. This is
    a documented, tested divergence, not a bug: `engineering/AREA_SPEC.md` §9 and
    `rewrite/STATUS.md`'s Area rebuild table both pin PHP-admits/TS-refuses as the
    expected behavior, gated by `area_security_differential.test.ts`.

    Also fixed in TS: the **model-vs-tipo dispatch quirk**. PHP dispatches an
    area read on the client-supplied `source.model` string without checking
    that the tipo it names actually resolves to that model — proven live by a
    fixture where a `field_text` tipo (`dd917`) was accepted as
    `area_ontology`. TS validates `source.model` against the tipo's real
    ontology model and refuses loudly on a mismatch; the fixture was corrected
    to the real ontology-area tipo (`dd5`) with both behaviors pinned
    (PHP's looseness as a quirk, TS's refusal as the strengthening).
...
The TS equivalent — no instance, and the superuser gate runs first:

```ts
const areaModel = 'area_ontology';
const areaTipo = 'dd5';

// dispatchAreaRead (src/core/area/read.ts) refuses here unless
// principal.userId === SUPERUSER_ID — before any ontology read happens
const item = await readAreaHierarchyData(areaModel, areaTipo, 'lg-spa', false);
```

`readAreaHierarchyData` (`src/core/area/tree.ts`) is the single resolver shared
with `area_thesaurus` — see
[area_thesaurus → Instantiation & lifecycle](area_thesaurus.md#instantiation-lifecycle)
for the full call shape; there is no `area_ontology`-specific TS function, only
the `areaModel` argument.
...
| --- | --- | --- | --- |
| `get_hierarchy_section_tipo()` | | Return `DEDALO_ONTOLOGY_SECTION_TIPO` (`'ontology35'`) — the section whose records are the ontology hierarchies. Overrides the thesaurus' `'hierarchy1'`. | the `'area_ontology'` branch of the `areaModel` switch in `readAreaHierarchyData` (`src/core/area/tree.ts`) — no override, one shared function |
| `get_main_table()` | | Return `ontology::$main_table` (`'matrix_ontology_main'`). Overrides the thesaurus' `'matrix_hierarchy_main'`. | same switch |

### Inherited from `area_thesaurus` (tree building + search)

See [area_thesaurus → Key methods](area_thesaurus.md#key-methods) for the full
TS mapping of `get_hierarchy_sections`/typology-and-name resolution/
`search_thesaurus`/`get_hierarchy_terms_sqo` — one shared implementation
(`readAreaHierarchyData` + `searchThesaurus`/`getHierarchyTermsSqo`), not a
per-area copy.

### Inherited from `area_common` (lifecycle, JSON, metrics)

See [area_common → Public API](area_common.md#public-api) for the TS mapping
of construction/identity, `get_json`'s fallback, and the dashboard/metrics
methods — `area_ontology` is a **tree**-behavior area so its read never
reaches the dashboard resolver in practice, but the underlying functions are
shared.
...
- **`area` / menu** — `area::get_areas()` enumerates `area_ontology` as one of
  ...
  with a graceful warning when the ontology is out of date). TS: the menu walk
  (`src/core/resolve/menu.ts`) must additionally hide `area_ontology`'s tipo
  (`dd5`) from every non-superuser menu — see the TS status note at the top of
  this page.
