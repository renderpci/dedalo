...
!!! note "TS rewrite status"
    The area family has been rebuilt in the TypeScript/Bun server
    (`engineering/AREA_SPEC.md`, phases A–E, all done): there is no per-model class —
    a single **`concepts/area.ts`** contract module owns the canonical model
    list, the dashboard/tree/maintenance behavior taxonomy and the walk-filter
    sets, consumed by resolvers in `src/core/area/` (`read.ts` the read
    dispatcher, `dashboard.ts` the statistics engine, `tree.ts` the thesaurus/
    ontology boot payload, `color.ts` the deterministic per-tipo color) and by
    `src/core/resolve/menu.ts` (the menu walk). All ten behavior-carrying
    models below resolve through this module; **`area_graph` is the one
    exception — dropped from the rewrite by user decision (2026-07-03) and its
    reads are refused loudly**, though it still occupies its fixed slot in the
    menu's root order for wire parity. `area_ontology` is additionally
    **superuser-only, fail-closed** in TS — a deliberate strengthening beyond
    PHP (see [area_ontology](area_ontology.md#ts-rewrite-status)).
...
!!! note "Menu roster"
    ...
    The TS rewrite keeps the exact same root order as `MENU_ROOT_MODEL_ORDER`
    in `src/core/concepts/area.ts`, consumed by `src/core/resolve/menu.ts` —
    including `area_graph`'s slot, for menu wire parity, even though `area_graph`
    is excluded as a *behavior* (§ below). The behavior-carrying model set is a
    *different*, deliberately shorter list: `AREA_MODELS` in the same module
    (ten models, `area_graph` absent).
