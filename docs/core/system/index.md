...
| --- | --- | --- |
| [common](common.md) | Base &amp; data | The horizontal ENGINE LAYER that replaced PHP's `common` base class — identity, structure-context, `request_config`, permissions and language resolved by per-model descriptors, not inheritance. |
| [base](base.md) | Base &amp; data | What replaced PHP's bootstrap/loader: native ES-module resolution + the component registry (no autoloader), the frozen config catalog and the `Bun.serve` process entry. |
...
| [logger](logger.md) | Ops | The user-activity audit trail (`activity_log.ts` → `matrix_activity`); the PHP severity facade / pluggable-backend registry and `debug_log()` stream are not ported (plain `console` diagnostics). |
| [services](services.md) | Ops | Reusable client-side helpers (file upload, rich-text editing, autocomplete search, …) copied as-is into the client tree; their server side is the generic TS dispatch actions. |
