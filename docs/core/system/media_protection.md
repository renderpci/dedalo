...
!!! danger "TS status: NOT ported — reporting shell only"
    Media protection stays a **PHP/web-server-owned subsystem** in the TS
    rewrite (`engineering/MEDIA_SPEC.md` §Out-of-scope; `STATUS.md`'s
    media-access-control row is ⬜). There is no TS
    `build_htaccess`/`sync_auth_markers`/cookie-persistence equivalent, and the
    TS server does not generate `.htaccess`, write auth markers, or set the
    `dedalo_media_auth` cookie. The one thing the TS server does is **report**
    on the shared media dir's state: `mediaControlGetValue()`
    (`src/core/resolve/widget_request.ts`) reads whether `.publication/` marker
    counts and `.htaccess` exist, and forwards a `media_index_status` RPC to
    the diffusion engine over its configured unix socket
    (the native media-index seam since the 2026-07-11 cutover; the old `callDiffusionEngine` socket RPC is deleted) — engine-native fields it cannot know
    (PHP config constants, `htaccess.up_to_date`) come back `null`, and its own
    posture is reported as `mode: 'private'` gated on the TS session cookie
    (`SESSION_COOKIE = 'dedalo_ts_session'`,
    `src/core/security/session_store.ts`) — a **different** cookie from the
    PHP `dedalo_media_auth` one described below, not a reproduction of it.
    `media_control.set_media_access_mode` is explicitly refused on the TS
    engine (`engineDenied`, same file): mutating the PHP install's config file
    and `.htaccess` from a coexisting server would be unsafe. Everything below
    this note documents the **PHP mechanism**, which remains the live,
    enforcing oracle.
...
| **Bun diffusion engine** | the `pub/` and `dbs/` markers (rule B), the matching key validation | `media_index.ts`, its own service (a separate diffusion engine process reached over a unix socket — not part of this TS server's `src/` tree; see the `dedalo-diffusion` skill) |
...
!!! warning "Three enforcement surfaces must stay in sync"
    ...
    `KEY_REGEX` in the diffusion engine's `media_index.ts`. Touch one, review
    all three. The TS server in this tree talks to that engine only through
    the native `getMediaIndexStatus()` seam (the socket RPC was deleted at the 2026-07-11 cutover); it
    does not implement `KEY_REGEX` or the marker writes itself.
...
- **Diffusion engine** (`media_index.ts`, its own separate service) — owns the
  `pub/` and `dbs/` markers (rule B). It validates keys with the same grammar
...
- **`media_control` widget, PHP side** (`core/area_maintenance/widgets/media_control/`) —
...
- **`media_control` widget, TS side** (`src/core/resolve/widget_request.ts`'s
  `mediaControlGetValue()`) — the **reporting-only** counterpart described in
  the TS status note above: reads the shared media dir's `.publication/` marker
  counts and `.htaccess` existence and forwards the same `media_index_status`
  RPC over the configured unix socket, but never writes `.htaccess`, never
  syncs auth markers, and refuses `set_media_access_mode` outright
  (`engineDenied`). `rebuild_media_index` is wired the same way as PHP,
  delegating to the diffusion engine.
...
- `core/media_protection/class.media_protection.php` — the source (still PHP;
  see the TS status note above — nothing here has a TS port yet).
- the separate Bun diffusion engine's `media_index.ts` — the owner of rule-B
  markers; it is a standalone service, not part of `src/` in this tree (see
  the TS status note above).
