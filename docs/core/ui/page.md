...
The server has no `page` model: the `start` action in
`dd_core_api::start()` assembles the page context (login context when not
logged; otherwise the `menu` element plus the section/area/tool resolved from
the URL) and returns it as `result.context` along with an `environment` payload.
