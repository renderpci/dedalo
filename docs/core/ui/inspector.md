...
context. It is carried on [`dd_object`](../dd_object.md)
(`get_show_in_inspector()` / `set_show_in_inspector()`), populated by
`tool_common::get_structure_context()` / `create_tool_simple_context()` from the
tool's registration, and mapped via `tool_ontology_map::SHOW_IN_INSPECTOR` in
`tools_register`. A tool authored with `show_in_inspector: true` in its
