...
The routing is *not* hardcoded per component. It is resolved through the static
registry `section_record_data::$column_map` via `get_column_name($model)`. All
four string components map to `string`:
...
when `set_data()` encounters an item without a valid `id`. See
[Sections — typed-column storage](../sections/index.md) for the full column set
