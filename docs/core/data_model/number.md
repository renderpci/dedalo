...
The model→column routing is **not** hardcoded in the component; it is resolved
through the static registry `section_record_data::$column_map`, which maps
`component_number → 'number'`. The column itself is declared in
