...
!!! info "Where these columns sit"
    ...
    split across typed JSONB columns. `section_record_data::$column_map` maps each
    component model to its column, and `section_record_data::$ar_columns` lists the
    canonical column set. See
...
Because the shapes differ wildly, `section_record_data::$column_map` flags every
`misc` entry as a *direct object* — the column store does not interpret the
...
the stored array to `$item->entries`, and the component JS reads it back:
...
- **`misc` is a role, not a type.** The five components that share it have no
  ...
  adding a direct-object component, register it as `'…' => 'misc'` in
  `section_record_data::$column_map` and implement the read/write of its own
