...
- **Child ordering.** `add_parent()` calls `set_child_order()` and `remove_parent()` calls `remove_child_order()`; both read the order component tipo from `section_map->thesaurus->order` and write the child's position (as a context-keyed `component_number`) so siblings keep a stable order. `recalculate_sibling_orders[_static]()` renumbers a parent's children 1..n. These count-then-write operations are race-free only inside the parent's advisory lock / transaction (a `SHOW_DEBUG` warning flags unprotected call sites).
