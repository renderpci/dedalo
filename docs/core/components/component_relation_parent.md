...
!!! note "Typology"
    `component_relation_parent` is a **related** component. In server context it extends the abstract base [`component_relation_common`](base_classes.md) (`core/component_relation_parent/class.component_relation_parent.php` → `core/component_relation_common/class.component_relation_common.php`), which in turn extends `component_common`. Like every related component it stores [locators](../locator.md) that point at other records rather than literal data, and resolves its displayed `value` from the *target* (parent) record. It is enumerated in `component_relation_common::get_components_with_relations()`.
...
- **Hierarchy integrity (server-enforced).** Adding a parent goes through `add_parent()`, which rejects two illegal cases before any save: an **auto-reference** (a record pointing at itself) and a **descendant cycle** (the prospective parent is already a descendant — checked with `is_ancestor()`, which walks the ancestor chain via `get_parents_recursive()`). Cycle rejections push a structured entry onto `component_relation_parent::$errors`, surfaced to the client through the datum. The recursive walk (`fetch_ancestors_recursive()`) is itself loop-guarded per path and de-dupes ancestors by `section_tipo_section_id`.
- **Child ordering.** `add_parent()` calls `set_child_order()` and `remove_parent()` calls `remove_child_order()`; both read the order component tipo from `section_map->thesaurus->order` and write the child's position as an **`id_key` dataframe** of the parent-link locator (`{value, id_key}` on a `component_number`) so siblings keep a stable order. `add_parent()` **pre-allocates** the parent locator's `id` (`set_data_item_counter()`) before ordering so the order can pair before the save mints ids. `recalculate_sibling_orders[_static]()` renumbers a parent's children 1..n, resolving each child's `id_key` via `component_relation_children::resolve_parent_link_id_key()`. These count-then-write operations are race-free only inside the parent's advisory lock / transaction (a `SHOW_DEBUG` warning flags unprotected call sites). The id_key contract is documented in [component_dataframe → Relation ordering](component_dataframe.md#relation-ordering-the-order-is-a-dataframe).
- **Convenience API.** `make_me_your_parent()` / `remove_me_as_your_parent()` add/remove a parent edge from the child's side; `get_parents()` returns the direct parents, `get_parents_recursive()` all unique ancestors, `get_parent_tipo()` / `get_component_relation_children_tipo()` resolve the paired component tipos, and `get_possible_root_hierarchy()` detects whether the record is a root node of a hierarchy (used for diffusion).
