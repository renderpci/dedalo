...
The search trait (`trait.search_component_iri.php`) builds JSONB-path SQL over the `iri` column and supports:
...
`conform_import_data()` also accepts plain-text CSV strings:
...
On export, `get_export_value()` emits one atom per value with `cell_type: "iri"`, joining `iri` + resolved title with `fields_separator` and pre-joining multiple values with `records_separator` (flat-table parity).

See the full URI import definition [here](../importing_data.md#uri) and the export model [here](../exporting_data.md).

## Notes

- **Title label dataframe (always injected).** `get_properties()` injects a fixed `component_dataframe` (slot tipo `dd560`, target section `dd1706` / component `dd1715`) into `source.request_config` for every `component_iri`. Titles are stored as structured data there and resolved at read time by `resolve_title()`, paired to each IRI value by the shared item `id`. Deleting an IRI value cascades the paired dataframe rows server-side (single-writer rule); the client does not delete the dataframe directly. See [component_dataframe](component_dataframe.md).
...
- **Diffusion.** `get_diffusion_data()` returns each value with its title resolved (cloned, not written back to live data), so published output carries both the URL and its label.
- **`dd_iri` DTO.** Each value is a `dd_iri` (`class.dd_iri.php`): `set_iri()` validates scheme+host, `set_id()` / `set_label_id()` cast to int, and unknown properties are rejected. `label_id` is explicitly not part of the persisted schema.
- **`is_empty()`** considers only the `iri` and `title` properties when deciding whether a value is empty, so a value with just an `id` (and no URL/title) is treated as empty.
