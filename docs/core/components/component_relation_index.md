...
!!! info "TS server implementation"
    The descriptor `src/core/components/component_relation_index/descriptor.ts` registers `resolveData: relationIndexResolver` (`src/core/relations/models/relation_index.ts`). In `list`/`tm` cells it resolves one page of dd96 inverse locators via `findInverseReferenceLocators` / `countInverseReferences` (`src/core/search/search_related.ts`) and, on the first page of a request, attaches each pointing section's `related_list` children (mode `related_list`, one level, no further own-config recursion) — a pinned per-request state-bleed quirk (`relationIndexSolvedSections`) that reproduces PHP's shared-instance re-parenting. Other modes fall through to the shared portal engine. Search (the dedicated computed-inverse trait) is not yet ported. See the *dedalo-relations-ts* skill.
...
!!! warning "TS gap: search"
    `src/core/components/component_relation_index/descriptor.ts` marks `search: { status: 'unported' }` (its computed-inverse search trait, incl. the `*` / `!*` easy-search operators, is not implemented) — `getRelationSearchFragmentBuilder()` (`src/core/relations/registry.ts`) throws loudly for this model rather than silently mis-searching. Filtering by "is/is not indexed" is not yet available in the TS server.
