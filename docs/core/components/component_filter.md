...
!!! note "Typology"
    `component_filter` is a **related** component. It extends `component_relation_common` (`core/component_relation_common/class.component_relation_common.php`), the abstract base shared by every relation component, which in turn extends `component_common`. Like all related components it stores [locators](../locator.md), not literal values, and resolves its display value from a *target* section — here, the **Projects** section (`dd153`).
