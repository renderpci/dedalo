...
In `edit` mode the JSON controller also ships a `datalist`: the complete ontology hierarchy (areas → sections → elements) used to render the tree. This is **derived, not stored data** — it is computed by `get_datalist()` and is identical for all profiles (permissions differ, the tree does not). Because resolving it is expensive (≈3–6 s), the login sequence pre-calculates it in the background and `dd_cache::cache_from_file()` caches it per application language as `cache_tree_<lang>.php`. Each datalist item carries its full parent chain in `ar_parent`, letting the client resolve children/parents (and propagate values up the tree) without re-walking the ontology:
...
- The constants `DEDALO_SECTION_PROFILES_TIPO` (`dd234`) and `DEDALO_COMPONENT_SECURITY_ACCESS_PROFILES_TIPO` (`dd774`) in `core/base/dd_tipos.php` bind the security subsystem to this node; `calculate_tree()` reads exactly this section_tipo/tipo pair to build the per-profile permission tree.
...
- **search** mode renders a placeholder (`"Working here! (search mode)"`); saving is blocked in search mode at the component level. The Profiles section is explicitly **not** filtered by the search WHERE machinery (`trait.where.php` profiles branch applies no row filter).
...
Because the meaning of each row depends on live ontology tipos, importing a permission matrix is only sound between installations that share the same ontology tipos. There is **no diffusion value**: `get_diffusion_value()` is overridden to return the literal placeholder `"There is no diffusion value for this component"`, so permission data is never published to diffusion targets.

See the full import model in [importing data](../importing_data.md) and the export model in [exporting data](../exporting_data.md).

## Notes

- **Single source of truth for permissions.** The stored matrix feeds `security::reset_permissions_table()` / the runtime permission table. The component itself is gated by the standard `get_component_permissions()` chain; note the special case that downgrades security-admin contexts (see `resolve_component_read_permission`). Editing it requires admin-level access.
- **`set_section_permissions()`** is the programmatic entry point used when generating hierarchies for users: it grants a profile a permission (default `2`) over a list of section tipos *and all their children*, merging into existing data, then resets the permissions table. It is the API behind "give this profile access to these new sections."
- **`calculate_tree()` / `get_datalist()`** build the tree once per language and cache it to file on login. If permissions look stale or the tree is missing nodes, suspect the `cache_tree_<lang>.php` file or `DEDALO_CACHE_MANAGER['files_path']`.
- **`get_sortable()` returns `false`** — this component never offers sort buttons in list headers.
