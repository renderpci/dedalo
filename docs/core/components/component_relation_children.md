...
!!! note "Typology"
    `component_relation_children` is a **related** component. It extends the abstract base
    `component_relation_common` (which extends `component_common`) and stores
...
must be related in the ontology so `get_ar_related_parent_tipo()` can pair them; if
that ontology relation is missing the component falls back to locating *any*
`component_relation_parent` in the section (and logs a "Bad definition in ontology"
warning).
...
- **Inverse of `component_relation_parent`.** Adds/removes are routed through the
  related parent component: `add_child()` / `remove_child()` resolve the parent tipo
  via `get_ar_related_parent_tipo()`, instantiate the `component_relation_parent`
  (preserving the current mode so a search-mode children component cannot
  accidentally persist) and call `make_me_your_parent()` /
  `remove_me_as_your_parent()`, then `save()` on the parent. `set_data()` diffs the
  desired list against the current one and issues those add/remove calls; prefer the
  explicit `add_child()` over `set_data()`.
- **Thesaurus integration.** `get_children_of_type()` and `has_children_of_type()`
...
- **Caching.** Resolved data is memoised in `data_resolved` and parent-tipo lookups
  in the static `ar_parent_tipo_cache`; mutating the relation clears `data_resolved`
