...
the section class. The component simply reads it:

```php
// class.component_section_id.php
...
**Export:** the component participates in the export atoms contract
...
```php
// one int atom per item, cell_type 'section_id'
...
The raw/`dedalo_raw` export path special-cases this model as well: the value is written
as an `int` under `cell_type` `section_id` instead of being wrapped as a `json` array —
so the exported cell is `1`, not `[1]`.
...
Search mode (handled by `trait.search_component_section_id`) resolves the query against
the integer `section_id` column (cast to `integer`), supporting these operators:
...
- **Ordering.** `get_order_path()` sets the order column literally to `section_id`
  (or `id` for the Time Machine `dd1573` instance) so that "order by id" sorts on the
  real integer column rather than on a JSONB path. `sortable` is `true` in the context.
