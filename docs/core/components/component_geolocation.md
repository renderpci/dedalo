...
default: the `DEDALO_GEO_PROVIDER` constant (`config.php`; the sample config ships `VARIOUS`).

Selects the base tile layer(s) of the Leaflet map. The server stamps the resolved value into `context.features.geo_provider` (`$properties->geo_provider ?? DEDALO_GEO_PROVIDER`) and the client switches on it:
...
!!! warning "Search mode under construction"
    `search` mode is wired (`render_search_component_geolocation`) but the search view is not currently exposed in the search list (`UNDER CONSTRUCTION` in the source). The component also reports `get_sortable()` → `false`, so it never offers column-sort buttons in lists.
...
- **PHP accessors.** `get_latitude()` / `get_longitude()` return the center as floats (or `null` for the default sentinel). `regenerate_component()` reloads then re-saves (used by `tool_update_cache`).
- **Diffusion.** Two diffusion helpers convert the stored item: `get_diffusion_value_socrata()` returns a GeoJSON `Point` object `{type, coordinates:[lon, lat]}` for Socrata; `get_diffusion_value_as_geojson()` returns an encoded `FeatureCollection` (layer wrapper, 16‑decimal `[lon, lat]` coordinates) and yields `null` for the default sentinel. Both keep the GeoJSON `[lon, lat]` order.
