...
When the item carries drawn shapes (a `lib_data` layer with at least one feature), the **geometry is the location** and the stored `lat`/`lon` are the **framing** — where the drawing is displayed from. Publication reads it that way on every path, and the standalone one drops `lat`/`lon` from what it emits, so no consumer can read framing as an entered coordinate. That is what makes re-framing storable work: panning or zooming a record that holds geometry updates its center and marks the component dirty, and it is correct rather than a fabricated coordinate. On a record with no value at all there is nothing to frame, so navigation writes nothing; and a geometry-only record opens **fitted to its own drawing**, not on the world view. See [Geolocation values](../data_model/geolocation.md#geometry-wins-over-the-center).

!!! danger "Legacy data: the retired Valencia pair"
    Earlier engines seeded the map with `39.462571 / -0.376295` — the coordinates of the Dédalo facilities, the client's factory default position — and read that exact pair back as "no location set". It is an ordinary coordinate now. An upgraded install carries items centred on it that nobody entered, and they must be repaired before the engine stops special-casing the pair — otherwise each of them publishes that studio position as a real location. The repair (`scripts/repair_geolocation_studio_default.ts`) clears the bare ones and, for an item that also carries `lib_data` features, **derives** a new center from that drawn geometry instead of keeping the fabricated pair — the one coordinate the engine authors anywhere, and framing rather than an asserted location. Because the pair is a factory default rather than a place, an item holding it exactly is always fabricated: a genuinely geocoded record carries its own coordinates, so nothing correct is caught. Full law and the operator caveats: [Geolocation values](../data_model/geolocation.md#absence-is-structural--there-is-no-magic-coordinate).
