...
When instantiated, `component_select_json.php` builds the structure context, resolves the target sections via `get_ar_target_section_tipo()` and attaches each as `context.target_sections` (with `permissions` and `permissions_new`, used by the client to show the *new* / *go to list* buttons), and — in `edit` mode — populates `data.datalist` from `get_list_of_values()`. On `save()`, the section is the single writer: the chosen locator is persisted into the section `relations` container, not a private data column.
