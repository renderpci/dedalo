...
use `component_html_text`, which shares this class text-handling logic but keeps
more HTML tags on import/sanitization.
...
single flat string (it is auto-wrapped in `<p>…</p>`, and line breaks `\n` / `<br>`
become paragraph breaks). `conform_import_data()` normalizes all of these into the
v7 `{value}` item shape, so a bare title imported into a text area still becomes
valid HTML. Strings that look like a malformed JSON array (begin `["` / end `"]`)
...
Export uses the atoms contract (`get_export_value`): one atom per non-empty data
item, with inline image tags resolved on the fly (`TR::add_tag_img_on_the_fly`) so
the exported cell carries the same value the grid showed. When the current language
is empty, the value falls back to the default language and is flagged
`is_fallback`. The `indexation_list` legacy grid mode is never used during export.

See [Exporting data](../exporting_data.md).

## Notes

- **Inheritance.** `component_text_area extends component_string_common extends
  component_common` (literal-direct). It is **not** new-ed directly — use
  `component_common::get_instance()` (see [index.md](index.md#instantiation)).
- **Save sanitization.** `save()` runs `sanitize_text()` over every value (when
  `clean_text=true`) before delegating to the standard
  `component_common::save()`; the section persists the data (the component never
  touches the DB).
- **Tag repair.** In edit mode the controller auto-fixes broken index/draw tag
  pairs (`fix_broken_index_tags()`); `delete_tag_from_all_langs()` removes a tag
  across every language and re-saves.
- **Plain text.** `get_plain_text()` strips all Dédalo tags and HTML (used by
  publication search).
- **API actions** (`dd_component_text_area_api`, SEC-024 allowlist): `delete_tag`,
  `get_tags_info`. Used mainly by `tool_indexation`.
- **Default tools.** Indexation/transcription tooling (`tool_indexation`,
  ...
  `tool_lang_multi`, `tool_propagate_component_data` and `tool_time_machine`.
- **Observer example (AV sync).** When the user clicks a `[TC_…]` time-code tag in
  the transcription, the component publishes an event the related `component_av`
  is subscribed to, jumping the player to that position; the AV insert-TC /
  play-pause keys are exposed via `context.features.av_player`.
- **Gotcha — tags vs locators.** The inline tags are a synchronised *copy* of the
  locators stored in the companion `tags_*` relation components. Editing the text
  directly can desynchronise them; rely on the toolbar buttons / tools and on
  `fix_broken_index_tags()` rather than hand-editing tag strings.
