...
Server-side instantiation follows the shared factory (never `new`):

```php
$component = component_common::get_instance(
    'component_json', // model
    'test18',         // tipo (mandatory)
    1,                // section_id
    'edit',           // mode
    DEDALO_DATA_NOLAN,// lang (forced to lg-nolan internally)
    'test3'           // section_tipo (mandatory)
);
```
...
The raw export (`dedalo_raw`) produces the `dedalo_data` wrapper automatically, so **export → import round-trips work without manual edits**. The edit-view **Download** button exports the current value as a plain `.json` file (not wrapped).
...
- **Search.** The search trait (`search_component_json` / `_tm`) builds PostgreSQL JSONB **jsonpath** predicates over the `misc` column (`@?` / `like_regex`, with `q_split` AND-splitting of multi-word queries). Search saves are blocked like in any component (search/tm modes do not persist).
