...
3. **Gate** (`index.php`) — runs in order: CORS check (`Access-Control-Allow-Origin` echoed only for an allowed origin), OPTIONS preflight short-circuit, `json_decode` of `php://input` with a JSON-error guard, `$_FILES`/legacy `$_REQUEST` XSS scrub (which **deliberately preserves** `rqo.sqo` and `rqo.options.sqo` so search operators survive), default `dd_api` assignment (`diffuse`/`validate`/`get_ontology_map` force `dd_diffusion_api`; otherwise `dd_core_api`), then the sanitizers: `search_query_object::sanitize_client_sqo()` strips server-only SQO fields (forces `parsed=false`, clamps `limit` to `DEDALO_SEARCH_CLIENT_MAX_LIMIT`), and `request_config_object::sanitize_client_ddo_map()` reduces the client `show.ddo_map` and `search.ddo_map` to a whitelist of display fields — including per-ddo `limit`/`offset` (accepted only as non-negative ints) which set a component's *output pagination slice* (`limit:0` = all rows; see [dd_object.md](dd_object.md#client-sent-ddos-the-security-whitelist)). Finally the per-session CSRF token is minted, and `session_write_close()` runs if `prevent_lock === true`.
...
| 4 | `sanitize_client_ddo_map` | `index.php` → `dd_manager::sanitize_client_rqo` | Injected ddo fields beyond the display whitelist — on `show.ddo_map` and `search.ddo_map`. The whitelist allows display + pagination fields only; `limit`/`offset` survive as non-negative ints (output slice), server-authoritative fields (`model`, `permissions`…) are stripped |
