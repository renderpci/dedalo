...
ar_ddo_to_export (chosen columns, user order)
        ...
        ▼
get_export_value()  per component → export atoms  (core/dd_grid/class.export_*.php)
        ▼
export_tabulator    atoms → flat-table NDJSON      (tools/tool_export/class.export_tabulator.php)
        ▼  stream
flat_table.js       accumulate lines → preview + CSV/TSV/ODS/XLSX/HTML/media
```

The server forces `sqo->limit = 'ALL'` in `setup()` (after the read-permission
gate), so the export always covers the full filtered selection rather than the
client's clamped page limit.

### API

`tool_export::get_export_grid(options)` — exposed through `dd_tools_api`
(`API_ACTIONS = ['get_export_grid' => null]`). Request fields:
...
Components resolve their export data through `get_export_value(export_context) :
export_value` — a flat list of `export_atom` `{path, value, cell_type}`. Relation
components recurse component-driven, descending with `export_context->descend()`.

- A new component only needs to override `get_export_value` if its shape differs
  from the base.
- `get_raw_export_value()` is **final** — it shares its builder with the legacy raw
  value path so the `dedalo_raw` wire shape cannot drift. **Do not override it.**
- The flat-join (`value` format) reference is `export_value::to_flat_string()`.

### Files

- `tools/tool_export/class.tool_export.php` — request handling, `setup`, records, atoms, streaming.
- `tools/tool_export/class.export_tabulator.php` — atoms → NDJSON, breakdown.
- `core/dd_grid/class.export_*.php` — `export_value` / `export_atom` / `export_path_segment` / `export_context`.
- `tools/tool_export/js/flat_table.js` — client accumulator, preview, downloads.
- `tools/tool_export/js/{render_tool_export,drag_tool_export}.js` — UI and drag-and-drop column model.
- `tools/tool_export/js/export_user_presets.js`, `core/section/js/view_export_user_presets.js` — per-user presets (section `dd1781`).
