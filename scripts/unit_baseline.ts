...
export const UNIT_TIER: TierSpec = {
	...
	exactCounts: false,
	whyRed:
		"Why these reds exist — and why, UNLIKE the parity tier, they are NOT permanent: the 7 unit reds the CENSUS measured on 2026-08-29 (727 files / 8800 cases / 8779 pass / 14 skip — the census's own numbers, which a hand-run `bun test` does not reproduce because scripts/lib/parity_census.ts childEnv() strips the nine PER_RUN_SEAMS keys and pins ORACLE_MODE=fixtures) fall in three classes. (1) 2 deterministic golden mismatches (ontology_parser dd1, info_widget_native component_info) — a stale expectation or a real parser/widget defect, either way fixable. (2) 2 order-dependent gates that PASS IN ISOLATION and fail only in full-suite order (ops_health_db_down, activity_aggregate_native) — test-isolation defects (leaked module state / shared fixture), NOT product defects. A third such gate, update_disk_space_native, is order-dependent in the same way but is NOT frozen here: it happened to pass in the runs that produced this baseline, and it is being made deterministic rather than frozen. That is the hazard this class carries — an order-dependent test flaps the ratchet red and green on its own, so the fix is always determinism, never an entry. (3) 3 blocked on backlog rows P1-14 / AIX-01 and P1-16 (rag_api, rag_ask, rag_pipeline): the RAG gates INHERIT the machine's embedding provider instead of pinning one, so they need a sidecar that is absent here, AND they write to the INSTALLATION's vector database because ragSql reads DEDALO_RAG_DB_NAME which the test preload never repoints — a violation of the suite-database law in its own right; retrieval.ts additionally launders `undefined` through an `as number[]` cast. EVERY entry here is expected to be FIXED. The list is shrink-only and a listed test that passes is red, so these numbers may only go DOWN.",
};
