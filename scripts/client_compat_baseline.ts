...
const RULE = [
	'SHRINK-ONLY ratchet on client reads of the envelope-v2 COMPAT keys `.msg` / `.errors` / `.result` (engineering/ERRORS_SPEC.md §3.1), per file, comments and string contents excluded, page_globals lines excluded, and the named non-envelope expressions of READ_EXCLUSIONS (browser APIs, server job stream frames, payload diagnostic lists, failure extension keys) excused one expression at a time with a reason.',
