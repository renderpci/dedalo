import type { Sqo, Filter, FilterRule, Locator } from '../types/index.js';
...
export class SqoBuilder {
	 ...
	 */
	filter(operator: 'AND' | 'OR' = 'AND', rules: FilterRule[]): this {
	 ...
	 */
	order(path: string, direction: 'ASC' | 'DESC' = 'ASC'): this {
	  if (!this.sqo.order) {
	    this.sqo.order = [];
	  }
