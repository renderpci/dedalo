...
function mapDedaloError(raw) {
  const errors2 = Array.isArray(raw.errors) ? raw.errors.map(String) : [];
  const msg = typeof raw.msg === "string" ? raw.msg : "Dédalo request failed";
  let code = "unknown";
  if (errors2.includes("permissions_denied")) {
    code = "permissions_denied";
  } else if (errors2.includes("csrf_failed")) {
    code = "csrf_failed";
  } else if (errors2.includes("not_logged")) {
    code = "not_logged";
  } else if (errors2.includes("invalid_api_class")) {
    code = "invalid_api_class";
  } else if (errors2.includes("invalid_action")) {
    code = "invalid_action";
  } else if (errors2.includes("login_failed")) {
    code = "login_failed";
  } else if (errors2.includes("maintenance_mode")) {
    code = "maintenance_mode";
  } else if (errors2.includes("update_lock")) {
    code = "update_lock";
  } else if (errors2.includes("db_connection_failed")) {
    code = "db_connection_failed";
  }
  return new DedaloError(`${msg} [${errors2.join(",")}]`, code, errors2, raw);
}
...
class WorkClient {
  ...
  async call(rqo) {
    ...
    if (json.result === false) {
      const errors2 = Array.isArray(json.errors) ? json.errors.map(String) : [];
      if (this.auth !== null && errors2.includes("not_logged") && this.autoLogin) {
        await this.login();
        body.csrf_token = this.csrfToken;
...
function isSessionAuth(config) {
  return config !== null && config.type === "session";
}
// src/auth/public_auth.ts
function validatePublicAuthConfig(config) {
