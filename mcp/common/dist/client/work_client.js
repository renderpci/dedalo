...
/**
    ...
    *
    * What: manages a PHP session cookie, CSRF-token rotation, and
    * transparent re-login so that MCP tool handlers can treat API calls
    * as stateless while the client handles all session machinery.
    *
    ...
    *   username: 'admin',
    *   password: process.env.DEDALO_WORK_PASSWORD!,
    *   autoLogin: true,
    * });
    * await client.bootstrapCsrf();
    * await client.login();
    * const res = await client.call(rqo('read','dd_core_api',{...}, sqo));
    * ```
    */
export class WorkClient {
     ...
     * CSRF-token extraction from the response body.
     */
     ...
     *
     * What: serialises the RQO into a JSON POST body, attaches the
     * current cookie and CSRF token, parses the response, and checks
     * `result`.  If `result === false`, inspects `errors` for `not_logged`;
     * when `autoLogin` is enabled, performs one re-login attempt and
     * retries the request.
     *
     * Why: the MCP server must behave as if the API is stateless, but
     * Dédalo sessions expire.  Moving the retry logic here keeps every
     * tool handler simple (single `await client.call(...)`).
     *
...
//# sourceMappingURL=work_client.js.map