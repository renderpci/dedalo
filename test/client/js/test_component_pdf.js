...
// element options for component_pdf
	const element = elements.find(el => el.model==='component_pdf')
	if (!element) {
		console.error('Error: component_pdf not found in elements');
	}
...
// mode/view matrix for component_pdf
...
describe(`COMPONENT_PDF LIFECYCLE`, function() {
		...
		describe(`${pair.mode} / ${pair.view}`, function() {
			...
			it(`init → build → render`, async function() {
				...
				// init
					...
					assert.equal(instance.model, 'component_pdf', 'model expected component_pdf')
			...
			it(`render output structure for ${pair.mode}/${pair.view}`, async function() {
				...
				// mode-specific assertions
					if (pair.mode==='edit' && pair.view==='default') {
						// edit default: content_data with pdf viewer container
					...
					if (pair.mode==='edit' && pair.view==='viewer') {
						// edit viewer: wrapper with view_viewer class
						assert.isOk(node.classList.contains('view_viewer'), 'expected view_viewer class in edit/viewer')
						assert.isOk(node.classList.contains('component_pdf'), 'expected component_pdf class in edit/viewer')
					}
...
describe(`COMPONENT_PDF DATA OPERATIONS`, function() {
	...
	it(`change data via change_value (update)`, async function() {
		...
		if (entries.length > 0) {
			...
			updated_entry.original_file_name = 'updated_test_file.pdf'
...
describe(`COMPONENT_PDF SEARCH DATA OPERATIONS`, function() {
