...
// element options for component_number
	const element = elements.find(el => el.model==='component_number')
	if (!element) {
		console.error('Error: component_number not found in elements');
	}

	const section_tipo	= element.section_tipo
	const section_id	= element.section_id
	const tipo			= element.tipo  // test211
...
// mode/view matrix for component_number
// edit: default, line, print, mini
...
describe(`COMPONENT_NUMBER LIFECYCLE`, function() {
		...
		describe(`${pair.mode} / ${pair.view}`, function() {
			...
			it(`init → build → render`, async function() {
				...
				// init
					...
					assert.equal(instance.model, 'component_number', 'model expected component_number')
			...
			it(`render output structure for ${pair.mode}/${pair.view}`, async function() {
				...
				// mode-specific assertions
					...
					if (pair.mode==='edit' && pair.view==='line') {
						...
						assert.isOk(node.querySelector('.input_value'), 'expected input_value in edit/line')
...
describe(`COMPONENT_NUMBER DATA OPERATIONS`, function() {
	...
	it(`data structure`, async function() {
		...
		// component_number entries have value (number) and id
		if (entries.length > 0) {
	...
	it(`clean_value removes invalid chars`, async function() {

		// comma to dot
		assert.equal(instance.clean_value('17,2'), '17.2', 'expected comma replaced with dot')
		// non-numeric chars removed
		assert.equal(instance.clean_value('12abc3'), '123', 'expected non-numeric chars removed')
		// null/empty returns null
		assert.equal(instance.clean_value(''), null, 'expected null for empty string')
		assert.equal(instance.clean_value(null), null, 'expected null for null input')
	});



	it(`fix_number_format returns number or null`, async function() {

		// valid float
		const result1 = instance.fix_number_format('85.3568')
		assert.isOk(typeof result1 === 'number', 'expected number from fix_number_format')
		// null/undefined/empty returns null
		assert.equal(instance.fix_number_format(null), null, 'expected null for null')
		assert.equal(instance.fix_number_format(''), null, 'expected null for empty string')
		assert.equal(instance.fix_number_format(undefined), null, 'expected null for undefined')
	});



	it(`get_steps returns number`, async function() {

		const steps = instance.get_steps()
		assert.isOk(typeof steps === 'number', 'expected number from get_steps')
	});
...
describe(`COMPONENT_NUMBER SEARCH DATA OPERATIONS`, function() {
	...
	it(`search mode has q_operator DOM element`, async function() {
		...
		// q_operator is a DOM input element, not a data property
		const q_operator = node.querySelector('.q_operator')
