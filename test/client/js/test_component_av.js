...
// element options for component_av
	const element = elements.find(el => el.model==='component_av')
	if (!element) {
		console.error('Error: component_av not found in elements');
	}
...
// mode/view matrix for component_av
// edit: default, line, print, player, viewer
...
describe(`COMPONENT_AV LIFECYCLE`, function() {
		...
		describe(`${pair.mode} / ${pair.view}`, function() {
			...
			it(`init → build → render`, async function() {
				...
				// init
					...
					assert.equal(instance.model, 'component_av', 'model expected component_av')
			...
			it(`render output structure for ${pair.mode}/${pair.view}`, async function() {
				...
				// mode-specific assertions
					if (pair.mode==='edit' && pair.view==='default') {
						// edit default: content_data with av player
					...
					if (pair.mode==='edit' && pair.view==='player') {
						// edit player: wrapper with player view
						const content_data = node.querySelector('.content_data')
						assert.isOk(content_data, 'expected content_data in edit/player')
					}

					if (pair.mode==='edit' && pair.view==='viewer') {
						// edit viewer: wrapper_component with view_viewer class
...
describe(`COMPONENT_AV DATA OPERATIONS`, function() {
	...
	it(`change data via change_value (update)`, async function() {
		...
		if (entries.length > 0) {
			...
			updated_entry.original_file_name = 'updated_test_file.mp4'
...
describe(`COMPONENT_AV SEARCH DATA OPERATIONS`, function() {
