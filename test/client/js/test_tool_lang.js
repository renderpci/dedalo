...
/**
 * TEST_TOOL_LANG
 * Client-side coverage for the component translation tool.
 *
 * The tool's deeper init/build/render path needs a host section with a configured
 * text component (main_element ddo_map entry), the browser IndexedDB 'status' table,
 * and the lazily-imported transformers.js / TranslateGemma worker, none of which is
...
describe('TOOL_LANG CLIENT TEST', function() {

	this.timeout(10000)

	it('module exports the tool constructor', function() {
		assert.equal(typeof tool_lang, 'function', 'expected tool_lang to be a constructor function')
	})

	it('construct seeds the documented instance properties', function() {
		const instance = new tool_lang()
	...
	it('prototype is wired with the lifecycle methods', function() {
		...
		assert.equal(typeof tool_lang.prototype.render, 'function', 'expected render wired')
		assert.equal(typeof tool_lang.prototype.destroy, 'function', 'expected destroy wired')
		assert.equal(typeof tool_lang.prototype.refresh, 'function', 'expected refresh wired')
		// render mode delegated to render_tool_lang
		assert.equal(typeof tool_lang.prototype.edit, 'function', 'expected edit wired')
		// tool-specific overrides defined on the module
		assert.equal(typeof tool_lang.prototype.init, 'function', 'expected init defined')
		assert.equal(typeof tool_lang.prototype.build, 'function', 'expected build defined')
		assert.equal(typeof tool_lang.prototype.automatic_translation_browser, 'function', 'expected automatic_translation_browser defined')
		assert.equal(typeof tool_lang.prototype.automatic_translation_server, 'function', 'expected automatic_translation_server defined')
	})
