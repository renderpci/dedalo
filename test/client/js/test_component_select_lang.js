...
// element options for component_select_lang
	const element = elements.find(el => el.model==='component_select_lang')
	if (!element) {
		console.error('Error: component_select_lang not found in elements');
	}
...
// mode/view matrix for component_select_lang
// edit: default, line, print
// list: default, mini, text
// search: default
// Note: component_select_lang is an alias of component_select in JS,
// so it shares the same render modules and lifecycle methods
...
describe(`COMPONENT_SELECT_LANG LIFECYCLE`, function() {
		...
		describe(`${pair.mode} / ${pair.view}`, function() {
			...
			it(`init → build → render`, async function() {
				...
				// init
					...
					assert.equal(instance.model, 'component_select_lang', 'model expected component_select_lang')
...
describe(`COMPONENT_SELECT_LANG DATA OPERATIONS`, function() {
	...
	it(`select change value (add/change data)`, async function() {
	...
	it(`select change to empty (remove data)`, async function() {
...
describe(`COMPONENT_SELECT_LANG PERMISSIONS`, function() {

	this.timeout(10000);



	it(`permissions 0 → no render`, async function() {
		...
		// inject permissions
		instance.permissions = 0

		const node = await instance.render()

		assert.equal(instance.status, 'built', 'status expected built with permissions 0')
		assert.isOk(node !== null, 'node expected not null even with permissions 0')
	...
	it(`permissions 1 → read only`, async function() {
		...
		// inject permissions
		instance.permissions = 1

		const node = await instance.render()

		assert.equal(instance.status, 'rendered', 'status expected rendered with permissions 1')
		assert.isOk(node !== null, 'node expected not null')
		// no buttons_container with permissions 1
		assert.equal(
			node.querySelector(':scope >.buttons_container'),
			null,
		...
		assert.notEqual(
			...
			null,
			'content_data must exist with permissions 1'
		)
	...
	it(`permissions 2 → full edit`, async function() {
		...
		assert.equal(instance.status, 'rendered', 'status expected rendered with permissions 2')
		assert.isOk(node !== null, 'node expected not null')
		// buttons_container exists with permissions 2
		assert.notEqual(
			node.querySelector(':scope >.buttons_container'),
			null,
			'buttons_container must exist with permissions 2'
		)
		// content_data exists
		assert.notEqual(
			node.querySelector(':scope >.content_data'),
			null,
			'content_data must exist with permissions 2'
		)
		// view_default class
		assert.equal(
			node.classList.contains('view_default'),
			true,
			'wrapper classList must contain "view_default"'
		)
		// note: component_select_lang does NOT show button_add
		// (view_default_edit_select.js line 359: self.model !== 'component_select_lang')
...
describe(`COMPONENT_SELECT_LANG SPECIFIC BEHAVIORS`, function() {

	this.timeout(10000);



	it(`no button_add for component_select_lang`, async function() {
		...
		const options = {
			...
			mode			: 'edit',
			view			: 'default',
			id_variant		: 'no_btn_add_' + Math.random()
		}
		...
		// component_select_lang should NOT have button_add
		// (unlike component_select which can have it)
		const button_add = node.querySelector('.button.add')
		assert.equal(button_add, null, 'button_add should NOT exist for component_select_lang')
	...
	it(`datalist contains language options`, async function() {
		...
		const options = {
			...
			mode			: 'edit',
			view			: 'default',
			id_variant		: 'datalist_' + Math.random()
		}
		...
		// datalist should contain language entries
	...
	it(`set_lang_value event is published on change`, async function() {
		...
		// listen for set_lang_value event
