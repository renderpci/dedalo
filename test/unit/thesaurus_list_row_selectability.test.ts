 ...
 *
 * Scratch hygiene: the two test6022 rows are minted here at reserved high ids and
 * dropped in afterAll — the clone carries no fixture rows of its own.
 */
// Migrated to the generic `test` TLD 2026-08-20 (AGENTS.md hard rule). The twin had to
// be a section that DECLARES the contract under test: `test6022`'s section_map
// (`test6096`) names `thesaurus.is_indexable` = `test6094`, the same shape the install
// thesaurus this regression was found on declared. Its records live in `matrix_test`,
// the clone's own table.
...
// Reserved scratch ids on test6022 (the thesaurus twin): one term flagged
// indexable, one not.
...
describe.if(hasDb)('list-row selectability (thesaurus picker)', () => {
	beforeAll(async () => {
		...
		await sql.unsafe(`
			...
			VALUES
				(${SCRATCH_INDEXABLE}, 'test6022', '{"label":"scratch indexable"}'::jsonb,
					'{"test6094":[{"section_tipo":"dd784","section_id":1}]}'::jsonb),
				(${SCRATCH_PLAIN}, 'test6022', '{"label":"scratch plain"}'::jsonb, '{}'::jsonb)
	...
	afterAll(async () => {
		await sql`
			...
			WHERE section_tipo = 'test6022' AND section_id IN (${SCRATCH_INDEXABLE}, ${SCRATCH_PLAIN})
		`;
	});

	test('test6022 declares the contract, so every entry carries the server answer', async () => {
		const thesaurus = (await getSectionMap('test6022'))?.thesaurus as { is_indexable?: unknown };
		expect(typeof thesaurus?.is_indexable).toBe('string');

		const entries = await readEntries('test6022', 'list_thesaurus');
	...
	test('the answer is the term’s own flag, not a blanket true/false', async () => {
		...
		// The flagged scratch term reads TRUE (test6094 → section_id 1 = "Yes");
	...
	test('plain list mode gets the same answer (a picker list is not always list_thesaurus)', async () => {
		const entries = await readEntries('test6022', 'list');
		const byId = new Map(entries.map((entry) => [entry.section_id, entry.is_indexable]));
		expect(byId.get(SCRATCH_INDEXABLE)).toBe(true);
	});
	...
	test('the two keys are mutually exclusive — a declaring section never sends the exemption', async () => {
		// test6022 declares the contract, so its rows answer per term and must NOT
		// carry the blanket exemption; otherwise a term explicitly flagged
		// NOT indexable would still be offered the arrow.
		expect(await declaresTermSelectability('test6022')).toBe(true);
