 ...
 *
 * Corpus census 39 = the 34 PHP-seeded column-keyed registers + the five
 * TS-only tools (tool_error_report WC-019, tool_sitebuilder, tool_identify,
 * tool_numisdata_acquisition, tool_bibliography_acquisition), which are
 * AUTHORED in the authoring format and must convert+validate instead. The pin
 * is deliberately a hard number: a new tool that forgets to declare which
 * format it ships in is exactly what this census catches (tool_error_report
 * landed 2026-07-10 and tool_sitebuilder 2026-07-15 without bumping it; both
 * reconciled here).
 */
...
describe('seeded register.json corpus', () => {
	...
	test('every tool has a register.json and there are 39 (34 seeded + 5 TS-authored)', () => {
		expect(toolDirs.length).toBe(39);
	});
