...
/**
 * COMPONENT_EMAIL_DATAFRAME_TEST
 * Tests for literal main component (component_email) with dataframe extension.
 * Validates the pairing contract: dataframe rows pair by the item stable `id`,
 * never the array index.
 */
final class component_email_dataframe_test extends BaseTestCase {
	...
	*/
	public function test_data_items_have_id() {
		...
		$component->set_data([
			(object)['value' => 'a@example.org'],
			(object)['value' => 'b@example.org']
		]);
	...
	*/
	public function test_pairing_key_survives_reorder_and_removal() {
		...
		$component->set_data([
			(object)['id' => 1, 'value' => 'a@example.org'],
			(object)['id' => 2, 'value' => 'b@example.org'],
			(object)['id' => 3, 'value' => 'c@example.org']
		]);
		...
		$this->assertEquals('a@example.org', $by_id[1]);
		$this->assertEquals('c@example.org', $by_id[3]);
	...
	*/
	public function test_main_component_data_isolation() {
		...
		$component->set_data([ (object)['value' => 'x@example.org'] ]);
	...
	*/
	public function test_build_dataframe_subdatum_contract() {
		...
		$value = [ (object)['id' => 1, 'value' => 'y@example.org'] ];
...
}//end class component_email_dataframe_test
