...
final class env_sync_Test extends TestCase {
	...
	public function test_compare_reports_no_drift_when_mapped_keys_match() : void {
		$php = ['DEDALO_DIFFUSION_INTERNAL_TOKEN' => 'tok', 'DEDALO_MEDIA_PATH' => '/srv/media'];
		$bun = ['DIFFUSION_INTERNAL_TOKEN' => 'tok', 'DEDALO_MEDIA_PATH' => '/srv/media'];
		$this->assertSame([], env_sync::compare($php, $bun));
	}

	public function test_compare_reports_drift_on_mismatch() : void {
		$php = ['DEDALO_DIFFUSION_INTERNAL_TOKEN' => 'secret'];
		$bun = ['DIFFUSION_INTERNAL_TOKEN' => 'WRONG'];
		$drift = env_sync::compare($php, $bun);
		$this->assertCount(1, $drift);
		$this->assertSame('DEDALO_DIFFUSION_INTERNAL_TOKEN', $drift[0]['php_key']);
		$this->assertSame('DIFFUSION_INTERNAL_TOKEN', $drift[0]['bun_key']);
	}
