...
/**
* TOOL_INDEXATION_TEST
* Server-side coverage for the indexation tool.
*
* tool_indexation is a purely UI-driven tool: the entire workflow (split-pane
* rendering, tag overlays, locator persistence) runs in the browser
* (tool_indexation.js / render_tool_indexation.js / tag_note.js) through the generic
* core data API. The server class is a thin tool_common registration stub that exposes
* NO callable API actions (empty API_ACTIONS allowlist). Coverage here therefore
* focuses on:
...
*/
final class tool_indexation_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_indexation(1, 'dd1324');

		$this->assertSame(
			'tool_indexation',
			get_class($tool),
			'expected get_class to be tool_indexation and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_indexation.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_indexation', $context->model, 'expected model is tool_indexation');
	...
	* tool_indexation exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_indexation::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_indexation::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_indexation');
		foreach (tool_indexation::API_ACTIONS as $key => $value) {
...
}//end class tool_indexation_test
