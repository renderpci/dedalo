...
/**
* TOOL_PRINT_TEST
* Server-side coverage for the visual print-layout / report designer tool.
*
* tool_print is a purely UI-driven tool for the browser-print MVP: all layout,
* rendering and persistence run in the browser (tool_print.js, flow_engine.js,
* canvas_tool_print.js, print_layout_presets.js) through the generic core data
* API. The server side therefore exposes NO callable API actions (empty
* API_ACTIONS allowlist; phase-2 generate_pdf is kept as commented dead code).
...
*/
final class tool_print_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_print(1, 'dd1324');

		$this->assertSame(
			'tool_print',
			get_class($tool),
			'expected get_class to be tool_print and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_print.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_print', $context->model, 'expected model is tool_print');
	...
	* tool_print exposes no server-side callable methods in the browser-print MVP.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_print::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_print::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_print');
		foreach (tool_print::API_ACTIONS as $key => $value) {
