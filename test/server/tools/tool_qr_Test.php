...
/**
* TOOL_QR_TEST
* Server-side coverage for the QR-code generation tool.
*
* tool_qr is a purely UI-driven tool: all rendering runs in the browser
* (tool_qr.js / render_tool_qr.js) and the server side exposes NO callable
* API actions (empty API_ACTIONS allowlist). Coverage here therefore focuses on:
...
*
* Follows the golden patterns in tool_common_Test.php and tool_security_Test.php.
*/
final class tool_qr_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_qr(1, 'dd1324');

		$this->assertSame(
			'tool_qr',
			get_class($tool),
			'expected get_class to be tool_qr and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_qr.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_qr', $context->model, 'expected model is tool_qr');
	...
	* tool_qr exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_qr::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_qr::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_qr');
		foreach (tool_qr::API_ACTIONS as $key => $value) {
