...
/**
* TOOL_NUMISDATA_EPIGRAPHY_TEST
* Server-side coverage for the numismatic-epigraphy transcription tool.
*
* tool_numisdata_epigraphy is a purely UI-driven tool: all rendering and the full
* lifecycle (component loading, relation queries, glyph transcription, saves) run
* in the browser (tool_numisdata_epigraphy.js / render_tool_numisdata_epigraphy.js)
* and the server side exposes NO callable API actions (empty API_ACTIONS allowlist).
...
*/
final class tool_numisdata_epigraphy_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_numisdata_epigraphy(1, 'dd1324');

		$this->assertSame(
			'tool_numisdata_epigraphy',
			get_class($tool),
			'expected get_class to be tool_numisdata_epigraphy and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_numisdata_epigraphy.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_numisdata_epigraphy', $context->model, 'expected model is tool_numisdata_epigraphy');
	...
	* tool_numisdata_epigraphy exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_numisdata_epigraphy::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_numisdata_epigraphy::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_numisdata_epigraphy');
		foreach (tool_numisdata_epigraphy::API_ACTIONS as $key => $value) {
	...
	*/
	public function test_tool_request_refuses_action() : void {
		...
		$rqo = (object)[
			...
			'action'	=> 'tool_request',
			'source'	=> (object)[
				'model'		=> 'tool_numisdata_epigraphy',
				'action'	=> 'get_json'
			],
...
}//end class tool_numisdata_epigraphy_test
