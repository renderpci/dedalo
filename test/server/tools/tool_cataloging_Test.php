...
/**
* TOOL_CATALOGING_TEST
* Server-side coverage for the hierarchical cataloging tool.
*
* tool_cataloging is a purely UI-driven tool: the entire drag-and-drop workflow
* (mosaic render, thesaurus interaction, portal update) runs in the browser
* (tool_cataloging.js / render_tool_cataloging.js) and the server side exposes NO
* callable API actions (empty API_ACTIONS allowlist). Coverage here therefore
* focuses on:
...
*/
final class tool_cataloging_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_cataloging(1, 'dd1324');

		$this->assertSame(
			'tool_cataloging',
			get_class($tool),
			'expected get_class to be tool_cataloging and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_cataloging.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_cataloging', $context->model, 'expected model is tool_cataloging');
	...
	* tool_cataloging exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_cataloging::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_cataloging::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_cataloging');
		foreach (tool_cataloging::API_ACTIONS as $key => $value) {
...
}//end class tool_cataloging_test
