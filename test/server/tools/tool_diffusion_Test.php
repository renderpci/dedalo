...
/**
* TOOL_DIFFUSION_TEST
* Server-side coverage for the diffusion tool.
*
* tool_diffusion is a purely UI-driven tool: all diffusion work (publishing
* records to SQL/RDF/XML targets) runs in the browser (tool_diffusion.js) and
* through the Bun diffusion server. The PHP class exposes NO callable API actions
* (empty API_ACTIONS allowlist) and only implements the is_available() lifecycle
* hook. Coverage here therefore focuses on:
...
*/
final class tool_diffusion_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_diffusion(1, 'dd1324');

		$this->assertSame(
			'tool_diffusion',
			get_class($tool),
			'expected get_class to be tool_diffusion and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_diffusion.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_diffusion', $context->model, 'expected model is tool_diffusion');
	...
	* tool_diffusion exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_diffusion::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_diffusion::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_diffusion');
		foreach (tool_diffusion::API_ACTIONS as $key => $value) {
