...
/**
* TOOL_SUBTITLES_TEST
* Server-side coverage for the subtitles editing tool.
*
* tool_subtitles is a purely UI-driven tool: all subtitle editing, TC parsing,
* and VTT generation happen in the browser (tool_subtitles.js /
* render_tool_subtitles.js) and the server side exposes NO callable API actions
* (empty API_ACTIONS allowlist, SEC-024 §9.2). Coverage here therefore focuses on:
...
*/
final class tool_subtitles_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_subtitles(1, 'dd1324');

		$this->assertSame(
			'tool_subtitles',
			get_class($tool),
			'expected get_class to be tool_subtitles and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_subtitles.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_subtitles', $context->model, 'expected model is tool_subtitles');
	...
	* tool_subtitles exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_subtitles::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_subtitles::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_subtitles');
		foreach (tool_subtitles::API_ACTIONS as $key => $value) {
