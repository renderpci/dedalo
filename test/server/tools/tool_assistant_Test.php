...
/**
* TOOL_ASSISTANT_TEST
* Server-side coverage for the AI Assistant tool.
*
* tool_assistant is a purely UI-driven tool: all interaction runs in the browser
* (tool_assistant.js / render_tool_assistant.js / ai_assistant.js) and the server
* side exposes NO callable API actions (empty API_ACTIONS allowlist). All data
* operations — including MCP tool calls — are routed through dd_mcp_api, not
* dd_tools_api. Coverage here therefore focuses on:
...
*/
final class tool_assistant_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_assistant(1, 'dd1324');

		$this->assertSame(
			'tool_assistant',
			get_class($tool),
			'expected get_class to be tool_assistant and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_assistant.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_assistant', $context->model, 'expected model is tool_assistant');
	...
	* tool_assistant exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_assistant::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_assistant::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_assistant');
		foreach (tool_assistant::API_ACTIONS as $key => $value) {
