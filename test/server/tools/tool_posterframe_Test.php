...
/**
* TOOL_POSTERFRAME_TEST
* Server-side coverage for the posterframe extraction tool.
*
* tool_posterframe exposes a non-empty SEC-024 API_ACTIONS allowlist with two
* callable actions (create_identifying_image, get_ar_identifying_image). Both
* are DB/FFmpeg-backed and require a live AV record + section scope, so this
* suite does NOT exercise their business logic. Coverage focuses on the stable,
* fixture-free contract:
*   - construct + dd_object context shape (get_json / get_structure_context)
*   - the API_ACTIONS contract (every entry is public static)
*   - tool_security::resolve_action allows listed actions and refuses unknowns
...
*/
final class tool_posterframe_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_posterframe(1, 'dd1324');

		$this->assertSame(
			'tool_posterframe',
			get_class($tool),
			'expected get_class to be tool_posterframe and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_posterframe.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_posterframe', $context->model, 'expected model is tool_posterframe');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_posterframe');
		foreach (tool_posterframe::API_ACTIONS as $key => $value) {
	...
	* SEC-024: tool_security::resolve_action accepts a real listed action and
	* refuses a bogus one (fail closed).
	* @return void
	*/
	public function test_resolve_action_allows_listed_and_refuses_unknown() : void {

		$ok = tool_security::resolve_action('tool_posterframe', 'create_identifying_image');
		$this->assertTrue($ok->ok, 'expected listed action create_identifying_image allowed');

		$ko = tool_security::resolve_action('tool_posterframe', 'not_a_real_action');
	...
	* allowlist (e.g. get_json) is refused before reaching business logic.
	* @return void
	*/
...
}//end class tool_posterframe_test
