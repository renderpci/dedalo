...
/**
* TOOL_USER_ADMIN_TEST
* Server-side coverage for the self-service user-profile editor tool.
*
* tool_user_admin is a purely UI-driven tool: all rendering and the live dd128
* (users) section / component wiring run in the browser (tool_user_admin.js /
* render_tool_user_admin.js) and the server side exposes NO callable API actions
* (empty API_ACTIONS allowlist). Coverage here therefore focuses on:
...
*/
final class tool_user_admin_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_user_admin(1, 'dd1324');

		$this->assertSame(
			'tool_user_admin',
			get_class($tool),
			'expected get_class to be tool_user_admin and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_user_admin.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_user_admin', $context->model, 'expected model is tool_user_admin');
	...
	* tool_user_admin exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_user_admin::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_user_admin::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_user_admin');
		foreach (tool_user_admin::API_ACTIONS as $key => $value) {
...
}//end class tool_user_admin_test
