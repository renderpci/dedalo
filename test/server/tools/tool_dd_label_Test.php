...
/**
* TOOL_DD_LABEL_TEST
* Server-side coverage for the multi-language tool-label editor.
*
* tool_dd_label is a purely UI-driven tool: all reading and writing of label
* data runs in the browser (tool_dd_label.js / render_tool_dd_label.js) and the
...
*/
final class tool_dd_label_test extends BaseTestCase {
	...
	*/
	public function test___construct() : void {
		...
		$tool = new tool_dd_label(1, 'dd1324');

		$this->assertSame(
			'tool_dd_label',
			get_class($tool),
			'expected get_class to be tool_dd_label and is : '.get_class($tool)
		);
	...
	* The dd_object context is a 'ddo' typed object for model tool_dd_label.
	* @return void
	*/
	public function test_get_structure_context() : void {
		...
		$this->assertEquals('tool_dd_label', $context->model, 'expected model is tool_dd_label');
	...
	* tool_dd_label exposes no server-side callable methods.
	* @return void
	*/
	public function test_api_actions_is_empty() : void {

		$this->assertIsArray(tool_dd_label::API_ACTIONS, 'expected API_ACTIONS to be an array');
		$this->assertSame([], tool_dd_label::API_ACTIONS, 'expected empty API_ACTIONS allowlist');
	...
	*/
	public function test_api_actions_are_public_static() : void {
		...
		$reflection = new ReflectionClass('tool_dd_label');
		foreach (tool_dd_label::API_ACTIONS as $key => $value) {
