...
const ACTIVITY: MatrixTablePolicy = {
	...
	entries: [
		// --- keep (load-bearing) -------------------------------------------------
		{
			...
			disposition: 'keep',
			reason: 'UNIQUE key — enables the WC-044 flattened SQL and the deep-page order-flip; the flip/late page scan is index-only on it.',
		},
		{
			signature: 'using btree (section_tipo, section_id desc)',
			disposition: 'keep',
			reason: 'Primary list index — newest-first browse (section_id DESC) and the flip page scan, index-only.',
		},
		{
		...
		// --- drop-redundant (covered by a kept index) ----------------------------
		{
			...
			reason: 'Prefix-superset of the (section_tipo, section_id DESC) list index; the trailing timestamp serves no emitted shape.',
		},
		{
			signature: 'using btree (section_tipo, "timestamp", section_id)',
			disposition: 'drop-redundant',
			requiresSingleTipo: true,
			reason: 'Single-tipo table ⇒ ≡ (timestamp, section_id); the (timestamp, id) INCLUDE index already covers timestamp-ordered scans.',
		},
		{
		...
		...['data', 'relation', 'relation_search', 'string', 'date', 'iri', 'geo', 'number', 'media', 'misc'].map(
...
const TIME_MACHINE: MatrixTablePolicy = {
	...
	entries: [
		...
		},
		{
			...
			disposition: 'drop-dead',
			reason: 'A standalone lang index is far too low-selectivity to be chosen; lang narrows only alongside (section_tipo, section_id).',
		},
		{
			signature: 'using brin (date("timestamp"))',
			disposition: 'drop-dead',
		...
		// --- review (plausibly useful; report, opt-in to drop) -------------------
		{
			...
			disposition: 'review',
			reason: 'The "search default" composite — oversized (multi-GB) for its scan count; confirm no history-search shape needs it before dropping.',
		},
		{
			signature: 'using btree (bulk_process_id)',
			disposition: 'review',
			reason: 'Bulk-process inspection filter — semantically real but cold; operator opt-in to drop.',
		},
		{
...
export function classifyIndex(
	...
	if (entry.isConstraint) {
		return { index: entry, signature, disposition: 'keep', action: 'keep', reason: 'constraint-backed (PK/UNIQUE) — not DROP INDEX-able.' };
	}
	const match = policy.entries.find((policyEntry) => policyEntry.signature === signature);
	if (match === undefined) {
		return { index: entry, signature, disposition: 'unclassified', action: 'unclassified', reason: 'not in policy — left in place for review.' };
	}
	...
	if (disposition === 'drop-redundant' && match.requiresSingleTipo === true && !options.singleTipo) {
		return { index: entry, signature, disposition: 'review', action: 'review', reason: `${match.reason} (DOWNGRADED: table is not single-tipo, so this is not redundant.)` };
	}
	if (disposition === 'keep') return { index: entry, signature, disposition, action: 'keep', reason: match.reason };
	if (disposition === 'drop-redundant') return { index: entry, signature, disposition, action: 'drop', reason: match.reason };
	if (disposition === 'drop-dead') {
		...
		if (entry.idxScan > 0) {
			return { index: entry, signature, disposition, action: 'review', reason: `${match.reason} (KEPT: ${entry.idxScan} scans on this DB — unexpectedly used.)` };
		}
		return { index: entry, signature, disposition, action: 'drop', reason: match.reason };
	}
	// review
	return { index: entry, signature, disposition, action: options.includeReview ? 'drop' : 'review', reason: match.reason };
}
