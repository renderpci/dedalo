 ...
 * lock-free. `allowShrink:false` refuses (inside the lock — no TOCTOU) a
 * recompute that would DROP entries, returning `skippedShrink` for the
 * caller to report.
 */
export async function recomputeExternalRelation(
	...
	userId: number,
	now: Date,
	options: { write?: boolean; allowShrink?: boolean } = {},
): Promise<{ changed: boolean; before: number; after: number; skippedShrink?: boolean }> {
	...
	if (typeof componentToSearch !== 'string') return unchanged;

	const table = await getMatrixTableFromTipo(targetSection);
	if (table === null) return unchanged;

	// The recompute law over one consistent snapshot: keep existing entries
	// still referenced (stored order), append new references with the next
	// item ids (PHP save id assignment).
	const compute = async (
		lock: boolean,
	): Promise<{ exists: boolean; existing: StoredLocator[]; finalData: StoredLocator[] }> => {
		...
		if (rows.length === 0) return { exists: false, existing: [], finalData: [] };
		...
		return { exists: true, existing, finalData };
	};

	// Dry run: diff only, no lock, no writes.
	if (options.write === false) {
		const { exists, existing, finalData } = await compute(false);
		if (!exists) return unchanged;
		return {
			changed: JSON.stringify(finalData) !== JSON.stringify(existing),
	...
	return withTransaction(async () => {
		const { exists, existing, finalData } = await compute(true);
		if (!exists) return unchanged; // target record does not exist
		// No change → no write (PHP re-saves anyway; we skip the no-op to
		// avoid TM noise — the stored VALUE converges either way).
		if (JSON.stringify(finalData) === JSON.stringify(existing)) {
			return { changed: false, before: existing.length, after: finalData.length };
		}
		if (options.allowShrink === false && finalData.length < existing.length) {
			return {
				changed: true,
				before: existing.length,
				after: finalData.length,
		...
		return { changed: true, before: existing.length, after: finalData.length };
	});
}
