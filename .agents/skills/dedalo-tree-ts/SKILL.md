...
- icon `M` → also stamps `model_value` (PHP `:518` `$component->get_value()`), the orange model badge area_ontology toggles with **Ctrl+M**. Gated on the RESOLVED value `'M'`, never the ddo `icon` key (so `component_relation_index`'s `"M:3"` cannot match). Resolution deliberately BYPASSES the target's `thesaurus` term scope (which yields the bare tld for a model record): raw locators off the record → the component's OWN `properties.source.request_config[].show.ddo_map` (`section_tipo:'self'`) tipos → `getDdoValueByLocator` with the WIDER grid fallback (data-lang → main → lg-nolan → any, translatable only — model names live in ONE lang, so a strict read empties every badge). Two live sites: `ontology6` (ontology tree), `hierarchy27` (hierarchies tree).
...
- `parseChildData` assumes HOMOGENEOUS children (order tipo from FIRST locator); it is now the section_id CANONICALIZATION BOUNDARY (WC-2026-08-10-section-id-int-canonical: `canonicalizeStoredSectionId` + `isSectionId`, non-addresses skipped) — downstream ts_object code takes plain `number`; dedup/prefetch keys use `${tipo}_${int(id)}`. dd_ts_api RQO-body ids coerce at the door via `coerceSectionId` (`rqo.dd_ts_api.<action>.<field>` counters).
...
## Testing
Unit: `test/unit/{tld,with_transaction,node_lock_coexistence,ts_tree_semantics,ts_tree_db_semantics}.test.ts`. Differentials vs live PHP: `test/parity/ts_{node_read,search,mutations}_differential.test.ts` + `ts_mutations_hardening.test.ts` (concurrency: 2 racing add_child → distinct ids + distinct sibling orders). Fixture tree = `tchi1` (children_tipo `tchi40`, node 602 descriptor, 620 parent) — **absent from `dedalo7ts_test`**, so those cases red on an install without the monedaiberica dataset; baseline before blaming a change. The `model_value` gate uses install ontology instead (`dd0/35`→`area_tool`, asserted in 3 langs). Client side: `client/dedalo/test/client/js/test_ts_object.js` "Model badge" (badge text + the `hide`/`show_models` toggle), run via `bun run test:client`. Differentials do two round-trips — pass `--timeout 60000`. Write tests self-revert (create→read-back→delete); NEVER leave scratch records.
