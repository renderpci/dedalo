...
**Read this first for any `src/` work.** Dédalo v7 TS is a from-scratch rewrite of the PHP monolith (`v7_php_frozen/master_dedalo`) on Bun 1.4.2 + strict TypeScript. During coexistence the **PHP server is a READ-ONLY oracle running on the SAME Postgres** — every TS behavior is verified differentially against it, and PHP can re-parse any row TS writes. Authoritative overview: `engineering/REWRITE_SPEC.md`. **Live measured state (where we are right now): `rewrite/LEDGER.md`.** Historical narrative (how we got here): `rewrite/STATUS.md`.
