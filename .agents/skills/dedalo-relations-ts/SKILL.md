...
description: The Dédalo v7 TypeScript/Bun rewrite of the RELATION component family — the strangler-fig subsystem in src/core/relations/ (registry dispatch, relation_core shared engine, per-model resolvers, v5/v6 request_config builders, dataframe id_key pairing, inverse/children/related/index engines, save hooks). Use when editing src/core/relations/**, src/core/resolve/read_rows.ts relation branches, src/core/search/conform.ts relation dispatch, or debugging why a portal/autocomplete/dataframe/children/index/related component resolves wrong vs the PHP oracle. Authoritative spec: docs/RELATIONS_SPEC.md; ledger: docs/STATUS.md "Relations rebuild".
---

# Dédalo v7 relations (TypeScript rewrite)

The relation family is being rewritten from the PHP monolith (`v7/master_dedalo`, read-only oracle) into `src/core/relations/`. PHP is the oracle: every behavior is verified differentially against the live PHP server (`dedalo-parity-debugging` skill). Never silently narrow scope — throw loudly + ledger the gap in `docs/STATUS.md`.
...
The five exports of `src/core/resolve/read_rows.ts` (`readSection`, `readComponentData`, `resolveSearchData`, `readSectionRows`, `buildGetDataContext`) keep their signatures; bodies delegate to the registry. Relation branches are DELETED from the monolith as each family migrates — emptied, never wrapped. Layering (no cycles): `relations/` imports `concepts/`, `ontology/`, `db/`, `search/search_related.ts`, `search/builders/*`; `read_rows.ts` / `save_component.ts` / `conform.ts` import `relations/registry.ts`. Child recursion goes through the `emitDdo` CALLBACK handed into resolvers — so `relations/` never imports `read_rows`.
...
src/core/relations/
  ...
  request_config/{build,v5,v6,external,filters}.ts
```
...
1. **`section_tipo: 'self'` resolves to the SQO TARGET sections, not the caller** (`resolve_ddo_self_references :250-255`). Caller scalar only for dataframes; `undefined` stays untouched. Getting this wrong made every self-declaring child skip the per-locator grouping → autocomplete cells emitted no subdatum. (`request_config/v6.ts processSingleDdo`.)
...
6. **`get_data` serves ANY component, not just relations.** The autocomplete_hi edit-in-place widget refreshes the chosen term's `component_input_text` value via `get_data`. `readComponentData` routes non-relation models through the generic `emitDdoData` path instead of throwing. (`read_rows.ts`.)
...
- `build.ts` = the v5/v6 data-driven branch (single entry `buildRequestConfigForElement`). v6 = `properties.source.request_config`; v5 = ontology graph walk (no-source components: `numisdata967/71/1562`; legacy source objects: `numisdata55`).
- `v6.ts`: `processSingleDdo` (the self→targets rule #1), `resolveGetDdoMap`, `parseBlock`, dynamic `hierarchy_types` + multi-section targets, implicit-self SQOs (`numisdata36/1006` — no section_tipo, filter_by_list only → resolve to caller), `filter_by_list`/`fixed_filter` expansion (disables cache), external `api_config` attach.
- `v5.ts`: graph-walk targets; parent/children throw (V5_UNSUPPORTED_MODELS); `getMainRelatedSectionTipo`.
