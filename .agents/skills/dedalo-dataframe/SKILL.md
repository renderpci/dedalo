...
2. JS views (default_edit, line_edit, default_list, mini): one call per data item — `attach_item_dataframe({self, item: current_value, container: content_value, view: self.view})`. No-op without `has_dataframe`. String-rendered list/mini views loop `self.data.entries`. Never inside a CKEditor container. **The default_edit dispatch has a read/write split — `(self.permissions===1) ? get_content_value_read(...) : get_content_value(...)`; attach in BOTH branches.** A writable-only attach leaves read-only renders without the dataframe: the Time Machine preview renders the main in edit mode with `permissions=1` (`render_tool_time_machine.js`), and read-only users hit the same path. (Fixed 2026-06-16 for input_text/date/text_area/number/email by attaching in the read-only branch loop; `component_iri`'s `get_content_value_read` already did it.)
3. No remove-flow JS (invariant 4).
4. Activation is ONTOLOGY data, not code: instance `properties->has_dataframe: true` + a `component_dataframe` ddo in request_config `show.ddo_map`; the dataframe node's portal request_config points at the frame target section. **`has_dataframe` is REQUIRED for literal mains but IGNORED by relation mains** — portal/autocomplete/select activate purely via the ddo in `show.ddo_map` (rendered through `section_record`) and never read the flag, so a working relation dataframe with no flag is NOT a template for a literal. For a coloured rating, add a `role:"rating"` ddo (→ a `component_radio_button` with a colour datalist) to the slot's `request_config.hide.ddo_map`. Full ontology walk-through: `docs/core/components/component_dataframe.md` → "Worked example — uncertainty rating on a literal".
...
- **Literal dataframe shows in edit but NOT in Time Machine / read-only**: the literal default-edit views attach per value inside `get_content_value` (writable) AND must also attach in the read-only branch (`get_content_value_read` path). The TM preview renders the main in edit mode with `permissions=1`, so a writable-only attach silently drops the button there and for read-only users. Symptom: the instance has `context.properties.has_dataframe===true` + the dataframe ddo in its RQO, but zero `.component_dataframe` nodes render. Fixed 2026-06-16 by adding a `permissions===1`-gated attach in the dispatch loop of input_text/date/text_area/number/email; `component_iri` already handled it. (Same debug session red herring: a stale browser cache of `core/common/js/events.js` made `component_json` fail to import `fade_in_on_reveal`, breaking the TM grid — a hard reload, not a code change, fixes that.)
