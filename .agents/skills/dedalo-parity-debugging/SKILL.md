...
The rewrite lives in `/Users/render/Desktop/trabajos/dedalo/v7_ts/master_dedalo`. The PHP reference (`v7/master_dedalo`) is READ-ONLY and is the ORACLE: when TS and PHP disagree, PHP is right unless it is a pinned live defect. Master spec `docs/REWRITE_SPEC.md`; per-subsystem specs in `docs/*_SPEC.md`; running ledger `docs/STATUS.md` (house style: ✅ rows cite their gate test; open items carry a blocking reason; NEVER silently narrow scope — throw loudly + ledger).
...
import { readSection } from '/Users/render/.../master_dedalo/src/core/resolve/read_rows.ts';
...
const tsData = await readSection(rqo);
...
When PHP is provably wrong (crashes, ignores documented inputs, corrupts), TS does the CORRECT thing and the gate asserts the divergence on BOTH sides (an asymmetric/pin test), with a `(!)` note + reproduction in `docs/STATUS.md`. Never silently match a defect. Examples accumulated: TM read ignores `sqo.filter`; `component_calculation` READ crashes on unstored value (`array_sum`); counters double-unwrap kills `modify_counter`; the PHP install's backup cron produces ZERO-BYTE dumps (pg_dump 17 vs server 18); DROP…CASCADE footgun in `rebuild_db_functions`. If you find a new one, PIN it (assert both behaviors) and ledger it numbered.
...
- Update `docs/STATUS.md` in the same commit as the fix — the ledger is the source of truth for what is/isn't covered.
