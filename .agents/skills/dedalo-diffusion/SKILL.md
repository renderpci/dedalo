...
- **Activity log**: section dd1758 (`matrix_activity_diffusion`, PostgreSQL). Components: dd1762 user, dd1761 date, dd1763 locator, dd1764 section_id, dd1765 section_tipo, dd1766 diffusion element, **dd1767 action** (component_select → value-list section dd1774: 1=published, 2=unpublished, 3=unpublish_pending). Query pending rows with `matrix_db_manager::search` using JSONB `@>` on the `relation` column; flip rows in place with `update()`.
- **Publication state per record**: `component_publication` (yes/no) + metadata components dd271/dd1223/dd1224/dd1225 set by `diffusion_utils::update_publication_data()`.
