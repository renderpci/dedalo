	...
	protected function load_structure_data() : bool {
		...
		if( !$this->bl_loaded_structure_data) {
			...
			// fix vars
				...
				$this->label	= ontology_node::get_label_by_tipo($this->tipo,DEDALO_APPLICATION_LANG,true);		#echo 'DEDALO_APPLICATION_LANG: '.DEDALO_APPLICATION_LANG ;#var_dump($this->label);	#die();
				...
				default:
					// try related. If section have TR of model name 'matrix_table' takes its matrix_table value
						...
						if ( isset($ar_related[0]) ) {
							...
							$matrix_table = ontology_node::get_label_by_tipo($ar_related[0], DEDALO_STRUCTURE_LANG, true);
						}
	...
	public static function get_main_lang( ?string $section_tipo, mixed $section_id=null ) : string {
		...
		if ($section_tipo===DEDALO_HIERARCHY_SECTION_TIPO) {
			...
			if (!is_null($section_id)) {
				...
				$model_name	= ontology_node::get_model_name_by_tipo(DEDALO_HIERARCHY_LANG_TIPO,true);
	...
	*/
	public static function get_ar_related_by_model(string $model_name, string $tipo, bool $strict=true) : array {
		...
		foreach ((array)$relations as $relation) foreach ((array)$relation as $current_tipo) {
			...
			$current_model_name = ontology_node::get_model_name_by_tipo($current_tipo, true);
			if ($strict===true) {
					...
					if (isset($properties->state_of_component)) {
						foreach ($properties->state_of_component as $soc_key => $soc_value) {
							if (isset($soc_value->target_component) && isset($soc_value->msg)) {
								...
								$component_name	= ontology_node::get_label_by_tipo($soc_value->target_component, DEDALO_APPLICATION_LANG, true);
			...
			// search
				if ($this->mode==='search') {
					...
					if (isset($parent_grouper)) {
						...
						$dd_object->config->parent_grouper_label = ontology_node::get_label_by_tipo($parent_grouper);
					}
				}
			...
			foreach ($request_config as $request_config_object) {
				...
				if(empty($request_config_object->show->ddo_map)) {
					debug_log(__METHOD__
						...
						. ' (tipo: '.$this->tipo.' - '. ontology_node::get_label_by_tipo($this->tipo) .')' . PHP_EOL
				...
				foreach($ar_ddo as $dd_object) {
					...
					// short vars
						...
						$model			= ( isset($dd_object->model) )
							? $dd_object->model
							: ontology_node::get_model_name_by_tipo($current_tipo, true);
						...
						switch (true) {
							...
							default:
								debug_log(__METHOD__
									...
									. ontology_node::get_label_by_tipo($current_tipo)
									, logger::WARNING
								);
								break;
						}//end switch (true)
					...
					// consolidate ddo items properties
						...
						foreach ($requested_show->ddo_map as $key => $current_ddo) {
							...
							if(!isset($new_ddo->label)) {
								$new_ddo->label = ontology_node::get_label_by_tipo($new_ddo->tipo, DEDALO_APPLICATION_LANG, true, true);
							}
	...
	public function get_ar_request_config() : array {
		...
		// check section tipo model (allow areas)
			if ($section_tipo!=='self') {
				$section_model = ontology_node::get_model_name_by_tipo($section_tipo,true);
						...
						// parsed_item (section_tipo). normalized ddo with tipo and label
							$parsed_item->sqo->section_tipo = array_map(function($current_section_tipo){
								$ddo = new dd_object();
									...
									$ddo->set_label(ontology_node::get_label_by_tipo($current_section_tipo, DEDALO_APPLICATION_LANG, true, true));
						...
						if (isset($item_request_config->search)) {
							...
							if($ar_search_ddo_map){
								...
								foreach ($ar_search_ddo_map as $current_search_ddo_map) {
									...
									// check valid tipo (The model is unsolvable)
										...
										if ( $tipo_is_valid === false ) {
											debug_log(__METHOD__
												...
												.' this->section_tipo: ' . $this->section_tipo . PHP_EOL
												.' this->section_id: ' . $this->section_id . PHP_EOL
												.' current_model: ' . ontology_node::get_model_name_by_tipo($current_search_ddo_map->tipo)
												, logger::WARNING
											);
											continue;
										}
									...
									// label. Add to all ddo_map items
										$current_search_ddo_map->label = ontology_node::get_label_by_tipo($current_search_ddo_map->tipo, DEDALO_APPLICATION_LANG, true, true);
					...
					// choose
						if (isset($item_request_config->choose)) {
							...
							foreach ($choose_ddo_map as $current_choose_ddo) {
								...
								// check valid tipo (The model is unsolvable)
									...
									if ( $tipo_is_valid === false ) {
										debug_log(__METHOD__
											...
											.' this->section_tipo: ' . $this->section_tipo . PHP_EOL
											.' this->section_id: ' . $this->section_id . PHP_EOL
											.' current_model: ' . ontology_node::get_model_name_by_tipo($current_choose_ddo->tipo)
											, logger::WARNING
										);
										continue;
									}
								...
								// model. Calculated always to prevent errors
									$current_choose_ddo->model = ontology_node::get_model_name_by_tipo($current_choose_ddo->tipo, true);
								...
								// label. Add to all ddo_map items
									$current_choose_ddo->label = ontology_node::get_label_by_tipo($current_choose_ddo->tipo, DEDALO_APPLICATION_LANG, true, true);
							...
							if ($model==='section') {
								...
								if(isset($ar_terms[0])) {
									...
									foreach ((array)$ar_related as $current_tipo) {
										$current_model = ontology_node::get_model_name_by_tipo($current_tipo,true);
				...
				// ddo_map
					$ddo_map = array_map(function($current_tipo) use($tipo, $target_section_tipo, $current_mode, $children_view){
						...
						$ddo = new dd_object();
							...
							$ddo->set_view($view);
							$ddo->set_label(ontology_node::get_label_by_tipo($current_tipo, DEDALO_APPLICATION_LANG, true, true));
	...
	public function get_buttons_context() : array {
		...
		// ar_button_objects create
			foreach ($ar_buttons_tipo as $current_button_tipo) {
				...
				// model
					$model = ontology_node::get_model_name_by_tipo($current_button_tipo, true);
					// skip exclude_models
				...
				// label $term, $lang=NULL, $from_cache=false, $fallback=true
					$button_label = ontology_node::get_label_by_tipo($current_button_tipo, DEDALO_APPLICATION_LANG, true, true);
