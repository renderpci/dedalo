		...
		if ($considerHtml===true) {
			...
			foreach ($lines as $line_matchings) {
				...
				if (!empty($line_matchings[1])) {
					// if it's an "empty element" with or without xhtml-conform closing slash (HTML5 self-closing tags)
					if (preg_match('/^<(\s*.+?\/\s*|\s*(img|br|input|hr|area|base|basefont|col|frame|isindex|link|meta|param|source|track|embed|wbr)(\s.+?)?)>$/is', $line_matchings[1])) {
						// do nothing
