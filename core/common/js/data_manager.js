...
/**
* POLL_PROCESS_STATUS
* Polling-based process status monitor. Replaces the SSE streaming approach
* (request_stream + read_stream) with simple periodic API requests.
*
* Uses the same on_read/on_done callback interface as read_stream, making it
* a drop-in replacement for SSE-based process monitoring.
*
* Advantages over SSE:
* - Compatible with RoadRunner worker (no streaming, no die(), no flush())
* - Does not block a PHP-FPM worker in a while(1) loop
* - No fragile chunk reassembly needed
* - Works through any proxy/CDN without buffering issues
*
* @param object options
* {
*   pid:         int|string  Process ID
*   pfile:       string      Process file name
*   update_rate: int         Poll interval in ms (default 1000)
* }
* @param function on_read
*   Callback fired on each poll response. Receives (sse_response, controller)
* @param function on_done
*   Callback fired when process finishes (is_running === false)
* @return object controller
*   { stop: function } — call controller.stop() to cancel polling
*/
data_manager.poll_process_status = function(options, on_read, on_done) {
	...
	const update_rate	= options.update_rate || 1000

	// controller for external cancellation (compatible with stream_readers pattern)
	let stopped = false
	const controller = {
		cancel : () => {
			stopped = true
	...
	// register controller (allow stop on page navigation, like stream_readers)
	page_globals.stream_readers.push(controller)

	// track elapsed time client-side (replaces SSE's server-side total_time)
	const start_time = Date.now()

	// format elapsed time as human-readable string
	const format_elapsed = () => {
		const elapsed_ms = Date.now() - start_time
		if (elapsed_ms < 1000) return elapsed_ms + ' ms'
		const elapsed_sec = elapsed_ms / 1000
		if (elapsed_sec < 60) return Math.round(elapsed_sec) + ' sec'
		const elapsed_min = elapsed_sec / 60
		if (elapsed_min < 60) return elapsed_min.toFixed(1) + ' min'
		const elapsed_hour = elapsed_min / 60
		return elapsed_hour.toFixed(1) + ' hour'
	}

	// initial notification
	on_read({
		data		: { msg : 'Preparing data...' },
		is_running	: true
	}, controller)

	// poll function (recursive via setTimeout)
	const poll = async () => {
		...
		try {
			...
			const response = await self.request({
				body : {
					dd_api		: 'dd_utils_api',
					action		: 'get_process_status_poll',
					prevent_lock: true,
					options		: {
						pid		: pid,
						pfile	: pfile
			...
			// add client-side total_time to response (SSE compat)
			response.total_time		= format_elapsed()
			response.update_rate	= update_rate

			// fire on_read callback
			on_read(response, controller)
			...
			console.error('poll_process_status error:', error)

			if (stopped) return

			// retry on error (network glitch etc.)
			setTimeout(poll, update_rate * 2)
		}
	}

	// start first poll after update_rate delay
	setTimeout(poll, update_rate)

	return controller
}//end poll_process_status
