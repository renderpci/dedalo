...
export const ui = {
	...
	* 	update_rate: int (milliseconds)
	* 	display_json: bool false (on true, add raw JSON view of data)
	*	callback_done: function (fired on reader finish)
	* 	callback_read: function (fired on reader reads a chunk)
	* }
	* @return HTMLElement result_node
	*/
	render_process_status : async function(options) {

		// options
			const id			= options.id // used to store DB local data status
			const pid			= options.pid
			const pfile			= options.pfile
			const container		= options.container
			const update_rate	= options.update_rate || 1000 // milliseconds
			const display_json	= options.display_json ?? (SHOW_DEBUG===true)
			const callback_done = options.callback_done || null
			const callback_read = options.callback_read || null
		...
		// call API to fire get_process_status
			...
			.then(function(stream){
				...
				// store local info about this process
					data_manager.set_local_db_data(
						{
							id		: id, // like 'make_backup_process',
							value	: {
								pid		: pid,
								pfile	: pfile
							}
				...
				// on_read. Fired on every read chunk by stream reader
				const on_read = (reader, sse_response) => {
					...
					if (callback_read) {
						callback_read(sse_response)
					}
				}

				// on_done. Fired on stream reader done (also cancel call)
				const on_done = () => {
					spinner.remove()
					if (callback_done) {
						callback_done()
