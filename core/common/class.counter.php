	...
	*/
	public static function update_counter(string $tipo, string $matrix_table='matrix_counter', $current_value=false) : int {
		...
		if( intval($current_value)===0 ) {
			...
			$ref		= ontology_node::get_label_by_tipo($tipo)." [".ontology_node::get_model_name_by_tipo($tipo,true)."]";
	...
	*/
	public static function check_counters() : object {
		...
		while ($rows = pg_fetch_assoc($result)) {
			...
			// model check
				$model_name = ontology_node::get_model_name_by_tipo($section_tipo,true);
			...
			// item_info
				$item_info = (object)[
					...
					'label'				=> ontology_node::get_label_by_tipo($section_tipo, DEDALO_DATA_LANG, true, true),
