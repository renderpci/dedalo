...
const render_opcache_section = function(self, container) {
	...
	// status panel (refreshed in place)
	...
	// reset opcache button
		add_action_button({
			parent			: container,
			body_response	: body_response,
			label			: get_label.reset_opcache || 'Reset opcache',
			run				: () => self.reset_opcache(),
			on_success		: async () => {
				self.value = await self.get_value()
				ui.update_node_content(panel, JSON.stringify((self.value||{}).opcache || {}, null, 2))
			}
		})

	// clear realpath / stat cache button (complements opcache reset)
		add_action_button({
			parent			: container,
			body_response	: body_response,
			label			: get_label.reset_realpath_cache || 'Clear realpath cache',
			run				: () => self.reset_realpath_cache()
		})
...
const render_directories_section = function(self, container) {
	...
	// remove old upload chunk files
		add_action_button({
			parent			: container,
			body_response	: body_response,
			label			: get_label.clear_chunk_files || 'Remove old upload chunks',
			run				: () => self.clear_chunk_files(),
			on_success		: refresh
		})
