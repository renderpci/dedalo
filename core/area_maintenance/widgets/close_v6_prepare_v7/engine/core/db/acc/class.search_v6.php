...
/**
* CLASS SEARCH_V6
* Base class for all Dédalo v7 SQL search operations over JSONB matrix tables.
*
* Translates a Search Query Object (SQO) — a tree of $and/$or filter nodes that
* reference component tipos and JSONB paths — into a complete PostgreSQL SELECT
* statement and executes it against the matrix table that corresponds to the
* requested section tipo(s).
*
* Responsibilities:
* - SQO validation and normalisation (set_up, pre_parse_search_query_object)
* - Per-component SQL fragment delegation (conform_search_query_object,
*   component_parser_select)
* - SELECT / FROM / WHERE / JOIN / ORDER / LIMIT construction
* - UNION ALL assembly when multiple section tipos map to different matrix tables
* - Project-access filtering (build_sql_projects_filter) based on logged-in user
* - Propagation of relation locators to the flat `relations` table
* - Supporting static helpers used by components (trim_tipo, get_query_path,
*   is_literal, search_options_title)
*
* Extended by:
* - search       — standard record search (concrete subclass, same file)
* - search_tm    — time-machine variant (uses matrix_time_machine)
* - search_related — cross-table relation search
*
* The SQO contract is documented in core/common/class.search_query_object.php.
* Security chokepoint: sanitize_client_sqo() in core/api/v1/json/index.php strips
* and clamps untrusted client SQOs before any instance of this class sees them.
*
* @package Dédalo
* @subpackage Core
*/
class search_v6 {
	...
	// main vars
		/**
		* Name of the flat relations table used for LEFT JOIN path traversal.
		* All cross-section relations are denormalised here by
		* propagate_component_dato_to_relations_table() on every save.
		* @var string $relations_table
		*/
		private static $relations_table = 'relations';

		/**
		* Snapshot of the raw SQO before any component pre-parsing.
		* Kept solely for debug/logging purposes; never used in query building.
		* @var mixed $search_query_object_preparse
		*/
		public $search_query_object_preparse;

		/**
		* Working copy of the SQO, normalised by set_up() and mutated in place
		* by pre_parse_search_query_object(). The `parsed` flag prevents re-entry.
		* @var object $search_query_object
		*/
		public $search_query_object;

		/**
		* SQL fragment for the FROM clause, e.g. "matrix_objects AS ob1".
		* Set by build_main_from_sql() and cached here so downstream methods
		* (notably the duplicated-value subquery in get_sql_where) can reuse it.
		* @var string $main_from_sql
		*/
		public $main_from_sql;

		/**
		* Accumulated LEFT JOIN clauses for multi-level component paths.
		* Each path step beyond the root table appends one relation JOIN and one
		* matrix JOIN via build_sql_join().
		* @var array|null $join_group
		*/
		protected $join_group;

		/**
		* SQL fragment for the main WHERE clause (section_tipo filter + section_id > 0 guard).
		* Set by build_main_where_sql() and reused by the duplicated-value subquery.
		* @var string $main_where_sql
		*/
		...
		/**
		* Primary matrix table name resolved from the first valid section tipo in ar_section_tipo.
		* Fixed once during set_up(); search_tm subclass overrides to 'matrix_time_machine'.
		* @var string|null $matrix_table
		*/
		protected $matrix_table;

		/**
		* SQL fragments for extra SELECT columns needed by ORDER BY expressions.
		* Populated by build_sql_query_order() when ordering on JSONB paths;
		* appended to the SELECT list by build_sql_query_select().
		* @var array $order_columns
		*/
		protected $order_columns;

		/**
		* When true, the inner window sub-select (SELECT DISTINCT ON … id) is used
		* to speed up offset pagination on large tables by first narrowing to a list
		* of row ids, then fetching only those rows with full column projection.
		* Disabled for autocomplete callers where it adds unnecessary overhead.
		* Default: true (set in set_up).
		* @var bool $allow_sub_select_by_id
		*/
		protected $allow_sub_select_by_id;

		/**
		* When true, the DISTINCT ON(section_id) guard is removed from the SELECT.
		* Required for thesaurus searches that intentionally query multiple section
		* tipos and rely on the full duplicate row set for their result logic.
		* Forced true whenever ar_section_tipo contains more than one entry.
		* Default: false.
		...
		*/
		...
		/**
		* When true, the per-user project access filter is skipped entirely.
		* Automatically set for system tables (matrix_list, matrix_dd, etc.) that
		* are not governed by the project/filter mechanism.
		* @var bool $skip_projects_filter
		*/
		protected $skip_projects_filter;

		/**
		* Overrides the default ORDER BY clause (section_id ASC/DESC).
		* Set when a distinct_values select is requested so that the deduplication
		* key also drives the sort.
		* @var string|null $sql_query_order_default
		*/
		protected $sql_query_order_default;

		/**
		* Specific ORDER BY SQL fragment used inside the window sub-select
		* (inner DISTINCT ON query). Allows the pagination window to use a
		* different sort key than the outer ORDER BY.
		* @var string|null $sql_query_order_window_subselect
		*/
		protected $sql_query_order_window_subselect;

		/**
		* Map of table alias → component tipos whose relation data has already been
		* added to the SELECT list. Prevents duplicate 'relations_xxx' columns when
		* multiple relation components from the same section are selected.
		* Shape: [ table_alias => [ component_tipo, ... ] ]
		* @var array|null $relations_cache
		*/
		protected $relations_cache;

		/**
		* List of distinct matrix table names involved in the query.
		* Computed lazily by build_union_query() when ar_section_tipo spans
		* more than one physical table, driving the UNION ALL assembly.
		* @var array $ar_matrix_tables
		*/
		protected $ar_matrix_tables;

		/**
		* Array of locator objects used to restrict results to specific records.
		* Each locator may carry section_id, section_tipo, and time-machine-only
		* fields (tipo, lang, matrix_id, section_id_key).
		* Sourced from SQO->filter_by_locators.
		* @var array|null $filter_by_locators
		*/
		protected $filter_by_locators;

		/**
		* Logical operator ('AND'|'OR') combining multiple filter_by_locators entries.
		* Sourced from SQO->filter_by_locators_op.
		* @var string|null $filter_by_locators_op
		*/
		protected $filter_by_locators_op;

		/**
		* Column names that map directly to physical matrix table columns and do
		* not require JSONB path extraction. Used to short-circuit select/order
		* building for these special identifiers.
		* @var string[] $ar_direct_columns
		*/
		public static $ar_direct_columns = ['section_id','section_tipo','id'];

		/**
		* When true, records with section_id <= 0 (the root/system record) are
		* included in results. Used by profile sections where the root record is
		* a valid entry that must be visible.
		* @var bool|null $include_negative
		*/
		public $include_negative;

		/**
		* All section tipos this query targets, normalised to an array.
		* Sourced from SQO->section_tipo (scalar or array).
		* @var array $ar_section_tipo
		*/
		public $ar_section_tipo;

		/**
		* The first (primary) section tipo in ar_section_tipo. Used as the
		* reference for table resolution, alias generation, and WHERE construction.
		* @var string $main_section_tipo
		*/
		public $main_section_tipo;

		/**
		* Short SQL alias derived from main_section_tipo via trim_tipo(), e.g.
		* 'oh1' → 'oh1', 'dd1324' → 'dd1324'. Set to 'mix' when ar_section_tipo
		...
		*/
		...
		/**
		* SQL LEFT JOIN fragment that attaches the flat `relations` table for the
		* project-access filter. Built by build_sql_projects_filter() in the
		* default branch and stored here so parse_search_query_object() can
		* inject it into the FROM clause.
		* @var string|null $filter_join
		*/
		public $filter_join;

		/**
		* SQL AND clause appended after the main WHERE to restrict results to rows
		* whose relation points to one of the logged-in user's allowed projects.
		* Complements $filter_join; built by build_sql_projects_filter().
		* @var string|null $filter_join_where
		*/
		public $filter_join_where;

		/**
		* Additional WHERE fragment that restricts results to an explicit list of
		* section_ids. Injected when DEDALO_FILTER_USER_RECORDS_BY_ID is true and
		* the current user has a per-record allowlist for this section tipo.
		* @var string|null $filter_by_user_records
		*/
		public $filter_by_user_records;

		/**
		* Complete custom ORDER BY + LEFT JOIN string built from SQO->order_custom.
		* When set, it is appended verbatim to the outer query and replaces the
		* standard ORDER BY clause.
		* @var string|null $sql_query_order_custom
		*/
		public $sql_query_order_custom;

		/**
		* Keyed map of unique SQL LEFT JOIN fragments added by build_sql_join().
		* Keys are the generated table alias strings; values are the full JOIN SQL.
		* De-duplicated by key so the same join is never emitted twice even when
		* the same path is referenced by multiple select/filter elements.
		* @var array|null $ar_sql_joins
		*/
		public $ar_sql_joins;

		/**
		* Guard flag for the duplicated-value search operator (!!).
		* When a filter element has `duplicated: true`, get_sql_where() must call
		* build_sql_filter() recursively to obtain the sibling filters; setting this
		* flag tells that recursive call to exclude the duplicated element itself,
		* preventing infinite recursion.
		* @var bool $skip_duplicated
		*/
		public $skip_duplicated = false;

		/**
		* Stores the last assembled SQL query string.
		* Never used by the query engine; exists only to simplify debug inspection
		* of the current instance without re-calling parse_search_query_object().
		* @var string|null $sql_query
		*/
		public $sql_query;
	...
	/**
	* GET_INSTANCE
	* Factory: constructs and returns the correct search subclass instance for the
	* given SQO. The concrete class is chosen from SQO->mode:
	*   'tm'      → search_tm    (time-machine table queries)
	*   'related' → search_related (cross-section relation queries)
	*   (default) → search       (standard matrix table queries)
	*
	* Callers should always use this factory rather than constructing subclasses
	* directly, so that mode dispatch stays in one place.
	* @param object $search_query_object - validated SQO (already sanitised by sanitize_client_sqo)
	* @return object - concrete search_tm|search_related|search instance
	*/
	...
	/**
	* __CONSTRUCT
	* Private: use get_instance() to obtain a properly typed subclass instance.
	* Delegates entirely to set_up() which normalises the SQO and initialises all
	* class properties before any query-building method is called.
	* @param object $search_query_object - raw or pre-sanitised SQO
	*/
	...
	/**
	* SET_UP
	* Normalises and validates the incoming SQO, then populates all instance
	* properties that the query-building methods depend on.
	*
	* After set_up() completes the instance is ready for pre_parse_search_query_object()
	* followed by parse_search_query_object(). No query-building method must be called
	* before set_up() finishes.
	*
	* Side effects:
	* - Sets $this->search_query_object with defaults applied.
	* - Resolves $this->matrix_table from the first valid tipo in ar_section_tipo
	*   (skips tipos whose ontology model or matrix table cannot be resolved).
	* - Throws Exception if section_tipo is absent or empty.
	* @param object $search_query_object - raw SQO; cast to object if not already
	* @return void
	* @throws Exception when section_tipo is missing
	*/
	protected function set_up(object $search_query_object) : void {
		...
		// System/internal tables never go through the project-access filter because
		// they are not owned by any project and may be queried by any authenticated user.
	...
	/**
	* SEARCH
	* Executes the full search pipeline and returns the matching records.
	*
	* Pipeline:
	* 1. parse_search_query_object() → SQL string (includes pre-parse if needed)
	* 2. JSON_RecordObj_matrix::search_free() → PostgreSQL result resource
	* 3. Hydrate each row: section_id cast to int, all other non-string columns
	*    JSON-decoded, text columns kept as strings.
	* 4. Optionally expand children_recursive: fetch all descendant records by
	*    component_relation_children::get_children_recursive(), rebuild a new SQO
	*    that targets them via an IN filter, and re-run search().
	* 5. Deprecated full_count path: still supported but logs a warning.
	*
	* Returns early with ar_records=[] if SQL generation fails or the database
	* query returns false (error already logged).
	*
	* @return object $response - stdClass with:
	*   ->ar_records  array of stdClass rows (section_id int, other fields mixed)
	*   ->debug       string (only when SHOW_DEBUG is true; contains timing + SQL)
	*/
		...
		// ar_records. Build a temporal list with array of records found in query
			...
			while ($rows = pg_fetch_assoc($result)) {
				...
				for ($i=0; $i < $pg_num_fields; $i++) {
					...
					// Type-safe hydration
					// section_id is always a plain integer; every other non-string column
					// arrives as a JSON-encoded string from PostgreSQL and must be decoded.
					// section_tipo is a plain string — exempt from json_decode.
	...
	/**
	* COUNT
	* Executes a COUNT query for the current SQO and returns the total number of
	* matching records.
	*
	* When SQO->children_recursive is true, a full search() is performed first so
	* that the recursive child-expansion logic can rewrite the SQO's filter; the
	* resulting modified SQO is then used for the count query.
	*
	* When SQO->group_by is set, the response includes a totals_group array listing
	* the count broken down by each group-by key (e.g. section_tipo for UNION results).
	*
	* Uses parse_search_query_object(true) which generates a COUNT(*) wrapper rather
	* than the full SELECT column list.
	*
	* @return object $records_data - stdClass with:
	*   ->total        int   total matching records (sum across all UNION branches)
	*   ->totals_group array (only when SQO->group_by is set) of stdClass{key, value}
	*   ->debug        stdClass (only when SHOW_DEBUG is true)
	*/
	...
	* Builds a replacement SQO whose filter targets the complete set of parent +
	* descendant records discovered by a previous children_recursive search pass.
	*
	* The optimised form uses a single IN(section_id, ...) clause (instead of one
	* OR branch per record) for better PostgreSQL planner performance.
	*
	* Mutations applied to the cloned SQO:
	* - parsed → false (forces re-parse with new filter)
	* - children_recursive → false (prevents infinite recursion on the second pass)
	* - filter_by_locators removed (the parent-locator filter was already consumed)
	* - full_count → false
	* - filter replaced with the new IN-based children filter, optionally wrapped in
	*   $and with the SQO's fixed_children_filter if one was provided.
	* @param array $ar_rows - merged array of parent rows + all descendant rows
	* @return object $new_sqo - cloned and modified SQO ready for search::get_instance()
	*/
	...
	* Dispatches each filter and select element in the SQO to the owning component
	* class so it can enrich the element with JSONB paths, operators, and q_parsed
	* values. Mutates $this->search_query_object in place.
	*
	* This is the first stage of query compilation. It must complete before any of
	* the build_sql_*() methods are called. Re-entry is guarded by the `parsed` flag:
	* if SQO->parsed is already true the method returns immediately.
	*
	* Delegate calls:
	* - filter  → conform_search_query_object() (recursive, component::get_search_query)
	* - select  → component_parser_select()     (component::get_select_query)
	* - order   → component_parser_select()     (same parser as select)
	*
	* @return void
	*/
	...
	/**
	* COMPONENT_PARSER_SELECT
	* Delegates a single select (or order) element to the owning component class so
	* the component can add its JSONB path, data type, and any component-specific
	* select modifiers.
	*
	* Direct columns (section_id, section_tipo, id) are returned unchanged because
	* they map to physical columns, not JSONB paths, and need no component involvement.
	*
	* Note: inherited from the v5 architecture; the comment "not used in v6" in the
	* original is misleading — it IS still called by pre_parse_search_query_object()
	* for both select and order arrays.
	* @param object $select_object - raw select/order element from the SQO
	* @return object $select_object - enriched with component-specific SQL fragments
	*/
	...
	* Recursively traverses the SQO filter tree, calling each component's
	* get_search_query() to enrich leaf elements with SQL-ready fragments
	* (q_parsed, operator, component_path, type, format, etc.).
	*
	* The tree is structured as nested $and/$or operator objects, each containing
	* an array of either sub-operator objects or leaf search elements. A leaf
	* element has a `path` property; a sub-operator object does not.
	*
	* Elements whose component model cannot be resolved (e.g. an ontology not
	* installed in this Dédalo instance) are silently skipped with an error log
	* rather than aborting the entire search.
	*
	* Elements that already carry `format='column'` and `q_parsed` (pre-parsed
	* elements, e.g. from request-config fixed filters) are passed through without
	* re-dispatching to the component.
	*
	* @param string $op - logical operator key, e.g. '$and' or '$or'
	* @param array $ar_value - array of sub-operator objects or leaf search elements
	*   Sample leaf element:
	*   {
	*     "q": {"section_id":"1","section_tipo":"dd64",...},
	*     "q_operator": null,
	*     "path": [{"section_tipo":"dd1324","component_tipo":"dd1354","model":"component_radio_button","name":"Active"}]
	*   }
	* @return object $new_ar_query_object - stdClass{ $op: enriched_array }
	*/
	...
	* Assembles the complete PostgreSQL SELECT statement from the pre-parsed SQO.
	*
	* Pre-parses the SQO first (component path enrichment) unless `parsed` is
	* already true. Then branches on three mutually exclusive cases:
	*
	* 1. full_count=true: wraps a DISTINCT section_id sub-select in COUNT(*).
	* 2. filter_by_locators set: fetches exact locator rows with a preserved-order
	*    LEFT JOIN VALUES trick.
	* 3. No order / With order: the default cases.
	*    - Without order and allow_sub_select_by_id=true: uses a two-level window
	*      (inner DISTINCT ON id + LIMIT/OFFSET, outer full column projection) for
	*      fast offset pagination on large tables.
	*    - Without order and allow_sub_select_by_id=false (autocomplete mode): a
	*      single flat SELECT with DISTINCT ON(section_id).
	*    - With order: wraps the base query in a sub-select so PostgreSQL can sort
	*      an already-paginated set efficiently.
	*
	* Multi-section queries (ar_section_tipo > 1) pass through build_union_query()
	* before the final ORDER / LIMIT are appended.
	*
	* @param bool $full_count = false - when true returns a COUNT(*) query instead
	* @return string $sql_query - complete, executable PostgreSQL statement (ends with ;)
	*/
	...
	/**
	* BUILD_UNION_QUERY
	* Rewrites a single-table SQL query as a UNION ALL across all matrix tables that
	* the current ar_section_tipo set resolves to.
	*
	* When all section tipos share the same matrix table (the common case), the
	* query is returned unchanged. When they span multiple tables, the method:
	* 1. Rebuilds ar_matrix_tables from the current ar_section_tipo (deduplicated).
	* 2. Produces one copy of the SQL per additional table, substituting the table
	*    name and alias via preg_replace/str_replace so the column aliases remain
	*    unambiguous.
	* 3. Joins all copies with UNION ALL (duplicates intentionally kept because
	*    remove_distinct is forced true for multi-section queries in set_up).
	*
	* (!) The regex substitution for FROM/AS is naive and would break if the FROM
	* clause contained subqueries. The current callers never produce that shape, but
	* this is a fragility worth noting.
	* @param string $sql_query - base single-table SQL to expand
	* @return string $sql_query - UNION ALL of all matrix table variants, or the
	*   original string when only one table is involved
	*/
	...
	* Builds the SQL SELECT column list from the SQO->select array, or falls back
	* to selecting all standard matrix columns (datos, data, relation, string, date,
	* iri, geo, number, media, misc, relation_search, meta) when select is empty.
	*
	* Each select element is processed by get_table_alias_from_path() to locate
	* the correct table alias for multi-level portal paths, and by build_sql_join()
	* to ensure the required LEFT JOINs are registered.
	*
	* Special handling:
	* - component_section_id: resolves to the direct column, no JSONB extraction.
	* - Relations columns: deduplicated via relations_cache so only one shared column
	*   is added per table alias; individual component tipos reference the shared column.
	* - distinct_values: wraps the selected JSONB expression in DISTINCT ON and
	*   reorders the SELECT list so it appears first (required by PostgreSQL DISTINCT ON).
	* - Activity section (DEDALO_ACTIVITY_SECTION_TIPO): path 'valor_list' is
	*   transparently rewritten to 'dato'.
	*
	* Side effect: populates this->ar_sql_joins via build_sql_join().
	*
	* @param bool $full_count = false - delegates to build_full_count_sql_query_select() when true
	* @return string $sql_query_select - comma-separated SELECT column expressions
	*/
	...
	* Generates the per-user project-access SQL filter for the current section tipo.
	*
	* Global admins receive an empty string (no restriction). All other users get a
	* WHERE fragment that constrains results to records associated with the user's
	* allowed projects. The exact fragment depends on the section tipo:
	*
	* - DEDALO_SECTION_PROFILES_TIPO: no filter (profiles are globally visible).
	* - DEDALO_FILTER_SECTION_TIPO_DEFAULT (projects section): no filter
	*   (projects are listed without restriction since 2018-03-31).
	* - DEDALO_SECTION_USERS_TIPO: compound filter — must match either a profile
	*   linked to one of the user's authorised areas OR a project from the user's
	*   filter_master component; throws if either list is empty.
	* - Default sections: filters by the section's component_filter locator, using
	*   a JSONB containment check against the user's project locators. Also
	*   populates $this->filter_join and $this->filter_join_where for the JOIN path.
	*
	* If DEDALO_FILTER_USER_RECORDS_BY_ID is defined and true, an additional
	* per-record allowlist filter is appended and stored in $this->filter_by_user_records.
	*
	* Results are cached in a static per-uid map (section_tipo + user_id) so repeated
	* calls within the same PHP request (e.g. count() after search()) are free.
	*
	* (!) The static cache is never invalidated within a request; if the user's project
	* membership changes mid-request the cached filter will be stale. This is acceptable
	* for normal web requests but can cause issues in long-running persistent workers.
	*
	* @param bool $force_calculate = false - bypass cache and global-admin short-circuit; for debug only
	* @return string $sql_projects_filter - SQL WHERE fragment (begins with AND or is empty)
	* @throws Exception when a non-global user has no authorised areas or no profile data
	*/
	...
	* Returns the fallback ORDER BY expression used when no explicit order is
	* requested in the SQO.
	*
	* Default: section_id ASC for all sections except the activity log section
	* (DEDALO_ACTIVITY_SECTION_TIPO), which uses DESC to show newest entries first.
	*
	* If $this->sql_query_order_default has already been set (e.g. by a distinct_values
	* select), that custom value is returned unchanged, allowing the caller to override
	* the default without re-entering this method.
	* @return string $sql_query_order_default - e.g. 'mix.section_id ASC'
	*/
	...
	* Builds the ORDER BY expression(s) from the SQO's order or order_custom property.
	*
	* Two modes:
	* - order_custom: each item provides section_tipo, column_name, and column_values.
	*   A LEFT JOIN VALUES(…) trick is used to assign an arbitrary sort position to each
	*   value, stored in $this->sql_query_order_custom for injection by the 'with order'
	*   branch of parse_search_query_object(). Returns empty string (no ORDER BY here).
	* - order (standard): each order element specifies a path and direction. Three
	*   sub-cases:
	*     a. `column` property set — full SQL column expression supplied by the component
	*        (e.g. component_date provides a pre-cast timestamp); aliased and added to
	*        $this->order_columns for inclusion in SELECT.
	*     b. Direct column (section_id, section_tipo, id) — used verbatim.
	*     c. JSONB path — extracted via datos#>>'{…}', aliased, and added to
	*        $this->order_columns.
	*
	* All non-empty order expressions get 'NULLS LAST' appended. If the order does
	* not already include section_id, it is appended as a tiebreaker.
	*
	* Side effects: may call build_sql_join() to register required JOINs; populates
	* $this->order_columns; may set $this->sql_query_order_custom.
	* @return string $sql_query_order - comma-separated ORDER BY terms (without 'ORDER BY'),
	*   or empty string when order_custom mode is active or no order is specified
	*/
	...
	/**
	* BUILD_MAIN_FROM_SQL
	* Produces the base FROM clause fragment, e.g. "matrix_objects AS ob1".
	* Also caches the result in $this->main_from_sql so the duplicated-value
	* subquery in get_sql_where() can reference it without rebuilding.
	* @return string $main_from_sql - "<matrix_table> AS <alias>"
	*/
	...
	* Builds the primary WHERE clause that restricts rows to the requested section
	* tipo(s) and excludes the root record (section_id <= 0) unless include_negative
	* is explicitly set.
	*
	* Uses IN(...) for multi-section queries and a plain = for single-section for
	* minor planner efficiency. Also caches the result in $this->main_where_sql so
	* the duplicated-value subquery can reuse it.
	*
	* The section_id > 0 guard prevents the synthetic root record (section_id = -1
	* or 0 used internally by profiles) from leaking into normal search results.
	* @return string $main_where_sql - e.g. "(mix.section_tipo IN ('oh1','rsc167')) AND mix.section_id>0"
	*/
	...
	/**
	* BUILD_SQL_FILTER
	* Converts the SQO->filter tree into a SQL AND (...) fragment by delegating
	* recursive parsing to filter_parser().
	*
	* Returns an empty string when the filter is empty or produces no SQL output.
	*
	* Duplicated-operator guard: when $this->skip_duplicated is true (set by
	* get_sql_where() during a duplicated-value subquery), elements that carry
	* `duplicated: true` are removed from the value array before parsing, ensuring
	* the outer filter is applied inside the dedup subquery without the dedup
	* element recursing into itself. The flag is reset to false after use.
	* @return string $filter_query - SQL fragment starting with " AND (" or empty string
	*/
	...
	* Builds a WHERE fragment that restricts results to the specific records listed
	* in $this->filter_by_locators. Each locator contributes an AND-ed set of column
	* equality predicates; multiple locators are joined with OR so any matching record
	* is returned.
	*
	* Supported locator properties (all optional; each becomes one AND predicate):
	*   section_id, section_tipo, tipo (time_machine only), type, lang (time_machine only),
	*   matrix_id (time_machine only → column 'id'), section_id_key (time_machine only).
	*
	* Properties only valid for matrix_time_machine are silently ignored with a warning
	* when the current matrix table is not matrix_time_machine.
	*
	* Returns empty string if filter_by_locators is empty.
	* @return string $sql_filter - SQL block starting with a comment header; individual
	*   locator predicates joined with OR, e.g. "(section_id=5 AND section_tipo='oh1') OR ..."
	*/
	...
	* Produces a LEFT JOIN VALUES(…) ORDER BY expression that restores the exact
	* order of the input locators in the query result.
	*
	* Uses the same PostgreSQL VALUES trick as order_custom: each locator gets a
	* row in the VALUES list with its section_tipo, section_id, and a 1-based
	* position index. The result is sorted by that index (ASC).
	*
	* Called only when no explicit SQO->order is present and filter_by_locators is set,
	* so the locator-specified order is preserved as-is.
	* @return string $string_query - complete ORDER fragment starting with 'LEFT JOIN (VALUES…'
	*/
	...
	/**
	* FILTER_PARSER
	* Recursively converts one level of the SQO filter tree into a SQL expression.
	*
	* Each element in $ar_value is either:
	* - A sub-operator object (no `path` property): the method recurses, wrapping the
	*   inner result in parentheses and joining with the current $op SQL keyword.
	* - A leaf search element (has `path`): delegates to get_sql_where() and
	*   registers required JOINs via build_sql_join() when the path depth > 1.
	*
	* The SQL operator keyword is derived from the $op string by stripping the leading
	* '$' and uppercasing: '$and' → 'AND', '$or' → 'OR'.
	*
	* Empty parsed strings (e.g. components that suppress themselves for a given query)
	* are skipped; their trailing operator is omitted.
	* @param string $op - operator key, e.g. '$and' or '$or'
	* @param array $ar_value - array of sub-operator objects or leaf search elements
	* @return string $string_query - SQL boolean expression (no outer parentheses)
	*/
	...
	* Returns the SELECT clause used in the deprecated inline full_count path
	* inside search(). The outer wrapper in parse_search_query_object() uses
	* COUNT(*) over a DISTINCT subquery instead; this method only applies when
	* full_count is requested from within the search() call itself.
	* @return string $sql_query_select - e.g. "count(DISTINCT mix.section_id) as full_count"
	*/
	...
	/**
	* BUILD_SQL_JOIN
	* Registers the LEFT JOIN clauses required to traverse a multi-level component
	* path (e.g. portal → related section → component). Idempotent: each unique
	* table alias is only added once to $this->ar_sql_joins regardless of how many
	* select/filter/order elements share the same path.
	*
	* For each step beyond the first:
	* 1. Joins the flat `relations` table (aliased as r_<path_alias>) on (section_id,
	*    section_tipo, from_component_tipo) to locate the linked record ids.
	* 2. Joins the target matrix table (aliased as <path_alias>) on the matched
	*    target_section_id/tipo.
	*
	* The alias key is an underscore-concatenation of trimmed tipo strings along the
	* path, e.g. 'oh25_rsc167' for a portal component oh25 leading to section rsc167.
	*
	* Steps whose matrix table cannot be resolved are skipped with an error log.
	*
	* @param array $path - array of path step objects, each with section_tipo and component_tipo
	*   Sample:
	*   [ {"name":"Publication","model":"component_publication","section_tipo":"rsc167","component_tipo":"rsc20"},
	*     {"name":"Value","model":"component_input_text","section_tipo":"dd64","component_tipo":"dd62"} ]
	* @return bool true always (errors are logged, not thrown)
	*/
	...
	/**
	* TRIM_TIPO
	* Contracts a Dédalo ontology tipo identifier to a short SQL alias by keeping
	* only the first $max characters of the alphabetic prefix plus the numeric suffix.
	*
	* Examples: 'dd1324' → 'dd1324' (prefix 'dd' already ≤ 2), 'numisdata303' → 'nu303'.
	*
	* The alias is used throughout query building to keep SQL readable and within
	* PostgreSQL identifier length limits. All join and table alias names are derived
	* from these trimmed forms.
	*
	* Special case: the sentinel value 'all' (used by search_related when the section
	* tipo is unknown) is returned unchanged.
	*
	* Returns null and logs an error on empty input or when the tipo string does not
	* match the expected [a-z]+[0-9]+ pattern.
	* @see search_Test::test_trim_tipo
	* @param string $tipo - ontology tipo identifier, e.g. 'dd1324', 'oh1', 'numisdata303'
	* @param int $max = 2 - maximum characters to keep from the alphabetic prefix
	* @return string|null $trimmed_tipo - shortened alias, or null on invalid input
	*/
	...
	/**
	* GET_SQL_WHERE
	* Converts a single pre-parsed leaf filter element into a SQL boolean expression.
	*
	* The output format is driven by $search_object->format:
	*
	* - 'direct' (default): JSONB path extraction (#>> or #>) with the component-
	*   supplied operator and q_parsed value. Wraps in f_unaccent() when unaccent=true.
	*   Special sub-case 'duplicated=true': emits an IN(SELECT … GROUP BY … HAVING count>1)
	*   deduplication subquery and sets skip_duplicated to prevent infinite recursion.
	*
	* - 'array_elements': delegates element-level matching to a PostgreSQL set-returning
	*   function check_array_component() via resolve_array_elements(). Used by
	*   component_date for temporal range queries.
	*
	* - 'typeof': jsonb_typeof() check; used to test whether a JSONB node is an array,
	*   object, string, etc.
	*
	* - 'column': direct physical column access (section_id, state, …); bypasses JSONB.
	*   Requires column_name property on the search_object.
	*
	* - 'in_column': section_id IN(…) or datos JSONB path IN(…) for comma-separated
	*   id lists. Uses IN or NOT IN depending on search_object->operator.
	*
	* - 'function': calls a named PostgreSQL function on the datos column.
	*
	* The lang property is appended to component_path once (guarded by prefix check)
	* when a language-specific search is needed; 'all' skips this step.
	*
	* @param object $search_object - pre-parsed leaf element from the SQO filter tree
	*   (must have path, format, type, operator, q_parsed at minimum)
	* @return string $sql_where - SQL boolean expression (no leading AND/OR)
	*/
			...
			switch ($search_object_format) {
				...
				case 'direct':
					...
					if(SHOW_DEBUG===true) {
						...
						$model_name				= $component_path_data->model; //ontology_node::get_model_by_tipo($component_tipo,true);
	...
	* Recursively translates a structured date/range filter object into the SQL
	* WHERE clause used inside a check_array_component() call.
	*
	* The input tree mirrors the SQO filter structure ($and/$or groups) but operates
	* on array element aliases produced by the check_array_component set-returning
	* function. Each leaf carries a component_path, type, operator, and q_parsed.
	*
	* Groups are wrapped in parentheses and joined with the SQL equivalent of the
	* operator ('AND'|'OR'). Nested groups recurse until leaves are reached.
	* @param object $array_elements - nested $and/$or filter tree, e.g. {"$or":[{...}]}
	* @param string $component_tipo - the component tipo alias used as the column prefix
	*   inside the SRF result set, e.g. 'numisdata35'
	* @return string $sql_where - SQL boolean expression without outer parentheses
	*/
	...
	/**
	* IS_SEARCH_OPERATOR
	* Returns true when any property key of the given object starts with '$',
	* indicating that it is a logical operator node ($and, $or, …) rather than a
	* leaf filter element.
	*
	* Note: this method is currently unused in the codebase (its call sites were
	* replaced by !property_exists($search_object,'path') checks for clarity),
	* but it is retained as a public utility for subclasses and tests.
	* @param object $search_object - any SQO node
	* @return bool true if the object is an operator node, false if it is a leaf
	*/
	...
	* Derives the SQL table alias that corresponds to the terminal step of a
	* component path, matching the alias assigned by build_sql_join().
	*
	* For single-step paths (direct columns in the root table), the main section
	* tipo alias is returned ('mix' or the trimmed tipo of the root section).
	*
	* For multi-step paths, the alias is built by concatenating the trimmed tipo
	* strings for each intermediate step (section_tipo + '_' + component_tipo)
	* followed by just the section_tipo of the final step — the same convention
	* used by build_sql_join() when naming its aliases.
	*
	* This method must stay in sync with build_sql_join()'s alias generation logic.
	* @param array $path - array of path step objects with section_tipo (and component_tipo for non-last steps)
	* @return string $table_alias - SQL alias for the terminal table in the path
	*/
	...
	/**
	* GET_QUERY_PATH
	* Recursively builds the full path array for a component tipo as used in SQO
	* select and filter elements. Called by component_common and section to
	* construct the `path` arrays that identify columns in the query.
	*
	* Always includes the component itself as the first path element. When
	* resolve_related=true and the component is a relation-type (portal, relation_*):
	* - Resolves the target section via ontology_node::get_ar_related_by_model.
	* - Recurses into the first resolvable component of the related section (or
	*   $related_tipo if explicitly supplied), appending its path element(s).
	*
	* Only the first related component is followed (break after first hit) to avoid
	* unbounded paths with multiple components.
	* @param string $tipo - component tipo to build the path for
	* @param string $section_tipo - owning section tipo of $tipo
	* @param bool $resolve_related = true - when true, follows relation links to build multi-step paths
	* @param bool|string $related_tipo = false - specific related component tipo to use;
	*   when false, the first related component is auto-selected
	* @return array $path - array of stdClass path step objects with name, model,
	*   section_tipo, component_tipo properties
	*/
	...
	/**
	* GET_SQL_JOINS
	* Flattens the ar_sql_joins map into a single SQL string for embedding in the
	* FROM clause. Returns empty string when no JOINs have been registered.
	* @return string $sql_joins - space-joined SQL LEFT JOIN fragments, or empty string
	*/
	...
	* Synchronises the flat `relations` table with the current locator set of a
	* single relation component instance. This is the write-side of the JOIN
	* optimisation: after any save, the component calls this method so that future
	* LEFT JOINs via build_sql_join() can use the flat table rather than scanning
	* the JSONB datos column.
	*
	* Strategy: DELETE all existing rows for (section_id, section_tipo,
	* from_component_tipo), then INSERT one row per distinct target locator. This is
	* a full replace, not a diff, so it is safe even if locators were removed.
	*
	* Skipped silently when section_id contains the DEDALO_SECTION_ID_TEMP sentinel
	* (unsaved temporary records have no permanent id yet).
	*
	* Duplicates (same 5-tuple) are guarded by the in_array() check before building
	* $ar_insert_values; they are NOT caught by a unique constraint.
	*
	* (!) Uses pg_query() directly rather than JSON_RecordObj_matrix::search_free()
	* for the INSERT, which means it bypasses any logging or instrumentation applied
	* by the wrapper. The DELETE still uses JSON_RecordDataBoundObject::search_free().
	*
	* @param object $options - configuration object with mandatory fields:
	*   ->section_tipo       string - owning section tipo
	*   ->section_id         string|int - owning record id
	*   ->from_component_tipo string - component tipo whose locators are being propagated
	*   ->ar_locators        array (optional) - locator objects; defaults to []
	* @return object $response - stdClass with:
	*   ->result bool   true on success or skip; false on validation failure
	...
	* Generates the HTML tooltip content for search operator help shown in the search
	* UI when a component's search mode is active.
	*
	* Each entry in $search_operators_info is an operator symbol → label key pair. The
	* label key is resolved through label::get_label() for the current UI language.
	* Returns an empty string when no operators are provided (component has no special
	* search modes).
	* @param array $search_operators_info - map of operator_symbol => label_key,
	*   e.g. ['=' => 'exact', 'BETWEEN' => 'between']
	* @return string $search_options_title - HTML string with styled operator list,
	*   or empty string when $search_operators_info is empty
	*/
	...
	/**
	* SEARCH_COUNT
	* Executes a direct GROUP BY / COUNT query against a matrix table column, useful
	* for building statistical breakdowns.
	*
	* Emits SQL of the form:
	*   SELECT <column_path> AS <column_tipo>, COUNT(<column_path>) AS count
	*   FROM "<matrix_table>"
	*   WHERE section_tipo = '<section_tipo>'
	*   GROUP BY <column_tipo>
	*   ORDER BY count;
	*
	* Returns an empty array when the matrix table cannot be resolved or the query fails.
	*
	* (!) This method was designed for activity-log statistics and has not yet been
	* integrated into any production UI as of 2018-12-14.
	*
	* @param object $options - query options:
	*   ->column_tipo  string|null - output alias for the grouped column, e.g. 'dd544'
	*   ->column_path  string|null - SQL path expression, e.g. "datos#>>'{components,dd544,dato,lg-nolan}'"
	*   ->section_tipo string|null - section tipo to filter, e.g. 'dd542'
	* @return array $ar_result - array of stdClass{ tipo: string, count: string }
	*/
	public static function search_count(object $options) : array {

		# (!) Designed for activity statistics but not yet implemented in production [2018-12-14]
	...
	/**
	* GET_DATA_WITH_PATH
	* Resolves actual component dato values by following a multi-step path of
	* locators, useful for reading data through portal chains without constructing
	* a full SQL query.
	*
	* Each path step is processed sequentially: the locators from the previous step
	* are used as input to resolve_path_level(), which instantiates each component
	* and calls get_dato_full(). The resulting locators become the input for the
	* next step, building a chain of resolved data.
	*
	* Returns an array of stdClass items, one per path step, each carrying:
	*   ->path  object  the path step descriptor
	*   ->value array   the locators/values resolved at that step
	*
	* Sample path input:
	*   [
	*     {"section_tipo":"oh1","component_tipo":"oh25","model":"component_portal","name":"Audiovisual"},
	*     {"section_tipo":"rsc167","component_tipo":"rsc25","model":"component_select","name":"Collection/archive"}
	*   ]
	* @param array $path - array of path step objects (section_tipo, component_tipo, model, name)
	* @param array $ar_locator - initial array of locator objects (the starting records)
	* @return array $data - array of stdClass{ path: object, value: array } per step
	*/
	...
	/**
	* RESOLVE_PATH_LEVEL
	* Instantiates the component identified by $path_item for each input locator,
	* calls get_dato_full(), and merges all results into a single flat array of
	* locators/values for the next path step in get_data_with_path().
	*
	* Each locator must have section_id and section_tipo; all other locator
	* properties are ignored. Returns an empty array if none of the component
	* instances return data.
	* @param object $path_item - path step descriptor with section_tipo and component_tipo
	* @param array $ar_locator - array of locator objects pointing to the current-level records
	* @return array $result - merged array of locators/values from all component instances
	*/
	...
	/**
	* IS_LITERAL
	* Determines whether a q value produced by a component is a SQL literal (i.e.
	* already enclosed in single quotes and ready for direct embedding) rather than
	* a raw value that still needs quoting.
	*
	* Components use this to decide whether to wrap q_parsed in quotes or pass it
	* through as-is. A string that starts and ends with a single quote and has length
	* greater than 1 is considered a literal.
	*
	* Example: "'hello'" → true (component pre-quoted it); "hello" → false.
	* @param string $q - the q or q_parsed string to inspect
	* @return bool true if $q is already a SQL-quoted literal, false otherwise
	*/
