<?php
/**
* FFMPEG SETTING: 576_PAL_16X9
* Encoding parameters for a PAL 16:9 widescreen H.264/MP4 derivative at 576-line height.
*
...
* Aspect   : 16:9 widescreen — output frame 1024 × 576 px
* Codec    : H.264 (libx264), MP4 container (faststart-ready)
* Video    : 1536 kbit/s, GOP 25 frames (one keyframe per second at 25 fps)
* Audio    : AAC, 128 kbit/s, stereo (two channels), 44.1 kHz
*
...
* $ab          — audio bitrate string passed to ffmpeg -b:a
...
*                is stored (e.g. "576"); must match the quality key expected by the
*                component and media-protection rules
*
* Relationship to sibling settings files
* ----------------------------------------
* - 576_pal_4x3.php   : same bitrate / audio, but scale is 720×576 (4:3 frame)
* - 576_pal.php       : identical parameters to this file; the un-suffixed variant
*                       is kept for legacy callers that do not distinguish aspect ratio
* - 576_ntsc_16x9.php : same frame size (1024×576) but g=30 (NTSC 30 fps base)
*                       and a lower video bitrate (1280 k) for NTSC material
*
* @package Dédalo
...
// (!) Header comment preserved verbatim from original; should read "576_pal_16x9"
...
// Audio parameters
// Stereo output (ac=2) is appropriate for the higher-quality 576-line profile,
// which targets broadcast-grade archival and online streaming use cases.
