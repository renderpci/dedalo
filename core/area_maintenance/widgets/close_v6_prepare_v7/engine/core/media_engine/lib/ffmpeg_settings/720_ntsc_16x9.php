<?php
/**
* FFMPEG SETTINGS PROFILE: 720_ntsc_16x9
...
*   - Broadcast standard: NTSC  (≥ 29 fps; keyframe interval tuned to 30 = one GOP per second)
*   - Quality tier label: '720'  (maps to the $target_path sub-directory name)
*
* This profile is selected when Ffmpeg::get_setting_name() resolves to '720_ntsc_16x9'
* for a source file whose frame rate is >= 29 fps (NTSC) and whose display aspect ratio
* resolves to '16x9' via Ffmpeg::get_aspect_ratio(). Related profiles for this quality
* tier are 720_ntsc.php (no explicit aspect ratio suffix) and 720_pal_16x9.php (PAL,
* < 29 fps). The '16x9' suffix is appended to the setting name only when ffprobe
* explicitly reports a 16:9 display aspect ratio; the plain '720_ntsc' variant acts as
* the non-16:9 fallback.
*
...
*
* (!) The file header comment says '404_pal' — this is a copy-paste error in the original
*     source; the actual profile is 720_ntsc_16x9. The code is correct; only the banner
*     comment was wrong. It has been corrected here.
*
...
$vb				= '2968k';			# video bitrate (kilobits per second)
$s				= '1280x720';		# output scale: width × height in pixels (HD 16:9)
$g				= 30;				# keyframe interval in frames (GOP size); 30 = one keyframe per second at 30 fps NTSC
...
$ac				= "2";				# number of audio channels: 2 = stereo, 1 = mono (pre-existing 'nomo' typo preserved in original)
