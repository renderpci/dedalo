...
function get_current_data_version() : array {
	...
	try {
		...
		// Query all updates records v7 sql
			...
			// (!) ASK for the column instead of discovering it by a failed prepare. Reading a
			//     NOT-yet-migrated v6 database (the v6→v7 migration runs against exactly that)
			//     is a normal state, but letting the v7 SQL fail first logs a pg ERROR on every
			//     process boot, which looks like a fault and is not one. The v6 fallback below
			//     is kept as a safety net for any case the probe cannot answer.
			$v7_column = (class_exists('DBi') && method_exists('DBi', 'check_column_exists'))
