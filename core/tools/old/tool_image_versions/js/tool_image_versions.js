...
var tool_image_versions = new function() {
	...
	*/
	this.get_the_external_source_to_file = function(button_obj){
		...
		//component IRI
		const wrap_div = container.querySelector('.wrap_component')
			if (wrap_div === null ) {
		...
		const js_promise = common.get_json_data(trigger_url, trigger_vars).then(function(response) {
		  ...
		  if (response===null) {
			...
			if(response.result === false){
				...
				console.warn("[tool_image_version.get_the_external_source_to_file] Error on load external_source image");
				alert(response.msg);

			}else{

		    	console.log("[tool_image_version.get_the_external_source_to_file] ",response.msg);

		    	// Show hidded table versions
		    		//const table_hide = document.querySelector(".table_versions")
		    		//if (table_hide) {
		    		//	table_hide.classList.remove("table_versions_off")
		    		//}
	...
	*/
	...
	this.change_media_quality = function ( button_obj ) {
		...
		const wrap_tool_image_versions_page = document.querySelector(".wrap_tool_image_versions_page")
		const wrap_image 					= wrap_tool_image_versions_page.querySelector(".css_wrap_image")

		// image url
		const quality 				= button_obj.dataset.quality
		const image_id 				= button_obj.dataset.image_id
		const tipo 					= button_obj.dataset.tipo
		const parent 				= button_obj.dataset.parent
		const aditional_path		= button_obj.dataset.aditional_path
		const initial_media_path 	= button_obj.dataset.initial_media_path
		const image_url 			= page_globals.media_base_path + initial_media_path +'/'+ quality + aditional_path +'/'+ image_id +'.'+ page_globals.media_extension +'?t='+ new Date().getTime()

		// Spinner in
		html_page.loading_content( wrap_tool_image_versions_page, 1 );
		...
		const image = new Image()
			...
			image.addEventListener("load",function(e){
				...
				component_image_read.raster_canvas({
					wrapper_id 	: wrap_image.id,
					image_url  	: image_url,
					image 		: this
				})

				// Spinner Out
				html_page.loading_content( wrap_tool_image_versions_page, 0 );
	...
	*/
	this.view_image = function(button_obj) {
		...
		const src_target 		= page_globals.media_base_path + initial_media_path + '/' + quality + aditional_path + '/' + image_id + '.' + page_globals.media_extension + '?t=' + (new Date()).getTime();		
