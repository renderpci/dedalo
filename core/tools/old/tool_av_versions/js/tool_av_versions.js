...
var tool_av_versions = new function() {
	...
	*/
	this.change_media_quality = function ( button_obj ) {
		...
		var quality 	= button_obj.dataset.quality,
			video_id 	= button_obj.dataset.video_id,
			date 		= new Date(),
		...
		var video   	 = ar_video_obj[0],
			video_source = video.getElementsByTagName('source')[0];

			if (!video_source) {
				return alert("Error on get video source");
			}
			video.pause();
		
		var src_target 	= page_globals.video_base_path + '/' + quality +'/' + video_id + '.' + page_globals.video_extension + '?&t=' + date.getTime();	

			// Change source media
			video_source.setAttribute("src", src_target);

			// Load and play new video stream
			video.load();
			video.play();			
			...
			*/
			...
			if(SHOW_DEBUG===true) console.log(src_target);
	...
	*/
	this.generate_version = function(button_obj) {
		...
		if(SHOW_DEBUG===true) console.log("->generate_version vars:" +video_id+' '+source_quality+' '+target_quality);
	...
	*/
	this.conform_header = function(button_obj) {
		...
		if(SHOW_DEBUG===true) console.log("->conform_header vars:" +video_id+' '+quality);
	...
	*/
	this.file_exists = function(button_obj, time) {
		...
		if(SHOW_DEBUG===true) console.log("->file_exists vars:" +video_id+' '+quality+ ' '+time);
	...
	*/
	this.delete_version = function(button_obj) {
		...
		if(SHOW_DEBUG===true) console.log("->delete_version vars:" +video_id+' '+quality);
	...
	*/
	this.download_file = function(button_obj) {
		...
		if(SHOW_DEBUG===true) console.log("->dowload_file vars:" +video_id+' '+quality);
	};
		...
		.done(function(received_data) {
			...
			if(error_response) {
				...
				if ( confirm('Download ' + received_data) ) {
					...
					if(SHOW_DEBUG===true) console.log("->dowload_file Ok filename:" +filename);
				}
			}
		})
		// FAIL ERROR
