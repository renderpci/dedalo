...
class area_graph extends area_common {
	...
	public function get_hierarchy_sections( ?array $hierarchy_types_filter=null, ?array $hierarchy_sections_filter=null, bool $terms_are_model=false ) : array {
		...
		foreach ($ar_records as $row) {
			...
			// hierarchy target section tipo
				$model			= RecordObj_dd::get_model_name_by_tipo($hierarchy_target_section_tipo,true);
			...
			// hierarchy target section name
				$model					= RecordObj_dd::get_model_name_by_tipo(DEDALO_HIERARCHY_TERM_TIPO,true);
			...
			// hierarchy order
				$model						= RecordObj_dd::get_model_name_by_tipo(DEDALO_HIERARCHY_ORDER_TIPO,true);
	...
	*/
	public static function get_siblings(string $ckey) : array {
		...
		$model_name 	= RecordObj_dd::get_model_name_by_tipo($tipo,true); // 'component_relation_children';
		$mode 			= 'list';
