	...
	*/
	public function get_grid_value( ?object $ddo=null ) : dd_grid_cell_object {
		...
		foreach ($data as $current_locator) {
			...
			$section_label 	= ontology_node::get_label_by_tipo($from_section_tipo,DEDALO_APPLICATION_LANG, true);
		...
		foreach ($dato as $current_locator) {
			...
			foreach ($inverse_show as $ikey => $ivalue) {
				...
				if ($ikey === 'section_label') {
					if(strlen($line)>0) $line .= ' ';
					$label = ontology_node::get_label_by_tipo($section_tipo, $lang);
					$line .= $label;
				}
				...
				if ($ikey === 'component_label') {
					if(strlen($line)>0) $line .= ' ';
					$label = ontology_node::get_label_by_tipo($component_tipo, $lang);
					$line .= $label;
				}
			}
