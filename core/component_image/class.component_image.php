	...
	*/
	public static function update_dato_version(object $options) : object {
		...
		switch ($update_version) {
			...
			case '6.0.1':
				// component instance
					$model		= RecordObj_dd::get_model_name_by_tipo($options->tipo, true);
					$component	= component_common::get_instance(
						$model,
						$options->tipo,
						$options->section_id,
				...
				if ($is_old_dato===true) {
					...
					// get the source file name
						...
						if(isset($properties->target_filename)) {
							...
							$original_name_model	= RecordObj_dd::get_model_name_by_tipo($original_name_tipo,true);
						...
						if(empty($source_file_name)) {
							...
							$previous_code_model		=  RecordObj_dd::get_model_name_by_tipo($previous_code_tipo,true);
