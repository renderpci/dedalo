	...
	*/
	public function get_external_source() : ?string {
		...
		if (isset($properties->external_source) && !empty($this->section_id)) {
			...
			$component_model	= ontology_node::get_model_name_by_tipo($component_tipo,true);
			$component			= component_common::get_instance(
				$component_model,
				$component_tipo,
				$this->section_id,
		...
		try {
			...
			// target_filename. Save original file name in a component_input_text if defined
				...
				if (isset($properties->target_filename)) {
					...
					$model_name_target_filename	= ontology_node::get_model_name_by_tipo($properties->target_filename,true);
	...
	*/
	public static function update_dato_version(object $options) : object {
		...
		switch ($update_version) {
			...
			case '6.0.1':
				// component instance
					$model		= ontology_node::get_model_name_by_tipo($options->tipo, true);
					$component	= component_common::get_instance(
						$model,
						$options->tipo,
						$options->section_id,
				...
				if ($is_old_dato===true) {
					...
					// get the source file name
						...
						if(isset($properties->target_filename)) {
							...
							$original_name_model	= ontology_node::get_model_name_by_tipo($original_name_tipo,true);
						...
						if(empty($source_file_name)) {
							...
							$previous_code_model		=  ontology_node::get_model_name_by_tipo($previous_code_tipo,true);
