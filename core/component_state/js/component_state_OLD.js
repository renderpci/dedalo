...
var component_state = new function() {
	...
	*/
	this.init = function(options) {		
		...
		// Set ar_chars
			for (var i = 0; i < options.ar_charts_data.length; i++) {
				var charts_data = JSON.parse(options.ar_charts_data[i])
				this.ar_charts.push(charts_data)
			}
	...
	*/
	this.build_charts = function (ar_value_stats_json, key) {
		...
		if(ar_value_stats_json===null || typeof ar_value_stats_json !== 'object') {
			if(SHOW_DEBUG===true) { console.log("build_charts: Empty 'ar_value_stats_json' received"); };
			return false;
		}
	...
	*/
	this.stats_pie = function(current_dato, chart_id) {
		...
		chart_object.setAttribute("height", n*6)
	...
	this.toggle_graphs = function( button_object, show ) {
		...
		if (typeof button_object=='undefined' || !button_object) {
			button_object = document.querySelectorAll('[data-state_graphs_button]')[0];			
		}		

		const content_div = button_object.nextElementSibling;
		if (content_div.style.display==='none') {
			var ar_charts_len = this.ar_charts.length
			for (var i = 0; i < ar_charts_len; i++) {
				
				// Build chart
				this.build_charts(this.ar_charts[i].graph, this.ar_charts[i].key)
			}
