...
/**
* COMPONENT_FILTER_MASTER — JSON CONTROLLER
* Included-file controller that builds the JSON context+data response for a
* component_filter_master instance. Executed via common::get_json() inside
* the calling object scope ($this = component_filter_master).
*
* Responsibilities:
* - Guard against direct HTTP access (SEC-026 §9.3). The .htaccess
*   <FilesMatch> rule is layer 1; the isset($this) guard below is a
*   server-agnostic layer 2 that fails closed regardless of the web server
*   or display_errors setting.
...
*   In the default (full) branch the resolved target-section metadata
*   (tipo + human-readable label) is attached to the context object so the
*   client knows which project section this filter master component points to.
*   Unlike component_filter, which targets DEDALO_SECTION_PROJECTS_TIPO for
*   record filtering, component_filter_master is used exclusively in the User
*   section (dd128) to record which projects a user is authorised to access.
* - Resolve component data for the current mode:
*     'list' / 'tm' — get_list_value() returns label strings for the projects
*                     the stored locators resolve to (filtered to only those
*                     visible to the current user).
*     'edit' (default) — get_data_lang() returns the raw array of stored
*                     locators, paired with get_datalist() so the client can
*                     render the checkbox grid of available projects. The
*                     datalist comes from
*                     component_filter_master::get_user_authorized_projects()
*                     and is sorted alphabetically by label.
* - component_filter_master (like its parent component_filter) always stores
*   data under DEDALO_DATA_NOLAN (language-neutral). The constructor enforces
...
*   inside component_filter_master::get_json() (inherited from
*   component_common). The variables $this (component_filter_master),
*   $options (request options object), and SHOW_DEBUG (global constant) are
...
*     from_component_tipo,
*     entries: [locator, …],   // raw stored project locators (or null if none)
*     datalist: [              // only in edit mode; absent in list/tm
*       {
*         type:         'project',
*         label:        string,       // project name in display language
*         section_tipo: string,       // e.g. DEDALO_SECTION_PROJECTS_TIPO
...
* (not locators) for the subset of assigned projects that the current user
* can see, or null when no projects are assigned.
*
* @package Dédalo
* @subpackage Core
*/
	...
	if($options->get_context===true) { //  && $permissions>0
		switch ($options->context_type) {
			...
			default:
				...
				// add_request_config is false because component_filter_master does not
				// embed a dataframe; project assignment uses fixed locators and does not
				// require an inline request_config object in the context payload.
				...
				// master component targets. get_ar_target_section_tipo() returns an
				// array containing DEDALO_SECTION_PROJECTS_TIPO (e.g. ['dd153']) when
				// the constant is defined, or [] as a safe fallback.
				// The label is resolved from the ontology using DEDALO_DATA_LANG
				// (interface language) so the client can display the human-readable
				// section name without an extra ontology look-up.
		...
		// is only true when the edit branch explicitly populates it.
			switch ($mode) {
				...
				default:
					...
					// (component_filter_master always uses the language-neutral slot), or
					// null when no project has been assigned yet.
					$value		= $this->get_data_lang();
					// get_datalist() delegates to
					// component_filter_master::get_user_authorized_projects() and returns
					// the parsed project list sorted alphabetically by label.
