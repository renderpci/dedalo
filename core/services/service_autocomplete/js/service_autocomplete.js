...
export const service_autocomplete = function() {
	...
	*/
	this.init = function(options) {
		...
		self.context_search			= self.instance_caller.sqo_context.search
	...
	*/
	...
	// * @return
	// */
	// this.render_source_selector = function(options) {
	//
	// 	const self = this
	//
	// 	// source elements
	// 		const ar_source 	= self.properties.source.config_context
	//
	//
	...
	*/
	this.render_sections_selector = function(){
		...
		const sections_list = ui.create_dom_element({
			...
			const ar_ddo_sections = self.context_search.filter((item) => item.type === 'section' && item.typo === 'ddo')

			for (let i = 0; i < ar_ddo_sections.length; i++) {
				const ddo_section = ar_ddo_sections[i]
	...
	*/
	this.render_inputs_list = function(){
		...
		const inputs_list = ui.create_dom_element({
			...
			const ar_components = []
			const sqo_leng = self.context_search.length


			for (let i = sqo_leng - 1; i >= 0; i--) {
	...
	*/
	this.init_OLD = function(request_options) {
		...
		const input_obj = self.get_input({
			parent : wrapper
		})
	...
	* @return
	*/
	this.search_zenon_OLD = function(search_options) {
	...
	*/
	this.build_filter_fields = function(selection, options) {
		...
		const js_promise = new Promise(function(resolve, reject) {
			...
			// split selector
				const split_selector = ui.create_dom_element({
					element_type 	: "div",
					class_name 	 	: "search_operators_div",
					parent 			: div_container
					})
					const label_split = ui.create_dom_element({
						element_type 	: "label",
						class_name 		: "css_label label",
						parent 			: split_selector,
						text_content 	: get_label["dividir_palabras"] || "Split words"
						})
					const select_split = ui.create_dom_element({
						element_type 	: "select",
						...
						const option_true = ui.create_dom_element({
							element_type 	: "option",
							parent 			: select_split,
							value 			: "true",
							text_content 	: get_label["si"]
							})
						const option_false = ui.create_dom_element({
							element_type 	: "option",
							parent 			: select_split,
							value 			: "false",
	...
	* @return
	*/
	this.build_source_search_selector_OLD = function(options) {
