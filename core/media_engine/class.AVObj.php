...
class AVObj extends MediaObj {
	...
	*/
	...
	/**
	* GET MEDIA STANDARD (PAL / NTSC) (! MOVED TO STATIC FUNCTION IN FFMPEG)
	...
	*/
		...
		// 	} catch (Exception $e) {
		// 		error_log("get_ar_movie_header_info error on read header (quality.$quality)");
		// 	}

		// 	$fps = 25; # default
		// 	if(isset($ar_data[$quality]['fps'])) {
		// 		$fps = $ar_data[$quality]['fps'];
		// 	}
	...
	# GET ASPECT RATIO (16x9 / 4x3) (!) MOVED TO FFMEPG CLASS 12-10-2023
		// public function get_aspect_ratio_DES() {
	...
	public function get_ar_movie_header_info($debug_mode=false) {
		...
		$media_header = Ffmpeg::get_media_streams($local_file);
		...
		// 	$header				= new Zend_Media_Iso14496($local_file, array("base" => "moov"));		#dump($header, ' $header ');die("E");
