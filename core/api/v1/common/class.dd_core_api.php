...
final class dd_core_api {
		...
		// context
			...
			if (true!==login::is_logged()) {
				...
				// section/area/section_tool. Get the page element from get URL vars
					$model		= $tool_name ?? ontology_node::get_modelo_name_by_tipo($tipo, true);
	...
	*/
	public static function save(object $rqo) : object {
		...
		// short vars
			$tipo				= $source->tipo;
			$model				= $source->model ?? ontology_node::get_modelo_name_by_tipo($tipo,true);
			$section_tipo		= $source->section_tipo;
			$section_id			= $source->section_id;
	...
	*/
	public static function count(object $rqo) : object {
		...
		// rqo vars
			...
			$model	= $rqo->source->model ?? ontology_node::get_modelo_name_by_tipo($tipo,true);
			$sqo	= $rqo->sqo;
	...
	*/
	private static function build_json_rows(object $rqo) : object {
		...
		// source vars
			...
			$model				= $ddo_source->model ?? ontology_node::get_modelo_name_by_tipo($ddo_source->tipo,true);
