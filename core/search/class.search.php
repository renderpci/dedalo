			...
			if(SHOW_DEBUG===true) {
				...
				$ar_sections = array_map(function($section_tipo){
					return $section_tipo==='all'
						? $section_tipo
						: $section_tipo .' - '. ontology_node::get_label_by_tipo($section_tipo, DEDALO_DATA_LANG, true, true);
				}, $ar_sections);
				...
				switch (true) {
					...
					default:
						...
						if (!isset($ar_component_filter[0])) {
							$section_name = ontology_node::get_label_by_tipo($section_tipo);
							debug_log(__METHOD__
	...
	public function build_sql_join(array $path) : bool {
		...
		foreach ($path as $key => $step_object) {
			...
			if (!isset($this->ar_sql_joins[$t_name])) {
				...
				$sql_join  = "\n";
				if(SHOW_DEBUG===true) {
					$section_name = ontology_node::get_label_by_tipo($step_object->section_tipo, null, true, false);
			...
			switch ($search_object_format) {
				...
				case 'direct':
					...
					if(SHOW_DEBUG===true) {
						...
						$component_name			= $component_path_data->name ?? '';	//ontology_node::get_label_by_tipo($component_tipo, null, true, false);
	...
	public static function get_query_path(string $tipo, string $section_tipo, bool $resolve_related=true, bool|string $related_tipo=false) : array {
		...
		// Add first level always
			$current_path = new stdClass();
				$current_path->name				= strip_tags(ontology_node::get_label_by_tipo($tipo, DEDALO_DATA_LANG, true, true));
				...
				if(!$result) {
					...
					if(SHOW_DEBUG===true) {
						if ($section_tipo!==DEDALO_ACTIVITY_SECTION_TIPO) {
							$msg .= ' ('.ontology_node::get_label_by_tipo($section_tipo).' - '.ontology_node::get_label_by_tipo($from_component_tipo).')';
