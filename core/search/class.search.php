...
class search {
			...
			if (!property_exists($search_object, 'path')) {
				...
				if (isset($search_object->format) && $search_object->format==='column' && isset($search_object->q_parsed)) {
					...
					if (!isset($search_component->model)) {
						$search_component->model = RecordObj_dd::get_model_name_by_tipo($search_component->component_tipo, true);
					}
	...
	*/
	public function build_union_query(string $sql_query) : string {
		...
		foreach ($this->ar_section_tipo as $key => $current_section_tipo) {
			...
			$model_name = RecordObj_dd::get_model_name_by_tipo($current_section_tipo, true);
			...
			if ($is_global_admin!==true || $force_calculate===true) {
				...
				switch (true) {
					...
					case ($section_tipo===DEDALO_SECTION_USERS_TIPO) :
						...
						# PROJECTS FILTER
							$component_filter_master_model	= RecordObj_dd::get_model_name_by_tipo(DEDALO_FILTER_MASTER_TIPO,true);
	...
	*/
	public static function get_query_path(string $tipo, string $section_tipo, bool $resolve_related=true, bool|string $related_tipo=false) : array {
		...
		$term_model = RecordObj_dd::get_model_name_by_tipo($tipo,true);
	...
	*/
	public static function resolve_path_level(object $path_item, array $ar_locator) : array {
		...
		foreach ($ar_locator as $locator) {
			...
			$model_name	= RecordObj_dd::get_model_name_by_tipo($path_item->component_tipo,true);
