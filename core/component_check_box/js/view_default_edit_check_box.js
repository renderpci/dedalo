...
* Default edit view for component_check_box.
*
...
*
* Selected by render_edit_component_check_box.prototype.edit when context.view
* is 'default', 'print', or absent. For 'print' the caller sets
* self.permissions = 1 before dispatching here so that get_content_data_edit
* produces read-only label nodes instead of interactive checkboxes.
*
* Main export: view_default_edit_check_box; callers invoke .render(self, options).
*/
...
*
* Permissions guard: action buttons (list-navigation and reset) are only built for
* users with write access (self.permissions > 1). Read-only users (permission === 1)
* still receive the full wrapper but with no buttons, and get_content_data_edit will
* render static label nodes instead of interactive checkboxes.
*
* Side effects:
...
view_default_edit_check_box.render = async function(self, options) {
	...
	// ui build_edit returns component wrapper
		...
		// that refresh() and external callers can locate individual checkbox rows
