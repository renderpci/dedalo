...
class diffusion_section_stats extends diffusion {
		...
		// merge and verticalize data to store it
			...
			foreach ($what_obj as $key => $value) {
				$item = new stdClass();
					...
					$item->tipo		= $key;
					$item->value	= $value;
					$item->label	= ontology_node::get_termino_by_tipo($key); // add label for easy human read
				$totals_data[] = $item;
			}
			// where
						...
						foreach ($what_totals as $item) {
							...
							if (isset($what_data_obj->{$item_key})) {
								...
								$what_data_obj->{$item_key} = new stdClass();
									...
									$what_data_obj->{$item_key}->label	= ontology_node::get_termino_by_tipo($item->tipo, $lang, true, true);
									$what_data_obj->{$item_key}->value	= $item->value;
							}
						}
				}
						...
						foreach ($where_totals as $item) {
							...
							if (isset($where_data_obj->{$item_key})) {
								...
								$where_data_obj->{$item_key} = new stdClass();
									...
									$where_data_obj->{$item_key}->label	= ontology_node::get_termino_by_tipo($item->tipo, $lang, true, true);
									$where_data_obj->{$item_key}->value	= $item->value;
							}
						}
				}
						...
						foreach ($publish_totals as $item) {
							...
							if (isset($publish_data_obj->{$item_key})) {
								...
								$publish_data_obj->{$item_key} = new stdClass();
									...
									$publish_data_obj->{$item_key}->label	= ontology_node::get_termino_by_tipo($item->tipo, $lang, true, true);
									$publish_data_obj->{$item_key}->value	= $item->value;
							}
						}
				}
	...
	*/
	public static function parse_totals_for_js(object $totals, string $tipo=USER_ACTIVITY_SECTION_TIPO) : array {
		...
		// who
			$title = ontology_node::get_termino_by_tipo(logger_backend_activity::$_COMPONENT_WHO['tipo'], DEDALO_DATA_LANG, true, true);
		...
		// what
			$title = ontology_node::get_termino_by_tipo(logger_backend_activity::$_COMPONENT_WHAT['tipo'], DEDALO_DATA_LANG, true, true);
			$current_obj = new stdClass();
				$current_obj->title			= $title;
				$current_obj->tipo			= $tipo;
				$current_obj->query			= '';
		...
		// where
			$title = ontology_node::get_termino_by_tipo(logger_backend_activity::$_COMPONENT_WHERE['tipo'], DEDALO_DATA_LANG, true, true);
			$current_obj = new stdClass();
				$current_obj->title			= $title;
				$current_obj->tipo			= $tipo;
				$current_obj->query			= '';
		...
		// publish
			$title = ontology_node::get_termino_by_tipo('dd222', DEDALO_DATA_LANG, true, true);
			$current_obj = new stdClass();
				$current_obj->title			= $title;
				$current_obj->tipo			= $tipo;
				$current_obj->query			= '';
		...
		// when
			$title = ontology_node::get_termino_by_tipo(logger_backend_activity::$_COMPONENT_WHEN['tipo'], DEDALO_DATA_LANG, true, true);
			$current_obj = new stdClass();
				$current_obj->title			= $title;
				$current_obj->tipo			= $tipo;
				$current_obj->query			= '';
