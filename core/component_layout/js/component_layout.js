...
var component_layout = new function() {
	...
	*/
	...
	this.Save_html = function(html_content){
		...
		const dato = this.html_parser_json(html_content)
		return dato
	...
	*/
	this.html_parser_json = function(html_content){
		...
		for (var i = p_len - 1; i >= 0; i--) {
			...
			var n_len = all_nodes.length
			for (var i = n_len - 1; i >= 0; i--) {
	...
	*/
	this.get_ar_components = function(ar_div_components) {
		...
		var d_len = ar_div_components.length
		for (var i = d_len - 1; i >= 0; i--) {
	...
	*/
	this.get_ar_freeText = function(ar_div_freeText){
		...
		var f_len = ar_div_freeText.length
		for (var i = f_len - 1; i >= 0; i--) {
