...
var component_security_areas = new function() {
	...
	*/
	this.update_checks_state = function(component_obj, callback) {
		...
		const checked   = component_obj.checked
		const container = component_obj.parentNode
		...
		// INDETERMINATE . Fijamos como indeterminate el camino hacia arriba cuando no están checkeados los padres
		...
		const update_parents = component_security_areas.checkSiblings( container, checked);
	...
	*/
	...
	this.Save = function(component_obj, event) {
		...
		component_security_areas.update_checks_state(component_obj, function(){				
			...
			component_common.Save(component_obj, component_security_areas.save_arguments).then(function(response) {					
					/*
	...
	*/
	this.get_dato = function (input_obj) {
		...
		const obj_checkbox 		= {};
		const elements_checked 	= $('[name="'+name+'"]:checked, [name="'+name+'"]:indeterminate')
		
		// Iterate checked and indeterminate input elements and store value in object
		...
		$.when.apply($,elements_checked).done(function(){
			...
			for (let tipo in obj_checkbox) {

				if ( tipo.indexOf('-admin') > -1 ) {
					//console.log("changed "+tipo);
					const tipo_clean = tipo.replace("-admin", "", "gi");
	...
	*/
	this.load_access_elements = function( button_obj, event ) {
		...
		const wrapper_obj 	= document.getElementById(button_obj.dataset.wrap_div_id)
		const tipo 	 		= button_obj.dataset.tipo
		const parent 	 	= button_obj.dataset.parent		

		let area_access_obj = get_localStorage('area_access_obj') || '{}';
	...
	*/
	this.toggle_area_childrens = function(button_obj, event) {
		...
		const ar_ul = li.getElementsByTagName('ul')
		...
		if (ar_ul[0]) {
			...
			let area_childrens_obj = get_localStorage('area_childrens_obj') || '{}';
	...
	*/
	this.update_area_childrens_obj_opened = function() {
		...
		for(let tipo in area_childrens_obj)	{
			let ar_ul = document.querySelectorAll('[data-button_tipo="'+tipo+'"]')
			if (ar_ul[0]) {
				ar_ul[0].click()
			}
		}
	...
	*/
	this.update_area_access_obj_opened = function() {
		...
		for(let tipo in area_access_obj)	{
			let ar_span = document.querySelectorAll('[data-button_tipo="'+tipo+'"]')
			if (ar_span[0]) {
				ar_span[0].click()
			}
		}
	...
	*/ /*  NO LONGER USED
	this.update_checks_security_areas_admin = function() {
			...
			if (uncheck_global_admin_areas === true) {
				...
				//if(SHOW_DEBUG===true) console.log("unchecking global_admin_areas");
