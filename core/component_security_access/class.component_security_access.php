			...
			if(isset($section_properties->source) && isset($section_properties->source->request_config)){
				...
				foreach ($v6_children as $ddo) {
					$item = (object)[
						...
						'label'			=> ontology_node::get_label_by_tipo($ddo->tipo, DEDALO_APPLICATION_LANG, true, true),
		...
		foreach($ar_children as $element_tipo) {
			...
			// get the ontology JSON format
				$item = (object)[
					...
					'label'			=> ontology_node::get_label_by_tipo($element_tipo, DEDALO_APPLICATION_LANG, true, true),
					'parent'		=> $tipo
				];
				$ar_elements[] = $item;
