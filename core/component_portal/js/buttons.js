...
buttons.render_list_from_component_data_button = (self) => {
	...
	const mousedown_handler = async (e) => {
		...
		const sqo = clone(self.caller.caller.rqo?.sqo) || {};
		...
		// perform the search and open data in windows
		const perform_open_related_data = async function(){
			...
			if( data_selection.selected_value === 'found' ){
				...
				if(filter_empty === true){
					...
					const label = printf(raw_label, self.label)
		...
		// body
			...
			const raw_body_label = get_label.open_relationships_of_field || 'Open relationships of field {0} from the current record (1) or from all found records ({1})?'
			const body_label = printf(raw_body_label, self.label, self.caller.caller.total)
