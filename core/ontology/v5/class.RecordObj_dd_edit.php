...
class RecordObj_dd_edit extends RecordDataBoundObject {
	...
	*/
	protected static function update_counter(string $tld, ?int $current_value=null) : int|false {
	...
	*/
	public static function get_descriptor_dato_by_tipo( string $terminoID, ?string $lang=null, ?string $tipo=null, bool $fallback=false ) : ?string {
		...
		// term object
		$RecordObject_dd	= new RecordObj_dd_edit($terminoID);
	...
	*/
	public function get_modelo_name() : string {
		...
		$model = RecordObj_dd_edit::get_termino_by_tipo($modelo_tipo, DEDALO_STRUCTURE_LANG, true, false);
	...
	*/
	public function get_legacy_model_name() : ?string {
		...
		$model_name = RecordObj_dd_edit::get_termino_by_tipo(
			$this->get_modelo() ?? '',
	...
	*/
	public static function get_ar_terminoID_by_modelo_name( string $modelo_name, string $prefijo='dd' ) : array {
		...
		// model to terminoID resolution
			$model_terminoID = RecordObj_dd_edit::get_model_terminoID($modelo_name);
			if (empty($model_terminoID)) {
				return $ar_result;
			}

		// search terms with given model
			$RecordObj_dd	= new RecordObj_dd_edit($model_terminoID);
			$ar_result		= $RecordObj_dd->search([
				'strPrimaryKeyName'	=> 'terminoID',
	...
	*/
	public function get_ar_all_models() : array {
		...
		$RecordObj_dd	= new RecordObj_dd_edit(NULL,$this->prefijo);
	...
	public static function get_ar_recursive_childrens( string $terminoID, bool $is_recursion=false, ?array $ar_exclude_models=null, ?string $order_by=null, bool $use_cache=true ) : array {
		...
		for ($i=0; $i < $ar_childrens_size; $i++) {
			...
			if (!empty($ar_exclude_models)) {
				$modelo_name = RecordObj_dd_edit::get_modelo_name_by_tipo($current_terminoID, true);
			...
			$ar_resolved = array_merge(
				$ar_resolved,
				RecordObj_dd_edit::get_ar_recursive_childrens($current_terminoID, true, $ar_exclude_models, $order_by, $use_cache)
			);
		}
	...
	*/
	public static function get_ar_recursive_childrens_with_exclude(string $terminoID, bool $is_recursion=false, array $ar_exclude=array()) : array {
		...
		*/
		...
		#$ar_childrens  = (array)RecordObj_dd_edit::get_ar_childrens($terminoID);
		$RecordObj_dd 	= new RecordObj_dd_edit($terminoID);
		...
		foreach($ar_childrens as $current_terminoID) {
			...
			# Recursion
			$ar_resolved = array_merge( $ar_resolved, (array)RecordObj_dd_edit::get_ar_recursive_childrens( $current_terminoID, true ) );
		}
	...
	*/
	public function get_ar_parents_of_this( bool $ksort=true ) : array {
		...
		do {
			...
			$RecordObj_dd	= new RecordObj_dd_edit($parent);
		...
		switch($relation_type) {
				...
				if(is_array($ar_childrens)) foreach($ar_childrens as $terminoID) {
					...
					if(empty($modelo)) {
						debug_log(__METHOD__
							...
							.' name: ' . RecordObj_dd_edit::get_termino_by_tipo($terminoID)
							, logger::ERROR
						);
						return [];
					}

					$current_modelo_name = RecordObj_dd_edit::get_termino_by_tipo($modelo);
			...
			case 'parent' :
				...
				if(is_array($ar_parents)) foreach($ar_parents as $terminoID) {
					...
					$current_modelo_name = RecordObj_dd_edit::get_termino_by_tipo($modelo);		#dump($modelo_name);
	...
	*/
	public static function get_color( string $section_tipo ) : string {
		...
		$RecordObj_dd	= new RecordObj_dd_edit($section_tipo);
	...
	*/
	public static function check_active_tld( string $tipo ) : bool {
		...
		$active_tlds = RecordObj_dd_edit::get_active_tlds();
		$current_tld = RecordObj_dd_edit::get_prefix_from_tipo($tipo);
		...
		if (!empty($this->terminoID)) {
			...
			if ($result) {
				...
				if( (int)$dedalo_version[0]<=6 && (int)$dedalo_version[1]<3 ){
					$RecordObj_descriptors_dd_edit = new RecordObj_descriptors_dd_edit(RecordObj_descriptors_dd_edit::$descriptors_matrix_table, NULL, $terminoID, 'lg-spa');
					$RecordObj_descriptors_dd_edit->set_tipo('termino');
					$RecordObj_descriptors_dd_edit->set_parent($terminoID);
					$RecordObj_descriptors_dd_edit->set_lang('lg-spa');
					$created_id_descriptors	= $RecordObj_descriptors_dd_edit->Save();
				}
			}
		}
