...
class dd extends dd_elements {
	...
	*/
	public static function get_ar_indexations(string $terminoID) : array {
		...
		$RecordObj_descriptors_dd_edit		= new RecordObj_descriptors_dd_edit($matrix_table, NULL);
		$ar_records						= $RecordObj_descriptors_dd_edit->search($arguments);

		if(count($ar_records)===1) {

			$dato = $ar_records[0];

			# Dato es un array en formato JSON. No obstante, de momento RecordObj_descriptors_dd_edit no convierte automáticamente
	...
	*****************************************************************************/
	function searchTSform(string $terminoID, string $termino, string $def, string $type, ?string $modelo=null) : array {
		...
		$RecordObj_descriptors_dd_edit	= new RecordObj_descriptors_dd_edit($matrix_table, NULL);
		$ar_result					= (array)$RecordObj_descriptors_dd_edit->search($arguments);
