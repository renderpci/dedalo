...
var common = new function() {
	...
	*/
	this.export_str = function(button_obj, wrap_obj) {
		...
		// HTTPX Request
		const js_promise = common.get_json_data(trigger, trigger_vars).then(function(response) {
			if(SHOW_DEBUG===true) {
			...
			if (!response) {
				...
				if (log_messages) {
					...
					alert("[common.export_str] Error. log_messages not found")
				}
			}
		})
	...
	*/
	this.build_version_from_git_master = function(button_obj, wrap_obj) {

		if( !confirm('\nAre you sure to CREATE A NEW VERSION OF DEDALO FROM GIT BRACH MASTER ?\n') ) {
			return false
		}
		...
		const js_promise = common.get_json_data(trigger, trigger_vars).then(function(response) {
			if(SHOW_DEBUG===true) {
				console.log("[common.build_version_from_git_master] response: "+trigger_vars.mode, response);
			}
			...
			if (!response) {
				...
				if (log_messages) {
					...
					console.log("[common.build_version_from_git_master] ", response.msg);
					alert("[common.build_version_from_git_master] Error. log_messages not found")
				}
			}
		})
...
*/
function when_in_dom(node, callback) {
