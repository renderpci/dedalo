...
| `test/parity/tool_export_breakdown_differential.test.ts` | tool_export get_export_grid DEEP BREAKDOWN (the 2026-07-08 export_tabulator rebuild) over a 3-hop multi-author bibliography: per-SEGMENT '\|n' suffix keys (collision-free); breakdown 'rows' explodes every level vertically with author rows nested under their publication and fill_the_gaps filling title/date down the span (nofill = sparse deeper rows); 'default' explodes the first indexed level into sub-rows and suffixes deeper levels into '\|n' columns with ' N+1' label suffixes (identical with fill on/off); 'columns' suffixes every level at height 1 in ddo-major order; relation-leaf FAN-OUT into the component's own request_config children (one column each); deterministic order + 'after' chain + ar_labels section-term/component-label alternation + authoritative 'end'; value format same-top-component dedupe into ONE column (last non-empty flat wins, DECLARED per-level fields_separator); dedalo_raw stored-locator {"dedalo_data":…} wrapper; WC-008 single-step portal COMPACT cells (TS side of the asymmetric pin). | `test/unit/tool_export_breakdown_native.test.ts` (situation `zzbib`, helper `test/helpers/zzbib_export_chain.ts`) — the parity gate STAYS as the frozen record (header notes the twin): only its frozen bodies still pin the PHP fan-out side of WC-008 and the install-record display strings. | 2026-08-23 |
| `test/parity/tool_export_differential.test.ts` | tool_export flat grid base rules: data_format 'rows' normalizes to 'value'; flat display-string rows {rec,sub,c} with '<section>_<component>' keys; multi-hop paths (column = FIRST declared step, value walks each hop's locators to the leaf, level-0 ' \| ' multi-item join, label extended by the fan segments); dedalo_raw literal items + relation locators verbatim under {"dedalo_data":…}, cell_type json (the framed-main fold is RETIRED per WC-2026-08-09, pinned by tool_export_raw_dataframe_native.test.ts); media cells = ABSOLUTE default-quality URL (exportBase + stored files_info file_path), cell_type 'img'. NDJSON protocol + stream≡buffered + per-column emission floors: test/unit/diffusion_export_unified.test.ts (built chain `zzexp`). | `test/unit/tool_export_native.test.ts` (situation `zzbib`) — the parity gate STAYS as the frozen record: only its frozen bodies still pin the multi-target autocomplete_hi thesaurus explosion (per-target-section term columns) and the install-record display strings. | 2026-08-23 |
| `test/parity/relation_corpus_config.test.ts` — the two `hierarchy_types` rows (testmint1006, rsc860) ONLY | `{source:'hierarchy_types'}` sqo resolution: the ACTIVE (hierarchy4 → dd64/1) hierarchy1 registry rows of the named typologies (hierarchy9 → hierarchy13/<id>) yield their hierarchy53 target section, in section_id order; inactive rows excluded; multi-type input unions; the declared request_config sqo is the production entry; cache invalidates on the hierarchy1 save event. The registry is DATABASE STATE — the frozen 26-target answer is monedaiberica's registry and is unassertable on any other database, so the replay rows stay red as the frozen record. | `test/unit/relation_hierarchy_types_native.test.ts` (situation `zzht` + scratch hierarchy1 registry rows in the reserved >= 900000 band, swept + residue asserted) — the parity gate STAYS; its other rows are live (test6228 re-cut green 2026-08-23 per WC-2026-08-23-test6228-clone-recut; testcult1020 vacuity closed same day). | 2026-08-23 |

| `test/parity/activity_read_differential.test.ts` | The dd542 activity listing through the standard read pipeline (search RQO, list mode): the sections envelope entries, the dd543 user portal serving its stored locator entries + per-component server-authoritative pagination, the dd545 what-select RESOLVING the dd42 datalist label in list mode, the dd546/dd544 input_texts serving the stored where/ip values. `matrix_activity` is the engine's own APPEND-ONLY audit log (every API call writes a row, including the suite's), so the frozen `ASC LIMIT 3` window can only ever return the suite's residue — the gate measured the residue, not the engine. | `test/unit/activity_read_native.test.ts` (three scratch dd542 rows at explicit ids in the reserved >= 900000 band; a DESC window is deterministic there regardless of residue; swept with a loud 0-row guard) — the parity gate STAYS as the frozen record of the PHP activity read (header notes the twin) and stays red on the suite DB by construction. | 2026-08-23 |
| `test/parity/get_ontologies_differential.test.ts` | tool_ontology_parser::get_ontologies — the census derivation over matrix_ontology_main (ontology35): the five-field wire descriptor {target_section_tipo, tld, name, typology_id, typology_name} incl. the hierarchy13/<id> → hierarchy16 typology-name hop; a record missing its tld skipped NON-fatally into the `errors` envelope extension key (never nested in `data`); the activeOnly hierarchy4 → dd64/1 filter (updateOntologyInfo's subset) vs the full walk; plus the three developer security gates (non-developer refused, export_ontologies registered + developer-gated, empty options passes). The frozen body is monedaiberica's ~230-TLD REGISTERED set — a fact about which install is loaded (its TLD set and the suite DB's are disjoint). | `test/unit/get_ontologies_native.test.ts` (three scratch ontology35 rows, zz* TLDs, reserved id band, swept + guarded; the security tests run UNGATED — they are fully native and were needlessly credential-gated in the differential) — the parity gate STAYS as the frozen record of the install census and stays red on the suite DB by construction. | 2026-08-23 |
