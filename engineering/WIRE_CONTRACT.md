...
`delete_tag` (PHP's second action on this class — tool_indexation's "remove
this index tag from every lang") is **NOT ported and NOT registered**: an
unregistered pair is the documented 400, which is the loud failure the house
rule wants, rather than a handler that reports success while leaving the tag in
the text. Ledgered as open in `rewrite/LEDGER.md`.

Gate: `test/unit/tags_info.test.ts`.
