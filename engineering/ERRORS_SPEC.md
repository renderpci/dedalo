...
That first census was one root short: browser JS SERVED from `src/` (the tool
subsystem's `src/core/tools/client/js/*.js`) was in neither scanned tree, and
its two surviving `.result` reads left every tool context null — a blank label
on every tool header — until the same day. The corpus is client code by
DESTINATION, not by directory: `SCAN_ROOTS` now also carries
`src/**/client/js/**/*.js`, and the gate pins `tool_common.js` as present in
the census (see the WC entry's addendum).
