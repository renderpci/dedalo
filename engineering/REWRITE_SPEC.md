...
- **PostgreSQL is the system of record** (same schema/matrix as PHP). Use Bun-native Postgres access with prepared statements; preserve the matrix read/write contract.
  ...
  > **ADDENDUM 2026-08-05 (S3-38's rule, applied to the OUTBOUND side).** `src/external/` — records resolved from a third-party service — is a **PEER of `src/core`, `src/diffusion` and `src/ai`, never a part of core**, and the seam grows the same way: `src/core/**` reaches it ONLY through lazy imports of the `src/external/api/` facade (`test/unit/boundary_seam_tripwire.test.ts` FACADE_PREFIX; rationale 2 in `engineering/CONVENTIONS.md` §2), so transport, breaker, cache and the per-service adapters stay private and an install with no external section loads none of it. Two rules are specific to this subsystem because it is the one place an editor's keystroke becomes a request LEAVING the institution: (a) **exactly one outbound door** — `external/transport.ts`, whose nine-step order is itself the invariant (`external_outbound_tripwire`); (b) **the operator's host allowlist is the door, not a narrowing of one** — empty refuses everything, because the target URL is assembled from the ontology, which is editable data. And it is READ-ONLY in both directions: Dédalo never writes to the service and never writes a remote value into a matrix record (`external_write_refusal_tripwire`). Authority: `engineering/EXTERNAL_SPEC.md`; wire law: the `WC-2026-08-0{5,6}-external-*` entries. Levers: `DEDALO_EXTERNAL_*` (../private/.env).
...
4. **Authorization / permissions matrix.** Three-tier ACL (schema-level (section_tipo,component_tipo)→0–3; per-record projects filter; hard-coded bypasses for superuser/tools/maintenance/time-machine), computed with a layered cache and correct invalidation, and enforced **before** search runs and **per-SQO-target**. Permissions are capped per caller and are **server-authoritative** — never trust client-supplied permission or `skip_projects_filter`-style flags. (`class.security.php`.)
   - **Own-user-record rules (dd128).** On the actor's OWN user record the matrix level is both RAISED and LOWERED, and the rules are a WRITE gate, not only a display stamp — every door onto a dd128 component must consult `resolveOwnUserRecordPermission` (`src/core/security/permissions.ts`) before the matrix: the security-administrator flag (dd244) is forced to read even for a global admin; profile/developer/username/section_id (dd1725/dd515/dd132/dd330) are forced to read for non-global-admins (anti self-elevation); name/email/password/image (dd452/dd134/dd133/dd522) are forced to write, which is the only reason the self-service profile editor works at all — dd128 is not project-assigned, so most profiles grant 0 on it. Dropping the downgrade half lets a user with a dd128 write grant assign themselves a profile or the developer flag through the ordinary save API. (`component_common::resolve_component_read_permission`.) Gated by `test/unit/user_self_record_permissions.test.ts`.
