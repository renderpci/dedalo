...
class tool_upload { // extends tool_common
	...
	*/
	...
	// 									$response->msg .= trim(" Error on read or create media DEDALO_TOOL_EXPORT_FOLDER_PATH default directory. Permission denied ");
	// 									return $response;
	// 								}
	...
	* @return
	*/
	public static function process_uploaded_file($request_options) {
		$start_time=microtime(1);
		...
		// options
			$file_data 		= $request_options->file_data;
			$component_tipo	= $request_options->tipo;
			$section_tipo	= $request_options->section_tipo;
			$section_id		= $request_options->section_id;
			$quality		= $request_options->quality;
