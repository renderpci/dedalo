					...
					foreach ($ar_zotero_data as $zotero_obj) {
						...
						// Processing Zotero record
							...
							foreach ($zotero_obj as $name => $value) {
								...
								switch ($name) {
									case 'id':
										$current_model	= RecordObj_dd::get_model_name_by_tipo($ddo->tipo,true); // component_input_text
									...
									case 'type':
										...
										if (empty($data)) {
											...
											$current_model_name = RecordObj_dd::get_model_name_by_tipo($ddo->tipo,true);
											$component = component_common::get_instance(
												$current_model_name,
												$ddo->tipo,
												$section_id,
									...
									case 'container-title':
										...
										if ($section_id_list>0) {
											...
											$current_model 				= RecordObj_dd::get_model_name_by_tipo($series_ddo->tipo,true);
										...
										// add locator
											$current_model	= RecordObj_dd::get_model_name_by_tipo($ddo->tipo,true);
											$component		= component_common::get_instance(
												$current_model,
												$ddo->tipo,
												$section_id,
									...
									case 'ISBN':
										...
										$current_model	= RecordObj_dd::get_model_name_by_tipo($component_tipo,true); // component_relation_select
										$component		= component_common::get_instance(
											$current_model,
											$component_tipo,
											$section_id,
						...
						// Processing temporal section
							...
							foreach ($ar_ddo_map as $ddo) {
								...
								$model			= RecordObj_dd::get_model_name_by_tipo($ddo->tipo,true);
