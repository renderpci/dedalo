...
const MAX_BODY_BYTES = 20 * 1024 * 1024; // 20MB - generous for a metadata listing page, still bounded.
	...
	for (let redirectCount = 0; redirectCount <= MAX_REDIRECTS; redirectCount++) {
		...
		const response = await fetch(currentUrl, {
			...
			headers: {
				...
				Accept: 'text/xml,application/xml,text/html,application/xhtml+xml',
			},
