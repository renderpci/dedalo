		...
		foreach ($ar_csv_data as $rkey => $columns) {
			...
			// Iterate fields/columns
				foreach ($columns as $key => $value) {
					...
					$process_info->compomnent_label = ontology_node::get_termino_by_tipo($component_tipo,DEDALO_APPLICATION_LANG, true);
	...
	public static function get_section_components_list(object $options) : object {
		...
		try {
			...
			if (!empty($components_list)) {
				...
				$response->label	= ontology_node::get_termino_by_tipo($section_tipo, DEDALO_APPLICATION_LANG, true);
