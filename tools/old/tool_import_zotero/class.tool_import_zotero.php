...
define('TOOL_IMPORT_ZOTERO_UPLOAD_DIR'	, DEDALO_MEDIA_PATH . DEDALO_PDF_FOLDER .'/temp'.'/files/' .'user_'.$_SESSION['dedalo']['auth']['user_id'].'/' );
define('TOOL_IMPORT_ZOTERO_UPLOAD_URL'	, DEDALO_MEDIA_URL  . DEDALO_PDF_FOLDER .'/temp'.'/files/' .'user_'.$_SESSION['dedalo']['auth']['user_id'].'/' );
	...
	static function set_up() {
		...
		if (isset($_REQUEST['button_tipo']) && isset($_REQUEST['t']) ) {
			...
			if (isset($properties->process_script)) {
				...
				tool_import_zotero::$process_script = $properties->process_script;	// Fix current path
				if(SHOW_DEBUG) {
