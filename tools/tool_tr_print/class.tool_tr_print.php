...
class tool_tr_print extends tool_common {
	...
	*/
	public function get_av_duration() {
		...
		$model_name		= ontology_node::get_modelo_name_by_tipo($related_component_av_tipo,true);
	...
	*/
	public function get_tr_data() {
		...
		# date
			$current_tipo	= 'rsc44';
			$model_name		= ontology_node::get_modelo_name_by_tipo($current_tipo, true); // 'component_date';
			$component		= component_common::get_instance($model_name,
														  $current_tipo,
														  $parent,
														  'edit',
		...
		# municipality
			$current_tipo	= 'rsc46';
			$model_name		= ontology_node::get_modelo_name_by_tipo($current_tipo, true); // component_autocomplete_ts
			$component		= component_common::get_instance($model_name,
														  $current_tipo,
														  $parent,
														  'edit',
	...
	public function get_interview_data() {
		...
		foreach ($inverse_locators as $current_locator) {
			...
			if ($current_section_tipo==='oh1') {
				...
				# Informants
					$current_tipo	= 'oh24';
					$model_name		= ontology_node::get_modelo_name_by_tipo($current_tipo, true); // component_portal
					$component		= component_common::get_instance($model_name,
																  $current_tipo,
																  $current_section_id,
																  'edit',
	...
	*/
	public function get_informants_data( $ar_locators ) {
		...
		foreach ($ar_locators as $current_locator) {
			...
			# name
				$current_tipo	= 'rsc85';
				$model_name		= ontology_node::get_modelo_name_by_tipo($current_tipo, true); // component_input_text
				$component		= component_common::get_instance($model_name,
															  $current_tipo,
