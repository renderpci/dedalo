...
export const chat_render = class chat_render {
	...
	constructor() {
		...
		// attachments (vision) — populated by build() when on_attach_file is given
		this._on_attach_file		= null
		this._attach_button			= null
		this._attach_input			= null
		this._attachment_chips		= null
	}//end constructor
	...
	build(options={}) {
		...
		this._on_attach_file		= options.on_attach_file || null
	...
	// attachment chips row (vision) — hidden until a file is attached
		this._attachment_chips = document.createElement('div')
		this._attachment_chips.classList.add('assistant_attachment_chips')
		this._attachment_chips.style.display = 'none'
		content_data.appendChild(this._attachment_chips)

	// footer with input
		const input_row = document.createElement('div')
		input_row.classList.add('assistant_footer')

		// image attach (vision models only; visibility via set_attachments_visible)
		if (this._on_attach_file) {
			this._attach_input = document.createElement('input')
			this._attach_input.type = 'file'
			this._attach_input.accept = 'image/jpeg,image/png,image/webp,image/gif'
			this._attach_input.style.display = 'none'
			this._attach_input.addEventListener('change', function() {
				const file = self._attach_input.files && self._attach_input.files[0]
				if (file) self._on_attach_file(file)
				self._attach_input.value = ''
			})
			this._attach_button = document.createElement('button')
			this._attach_button.type = 'button'
			this._attach_button.classList.add('assistant_attach_button')
			this._attach_button.title = t('attach_image', 'Attach image')
			this._attach_button.textContent = '📎' // 📎
			this._attach_button.style.display = 'none'
			this._attach_button.addEventListener('click', function() {
				self._attach_input.click()
			})
			input_row.appendChild(this._attach_input)
			input_row.appendChild(this._attach_button)
		}
	...
	/**
	 * SET_MODEL_BADGE
	 * Updates the toolbar badge showing the active model and its EGRESS class
	 * ("local / private" vs "external service") \u2014 the user-facing signal of
	 * where this conversation's data may travel.
	 *
	 * @param {string} label        - Catalog model label.
	 * @param {string} egress_label - Localised egress text.
	 * @param {string} egress_class - 'local' | 'external' (drives the badge color class).
	 */
	set_model_badge(label, egress_label, egress_class) {
		if (!this._model_badge) return
		this._model_badge.textContent = (label || '?') + ' · ' + (egress_label || '')
		this._model_badge.title = label || ''
		this._model_badge.classList.remove('assistant_egress_badge_local', 'assistant_egress_badge_external')
		this._model_badge.classList.add(
			egress_class === 'local' ? 'assistant_egress_badge_local' : 'assistant_egress_badge_external'
		)
	}//end set_model_badge
	...
	/**
	 * SET_ATTACHMENTS_VISIBLE
	 * Shows/hides the image-attach affordance (vision-capable models only).
	 * @param {boolean} visible
	 */
	set_attachments_visible(visible) {
		if (!this._attach_button) return
		this._attach_button.style.display = visible ? 'flex' : 'none'
		if (!visible) this.clear_attachment_chips()
	}//end set_attachments_visible



	/**
	 * ADD_ATTACHMENT_CHIP
	 * Renders a pending-attachment chip above the input.
	 * @param {string} name - file name
	 */
	add_attachment_chip(name) {
		if (!this._attachment_chips) return
		const chip = document.createElement('span')
		chip.classList.add('assistant_attachment_chip')
		chip.textContent = '🖼 ' + (name || 'image')
		this._attachment_chips.appendChild(chip)
		this._attachment_chips.style.display = 'flex'
	}//end add_attachment_chip



	/**
	 * CLEAR_ATTACHMENT_CHIPS
	 * Removes all pending-attachment chips (after send or model change).
	 */
	clear_attachment_chips() {
		if (!this._attachment_chips) return
		this._attachment_chips.innerHTML = ''
		this._attachment_chips.style.display = 'none'
	}//end clear_attachment_chips
	...
	// model select — the SERVER-defined catalog (agent_models): each entry is
	// {id, label, egress: 'local'|'external', vision, default}. No endpoints,
	// no keys, no provider-native ids ever reach the browser.
		const model_label = document.createElement('label')
		model_label.classList.add('assistant_settings_label')
		model_label.textContent = t('model', 'Model')
		...
		models.forEach(function(m) {
			...
			opt.value = m.id
			const egress_text = m.egress === 'local'
				? t('local_private', 'local / private')
				: t('external_service', 'external service')
			const vision_text = m.vision === true ? ' · ' + t('vision_capable', 'vision') : ''
			opt.textContent = (m.label || m.id) + ' — ' + egress_text + vision_text
			opt.dataset.egress = m.egress || 'external'
			opt.dataset.vision = m.vision === true ? '1' : '0'
			if (m.id === settings.model_id) opt.selected = true
			model_select.appendChild(opt)
		})
		model_label.appendChild(model_select)
		this._settings_popover.appendChild(model_label)

	// write-mode toggle — rendered ONLY when the server says write is available
	// to this user (the server re-validates regardless; this is UX, not authz)
		let write_checkbox = null
		if (settings.write_allowed === true) {
			const write_label = document.createElement('label')
			write_label.classList.add('assistant_settings_label', 'assistant_write_toggle')
			write_checkbox = document.createElement('input')
			write_checkbox.type = 'checkbox'
			write_checkbox.checked = settings.write_mode === true
			const write_text = document.createElement('span')
			write_text.textContent = t('write_mode', 'Allow the assistant to propose changes')
			write_text.title = t('write_mode_hint', 'Changes are proposed as a plan you review and confirm before anything is saved.')
			write_label.appendChild(write_checkbox)
			write_label.appendChild(write_text)
			this._settings_popover.appendChild(write_label)
		}
	...
	// apply button
		...
		apply.addEventListener('click', function() {
			...
			self._on_settings_change({
				...
				write_mode	: write_checkbox ? write_checkbox.checked : false
			})
		})
	...
	/**
	 * CONFIRM_PLAN
	 * Renders an INLINE change-plan confirm card into the message list (not
	 * window.confirm): the plan summary as title, one row per op (the op's
	 * human summary + a monospace tool chip), and Apply / Cancel buttons.
	 * Resolves once the user decides; the card then flips to a static state.
	 *
	 * The card is UX only — the server re-validates every gate at apply time
	 * (agent_apply re-checks the plan hash and all permissions).
	 *
	 * @param {Object} plan - {plan_version, summary, ops:[{op_id, tool, args, summary}], plan_hash}
	 * @returns {Promise<boolean>} true = user clicked Apply
	 */
	confirm_plan(plan) {
		...
		return new Promise(function(resolve) {
			...
			card.classList.add('assistant_plan_card')

			const title = document.createElement('div')
			title.classList.add('assistant_plan_title')
			title.textContent = t('proposed_changes', 'Proposed changes') + ': ' + (plan.summary || '')
			card.appendChild(title)

			const ops = Array.isArray(plan.ops) ? plan.ops : []
			const list = document.createElement('ul')
			list.classList.add('assistant_plan_ops')
			ops.forEach(function(op) {
				const row = document.createElement('li')
				row.classList.add('assistant_plan_op')
				const tool_chip = document.createElement('code')
				tool_chip.classList.add('assistant_plan_tool')
				tool_chip.textContent = op.tool || ''
				const text = document.createElement('span')
				text.textContent = ' ' + (op.summary || op.op_id || '')
				row.appendChild(tool_chip)
				row.appendChild(text)

				// The op SUMMARY is model-authored prose; the ARGS are what will
				// actually execute (the plan hash covers them). Showing only the
				// summary would let a prompt-injected plan describe one change and
				// perform another — the human must confirm the real target/value.
				const args = document.createElement('pre')
				args.classList.add('assistant_plan_args')
				args.textContent = chat_render._stringify(op.args, 600)
			...
			actions.classList.add('assistant_plan_actions')

			const decide = function(confirmed) {
				apply_button.disabled = true
				cancel_button.disabled = true
				card.classList.add(confirmed ? 'is_applying' : 'is_discarded')
				title.textContent = confirmed
					? t('applying', 'Applying…') + ' ' + (plan.summary || '')
					: t('plan_discarded', 'Proposed changes discarded')
				resolve(confirmed)
			}

			const apply_button = document.createElement('button')
			apply_button.type = 'button'
			apply_button.classList.add('assistant_plan_apply')
			apply_button.textContent = t('apply', 'Apply')
			apply_button.addEventListener('click', function() { decide(true) })

			const cancel_button = document.createElement('button')
			cancel_button.type = 'button'
			cancel_button.classList.add('assistant_plan_cancel')
			cancel_button.textContent = t('cancel', 'Cancel')
			cancel_button.addEventListener('click', function() { decide(false) })

			actions.appendChild(apply_button)
			actions.appendChild(cancel_button)
			card.appendChild(actions)

			self._messages.appendChild(card)
	...
	/**
	 * ADD_APPLY_REPORT
	 * Renders the agent_apply outcome: one ✅ row per applied op (with created
	 * record ids), a ❌ row for the failed op (code/message/hint), ⏭ rows for
	 * skipped ops. A top-level {error} (hash mismatch, re-validation failure)
	 * renders as a single labeled error row.
	 *
	 * @param {Object} report - {applied:[{op_id}], failed?:{op_id,error}, skipped:[], created:{}} | {error:{code,message,hint}}
	 */
	add_apply_report(report) {

		const card = document.createElement('div')
		card.classList.add('assistant_apply_report')

		if (report && report.error) {
			const row = document.createElement('div')
			row.classList.add('assistant_apply_row', 'is_failed')
			const hint = report.error.hint ? ' — ' + report.error.hint : ''
			row.textContent = '❌ ' + t('apply_failed', 'Apply failed') + ': '
				+ (report.error.code || '') + ' ' + (report.error.message || '') + hint
			...
			applied.forEach(function(op) {
				const row = document.createElement('div')
				row.classList.add('assistant_apply_row', 'is_applied')
				const created_id = created[op.op_id] !== undefined
					? ' → ' + t('created', 'created') + ' #' + created[op.op_id]
					: ''
				row.textContent = '✅ ' + (op.op_id || '') + created_id
				card.appendChild(row)
			})
			...
			if (report.failed) {
				const row = document.createElement('div')
				row.classList.add('assistant_apply_row', 'is_failed')
				const error = report.failed.error && report.failed.error.error
					? report.failed.error.error
					: (report.failed.error || {})
				const hint = error.hint ? ' — ' + error.hint : ''
				row.textContent = '❌ ' + (report.failed.op_id || '') + ': '
					+ (error.code || '') + ' ' + (error.message || '') + hint
				card.appendChild(row)
			}

			skipped.forEach(function(op_id) {
				const row = document.createElement('div')
				row.classList.add('assistant_apply_row', 'is_skipped')
				row.textContent = '⏭ ' + op_id + ' (' + t('skipped', 'skipped') + ')'
				card.appendChild(row)
			})

			if (card.children.length === 0) {
				const row = document.createElement('div')
				row.classList.add('assistant_apply_row')
				row.textContent = t('applied', 'Applied')
				card.appendChild(row)
			}
		}
