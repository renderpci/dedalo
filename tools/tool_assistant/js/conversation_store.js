...
// v2 (WC-013, server-driven assistant): threads gain `history` (the
// SERVER-returned text-only wire history, resent verbatim each turn) and
// `model_id` (the per-thread catalog model choice). Bumping the suffix means
// v1 blobs are simply ignored — no migration step (the module's design).
const STORAGE_PREFIX	= 'dedalo_assistant_threads_v2::'
const MAX_THREADS		= 25
const MAX_MESSAGES		= 200
// cap on the serialized wire history per thread — oldest entries drop first
const MAX_HISTORY_BYTES	= 200 * 1024
...
export const conversation_store = class conversation_store {
	...
	save(id, messages, extra={}) {
		...
		// wire history (server-returned, resent verbatim): cap by serialized size,
		// dropping the OLDEST entries first so the recent context survives
		let history = Array.isArray(extra.history)
			? extra.history
			: (existing && existing.history) || []
		while (history.length > 2 && JSON.stringify(history).length > MAX_HISTORY_BYTES) {
		...
		data.threads[id] = {
			...
			messages	: trimmed,
			history		: history,
			model_id	: extra.model_id !== undefined ? extra.model_id : ((existing && existing.model_id) || null)
		}
