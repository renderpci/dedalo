 ...
 * single `ai_assistant` instance, delegates DOM construction to that instance's
 ...
 *      UI re-renders.
 *   4. `build_chat_ui()` performs the async boot sequence (MCP handshake + model
 *      load) and returns a `content_data` DOM element ready to insert.
 ...
 *
 * Dependencies: ui (common/js/ui.js), ai_assistant (./ai_assistant.js).
 */
 ...
 *
 * The `ai_assistant` instance is created once and cached on `self.assistant_instance`
 * so that reloading the tool panel does not tear down an in-progress conversation
 * or reload model weights.
 *
 * `render_level` values:
...
render_tool_assistant.prototype.edit = async function(options={}) {
	...
	// build chat UI content (returns content_data div)
		if (!self.assistant_instance) {
			// Lazy-create: keep a single ai_assistant alive across re-renders so
			// conversation history and model state are preserved.
