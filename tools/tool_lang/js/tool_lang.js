...
* → stream → restore → save) to translate_component_browser() in browser_translation.js.
* The worker is module-level and reused across calls so the model is downloaded
* and compiled only once per page session.
*
...
* @param {Function} [options.on_uncertain] - (report) => Promise<boolean>. Asked to confirm the save when
*   the translated value could not be verified to have kept every Dédalo mark intact. Without it the
*   engine refuses the save, so a damaged value is never persisted unnoticed.
...
tool_lang.prototype.automatic_translation_browser = async function(options) {
	...
	// json_langs is cached on the instance to avoid re-fetching the lang map.
		...
		return translate_component_browser({
			...
			on_uncertain				: options.on_uncertain
		})
