...
// import
	...
	import {render_tool_revert} from './render_tool_revert.js'
	import {
		on_dragstart,
		// on_dragend,
		on_dragover,
...
*/
export const tool_revert = function () {
...
*/
// prototypes assign
	tool_revert.prototype.render						= tool_common.prototype.render
	tool_revert.prototype.destroy						= common.prototype.destroy
	tool_revert.prototype.refresh						= common.prototype.refresh
	tool_revert.prototype.edit							= render_tool_revert.prototype.edit
	tool_revert.prototype.build_export_component		= render_tool_revert.prototype.build_export_component
	// get and render list of components from common
	tool_revert.prototype.get_section_elements_context	= common.prototype.get_section_elements_context
	tool_revert.prototype.calculate_component_path		= common.prototype.calculate_component_path
	// drag
	tool_revert.prototype.on_dragstart					= on_dragstart
	// tool_revert.prototype.on_dragend					= on_dragend
	tool_revert.prototype.on_dragover					= on_dragover
	tool_revert.prototype.on_dragleave					= on_dragleave
	tool_revert.prototype.on_drop						= on_drop
...
*	section_tipo: "rsc167"
*	tipo: "rsc36"
*	tool_config: {section_id: "2", section_tipo: "dd1324", name: "tool_revert", label: "Tool Indexation", icon: "/v6/tools/tool_revert/img/icon.svg", …}
* }
* @return bool common_init
*/
tool_revert.prototype.init = async function(options) {
...
*/
tool_revert.prototype.build = async function(autoload=false) {
...
* 	as 'tmp_export_1'
*/
tool_revert.prototype.get_section_id = function() {
...
*/
tool_revert.prototype.get_export_grid = async function(options) {
...
*/
	// tool_revert.prototype.get_export_csv = async function () {
...
* @return bool
*/
tool_revert.prototype.get_export_xsl = async function (options) {
...
* @return promise: bool
*/
tool_revert.prototype.on_close_actions = async function(open_as) {
...
* Read and replaces the tool_revert_config data portion from current section
* Saves the updated value read from self.ar_ddo_to_export
* @return bool
*/
tool_revert.prototype.update_local_db_data = async function() {
	...
	// get_local_db_data
	const id		= 'tool_revert_config'
	const response	= await data_manager.get_local_db_data(
		id,
		'data'
	)

	// tool_revert_config. Current section tool_revert_config (fallback to basic object)
	const tool_revert_config = response && response.value
		? response.value
		: {
			[target_section_tipo] : []
		  }
	...
	// update current key only and save whole object
		tool_revert_config[target_section_tipo] = self.ar_ddo_to_export

	// save
		const cache_data = {
			id		: 'tool_revert_config',
			value	: tool_revert_config
		}
	...
	// check if already exists current target section_tipo config ddo
		// const found = tool_revert_config[target_section_tipo]
		// 	? tool_revert_config[target_section_tipo].find(el => el.id===ddo.id)
		// 	: undefined

	// if not exists current ddo (as expected), add it to local database using current target section_tipo as key
		// if (!found) {
		// 	tool_revert_config[target_section_tipo] = tool_revert_config[target_section_tipo] || []
		// 	tool_revert_config[target_section_tipo].push(ddo)
		// 	// save
		// 	const cache_data = {
		// 		id		: 'tool_revert_config',
		// 		value	: tool_revert_config
