			...
			if(empty($tool_by_name)) {
				...
				if (!empty($reg_result->ar_records)) {
					...
					// config_name_component. create the name component in the config section
						$config_name_component_model	= ontology_node::get_model_name_by_tipo($component_tipo_tool_name,true);
	...
	public static function get_all_config_tool() : array {
		...
		// map result as ar_config
			$ar_config = array_map(function($record) use($name_tipo, $config_tipo){
				...
				// name
					$model		= ontology_node::get_model_name_by_tipo($name_tipo,true);
					$component	= component_common::get_instance(
						$model,
						$name_tipo,
						$record->section_id,
				...
				// config
					$model		= ontology_node::get_model_name_by_tipo($config_tipo,true);
					$component	= component_common::get_instance(
						$model,
						$config_tipo,
						$record->section_id,
