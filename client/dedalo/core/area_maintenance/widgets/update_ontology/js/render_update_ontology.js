...
*    b. **Config panel** (`render_server_vars`) — reads-only display of the two config
*       keys that control ontology source: `STRUCTURE_FROM_SERVER` and
...
*       form input for the active ontology TLDs updates accordingly.
...
*   active_ontology_tlds : string[],   // e.g. ['dd','ontology','rsc']
...
const get_content_data_edit = async function(self) {
	...
	// server config vars (STRUCTURE_FROM_SERVER, ACTIVE_ONTOLOGY_TLDS)
		...
		if (self.caller?.init_form) {
			self.caller.init_form({
				...
				on_render : (nodes) => {
					...
					// When the user selects a server via the radio buttons, update the TLD
				...
				on_submit : async (e, values) => {
					...
					// ar_active_ontology_tlds: parse the comma-separated TLD input into a clean array.
					// values[0] is always the first (and only) input: 'active_ontology_tlds'.
						// sample value:
						// [{
						// 	name : "active_ontology_tlds",
						...
						if (!ar_active_ontology_tlds.length) {
							alert("Error: no TLD is selected");
							return
						}
	...
	// render export to translate
		const export_to_translate_container = render_export_to_translate(self, active_ontology_tlds)
...
*   - `ACTIVE_ONTOLOGY_TLDS` — comma-joined list of the top-level domains of the
*     ontologies active in this installation (e.g. `"dd, ontology, rsc"`).  This is
*     the default set fed into the update form's TLD input.
*
* The output is a two-column CSS grid (class `config_grid`) with a bold "Config"
* header spanning both columns.  Each key occupies one label/value row.
*
* This function is intentionally read-only display; the `ACTIVE_ONTOLOGY_TLDS` value
...
* @param {Array}   value.active_ontology_tlds  - Current value of `ACTIVE_ONTOLOGY_TLDS`
...
const render_server_vars = function (value) {
	...
	// add vars: local helper that appends a label/value row pair to the config grid
		...
		add_to_grid('ACTIVE_ONTOLOGY_TLDS', active_ontology_tlds.join(', '), config_grid)
...
*   2. `export_ontology_tld_list`       — comma-separated TLDs to export
*                                         (pre-filled from `active_ontology_tlds`).
...
* @param {Array}  active_ontology_tlds  - Default TLD list from `class.update_ontology::get_value()`
* @returns {HTMLElement} The assembled `export_to_translate_container` node
*/
const render_export_to_translate = function (self, active_ontology_tlds) {
