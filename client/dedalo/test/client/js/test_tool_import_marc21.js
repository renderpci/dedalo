 ...
 * Client-side coverage for the MARC 21 import tool.
 *
 * The tool's deeper build/render path needs a host section/caller plus the
 * service_upload (multi-file queue mode) and service_tmp_section sub-instances resolved through
 * get_instance, none of which is guaranteed in the headless harness. This suite
...
import {tool_import_marc21} from '../../../tools/tool_import_marc21/js/tool_import_marc21.js'



describe('TOOL_IMPORT_MARC21 CLIENT TEST', function() {

	this.timeout(10000)

	it('module exports the tool constructor', function() {
		assert.equal(typeof tool_import_marc21, 'function', 'expected tool_import_marc21 to be a constructor function')
	})

	it('construct seeds the documented instance properties', function() {
		const instance = new tool_import_marc21()
	...
	it('prototype is wired with the lifecycle methods', function() {
		...
		assert.equal(typeof tool_import_marc21.prototype.render, 'function', 'expected render wired')
		assert.equal(typeof tool_import_marc21.prototype.destroy, 'function', 'expected destroy wired')
		assert.equal(typeof tool_import_marc21.prototype.refresh, 'function', 'expected refresh wired')
		// render mode delegated to render_tool_import_marc21
		assert.equal(typeof tool_import_marc21.prototype.edit, 'function', 'expected edit wired')
		// tool-specific overrides defined on the module
		assert.equal(typeof tool_import_marc21.prototype.init, 'function', 'expected init defined')
		assert.equal(typeof tool_import_marc21.prototype.build, 'function', 'expected build defined')
	})
