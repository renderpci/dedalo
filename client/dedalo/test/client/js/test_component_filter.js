...
describe(`COMPONENT_FILTER DATA OPERATIONS`, function() {
	...
	it(`checkbox uncheck (remove)`, async function() {
		...
		// component_filter enforces a minimum of one checked option in edit mode
		// (render_edit_component_filter: it restores the box and alert()s). So
		// stage a SECOND checked box first — otherwise this case tests the refusal
		// path while claiming to test remove, and raises a blocking alert().
			const checkboxes = node.querySelectorAll('input[type="checkbox"]')
			const all = Array.from(checkboxes)
			if (all.filter(cb => cb.checked).length < 2) {
				const spare = all.find(cb => !cb.checked)
				assert.isOk(spare, 'a second checkbox expected so one can be unchecked')
				spare.checked = true
			...
			const checked = all.find(cb => cb.checked)
			assert.isOk(checked, 'a checked checkbox expected to uncheck')
			assert.isAbove(all.filter(cb => cb.checked).length, 1, 'setup: more than one option must be checked')

			// simulate uncheck
			checked.checked = false
			checked.dispatchEvent(new Event('change', { bubbles: true }))

			// the uncheck save is async like the check one
			for (let i=0; i<40 && instance.data.changed_data?.[0]?.action!=='remove'; i++) {
				await new Promise(resolve => setTimeout(resolve, 50))
			}
