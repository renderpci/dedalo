...
// Per-suite test3 record isolation (WC-021 in-run isolation): this is the
// all-component save SWEEP — it clears and re-inserts random values into
// EVERY test3 element. It writes to its OWN record (test3/10, a clone of
// record 1 provisioned by the canonical seed) so its churn never poisons the
// record another (read) suite is asserting against. Non-test3 elements (e.g.
// component_external → zenon1) keep their own section_id.
const SWEEP_SECTION_ID = 10
...
describe("COMPONENTS DATA CHANGES", async function() {
	...
	for (let i = 0; i < elements.length; i++) {
		...
		// Redirect test3 writes to this sweep's private record (leaves other
		// sections — e.g. component_external's zenon1 — untouched).
		const sweep_section_id = element.section_tipo === 'test3'
			? SWEEP_SECTION_ID
			: element.section_id
