...
// element options for component_email
	const element = elements.find(el => el.model==='component_email')
	if (!element) {
		console.error('Error: component_email not found in elements');
	}

	const section_tipo	= element.section_tipo
	const section_id	= element.section_id
	const tipo			= element.tipo  // test208
...
// mode/view matrix for component_email
...
describe(`COMPONENT_EMAIL LIFECYCLE`, function() {
		...
		describe(`${pair.mode} / ${pair.view}`, function() {
			...
			it(`init → build → render`, async function() {
				...
				// init
					...
					assert.equal(instance.model, 'component_email', 'model expected component_email')
...
describe(`COMPONENT_EMAIL DATA OPERATIONS`, function() {
	...
	it(`data structure`, async function() {
		...
		// component_email entries have value (string) and lang
		if (entries.length > 0) {
	...
	it(`verify_email validates correctly`, async function() {

		// valid email
		assert.equal(instance.verify_email('test@example.org'), true, 'expected valid email')
		// invalid email
		assert.equal(instance.verify_email('invalid-email'), false, 'expected invalid email')
		// empty email (allowed for deletion)
		assert.equal(instance.verify_email(''), true, 'expected empty email allowed')
	});
...
describe(`COMPONENT_EMAIL SEARCH DATA OPERATIONS`, function() {
