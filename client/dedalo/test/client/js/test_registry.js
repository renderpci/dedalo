...
export const generic_suites_green = [
	...
	'test_diffusion',                // green after the selector fix (the inspector's diffusion opener is a SPAN, `.button.diffusion`, never a `button.diffusion`) AND the 2026-08-22 move off the `rsc` install section: it renders test3 now, with tool_diffusion made available by the run's own diffusion situation (src/core/test_data/situations/client_diffusion.ts) instead of by whatever domain the ambient database carried. Its Publish cases now assert the button renders and is confirm-gated, and ANSWER THE CONFIRM WITH NO: a confirmed click fires a real publish to the configured target and returns before the stream ends, so the old click asserted nothing while writing to a live target every run.
	'test_additional_text_area'      // green after (1) dispatch catches handler throws → graceful result:false (was raw 500), (2) the generic test3 pairing test84 (text_area) / test100 (geolocation) with test100's key_up_f2 observe — was the `dmm` install's "map of grapes" demo ontology until 2026-08-22, (3) the text_area edit-mode features.av_player port (structure_context.ts) that binds F2→build_tag. Matches the PHP oracle client 16/16.
]
...
export const lifecycle_suites_green = [
	...
	'test_component_filter',           // green after filterResolver search-datalist + entries→[] fix; its check/uncheck cases need MORE THAN ONE project to exist, which the run now provisions (src/core/test_data/filter_projects_fixture.ts) instead of borrowing from whatever the ambient database held
