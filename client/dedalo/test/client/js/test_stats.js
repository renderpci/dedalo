...
export function mark_test_status(test_name, status) {
	...
	for (const card of test_cards) {
		...
		if (status === 'pass' || status === 'fail') {
			// Deferred suites are NOT counted in the group/global tally (they were
			// excluded at init), so a manual run of one must not adjust the counters
			// (it would drive pending negative). Its dot still updates for feedback.
