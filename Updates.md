...
In the future this update file will be used only for deep changes and now the commits will be auto explained.
...
The next step will be the adaptation of our model to the SQL/JSON:2016 that will be implemented in postgres this year.
...
The component_filter change the format from the traditional object {"1":"2"} to the normalized locator [{"section_id":"1", "section_tipo":"dd153"}].

And the new relation model add to the locator the new two proprieties "from_component_tipo" with the tipo of the component that has the information and the "type" with the normalized type relation:
...
This version implement the new search to the thesaurus context. The new filter do joins between main section to sections that has relations and we update the concept to build joins with multiple sections to multiple sections. The concept is that the user need search between multiple main sections, for ex: toponymy that the main sections can be: Spain, France, Italy, etc... es1, fr1, it1, etc... Normally the components of this sections will be the same, like term component, that is because the thesaurus, normally, is built like virtual sections of the "real" thesaurus section.

We introduce into the JSON_QUERY_OBJECT the property "section", one array of main sections. If this array is configured with more that 1 section, the joins will be named with "mix" in the main path (first section_tipo in normal position). And will be built the WHERE clause with all sections of the array.
...
This version implement the new model for the search but sustain the old search, the new model is implemented in the component_autocomplete and is tested only in the monedaiberica instalation, this version is ONLY A TEST version.
...
Components:
	...
	component_number : 	set js validator/format to force always . instead , php controller to format float numbers with "." instead "," when show, print, etc
	component_text_area : added method fix_broken_person_tags to fix untranslated person info

Tools:
	refactoring of the tool_import_files, now the tool can be configured for upload files, and control the destination of the uploads, now the tool has three new concepts;
			...
			3 - the name of the files can indicate same record when the names are the same:
						mi foto-a.jpg
						mi foto-b.jpg
							the same name of the images "mi foto" indicate that the images are of the same "section_id" the -a or -b indicate the field. If the field are not indicated the images, the images will be stored in the same section_id inside the default portal of the image in the same row, (the second image replace the first). If the field is indicated the photo "-a" will go to the field "a" indicated in the structure, the image "-b" will be stored in the same "section_id" but in the field "b".

			This 3 configurations can be combined for control the destinations.
...
This version change all relation components to new format, now the locator(dato) is stored in a unique container in matrix, this container named "relations" is inside to the section and is outside of the clasical "components" container.
...
locate the destination information, "children information" of the component.
Now the locator introduce the origin concept:
	{
		"section_tipo":"oh1",
		"section_id":1,
		"component_tipo":"oh24",
...
from_component_tipo, say the origin of the locator, and it open the possibility that eliminate the "tipo" inside the section JSON in matrix.
...
Now the section has two dato containers:
	...
	-"components": will store the dato information of the components that has alphanumeric, date, ... information, this container is not changed in this version, the structuration of this container is the same that in previous version.
...
MATRIX:
	...
	This table is only a cache of the relations, this information can be recreated if the information store inside was de-synchronized with the real information.
	The components_relations will store your own data in locator format in the matrix sections and in the new relations table.


SEARCH:
		The 4.8 versions will replace the old search and filter, the new search concept is based in the relations of the documents that Dédalo manage. The target of the new search architecture is make possible the find in the deeper data, following the locator indications to the documents that has links with the origin section. The new search will can locate any related information, independent of the steps or relations that it has with the origin document.

		New "search_object" based in JSON. Every component can defined your path and the operators that will be parse to SQL. the search_object unify the model and options of search. This has a client part in js that transform the DOM to the object that will send to PHP to be parsed for the components for analyze the operators and options and finally will be parse to SQL.
...
jer_xx (every public table that begin with jer_ except the internal table "jer_dd" - stucture -that will not removed)
jerarquia
jerarquia_tipos
...
NEWS AND CHNAGES:
This version change the communication form javascript to php of all triggers, now the communication is make in JSON native format in both directions. All information store and change in the components is transferred in JSON. All  JQuery versions of the AJAX are remove and the calls now are made with the native XMLHttpRequest. In the PHP side the POST variables are removed.

New lang tools for the components, and new langs options for the transcriptions, indexations, chapters, etc.
...
New tools in the struct, this version add the options for sustain the correlation between translations of the "structurations", now is possible re-asig the original structurations from the original lang to the target lang.

New encryption of the 256bits for the passwords based in OpenSSL implementation.

Update the component_autocomplete for support the deep search in multiples sections. Now is possible search inside the 2, 3, 4, etc levels of the deep information in sections linked by locators.
Object -> portal -> mint -> group
PCI -> Interview -> Informant -> Profession -> Place -> actual situation
this searches are created with subquery in the JSON information. (important: for now the sub-query is not possible index it)
...
Updated the search and replace tool for do multiples changes without reset the grouping search in every change.
...
This newer version has a new dataframe into the JSON matrix, dataframe is a data companion of the dato. Dataframe in this first version store the uncertainty of the dato, if the dato is not certain, it will be store and processed by dataframe. In the near future will be incorporate the time and space information of the dato. Ex: one mint can change own name (dato) in the time (dataframe) and space (dataframe) and we can't be sure that the name in one time can be certain (dataframe)

Components that are affected:
- component_date is update to support multiple values of the dato. Now is a raid. Is necessary update the dato in the matrix tablet. Date is the first component that support "dataframe" and can manage it.


New components
- component_calculation, this new component can do calculations with formulas that can be configurable inside properties of the structure. The values for do calculations are get from other components, the get_dato is processed in php and the formula is calculated in javascript.
...
New matrix_indexation, for implement labels, comments and annotations to the indexations.
...
- All "component_autocomplete_ts" are deprecated and removed from the structure. The new component_autocomplete_hi change all formats to the new hierarchy format. The data format is compatible form autocomplet_ts to auocomple_hi and no is necessary change the JSON.

- All thesaurus need to be convert to the new JSON format of matrix. You can do the transformation in some ways.

Way 1.- This way is faster but require some manual changes into the Dédalo, if you don't have a lot experience use the way 2 (slow but more easy). If you don't use the toponymy thesaurus (like "España", "Cuba", "France", etc) in the indexation you can import all toponymy from:

	/dedalo/install/import/hierarchy

All hierarchies can be imported with the postgreSQL copy command, similar to:
...
This action is very fast but you will need change the counters of the tables you can use the:
- DEDALO COUNTERS STATE
section in the administration tool, you will see the difference and Dédalo show the SQL command to apply into the postgreSQL, similar to this:

- UPDATE "matrix_counter" SET dato = 75845 WHERE tipo = 'es1';

 This command put the counters to the new imported values.


Way 2.- Slow way, but the script do all hard work for transform the hierarchies. (You will need a lot of time and resources for PHP like RAM and SWAP space in HD, we recommended that you up the php memory for scripts to 4GB o more)

If you hierarchy is used in indexation, for example "ts1" (typical thematic hierarchy), you need transform the hierarchy from the version V3 to the V4.

In the administration tool you have a script named "update_jer_from_V3_to_4_5" that do all processes for change the hierarchies.

Put the tld of the original V3 hierarchy like "es" or "ts" and press the "Update" button.

if the hierarchy have a model associated, check the "modelo" check_box for do this task with the "modelo" data (this process is independent of the terms hierarchy).
...
select the hierarchy that you want open in the list, "es1" for example, edit and press the "Generate" button. This action create all new structure for the new hierarchy and link the hierarchy to new thesaurus terms.
...
Changes: this update change the trigram method for access to terms in matrix_hierarchy. Dédalo use the "ILIKE" operator for search the terms into the hierarchies with the "%term" into json. This update add new trigram method for postgreSQL that do possible index the term of thesaurus. This feature is used in "componet_autocomplete_hi" with auto-search field (toponymy)
...
ATENTION: This update is a security update, and you can't run it twice!, the update change all passwords on your installation to the OpenSSL format from the original and deprecated mycrpt lib, if you run this update twice you will lose the encryption, because the update re-encrypt the passwords twice and you can't decrypt it.
...
- Change into the "time_code" tag for using milliseconds into the time_code tags. the format will be: [TC_00:00:00.000_TC]
...
Changes:
- component_relation_parent : New component and model. the parent in the new thesaurus
- component_relation_children : New component and model. "CH" in the new thesaurus
- component_relation_related : New component and model. "TR" in the new thesaurus
...
Before run the update you need change the config.php file of your installation:
...
- section : created new structure section element named 'section_map' to mapping section components to different uses
...
- tool_import_dedalo_csv : the first version of the import the Dédalo format in csv files:
			...
			- is necessary update the first the portals and autocompletes sections before import the main section:
				- first import the "list of values"
				- second import the "resources"
				- and the last import need to be the main "inventory" section
...
- component_autocomplete_ts : new tree format to access to the thesaurus. Now the autocomplete_ts can access to all thesaurus tree for select the descriptor term.
...
- New porta_list views: now is possible change the portal list to different edit views: normal, view_single_line, view_mosaic. TO DO: Is necessary the code for change the view for the users.
- Tool_diffusion : add window scroll top on publish record
...
- New component_relation_children : the actual thesaurus (v3) only have the parent relation between the descriptors terms, in the new version the descriptors terms will be "bi-directional" relation, with children. The component will be the control of the relationship with the children of the descriptors terms of the thesaurus.
...
- tool_administrator : new option 'Ignore temporarily the publication status when publishing' to pass-thru component_publication value in some necessary cases
...
Changes:
- New 'sample_config4_db.php' that include the standard connection of MySQL for publication.
- New 'sample_config4.php' that include the default diffusion_domain for MySQL publication with OH.
...
This table will be used in future version 4.1 for sync components used by users in real-time (with server side events «SSE» and NODE), this version have the functionality but is disable by default. We recommend that you install NODE in you server and do tests of the functionality in a test or development server NO in a production system. Please tell us your experience with the new functionality.

For use the sync functionality you need change in the lib/dedalo/config4.php this lines to "true":

	define('DEDALO_LOCK_COMPONENTS' 	, true);
	define('DEDALO_NOTIFICATIONS'		, true);

Use: The new funcionality lock the components for multiple simultaneous editions. When one user "focus" one field Dédalo will lock the edition for all other users, when the user go out of the field "onblur", the component will unlocked the edition for all users.
...
The component_security_areas, component_security_access and component_security_tools ONLY work for the profiles and are deprecated for Users.
The root user (dev-user) can see this components for some time in the Users records.
...
VERY IMPORTANT:
- The final version is the first official version of Dédalo with Postgres.
...
Finally Postgres community has made a amazing job with the JSONB.
...
Very, very thanks for the excellent work of Oleg Bartunov, Teodor Sigaev and Alexander Korotkov.
...
- If you need install the beta 1, we recommended MySQL 5.6 and PHP 5.6.

- For Intangible Heritage with the Render model (standard schema) for the IPCE you will need install Memcache or Redis.
