...
- **Config files:** User-managed files are `config/config_core.php` and `config/config_areas.php`, copied from their `sample.*.php` counterparts. All other configuration lives in `.env` files under `/private/` (loaded by `config/bootstrap.php`). Do not edit `sample.*` files directly.
...
- **Config-driven behavior:** Search for `DEDALO_ENTITY` and other defines in `config/bootstrap.php` — many runtime behaviors come from those constants.
