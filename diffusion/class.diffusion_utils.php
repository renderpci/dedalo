...
class diffusion_utils {
	...
	// publication metadata component tipos (set on every record publication)
	...
	// bulk process generated by tool_diffusion when the user performs a new
	// publication. Saved into time_machine to group all publication sections
	...
	* Saves the publication metadata components of a record after publish:
	* first publication date/user (set once) and last publication date/user
	* (always updated). Prevents time machine records and modified flags.
	...
	*/
	public static function update_publication_data(string $section_tipo, string|int $section_id) : bool {
		...
		// tipos
			$publication_first_tipo			= self::$publication_first_tipo;
			$publication_last_tipo			= self::$publication_last_tipo;
			$publication_first_user_tipo	= self::$publication_first_user_tipo;
			$publication_last_user_tipo		= self::$publication_last_user_tipo;
		...
		// last publication. save updated date always
			// date
				...
				$component->set_bulk_process_id(self::$bulk_process_id);
				// section avoid save_modified by user in diffusion
					$section = $component->get_my_section();
					$section->save_modified = false;
				$component->Save();
