...
function pieChart() {
  ...
  var maxLength		= 55;			// maxima longitud del texto mostrado al pinchar sobre la tarta			
  ...
  function init() {
	...
	$('#chartData td').each( function() {
      ...
      if ( currentCell % 2 != 0 ) {		//if ( currentCell % 2 != 0 ) {
        currentRow++;
        chartData[currentRow] = [];
        chartData[currentRow]['label'] = $(this).text();	alert('\n currentCell:'+currentCell +' 	\n currentRow:' +currentRow )
      } else {
       var value = parseFloat($(this).text());
       totalValue += value;
       value = value.toFixed(2);
       chartData[currentRow]['value'] = value;				//alert(value)
      }
	...
	*/
	$('#chartData tr').each( function() {
		...
		$(this).find("td").each(function(cell,v) {
			...
			if(cell==0) {
				var nombre = $(this).text();			//alert(nombre)
				
				chartData[currentRow] = [];
				chartData[currentRow]['label'] = nombre;
			}
			if(cell==1) {
				var valor = $(this).text();			//alert(valor)
			...
			if(cell==2) {
				//var color = $(this).text();			
				var color = $(this).attr('alt');	//alert(color)
				
				// convert color from hex to rgb
				if(!color || color=='undefined') color = '#FFFFFF';
				hex = color.match(/#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})/);
				chartColours[currentRow] = [ parseInt(hex[1],16) ,parseInt(hex[2],16), parseInt(hex[3], 16) ];				
			}
  ...
  function handleChartClick ( clickEvent ) {
	...
	//alert(distanceFromCentre + ' - '+ chartRadius + ' - '+  this.offsetLeft)
  ...
  function startPullOut ( slice ) {
    ...
    if ( currentPullOutSlice == slice || slice==0) return;	
    ...
    var labelCell = $('#chartData td:eq(' + ((slice-1)*3) + ')');
    var valueCell = $('#chartData td:eq(' + ((slice-1)*3+1) + ')');
	var valueColor = $('#chartData td:eq(' + ((slice-1)*3+2) + ')');
    labelCell.addClass('highlight');
    valueCell.addClass('highlight');
	valueColor.addClass('highlight');
  ...
  function drawSlice ( context, slice ) {
	...
	if ( slice == currentPullOutSlice ) {
		...
		context.font 				= pullOutLabelFont;
		// texto
		var current_text 			= chartData[slice]['label'];		
		if(current_text.length>maxLength) current_text = current_text.substring(0,maxLength)+'..';
		
		context.fillText( current_text, centreX + Math.cos(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ), centreY + Math.sin(midAngle) * ( chartRadius + maxPullOutDistance + pullOutLabelPadding ) );
