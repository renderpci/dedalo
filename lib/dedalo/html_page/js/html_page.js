...
var html_page = new function() {
	...
	$(function() {
		...
		window.addEventListener('focus', html_page.refresh_components, false)
	...
	window.addEventListener('unload', function (event) {		
		...
		if (window.opener && page_globals.context_name=='list_in_portal') {
			...
			if (wrapper.length==1) {
				...
				if (DEBUG) console.log("Error on update portal component. Portal id_wrapper not exist in DOM : "+id_wrapper)
			}
		}
	});
	...
	*/
	this.refresh_components = function() {
		...
		if (!page_globals.components_to_refresh || page_globals.components_to_refresh.length<1) return false
		...
		for (var i = 0; i < page_globals.components_to_refresh.length; i++) {
