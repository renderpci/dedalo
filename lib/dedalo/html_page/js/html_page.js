...
$(window).bind('beforeunload', function(event){	
	...
	try{
		...
		document.activeElement.blur(); // Blur component trigger save
	...
	*/
	...
	this.loading_content = function (target_obj, mode) {
		...
		if( $target_obj.length != 1 ) {
			if (DEBUG) console.log("loading_content: Invalid target lenght ("+$target_obj.length+") in mode:" +mode)
			alert("Sorry. Error on html_page.loading_content mode "+mode)
			return false;
		}
		...
		if (mode===1) {
			...
			if (this.loading_content_active>0) return false;

			$page_area.css('cursor','progress');
			...
			$('<div class="loading_content_overlay"> </div>')
				.css( {'position':'fixed', 'left':position.left, 'top':position.top, 'width': width, 'height': height} )
				.appendTo($target_obj);
