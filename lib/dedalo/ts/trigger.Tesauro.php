...
if($accion=='insertTS') {
	...
	$created_id_ts = $RecordObj_ts->Save();				#dump($created_id_ts,'created_id_ts'," "); #die();
	
	# seleccionamos el último terminoID recien creado		
	$terminoID	= $RecordObj_ts->get_terminoID();		 #dump($RecordObj_ts,'RecordObj_ts',"new obj created terminoID:$terminoID, id:$created_id_ts "); die( );			
	...
	# DESCRIPTORS : finally we create one record in descriptors with this main info 
		...
		$RecordObj_descriptors	= new RecordObj_descriptors($matrix_table, NULL, $terminoID, $lang);	#($id=NULL, $parent=NULL, $lang=NULL, $tipo='termino', $fallback=false)
		$RecordObj_descriptors->set_tipo('termino');
		$RecordObj_descriptors->set_parent($terminoID);
		$RecordObj_descriptors->set_lang($lang);
		$created_id_descriptors	= $RecordObj_descriptors->Save();		#error_log('lang: '.var_dump($ts_lang,true));
...
if($accion=='deleteTS') {
	...
	*/									
	...
	# caso general (arbol)
		...
		$html .= $codHeader ;
		$html .= "<script type=\"text/javascript\">						
					//document.getElementById('$divName').innerHTML += '<div class=\"ok\"> Deleted $terminoID ($termino) ok ! </div>';	//alert(\"Registro borrado 2 \");
					//setTimeout('document.location=\"?modo=$modo\"',1000);
					//document.location = '?modo=$modo&k=1'
					window.openDivTrack('$parentPost',1,'$terminoIDpost');						
				</script>";
		*/
...
if($accion=='editTS') {
	...
	#$vars = array('parentInicial','parent','esdescriptor','propiedades','nHijos'); foreach($vars as $name) $$name = setVarPost($name);
	...
	/*
	INNECESARIO ???
	# update termino / def from mainLang
	if(isset($_REQUEST['descriptors_id']) && isset($_REQUEST['terminoID'])) {
		
		$id 			= $_REQUEST['descriptors_id'];
		$mainLang		= $_REQUEST['descriptors_mainLang'];
		
		$value_termino	= $_REQUEST["termino_".$id];
		$value_def		= $_REQUEST["def_".$id];
		
		$matrix_table			= RecordObj_descriptors::get_matrix_table_from_tipo($terminoID);
		$RecordObj_descriptors	= new RecordObj_descriptors($matrix_table, NULL, $terminoID, $mainLang);
		$RecordObj_descriptors->set_termino($value_termino);
		$RecordObj_descriptors->set_def($value_def);
		$RecordObj_descriptors->Save();
	}
	*/	
	...
	# Caso general (desde listado jerárquico)	
		...
		$terminoIDpost		= $_REQUEST['terminoID'];				
...
# listados (al abrir la flecha,etc..)
