...
class Tesauro extends TesauroElements {
	...
	*/
	public static function get_ar_indexations($terminoID) {
		...
		if(count($ar_records)==1) {
			...
			# Dato es un array en formato JSON. No obstante demomento RecordObj_descriptors no convierte automáticamente 
	...
	*/
	...
	/*************************************************************************************************************
	* AUTOSUGEST SEARCH . searchAutoSugest
	* Busca en todas las tablas (UNION)	
	* Ojo: Si no pasamos nada como filtroR, sólo se buscarán descriptores ($filtroR=" AND esdescriptor = 'si' ") 
	* Para casos como la web pública en que interesan también los No descriptores, sobreescrir $filtroR
	*************************************************************************************************************/
	function searchAutoSugest__DESACTIVO_XXXXXXXX($busquedaF, $filtroR=" AND esdescriptor = 'si' ") {		
		
		$html	= false ;
		$sql	= false ;
		$i		= 0 ;		
					
		if($busquedaF && $busquedaF !='') {			
		
			# búsqueda en todas las tablas activas de jerarquia simultáneamente (UNION)
			$arrayTablas = $this->arrayTablas ;												#print_r( $arrayTablas );
						
			# case not tables (clean install ...)
			if(!$arrayTablas || !is_array($arrayTablas)) {
				
				echo " Hierarchies are not enabled. Please enable at least a hierachie in Hierarchies Panel ";	
				return false;
			}
			...
			if(!$res) 	die(__METHOD__."<br>searchAutoSugest: Failed getting data from terminoID: {$sql} <hr> $sql");			
			...
			*/
			...
			$arguments['sql_code']			= '';	
			$arguments['sql_code']			.= "AND dato LIKE '$busquedaF%' ";
			$arguments['sql_code']			.= "AND tipo = 'termino' ";
			$matrix_table					= RecordObj_descriptors::get_matrix_table_from_tipo($terminoID);	
			$RecordObj_descriptors			= new RecordObj_descriptors($matrix_table, NULL);
			$ar_id							= $RecordObj_descriptors->search($arguments);	#print_r($ar_id);
			...
			if(count($ar_id)>0) {
				...
				foreach($ar_id as $id) {
					...
					$esmodelo		= $RecordObj_ts->get_esmodelo();
					$esdescriptor	= $RecordObj_ts->get_esdescriptor();					
					#$n_captaciones	= $RecordObj_ts->get_n_captaciones();
					
					switch($this->type) {
						case 'tesauro'	: $type_n = 1 ;	break;
						case 'toponimia': $type_n = 2 ;	break;
						case 'lenguaje'	: $type_n = 3 ;	break;
						case 'all'		: $type_n = " ";	
					}
					
					if($esmodelo=='no' && $esdescriptor=='si') {						
						$termino = RecordObj_ts::get_termino_by_tipo($terminoID);						
						
						#if($this->modo=='topo') $html .= " [$modeloActual] ($pais) ";
						if($this->type=='toponimia') {
							
							$modelo		= $RecordObj_ts->get_modelo();							
							if(!empty($modelo))	$modelo = RecordObj_ts::get_termino_by_tipo($modelo);
							
							$pais		= $RecordObj_ts->get_pais();
							if(!empty($pais))	$pais = RecordObj_ts::get_termino_by_tipo($pais);
								
							$html .= "$termino";
							
							if($modelo) $html .= " [$modelo]";
							if($pais)	$html .= " ($pais)";
							
							$html .= "|$terminoID\n";	
							
						}else{
							$html .= "$termino|$terminoID\n";		
						}
					}
					
					/*
					$prefijo 		= Tesauro::terminoID2prefix($terminoID);				
					$paisIndex		= array_search($prefijo, $arrayTablas['prefijo']);				
					$pais 			= $arrayTablas['nombre'][$paisIndex];
					
					$texto 			= $row_RS['texto'];
					$modelo 		= strtolower($row_RS['modelo']);
									
					$modeloActual	= false;
					if($modelo && strlen($modelo)>2) $modeloActual 	= RecordObj_ts::get_termino_by_tipo($modelo);
				...
				} #while ($rows = mysql_fetch_assoc($res));	mysql_free_result($res);
