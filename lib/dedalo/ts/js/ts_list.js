...
var ts = new function() {
	...
	*/
	this.show_indexations = function(terminoID, termino) {
		
		var target_div  = $('#u'+terminoID);
			if($(target_div).length!=1) return alert('Div not exist !');		
		
		termino = urldecode(termino);
		
		if (target_div.css( 'display') == 'block') {
			// si está visible, la ocultamos
			target_div.slideUp(200);

		} else {
			// si no está visible, hacemos la búsqueda y cargamos los datos			
			var accion	= 'show_indexations';
			var mydata	= { 'accion': accion, 'terminoID': terminoID };

			//html_page.loading_content( target_div, 1 )
			target_div.html('<div><img src="../themes/default/spinner.gif" alt="Wait" align="absmiddle" /></div>');
	...
	*/
	this.load_tree = function(terminoID, div_destino, modo, slide, terminoIDresalte, target, reloaded) {
		...
		if( !div ) {
			...
			if( typeof terminoID =='undefined' || terminoID.length==0 ) return;
			/*
			...
			*/
			...
			//if (DEBUG) console.log("->load_tree deleted cookie: "+terminoID)
			/*
			if(reloaded!=1) {
				setTimeout(function(){
					ts.load_tree(terminoID, div_destino, modo, slide, terminoIDresalte, target, 1)
				}, 1000)
			}
			*/
			...
			*/
			if(index == -1) {		
				...
				var mydata		= { 'accion': accion, 'modo': modo, 'terminoID': terminoID, 'terminoIDresalte': terminoIDresalte, 'ts_lang' : ts_lang , 'type' : type };
				var myURL		= this.trigger_url
				
				if(modo=="buildTree") {
					$(divJQobj).html('<div id="spinnerDiv"> Building Thesaurus  <br> <img src="../themes/default/spinner.gif" alt="Wait" align="absmiddle" /><br> Please wait</div>');
				}else{					
					$(divJQobj).html('<div style="margin-left:55px"><img src="../themes/default/spinner.gif" alt="Wait" align="absmiddle" /></div>');
	...
	*/
	this.add_index_common = function(obj) {
		...
		//return alert(parent.page_globals.modo)
	...
	*/
	this.update_tr_order = function(terminoID, dato, obj) {
		...
		var mydata		= { 'accion': accion, 'terminoID': terminoID, 'dato': dato};
	...
	this.delete_term = function(divContTesauroID, terminoID, children, indexaciones, parent, termino) {
		...
		if(children != 0) {
			...
			if(divObj==null) {
				...
				var myURL	= this.trigger_url;
				var accion	= 'deleteTS';
				var mydata	= { 'accion': accion, 'modo': modo, 'terminoID': terminoID, 'children': children };
	...
	*/
	this.insertTS = function(parent) {
		...
		if(div==null){
			...
			var myURL	= this.trigger_url
			var accion	= 'insertTS';
			var mydata	= { 'accion': accion, 'modo': modo, 'parent': parent };		//return(alert(modo))
	...
	*/
	this.saveDescriptorFromList = function(obj, terminoID, termino) {
		...
		var mydata		= { 'accion': accion, 'terminoID': terminoID, 'termino': termino, 'lang': ts_lang };	
...
function cambiarNorden(nordenV, terminoID, padre, termino) {
	var myurl = "../ts/ts_norden.php?nordenV="+nordenV+"&padre="+padre+"&terminoID="+terminoID+"&termino="+termino ;
