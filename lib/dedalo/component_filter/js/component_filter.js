...
var component_filter = new function() {
	...
	*/
	this.get_dato = function(wrapper_obj) {
		...
		if (wrapper_obj.dataset.modo && wrapper_obj.dataset.modo==='search') {

			const component_obj = wrapper_obj.querySelector('select')
			...
			if (select_value.length>0) {
				const value_obj = JSON.parse(select_value)
				dato.push(value_obj)			
			}
		...
		for (let i = len - 1; i >= 0; i--) {
		      if(inputElements[i].checked){
		      	const val = inputElements[i].value
		      		? JSON.parse(inputElements[i].value)
		      		: null
