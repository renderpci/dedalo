...
var component_filter = new function() {
	...
	*/
	this.get_dato = function(wrapper_obj) {
		...
		// ul list of inputs
		let	parent_ul = wrapper_obj.getElementsByTagName('ul')[0] //wrapper_obj.querySelector('ul.filter_projects_list')
		...
		for (let i = 0; i < len; i++) {
			...
			let checkbox_value 	= String(checkbox_item.value)
			if(checked===true) {								
				dato[checkbox_value] = 2				
			}
		}
