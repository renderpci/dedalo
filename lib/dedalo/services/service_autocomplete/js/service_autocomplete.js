...
var service_autocomplete = new function() {
	...
	this.activate = function( input_obj, component_js ) {
		...
		// Vars
			...
			const propiedades 	 	 = component_info.propiedades || {}
				console.log("component_info:",wrap_div.dataset.component_name);
		...
		$(input_obj).autocomplete({
			...
			source: function( request, response ) {
				...
				if (wrap_div.dataset.component_name==="component_autocomplete" || wrap_div.dataset.component_name==="component_relation_related") {
					var search_query_object = self.rebuild_search_query_object2(wrap_div, q);
				}else{
					var search_query_object = self.rebuild_search_query_object(wrap_div, q);
				}
	...
	*//**/
	this.rebuild_search_query_object = function(wrap_div, q) {
		...
		// hierarchy_sections get from wrapper dataset
		const hierarchy_sections 	= JSON.parse(wrap_div.dataset.hierarchy_sections)

		// Default
		let query_section_tipo = hierarchy_sections

		// Check if filter_sections exists (autocomplete hi)
		const filter_sections  = self.get_filter_elements(wrap_div)
		if (filter_sections) {
			query_section_tipo = filter_sections
		}

		// Update sections property in search_query_object
		search_query_object.section_tipo = query_section_tipo
			...
			const js_promise = new Promise(function(resolve, reject) {
				...
				// ul container
					const ul = common.create_dom_element({
						element_type 	: "ul",
						class_name 	 	: "filter_by_list text_unselectable",
						parent 			: options_wrapper
					})
				...
				self.set_filter_elements(options_wrapper, cookie_name)
			})
		...
		*/
		this.get_filter_elements = function(wrap_div) {
			...
			if (list_ul) {
				...
				selected_values = component_autocomplete_hi.get_hierarchy_sections(wrap_div)
			}
		...
		/**
		* SET_FILTER_ELEMENTS
		* Reads cookie if exists and apply cookie values to current list
		*/
		this.set_filter_elements = function(wrap_div, cookie_name) {

			const ar_values = readCookie(cookie_name)
				//console.log("set_filter_elements ar_values from cookie",ar_values,cookie_name);
				if (!ar_values) {
					return false;
				}
