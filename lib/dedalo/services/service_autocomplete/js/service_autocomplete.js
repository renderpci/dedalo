	...
	*/
	this.autocomplete_search = function(search_options){
		...
		// search source selector
			const search_engine = (search_options && search_options.search_engine)
				? search_options.search_engine
				: 'search_dedalo';

			if(SHOW_DEBUG===true) {
				console.log("[service-autocomplete.autocomplete_search] search_options:",search_options);
	...
	*/
	this.get_search_sections = function(wrap_div) {
		...
		if (selected_values.length<1) {
			...
			// from selector if exists
				const source_selector = wrap_div.querySelector(".source_selector")
				if (source_selector) {
					selected_values = [source_selector.value]
				}
