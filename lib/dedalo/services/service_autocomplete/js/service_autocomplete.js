			...
			source: function( request, response ) {
				...
				// rebuild_search_query_object with fallback
					...
					// search engine
						...
						if (source_selector) {
							...
							const search_engine 		= selector_source.search_engine;
							const original_ar_elements 	= JSON.parse(source_selector.dataset.source);
							const selected_fields 		= original_ar_elements.ar_elements.filter(element => element.section_tipo === selector_source.section_tipo)
						}						
				...
				// Search
					self.autocomplete_search({						
						...
						wrap 				: wrap_div,
						search_engine		: (typeof search_engine!=="undefined") ? search_engine : "search_dedalo",
						selected_fields 	: (typeof selected_fields!=="undefined") ?  selected_fields : null,
						q 					: q
					})
	...
	this.build_source_search_selector = function(options) {
		...
		// search elements
			...
			const ar_fields 	= options.ar_elements.map(element => element.fields_map);
				console.log("ar_fields:",ar_fields);
			//const ar_search = [...new Set(ar_elements)]
		...
		// switcher source
			...
			const select = common.create_dom_element({
				...
				for (var i = 0; i < ar_search_length; i++) {
