...
var service_autocomplete = new function() {
	...
	*/
	this.autocomplete_search = function(search_options){
		...
		// search_way . determine way to search from search_options search_engine property and call
			const search_way = search_options.search_engine || "search_dedalo_default"

		// exec search
			const js_promise = this[search_way](search_options)
	...
	/**
	* SEARCH_DEDALO_DEFAULT
	* @return promise 
	*/
	this.search_dedalo_default = function(search_options) {
	...
	*/
	this.search_zenon = function(search_options) {
		...
		// promise JSON XMLHttpRequest
			const js_promise = common.get_json_data(url_trigger, trigger_vars).then(function(response){
				if (SHOW_DEBUG===true) {
					console.log("[search_zenon] response:",response);
					if (!response.result) {
						console.warn("[search_zenon] response:",response);
						//console.log("[services.autocomplete_search] response "+response.msg +" "+ response.debug.exec_time);
					}
				}
				...
				if (!response) {
					// Notify to log messages in top of page
					console.error("[search_zenon] Error. response is null", response);
					return null
				}else{
