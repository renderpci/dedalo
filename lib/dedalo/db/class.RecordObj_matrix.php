...
class RecordObj_matrix extends RecordDataBoundObject {
	...
	*/
	protected function calculate_ID() {
		...
		#unset($_SESSION['config4']['calculate_ID']);
		#$idu = $parent.'-'.$tipo.'-'.$lang.'-'.$this->matrix_table;
		#if(isset($_SESSION['config4']['calculate_ID'][$idu])) return $_SESSION['config4']['calculate_ID'][$idu];
		...
		if(!empty($id)) {
			$this->set_ID($id);
			#if($tipo!=DEDALO_ACTIVITY_SECTION_TIPO) $_SESSION['config4']['calculate_ID'][$idu] = $id;
		}		
	...
	*/
	private function test_can_save() {
		...
		# TEST VALID USER
		#$userID		= navigator::get_userID_matrix();
	...
	*/
	public function Save() {
		...
		if($this->save_time_machine_version===true)	{
			...
			$this->time_machine_last_id = $this->save_time_machine();
		}
	...
	*/
	protected function save_time_machine() {
		...
		$RecordObj_time_machine->set_userID(navigator::get_userID_matrix());
