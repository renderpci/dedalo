...
var ts_object = new function() {
					...
					switch(true) {
						...
						case (ar_childrens_data[i].ar_elements[j].type==='term'):
							...
							// Term terminoID like [ts1_52]
								const term_add = " ["+ar_childrens_data[i].section_tipo+'_'+ar_childrens_data[i].section_id+"]"
									if(SHOW_DEBUG===true && node_type!=='hierarchy_node') {
										const current_link_childrens = ar_childrens_data[i].ar_elements.filter(item => item.type==="link_childrens")[0]
										const childrens_tipo 		 = current_link_childrens.tipo
										const a = "../ts_object/trigger.ts_object.php?mode=get_childrens_data&section_tipo="+ar_childrens_data[i].section_tipo+"&section_id="+ar_childrens_data[i].section_id+"&tipo="+childrens_tipo+"&node_type=thesaurus_node"
	...
	this.on_drop = function(obj, event) {
		...
		// data_transfer_json case
			...
			if (data_transfer_json.length>0) {
				...
				//console.log("source:",source, "obj",obj);
		...
		// target_section_tipo check on add_children_from_hierarchy mode
			if (mode==="add_children_from_hierarchy") {
				if (typeof wrap.dataset.target_section_tipo==='undefined') {
					...
					return console.log("[ts_object.add_children] Error on find target_section_tipo dataset on wrap");
				}
			}
	...
	*/
	this.select_first_input_in_editor = function(element_data_div) {
		//console.log(element_data_div); return;
