...
var ts_object = new function() {
	...
	*/
	this.init = function() {
		let url_vars = get_current_url_vars()
	...
	*/
	...
	this.get_childrens = function( childrens_element ) {
		...
		let wrap_childrens = wrap.childNodes
		let wrap_childrens_len = wrap_childrens.length
		...
		// AJAX REQUEST
		var js_promise = ts_object.get_json(trigger_vars).then(function(response) {
				if(SHOW_DEBUG===true) {
				...
				if (response && response.result) {
					// DOM_PARSE_childrens
					var ar_childrens_data = response.result
					var options = {
		...
		var promise = new Promise(function(resolve) {
			...
			for (let i = 0; i < ar_childrens_data_len; i++) {
				...
				// WRAP_TS_OBJECT . ts_object wrapper
					...
					var wrap_ts_object 		= common.create_dom_element({
																		element_type			: 'div',
				...
				// ID COLUMN . id column content
					var id_colum_content 	= common.create_dom_element({
																		element_type			: 'div',
				...
				// ELEMENTS CONTAINER . elements container
					...
					var elements_container 	= common.create_dom_element({
																		element_type			: 'div',
																		parent 					: wrap_ts_object,
				...
				// DATA CONTAINER . elements data container
					...
					var data_container 		= common.create_dom_element({
																		element_type			: 'div',
																		parent 					: wrap_ts_object,
				...
				// ND CONTAINER
					...
					var nd_container 		= common.create_dom_element({
																		element_type 			: 'div',
																		parent 		 			: wrap_ts_object,
				...
				// CHILDRENS CONTAINER . childrens container
					...
					var childrens_c 		= common.create_dom_element({
																		element_type			: 'div',
																		parent 					: wrap_ts_object,
					...
					switch(ts_object.thesaurus_mode) {
						...
						default:
							...
							// ADD . button + add element
								...
								var link_add 			= common.create_dom_element({
																			element_type			: 'a',
																			parent 					: id_colum_content,
							...
							// MOVE DRAG . button drag element
								...
								var link_drag 			= common.create_dom_element({
																		element_type			: 'div',
																		parent 					: id_colum_content,
							...
							// DELETE . button delete element
								...
								var link_delete 	= common.create_dom_element({
																		element_type			: 'a',
																		parent 					: id_colum_content,
							...
							// ORDER number element
								...
								if(is_descriptor===true && node_type!=='hierarchy_node') {								
									...
									var order_number 		= common.create_dom_element({
																			element_type			: 'a',
																			parent 					: id_colum_content,
							...
							// EDIT . button edit element
								...
								var link_edit 			= common.create_dom_element({
																		element_type			: 'a',
																		parent 					: id_colum_content,
													...
													// section_id number
													var section_id_number = common.create_dom_element({
																		element_type			: 'div',
																		parent 					: link_edit,
													...
													// edit icon
													var edit_icon 	= common.create_dom_element({
																		element_type			: 'div',
																		parent 					: link_edit,
	...
	*/
	...
	/**
	* ON_ORDER_DROP__UNUSED
	* Manages user custom terms sort using drag and drop
	*//*
	this.on_order_drop__UNUSED = function(obj, event) {	
		event.preventDefault();
		event.stopPropagation();		
	...
	*/
	this.on_drop = function(obj, event) {
		...
		let nodes 			= obj.children // childNodes
		...
		//var element_childrens_target = wrap_target.querySelector('.list_thesaurus_element[data-type="link_childrens"]')
		var element_childrens_target = ts_object.get_link_childrens_from_wrap(wrap_target)
		//var element_childrens_source = ts_object.old_parent_wrap.querySelector('.list_thesaurus_element[data-type="link_childrens"]')
		var element_childrens_source = ts_object.get_link_childrens_from_wrap(ts_object.old_parent_wrap)
	...
	*/
	this.update_parent_data = function(wrap_ts_object) {
		...
		*/
		...
		// JSON GET CALL
		let js_promise = ts_object.get_json(trigger_vars).then(function(response) {
			if(SHOW_DEBUG===true) {
	...
	*/
	this.toggle_view_childrens = function(link_childrens_element) {
		...
		let childrens_container = ts_object.get_my_parent_container(link_childrens_element, 'childrens_container')
	...
	*/
	...
	this.save_opened_elements = function(link_childrens_element, action) {
		...
		if (action==='add') {
			...
			let open_childrens_elements = wrap.getElementsByClassName('ts_object_childrens_arrow_icon_open')
	...
	*/
	this.hilite_element = function( element, clean_others ) {
		...
		let matches = document.querySelectorAll('.list_thesaurus_element[data-type="'+element.dataset.type+'"][data-section_tipo="'+element.dataset.section_tipo+'"][data-section_id="'+element.dataset.section_id+'"]');
	...
	*/
	this.reset_hilites = function() {
		let matches = document.querySelectorAll('.element_hilite');
		const len 	= matches.length;
		for (let i = len - 1; i >= 0; i--) {
	...
	*/
	...
	this.edit = function(button_obj, event, section_id, section_tipo) {
		...
		let wrap = button_obj.parentNode.parentNode;
			if(!wrap) {
		...
		if(ts_object.edit_window === null || ts_object.edit_window.closed) { //  || edit_window.location.href!=url || ts_object.edit_window.closed
			...
			let current_query 	= ts_object.edit_window.location.href.split("?")[1]
			let new_query 		= url.split("?")[1]
	...
	*/
	this.add_children = function(button_obj) {
		...
		let children_element = ts_object.get_link_childrens_from_wrap(wrap)
			if(!children_element) {
				return console.log("[ts_object.add_children] Error on find children_element 'link_childrens'");
			}			
		let tipo = children_element.dataset.tipo
			if (!tipo) {
		...
		var js_promise = ts_object.get_json(trigger_vars).then(function(response) {
				...
				var update_childrens_promise = ts_object.get_childrens(children_element)
					...
					update_childrens_promise.then(function() {
						...
						// Open edit window
						var new_section_id = response.result
	...
	*/
	this.select_first_input_in_editor = function(element_data_div) {
		...
		// Focus first input element
		let first_input = element_data_div.querySelector('input')
			if (first_input) {
				// Select all content
				first_input.select()
	...
	*/
	this.show_indexations = function(button_obj, event, section_tipo, section_id, component_tipo, container_id) {
		...
		let target_div = document.getElementById(container_id);
			if (!target_div) {
	...
	*/
	this.link_term = function(button_obj) {
		...
		let source_window = window.opener || window.parent
		...
		let component_name = ts_object.component_name
		...
		if(wrap.querySelector('.list_thesaurus_element[data-type="term"]').hasChildNodes()){
			label = wrap.querySelector('.list_thesaurus_element[data-type="term"]').firstChild.innerHTML;
		}else{
			label = ' '
		}		
			
		let response = source_window[component_name].link_term(wrap.dataset.section_id, wrap.dataset.section_tipo, label)
		...
		*/
		...
		for (let key in data) {
			...
			if(is_recursion===false) {
				...
				if(!main_div) {
					console.log("[ts_object.parse_search_result] Error on locate:  "+'.hierarchy_root_node[data-section_id="'+data[key].section_id+'"] > .childrens_container')
				}else{
					// Clean main div
			...
			if(!main_div) {
				console.log("[ts_object.parse_search_result] No main_div found ! ar_childrens_data",ar_childrens_data);
			}
			...
			let promise = ts_object.dom_parse_childrens(ar_childrens_data, main_div, options)
				/*
				.then(function(result) {
			...
			// Note var current_main_div is set on each dom_parse_childrens call
			if (typeof element.heritage!=='undefined') {
				...
				element = current_main_div.parentNode.querySelector('.elements_container > [data-type="term"]')
	...
	*/
	this.build_order_form = function(button_obj, evt) {
		...
		let order_inputs = document.querySelectorAll('input.input_order')
		const len = order_inputs.length
		for (let i = len - 1; i >= 0; i--) {
			order_inputs[i].remove()
		}

		let old_value = parseInt(button_obj.textContent)
	...
	*/
	this.toggle_nd = function(button_obj) {
		...
		let nd_container = ts_object.get_my_parent_container(button_obj, 'nd_container')
			if (!nd_container) {
				if(SHOW_DEBUG===true) {
		...
		let childrens_container 	= ts_object.get_my_parent_container(button_obj, 'childrens_container')
		//var link_childrens_element 	= button_obj.parentNode.querySelector('[data-type="link_childrens"]')
		let wrap 					= button_obj.parentNode.parentNode
		let link_childrens_element 	= ts_object.get_link_childrens_from_wrap(wrap)
