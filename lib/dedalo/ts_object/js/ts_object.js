...
var ts_object = new function() {
	...
	*/
	this.update_arrow_state = function(link_childrens_element, toggle) {
		...
		console.log("link_childrens_element",link_childrens_element, toggle);
							...
							// Term terminoID like [ts1_52]
								var term_add = " ["+ar_childrens_data[i].section_tipo+'_'+ar_childrens_data[i].section_id+"]"
									if(SHOW_DEBUG===true && node_type!=='hierarchy_node') {
										var current_link_childrens = ar_childrens_data[i].ar_elements.filter(item => item.type==="link_childrens")[0]
										var childrens_tipo 		 = current_link_childrens.tipo
										var a = "../ts_object/trigger.ts_object.php?mode=get_childrens_data&section_tipo="+ar_childrens_data[i].section_tipo+"&section_id="+ar_childrens_data[i].section_id+"&tipo="+childrens_tipo+"&node_type=thesaurus_node"
	...
	this.on_drop = function(obj, event) {
		...
		// data_transfer_json case
			...
			if (data_transfer_json.length>0) {
				if (wrap_target.dataset.section_tipo==='hierarchy1') {
					console.log("Ignored root node drop");
					return true
				}
