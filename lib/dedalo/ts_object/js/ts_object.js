/* global get_label, page_globals, SHOW_DEBUG, DEDALO_LIB_BASE_URL, common, get_current_url_vars */
...
var ts_object = new function() {
	...
	*/
	//var start = null
	this.get_childrens = function(childrens_element, options) {

		if (SHOW_DEBUG===true) {
			//let start_time = new Date().getTime()
			// console.log('--> get_childrens childrens_element:', childrens_element, childrens_element.dataset);
			// console.log('--> get_childrens options:', options);
		}
		...
		if (!parent_section_tipo || typeof parent_section_tipo==="undefined") {
			console.log("[get_childrens] Error. parent_section_tipo is not defined");
			return false
		}
		if (!parent_section_id || typeof parent_section_id==="undefined") {
			console.log("[get_childrens] Error. parent_section_id is not defined");
			return false
		}
		if (!tipo || typeof tipo==="undefined") {
			...
			console.error("[get_childrens] Error. tipo is not defined");
			return false
		}
		...
		// debug
			if(SHOW_DEBUG===true) {
				console.log("[ts_object.get_childrens] trigger_vars", trigger_vars); //console.log(new Error().stack);
			}
	...
	*/
	this.update_arrow_state = function(link_childrens_element, toggle) {
		...
		if(SHOW_DEBUG===true) {
			//console.log("[update_arrow_state link_childrens_element]",link_childrens_element, toggle);
		}
	...
	*/
	this.dom_parse_childrens = function(ar_childrens_data, childrens_container, options) {
		...
		// Options set values
			...
			const wrap							= childrens_container.parentNode
			const element_childrens_target		= ts_object.get_link_childrens_from_wrap(wrap)

		// // debug
		// 	console.log('+++++ childrens_container:', childrens_container);
		// 	console.log('+++++ wrap:', wrap);
		// 	console.log('+++++ element_childrens_target:', element_childrens_target);
		...
		// nd_container
			...
			const wrapper_children 	 	= childrens_container.parentNode.children
		...
		const promise = new Promise(function(resolve) {
			...
			const ar_childrens_c = build_ts_list(ar_childrens_data)

			function build_ts_list(ar_childrens_data) {
				...
				for (let i = 0; i < ar_childrens_data_len; i++) {
					...
					// wrap_ts_object . ts_object wrapper
						...
						let wrap_container, wrap_class, event_function
						if (is_descriptor===true) {
							wrap_container	= childrens_container
							wrap_class		= 'wrap_ts_object'
							event_function	= [
							...
							//if (typeof parent_nd_container==="undefined") {
								...
								//}
							//}
							wrap_container 	= parent_nd_container
							wrap_class 		= 'wrap_ts_object wrap_ts_object_nd'
							event_function 	= null
						}
					...
					// CHILDRENS CONTAINER . childrens container
						if (is_descriptor===true) {
							const childrens_c_class_name = (childrens_container_is_loaded===true)
								? 'childrens_container'
								: 'childrens_container js_first_load'
							const childrens_c = common.create_dom_element({
								element_type	: 'div',
								class_name		: childrens_c_class_name,
								data_set		: {
									role		:'childrens_container',
									section_id	: ar_childrens_data[i].section_id
								},
								parent			: wrap_ts_object
							})
					...
					// ID_COLUM_CONTENT elements
						switch(ts_object.thesaurus_mode) {
							...
							case 'relation':
								...
								const link_related	= common.create_dom_element({
									element_type			: 'a',
									class_name				: 'id_column_link ts_object_related',
									title_label				: 'add',
									custom_function_events	: [{'type':'click','name':'ts_object.link_term'}],
							...
							default:
														...
														.then(function(response){
															...
															// vars from response
																...
																// children_element. list_thesaurus_element of current wrapper
																	const children_element 	= ts_object.get_link_childrens_from_wrap(response.wrap)
																	if(!children_element) {
																		return console.log("[ts_object.add_children] Error on find children_element 'link_childrens'");
																	}

															// refresh childrens container
																ts_object.get_childrens(children_element)
																.then(function(){
									...
									if (ar_childrens_data[i].permissions_button_new>=2) {
										if(is_descriptor===true) {
											const link_drag = common.create_dom_element({
												...
												custom_function_events	: [{'type':'mousedown','name':'ts_object.on_drag_mousedown'}],
												title_label				: 'drag',
												parent					: id_colum_content
											})
											// drag icon
								...
								// DELETE . button delete element
									if (ar_childrens_data[i].permissions_button_delete>=2) {
										const link_delete 	= common.create_dom_element({
											...
											custom_function_events	: [{'type':'click','name':'ts_object.delete'}],
											title_label				: 'delete',
											parent					: id_colum_content
										})
										// delete icon
									...
									if (ar_childrens_data[i].permissions_button_new>=2) {
										if(is_descriptor===true && node_type!=='hierarchy_node') {
											...
											common.create_dom_element({
												...
												custom_function_events	: [{'type':'click','name':'ts_object.build_order_form'}],
												//data_set				: order_dataset,
												//title_label			: null,
												text_node				: i + 1 + (pagination.offset || 0),
												parent					: id_colum_content
											})
										}//if(is_descriptor===true && node_type!=='hierarchy_node')
									}
								...
								// EDIT . button edit element
									...
									const link_edit		= common.create_dom_element({
										...
										custom_function_events	: [{'type':'click','name':'ts_object.edit'}],
										title_label				: 'edit',
										parent					: id_colum_content
									})
									// section_id number
					...
					// LIST_THESAURUS_ELEMENTS
						const ts_line_node = build_ts_line( ar_childrens_data[i], indexations_container_id )
						elements_container.appendChild(ts_line_node)
				}//end for (let i = 0; i < ar_childrens_data_len; i++)


				return ar_childrens_c
			}//end build_ts_list
			...
			function build_ts_line(child_data, indexations_container_id) {
				...
				for (let j = 0; j < child_data_len; j++) {
					...
					const children_dataset	= {
						...
						//modo			: child_data.modo,
						//lang			: child_data.lang,
					}
					...
					switch(true) {
						...
						case (child_data.ar_elements[j].type==='term'):
							...
							var text_node = child_data.ar_elements[j].value
							switch(ts_object.thesaurus_mode) {
								case 'relation':
									var event_function 	= [];
									break;
							...
							// Term terminoID like [ts1_52]
								let term_add = " ["+child_data.section_tipo+'_'+child_data.section_id+"]"
									if(SHOW_DEBUG===true && node_type!=='hierarchy_node') {
										let current_link_childrens 	= child_data.ar_elements.filter(item => item.type==="link_childrens")[0]
										if (current_link_childrens) {
											let childrens_tipo = current_link_childrens.tipo
											let a = "../ts_object/trigger.ts_object.php?mode=get_childrens_data&section_tipo="+child_data.section_tipo+"&section_id="+child_data.section_id+"&tipo="+childrens_tipo+"&node_type=thesaurus_node&target_section_tipo="+target_section_tipo
						...
						// ND
						case (child_data.ar_elements[j].type==='link_childrens_nd'):
						...
						// ARROW ICON
						case (child_data.ar_elements[j].type==='link_childrens'):

							// Case link open childrens (arrow)
							const arrow_icon_node = common.create_dom_element({
								element_type	: 'div',
								parent			: fragment,
								class_name		: class_for_all,
								dataset			: children_dataset
							})
							arrow_icon_node.addEventListener('click', function(e) {
								ts_object.toggle_view_childrens(this, e)
							})

							let class_name = 'ts_object_childrens_arrow_icon'
								if (child_data.ar_elements[j].value==='button show childrens unactive') {
									class_name += ' arrow_unactive'
								}else if (show_arrow_opened===true){
									class_name += ' ts_object_childrens_arrow_icon_open'
								}
							const arrow_icon = common.create_dom_element({
								element_type	: 'div',
								parent			: arrow_icon_node,
								class_name		: class_name
							})
							break;

						// BUTTON SHOW INDEXATIONS AND STRUCTURATIONS
						case (child_data.ar_elements[j].model==='component_relation_index'):
						case (child_data.ar_elements[j].model==='component_relation_struct'):

							if (   (child_data.ar_elements[j].model==='component_relation_index'  && child_data.permissions_indexation>=1)
								|| (child_data.ar_elements[j].model==='component_relation_struct' && child_data.permissions_structuration>=1)
								) {
								...
								var event_function 	= [{
									...
									function_arguments	: [
										child_data.section_tipo,
										child_data.section_id,
										child_data.ar_elements[j].tipo,
										indexations_container_id
									]
								}]
								var element = common.create_dom_element({
			...
			function build_ts_pagination(){
				...
				button_show_more.addEventListener('click', function(e) {
					...
					// debug
						...
						// console.log('++++++ element_childrens_target.dataset.tipo:', element_childrens_target.dataset.tipo);
					...
					const tipo			= element_childrens_target.dataset.tipo // usually 'hierarchy45'
					...
					if(SHOW_DEBUG===true) {
						console.log('build_ts_pagination trigger_vars:', trigger_vars);
					}
	...
	* @param DOM object
	*/
	this.toggle_view_childrens = function(link_childrens_element, event) {
		...
		for (let i = len - 1; i >= 0; i--) {
			...
			let element_childrens = ts_object.get_link_childrens_from_wrap(parent_wrap)
				...
				if(element_childrens) {
					...
					ts_object.get_childrens( element_childrens )
					.then(function(response) {
	...
	*/
	...
	this.edit = function(button_obj, event, section_id, section_tipo) {
		...
		let url = DEDALO_LIB_BASE_URL + '/main/?t='+section_tipo+'&id='+section_id+'&menu=0'
	...
	this.add_children = function(button_obj) {
		...
		// children_element
			const children_element = ts_object.get_link_childrens_from_wrap(wrap)
				if(!children_element) {
					console.log("[ts_object.add_children] Error on find children_element 'link_childrens'");
					return false
				}
		...
		// tipo
			const tipo = children_element.dataset.tipo
				if (!tipo) {
					console.log("[ts_object.add_children] Error on find tipo on children_element 'link_childrens'");
					return false
				}
		...
		const js_promise = ts_object.get_json(trigger_vars).then(function(response) {
				...
				if (response===null) {
					...
					if (response.result===false) {
						...
						// All is OK
							// Refresh childrens container
							// ts_object.get_childrens(children_element).then(function(){
	...
	this.toggle_nd = function(button_obj) {
		...
		if (!nd_container.style.display || nd_container.style.display==='none') {
			...
			// Load all childrens and hide descriptors
				// Load element by ajax
				ts_object.get_childrens(button_obj)
				.then(function(response) {
	...
	* Locate link_childrens node iterating direct children of wrap->elements_container
	* @return DOM element|null link_childrens
	*/
	this.get_link_childrens_from_wrap = function(wrap) {

		// link_childrens . search component_relation_children tipo from wrap
		let link_childrens = null;
		...
		for (let i = wrap_children_len - 1; i >= 0; i--) {
			...
			const wrap_child = wrap_children[i];

			// elements_container
			if (wrap_child.classList.contains('elements_container')) {

				const elements_container_children		= wrap_child.childNodes;
				const elements_container_children_len	= elements_container_children.length
				for (let j = 0; j < elements_container_children_len; j++) {

					if( elements_container_children[j].dataset.type &&
						elements_container_children[j].dataset.type==='link_childrens') {

						link_childrens = elements_container_children[j]
						break;
					}
				}
				break;
		...
		// debug
			if(SHOW_DEBUG===true) {
				if (link_childrens===null) {
					console.warn("[ts_object.get_link_childrens_from_wrap] Error on locate link_childrens from wrap: ", wrap);
				}
			}
