...
var ts_object = new function() {
	...
	*/
	this.update_parent_data = function(wrap_ts_object) {
		...
		*/
		...
		let old_parent_wrap = ts_object.old_parent_wrap			
			if (!old_parent_wrap) {
		...
		let parent_wrap = wrap_ts_object.parentNode.parentNode;
			if(!parent_wrap) {
		...
		let parent_node_type 		 = parent_wrap.dataset.node_type
		/*
	...
	*/
	this.refresh_element = function( section_tipo, section_id ) {
		...
		// Locate all term elements
		let type 	= 'term';
		let matches = document.querySelectorAll('.list_thesaurus_element[data-type="'+type+'"][data-section_tipo="'+section_tipo+'"][data-section_id="'+section_id+'"]');
		const len 	= matches.length;
			if (len===0) {
	...
	this.show_component_editor = function(button_obj, html_data, role, callback) {
		...
		if (element_data_div) {
			...
			if (element_data_div.style.display==='none') {
				...
				let len = all_element_data_div.length
					for (let i = len - 1; i >= 0; i--) {
	...
	*/
	this.show_component_in_ts_object = function(button_obj) {
		...
		let wrap 	  	= button_obj.parentNode.parentNode
		let section_tipo= wrap.dataset.section_tipo
		let section_id 	= wrap.dataset.section_id
		let tipo 		= button_obj.dataset.tipo
		let modo 		= 'edit'
		let lang 		= page_globals.dedalo_data_lang
		let html_data 	= '...';	//" show_component_in_ts_object here! "
		let role 	  	= 'component_input_text' + '_' + section_tipo + '_' + section_id + '_' + tipo
	...
	*/
	this.build_order_form = function(button_obj, evt) {
		...
		for (let i = order_inputs.length - 1; i >= 0; i--) {
