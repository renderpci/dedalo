...
var component_portal = new function() {
		...
		return new Promise(function(resolve, reject) {
			...
			const js_promise = component_common.get_component_json_data(component_obj).then(function(response){
				...
				if (response && response.result) {
					...
					console.error("[component_portal.get_dato] ERROR on get dato portal data. Invalid trigger response: ",response);
				}
	...
	this.add_new_element = function (button_obj, wrapper) {
		...
		const js_promise = common.get_json_data(trigger_url, trigger_vars).then(function(response){
			...
			if (response && response.result) {
				...
				let created_record_id = parseInt(response.section_id)
				...
				if ( Number.isInteger(created_record_id) && created_record_id>0 ) {
					...
					var element_id = 'portal_link_open_' + portal_section_tipo + '_' + created_record_id
						console.log("[component_portal.add_new_element] element_id", element_id);
	...
	* @return promise
	*/
	this.render_html = function(wrapper) {
		...
		return new Promise(function(resolve, reject) {
			...
			const component_json_data_promise = self.build_component_json_data(wrapper)
	...
	* @return 
	*/
	this.build_component_json_data = function(wrapper) {
	...
	this.build_component_html = function(wrapper, component_json_data) {
		...
		return new Promise(function(resolve, reject) {
			...
			//if (component_json_data.dragable_connectWith) {
				if (edit_view==="view_tool_description") {
					if(SHOW_DEBUG===true) {
	...
	this.build_pagination = function(options) {
		...
		if(SHOW_DEBUG===true) {
			if (wrapper.dataset.tipo==="mdcat1477") {
				console.log("build_more_items_links - ","n_rows",n_rows,"max_records:",max_records,"offset:",offset,"maximun_showed:",maximun_showed)
			}
		}		
	...
	this.build_rows = function(data) {
		...
		switch(true) {
			case (edit_view && edit_view==="view_mosaic") :
				...
				column_width = null
				break;
			case (edit_view && edit_view==="view_tool_description") :				
		...
		for (let i = 0; i < result_len; i++) {
			...
			switch(true) {
				case (edit_view==='view_single_line') :
					...
					break;
				case (edit_view==='view_tool_description') :
	...
	*/
	this.build_header = function(data) {
		...
		for (let i = 0; i < columns_len; i++) {
			...
			let column 		= ar_columns[i]
			let column_tipo = column.tipo	
	...
	*//*
	this.get_more_items = function(button, event, instance, forced_offset) {
		...
		const wrapper = component_common.get_wrapper_from_element(button)

		html_page.loading_content( wrapper, 1 )

		const propiedades 	= self.propiedades
		const max_records 	= self.max_records
		...
		// 	}
		console.log("++ get_more_items component_options:",component_options);

		self.build_component_json_data(component_options).then(function(response){
				//console.log("response ++++ :",response); return

				const component_json_data = response.result


				// Build html with js
				self.build_component_html(component_json_data, wrapper)
		})
		return
	...
	*/
	this.show_more_items = function(button, wrapper) {
		...
		let new_max_records = parseInt(wrapper.dataset.max_records) + 10  
