/*globals page_globals, SHOW_DEBUG, common, document, console, get_label, find_ancestor, tool_common, component_common, html_page, inspector, exec_scripts_inside, section, DEVELOPMENT_SERVER, DEDALO_LIB_BASE_URL */
...
var component_portal = new function() {
	...
	*/
	this.build_delete_dialog = function(options) {
		...
		const footer = document.createElement("div")
			...
			// <button type="button" class="btn btn-primary">Save changes</button>
			if (options.permission_target_section_delete >=2 && options.delete_all) {
	...
	*/
	this.open_delete_dialog = function(button_obj) {
		...
		const delete_all			= propiedades.delete_all
		...
		const delete_dialog = this.build_delete_dialog({
			...
			delete_all							: typeof(delete_all)!=="undefined" ? delete_all : true
		})
	...
	*/
	this.sort_items = function(element) {
		...
		const offset		= JSON.parse(wrapper.dataset.offset)
		const max_records	= JSON.parse(wrapper.dataset.max_records)
		const min_rage 		= parseInt(offset)
		const max_range 	= min_rage + parseInt(max_records) -1

		const ar_elements_row 		= wrapper.querySelectorAll('.portal_element_sortable')
		const ar_elements_row_len 	= ar_elements_row.length

		// Fill ordered_dato from dom elements
		const ordered_dato = []
		for (let i = 0; i < ar_elements_row_len; i++) {
			let current_locator = JSON.parse(ar_elements_row[i].dataset.dato)
			ordered_dato.push( current_locator )
		}

		// Compare old and ordered dato to check changes
		const final_dato = []
		for (let i = 0; i < dato_len; i++) {
			const current_dato = dato[i]
			if(i >= min_rage && i <= max_range){
				const new_order_position = i - parseInt(offset)

				final_dato.push(ordered_dato[new_order_position])
			}else{
				final_dato.push(current_dato)

			}
		}
		// check if the new order is the same that current dato, if is the same doesn't save the dato
		// changed state
		let changed = false
		for (let i = 0; i < dato_len; i++) {
		...
		const js_promise = common.get_json_data(trigger_url, trigger_vars).then(function(response){
			...
			if (response && response.result) {
				...
				if ( Number.isInteger(created_record_id) && created_record_id>0 ) {
					...
					// update wrapper dataset n_rows
						wrapper.dataset.n_rows = parseInt(wrapper.dataset.n_rows)+1

					// update wrapper dataset dato
						const added_locator = response.added_locator || null
						if (added_locator && added_locator.section_id) {
							const dato = wrapper.dataset && wrapper.dataset.dato && wrapper.dataset.dato.length>2
								? JSON.parse(wrapper.dataset.dato)
								: []
							dato.push(added_locator)
							// replace dataset.dato with new whole string value
							wrapper.dataset.dato = JSON.stringify(dato)
						}
	...
	*/
	this.render_html = function(wrapper, dato, is_init) {
		...
		return new Promise(function(resolve, reject) {
			...
			self.build_component_json_data(wrapper, dato)
			.then(function(response){
				...
				if (!response || !response.result) {
					...
					self.build_component_html(wrapper, response.result)
					.then(function(){

						// Emit event like save . Attach custom event to render_html like save component
							const event_detail 	= {
								dataset : cloneDeep( wrapper.dataset ),
								wrapper : wrapper
							}
							const save_event = new CustomEvent('portal_ready', {detail:event_detail})
	...
	this.build_component_html = function(wrapper, component_json_data) {
		...
		return new Promise(function(resolve, reject) {
			...
			//if (component_json_data.dragable_connectWith) {
				if (edit_view && edit_view.startsWith("view_tool_description")) {
					if(SHOW_DEBUG===true) {
	...
	this.build_rows = function(data) {
		...
		// rows styles
			switch(true) {
				...
				case (edit_view && edit_view.startsWith("view_tool_description")) :
					...
					if (typeof(response.propiedades.edit_view_options)!=="undefined") {
						var row_styles = {
							width	: response.propiedades.edit_view_options.element_width,
							height	: response.propiedades.edit_view_options.element_height
						}
					}else{
						// Defaults
						var row_styles = null; //{width:'20%', height:'100px'}
		...
		// columns
			...
			for (let i = 0; i < result_len; i++) {
				...
				// Items (columns)
					...
					}//end (let z = 0; z < columns_len; z++)

				// Edit / delete column/s
					switch(true) {
						case (edit_view && edit_view==='view_single_line') :
		...
		/**/
			...
			if(SHOW_DEBUG===true && DEVELOPMENT_SERVER===true
				&& typeof data.response.json_rows!=="undefined"
				&& typeof data.response.json_rows.data!=="undefined"
				&& data.response.json_rows.data.length>0) { //

				console.log("++data.response.json_rows:",data.response.json_rows.data);
				//console.log("data.response.json_rows:",data.response.json_rows);

				const portal_test_container = common.create_dom_element({
												element_type 	: 'div',
												class_name 	 	: "rows_container", // portal_test_container
												parent 			: wrapper
											})

				section.parse_json_rows({
					json_rows 		: data.response.json_rows,
					container 		: portal_test_container,
					build_header 	: true
				}).then(function(response){
	...
	*/
	this.build_header = function(data) {
		...
		if(edit_view && edit_view==='view_single_line') {
			common.create_dom_element({
					element_type 	: 'div',
					class_name 	 	: 'column delete_column',
					style 			: { "width" : column_width },
	...
	*/
	this.build_add_buttons = function(options) {
		...
		// Options vars
			...
			const propiedades			= component_json_data.propiedades
			const rows_limit			= propiedades.rows_limit
		...
		// portal edit buttons
			...
			for (let i = 0; i < len; i++) {
				...
				const target_section_tipo = ar_target_section_tipo[i]
	...
	*/
	this.goto_last_page = function(button, wrapper) {
		...
		// last_offset. calculate last possible offset for all records
		let last_offset 	= Math.floor(n_rows / max_records) * max_records

		if (n_rows <= last_offset) {
			last_offset = 0
		}
