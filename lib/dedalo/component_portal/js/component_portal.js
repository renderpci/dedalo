...
var component_portal = new function() {
	...
	*/
	this.get_dato = function(wrapper_obj) {
		...
		return new Promise(function(resolve, reject) {
			// DB ajax Call
			var js_promise = component_common.get_component_json_data(component_obj).then(function(response){

				if (response && response.result) {

					let response_result = response.result		
					let dato 			= response_result.dato || null
					resolve(dato);
	...
	*/
	this.add_new_element = function (button_obj) {
		...
		let component_wrap = component_common.get_wrapper_from_element(button_obj)
			if (!component_wrap) {
				alert("[component_portal.add_new_element] Error on select component wrap")
				return false;
			}
			let id_wrapper = component_wrap.id
			//console.log(component_wrap)

		let component_info	= JSON.parse(component_wrap.dataset.component_info);
		let propiedades 	= component_info.propiedades
		const rows_limit	= propiedades.rows_limit
		const n_rows		= button_obj.dataset.n_rows
		...
		let js_promise = common.get_json_data(component_portal.url_trigger, trigger_vars).then(function(response){
			if(SHOW_DEBUG===true) {
			...
			if (response && response.result) {
				...
				var created_record_id = parseInt(response.section_id)
	...
	*/
	this.remove_element = function (button_obj, remove_mode) {
		...
		const trigger_vars = {
				...
				tipo  			: component_wrap.dataset.tipo,
				parent 			: component_wrap.dataset.parent,
