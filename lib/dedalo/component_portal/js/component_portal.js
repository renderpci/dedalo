...
$(document).ready(function() {
	...
	//component_portal.active_portal_drag();

		//$('.portal_table_dd906 tbody')
...
var component_portal = new function() {
	...
	*/
	this.save_order = function(table_object) {
		...
		var ar_final 	= [];
		var ar_tr_dato 	= $(table_object).find('tr.tr_sortable');	//console.log(ar_tr_dato)
		...
		for (var i = 0; i < ar_tr_dato.length; i++) {
			...
			if (typeof dato=='undefined' || typeof dato.section_id=='undefined' || dato.section_id.length<1) {
				console.log("Error: tr wrong in list !!!! Please fix this ASAP.");
				return alert("Error. Sort rows unavailable")
			}else{
				ar_final.push(dato)
			}			
		}
