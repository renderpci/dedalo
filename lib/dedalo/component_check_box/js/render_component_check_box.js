...
* @return DOM node
*/
render_component_check_box.prototype.edit = async function() {
...
// update value, subscription to the changes: if the dom input value was changed, observers dom elements will be changed own value with the observable value
		...
		function update_value (component) {
			...
			const changed_node = wrapper.querySelector('input[data-key="'+component.selected_key+'"]')
				console.log("changed_node:",changed_node);
			changed_node.checked = (changed_data.value === null) ? false : true
		}

	// add button element, subscription to the events
		self.events_tokens.push(
			event_manager.subscribe('edit_element_'+self.id, self.selector, edit_element)
		)
	...
	// remove button element, subscription to the events
		self.events_tokens.push(
			event_manager.subscribe('reset_element_'+self.id, self.selector, reset_element)
		)
	...
	// change event, for every change the value in the imputs of the component
		wrapper.addEventListener('change', (e) => {
			...
			if (e.target.matches('input[type="checkbox"]')) {
				...
				// set the value for the checkbox changed (in the main object of the component)
				let done = false
				if (input.checked===true) {
					done = self.add_value(value)
				}else{
					done = self.remove_value(value)
				}
				if(done===true){
