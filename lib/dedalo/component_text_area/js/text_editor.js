	...
	*/
	...
	this.save_command = function(ed,evt,text_area_component) {
		...
		if($(text_area_component).length>0) {
			...
			component_text_area.Save(text_area_component);
			if (DEBUG) {
				//console.log("-> save_command trigger Save from tinyMCE " + text_area_id);
				//console.log($(component_text_area))
			}
		...
		switch(true) {
			...
			case ( tag.indexOf('index')!=-1 ) :
					...
					var tipo 			= $(text_area_component).data('tipo'),
						...
						section_tipo 	= $(text_area_component).data('section_tipo'),
						parent 			= $(text_area_component).data('parent');					
