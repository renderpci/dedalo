	...
	// input contenteditable
		...
		// paste event
			input.addEventListener('paste', async (event) => {
				...
				//const paste_content = (event.clipboardData || window.clipboardData).getData('text/plain')
				//const paste_node 	= document.createTextNode(paste_content)



				const paste_content = (event.clipboardData || window.clipboardData).getData('text/html')

				const elements = document.createElement("div")
					  elements.innerHTML = paste_content
					  	console.log("elements:",elements);

				const paste_node = await clean_nodes(elements) // document.createElement("div")	//paste_content//document.createTextNode(paste_content)
					console.log("paste_node:",clean_node);


			    const selection = window.getSelection();
			    if (!selection.rangeCount) return false;
			    selection.deleteFromDocument();
			    selection.getRangeAt(0).insertNode(paste_node);
...
* CLEAN_NODES
*/
const clean_nodes = async (container) => {
	...
	for (var i = container.childNodes.length - 1; i >= 0; i--) {
		...
		// 	case 'STRONG':
		// 		const new_node_strong = document.createElement('b')
		// 		new_node_strong.innerHTML = node.innerHTML
		// 		node.parentNode.replaceChild(new_node_strong, node);
		// 		break;

		// 	case 'EM':
		// 		const new_node_em = document.createElement('i')
		// 		new_node_em.innerHTML = node.innerHTML
		// 		node.parentNode.replaceChild(new_node_em, node);
		// 		break;
		...
		if(node.hasChildNodes()) {
			console.log("node hasChildNodes:", node.nodeName, node);
			await clean_nodes(node)
		}
