...
var mce_editor = new function() {
	...
	*/
	this.init = function (text_area_id, modo, propiedades) {
		...
		const text_area_component = document.getElementById(text_area_id) // Use let (reasignable)
		if(!text_area_component) {
			if(SHOW_DEBUG) console.log("Ops.. text_area_component not found. text_area_id: "+text_area_id)
			return false;
		}

		const text_area_wrapper	= component_common.get_wrapper_from_element(text_area_component) // Use let (reasignable)
		let	cssFile				= DEDALO_LIB_BASE_URL + '/component_text_area/css/' + 'mce_editor_default.css?' + page_globals.dedalo_version

		// editor_height from properties content_data. 
			let	editor_height	= 125 // default height
			// look component css height definition if exists
			// Use format like: 
			// ".content_data": {
			// 	"style": {
			// 		"height": "450px"
			// 	}
			// }			
			const content_data			= text_area_wrapper.querySelector(".content_data")
			const content_data_height	= window.getComputedStyle(content_data).height
			if (content_data_height.indexOf("px")!==-1) {
				const content_data_css_height	= parseInt(content_data_height)
				const editor_bars_height		= 34 + 40
				const bottom_items				= 0 //27
				const target_height			= content_data_css_height - editor_bars_height - bottom_items
				if (target_height>editor_height) {
					editor_height = target_height
				}
			}
		...
		// propiedades_obj
			const propiedades_obj = (typeof propiedades==="string")
				? JSON.parse(propiedades)
				: (propiedades || {})
		...
		// CONFIG DEFAULT
			...
			// toolbar
				const image_tag_toolbar = propiedades_obj.image_tag ? " | button_image" : ""
				const basic_buttons = "bold italic underline undo redo searchreplace pastetext code fullscreen"
				let	current_toolbar = basic_buttons + image_tag_toolbar + " | button_save"
		...
		switch(modo) {
			case 'tool_transcription':
					if(context_name==='component_geolocation') {
						current_toolbar  = basic_buttons + " | button_geo | button_note | button_save"
					}else{
						current_toolbar  = basic_buttons + " | button_note button_person | button_save"
					}
			...
			case 'tool_structuration':
					current_toolbar  = basic_buttons + " | button_note button_reference | button_save"
					cssFile 		 = DEDALO_LIB_BASE_URL + '/component_text_area/css/' + 'mce_editor_tool_transcription.css?' + page_globals.dedalo_version
					editor_height 	 = 597 -70
					break;*/
			case 'tool_lang':
					...
					current_toolbar  = basic_buttons + " | button_save"
					}else{
					current_toolbar  = basic_buttons + " | button_add_structuration button_change_structuration button_delete_structuration | button_save"
					}
					break;
		...
		const html_tags_allowed  = "strong/b,em/i,div[class],span[class|style],img[id|src|class],br,p,apertium-notrans,section[id|class],reference[id|class],h2[class],label[class]"
				...
				*/
				...
				setup : function(ed) {
						...
						ed.addButton('button_image', {
							tooltip: 'Add image',
							image:  '../themes/default/buttons/image.svg',
							onclick: function(evt) {
								component_text_area.create_new_image_tag(ed, evt, text_area_component)
							}
						});
							...
							switch(modo) {
								case 'edit':
									...
									if(text_area_wrapper) {
										// when_in_dom(text_area_wrapper, function(){
										setTimeout(function(){
											// update_editor_height
											self.update_editor_height(text_area_wrapper, this)
										}, 500)
										// })										
									}									
									break;
								case 'tool_transcription' :
									...
									//setTimeout(function(){
										if(text_area_wrapper) {
											let warp_height = text_area_wrapper.offsetHeight //parseInt(text_area_wrapper.offsetHeight) -100;
											if (warp_height>200 && ed.theme) {
						...
						ed.on('PostRender', function(evt){
							...
							switch(modo) {
								case 'tool_lang' :
									...
									const base_height = window.innerHeight - 250
						...
						ed.on('focus', function(evt) {
							...
							const current_text_area_component	= document.getElementById( this.id );
							const current_text_area_wrapper		= component_common.get_wrapper_from_element(current_text_area_component)

							component_common.select_wrap(current_text_area_wrapper)	

							// update_editor_height
							self.update_editor_height(current_text_area_wrapper, this)
						...
						ed.on('click', function(evt) {
							...
							if( evt.target.nodeName==='IMG' || evt.target.nodeName==='REFERENCE' ) {
								...
								switch(evt.target.className) {
									...
									case 'image':
										// Show image info
										component_text_area.show_image_info( ed, evt, text_area_component )
										break;
									case 'reference':
										if(evt.altKey===true){
						...
						*/
						...
						ed.on('KeyPress', function(evt) {
							...
							const minor_that_key	= 60 // 188
							const more_that_key		= 62	// 190
	...
	/**
	* UPDATE_EDITOR_HEIGHT
	* @return number target_height
	*/
	this.update_editor_height = function(text_area_wrapper, ed) {
		...
		if (ed.theme) {
			...
			// look inside for possible additional elements like a component state
				const elements_inside = content_data.children
				for (var i = 0; i < elements_inside.length; i++) {
					const element = elements_inside[i]
					if (!element.classList.contains("mce-container")) {
						target_height = target_height - element.offsetHeight
					}
				}

			target_height = target_height - 45

			if (target_height>160) { // 227
