...
var mce_editor = new function() {
	...
	*/
	this.init = function (text_area_id, modo, propiedades) {
		...
		//if(DEBUG) console.log("->mce_editor.init: "+text_area_id)
		...
		var	text_area_wrapper	= component_common.get_wrapper_from_element(text_area_component)	//document.getElementById(text_area_component.dataset.id_wrapper),
		var	cssFile				= DEDALO_LIB_BASE_URL + '/component_text_area/css/' + 'mce_editor_default.css?' + page_globals.dedalo_version
		var	editor_height		= 126


		if(typeof text_area_wrapper.dataset.related_name != 'undefined'){

			var related_component_name = JSON.parse(text_area_wrapper.dataset.related_name)
			var related_component_length = related_component_name.length
		//console.log(context_name);
			if(related_component_length > 0 ){
				for (var i = related_component_length - 1; i >= 0; i--) {
		...
		var	current_toolbar  = "bold italic undo redo searchreplace pastetext code fullscreen | button_save"
		var current_statusbar= true
		var	current_plugings = ["paste","image","print","searchreplace","code","fullscreen","noneditable"]// "wordcount"							
		// TR rsc36 case
		...
		var html_tags_allowed  = "strong/b,em/i,div[class],span[class],img[id|src|class],br,p,apertium-notrans,section[id|class],reference[id|class],h2[class],label[class]"
				...
				*/
				...
				setup : function(ed) {
						...
						ed.on('focus', function(evt) {
							...
							//text_area_wrapper 			= document.getElementById( text_area_component.dataset.id_wrapper );
							...
							switch(context_name) {
								...
								case 'component_image':
										...
										if(evt.keyCode==tag_insert_key) {
											mce_editor.write_tag('svg', ed, evt, text_area_component)
										}
										break;
								case 'component_geolocation':
