...
var mce_editor = new function() {
	...
	*/
	this.init = function (text_area_id, modo, propiedades) {
		...
		if(propiedades && typeof propiedades.statusbar === 'undefined'){
			propiedades.statusbar = true
		}

		// CONFIG DEFAULT
		let current_inline 	 = false
		let	current_menubar  = false
		let	current_toolbar  = "bold italic undo redo searchreplace pastetext code fullscreen | button_save"
		let current_statusbar= propiedades.statusbar
		...
		let html_tags_allowed  = "strong/b,em/i,div[class],span[class],img[id|src|class],br,p,apertium-notrans,section[id|class],reference[id|class],h2[class],label[class]"

		// CONFIG CUSTOM PROPIEDADES (configure editor options in component propiedades)		
		if(typeof propiedades!=='undefined' && propiedades) {
			//var propiedades_obj = $.parseJSON(propiedades);
			let propiedades_obj = propiedades;
