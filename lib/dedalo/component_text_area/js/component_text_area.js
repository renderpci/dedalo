...
var component_text_area = new function() {
	...
	*/
	this.Save = function(component_obj, save_arguments, ed) {
		...
		if (component_obj === null || typeof component_obj !== 'object') {
			alert("Error: component_text_area is empty")
			return false;
		}
		...
		if (ed === null || typeof ed !== 'object') {
			...
			if(DEBUG) console.log("Warning: text editor is not received as argument. Global active editor is selected instead !");		
		}
		if (ed === null || typeof ed !== 'object') {
		...
		if(!ed.isDirty()) {
			if (DEBUG) {
				console.log("-> Info: Nothing is saved because ed.isDirty is false (no changes are detected in editor)");
			}
			return false;
		}
	...
	*/
	this.load_tr = function(component_obj, ed) {
		...
		if (typeof component_obj!='object' || typeof component_obj.dataset=='undefined') {
			console.log("Error on load_tr. Invalid component_obj: "); 	console.log(component_obj);
			return false;
		}
		...
		//var ed = tinymce.activeEditor;	//	component_obj.id
		...
		//var jsPromise = Promise.resolve(
			...
			// DONE
			.done(function(updated_received_data) {

				// DEBUG CONSOLE Console log
				if (DEBUG) {
					console.log("Loaded tr content length: "+updated_received_data.length);
				...
				*/
				if ( /Auth error/i.test(updated_received_data) ) {
					...
					alert("Error on save: "+updated_received_data+"<hr>"+ get_label.conservar_los_cambios_transcripcion)
				}else{
					//console.log(updated_received_data);
	...
	*/
	this.change_tag_state = function (obj) {
		...
		var tag 			= $(obj).data('tag')
			id 				= component_text_area.tag_to_id(tag)
			current_state 	= component_text_area.tag_to_state(tag)
			new_state 		= $(obj).val();
		...
		if (!new_state) return false;
			...
			return component_text_area.Save( $('.css_text_area')[0], null, tinyMCE.activeEditor );
	...
	*/
	this.update_geo_tag = function(tagOriginal, id, state, data){
		...
		// SAVE : Save component data
		component_text_area.Save( $('.css_text_area')[0], null, ed);
	...
	*/
	this.show_button_link_fragmet_to_portal = function(tagName, tipo, parent, section_tipo) {
		...
		return false; // NO MORE IS NEEDED ANYMORE
	...
	*/
	this.reload_component_with_lang = function(data) {
		...
		var selector = '[role="wrap_component_text_area"][data-tipo="'+data.tipo+'"][data-section_tipo="'+data.section_tipo+'"][data-parent="'+data.parent+'"]',
			...
			//console.log(wrapper);
		if (wrapper && typeof wrapper[0]!='undefined') {
			wrapper = wrapper[0];
	...
	*/
	this.show_component_in_row_thesaurus = function(button_obj) {
		...
		var role 	  = 'component_text_area' + '_' + button_obj.dataset.section_tipo + '_' + button_obj.dataset.parent + '_' + button_obj.dataset.tipo
