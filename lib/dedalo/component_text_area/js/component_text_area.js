...
var component_text_area = new function() {
	...
	*/
	this.get_dato = function(wrapper_obj) {
		...
		if (wrapper_obj.dataset.modo==="search") {
			...
			let input_text  = wrapper_obj.getElementsByTagName('input')[0]
			...
			if (text_preview_div) {
				...
				let textarea  = wrapper_obj.getElementsByTagName('textarea')[0]			
				let ed 		  = tinyMCE.get(textarea.id)
	...
	*/	
	this.save_text_preview = function(editor_obj) {
		...
		let component_obj 	= editor_obj //component_common.get_wrapper_from_element(editor_obj)
		let save_arguments 	= null
		let ed 				= null
	...
	*/
	this.Save = function(component_obj, save_arguments, ed) {
		...
		let clean_start = new Date().getTime();	
		let dato_clean = self.preprocess_text_to_save(container)
			if (dato_clean) {
		...
		let js_promise = component_common.Save(component_obj, save_arguments).then(function(response) {
			if(SHOW_DEBUG===true) {
