	...
	*/
	this.load_button_link_fragmet_to_portal = function(tagName, tipo, parent, section_tipo) {
		...
		var vars = new Object();
			...
			// Verify vars values
			if(!test_object_vars(vars,'remove_locator_from_portal')) {
				console.log("ERROR: test_object_vars not passed! (load_button_link_fragmet_to_portal)");
				return false;
			}
		...
		// Target div (contains all data info required for create the component to load)
		var wrapper_id 	= 'relations_ajax_div_'+tipo,
			target_obj 	= document.getElementById(wrapper_id);
		...
		component_common.load_component_by_wrapper_id(wrapper_id, arguments,
							function(){
								...
								document.querySelector(".btn_relate_fragment").style.display = 'inline-block'
							});
	...
	this.change_tag_state = function (obj) {
		...
		if (!new_state) return false;
			...
			*/
			...
			component_text_area.Save( $('.css_text_area')[0], null, tinyMCE.activeEditor );
