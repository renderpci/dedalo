...
var component_text_area = new function() {
	...
	*/
	...
	this.init = function(data) {
		...
		// Create a instance of component_text_area
		let self = this;	//Object.create(component_text_area)
			...
			let wrapper_obj = document.getElementById(id_wrapper)
				if (!wrapper_obj) {
					...
					if (data.modo!=="portal_list") {
						...
						var msg = "[component_text_area:init] ERROR. wrapper_obj not found! ("+id_wrapper+")"
		...
		let init_uid = self.section_tipo +"_"+ self.section_id +"_"+ self.component_tipo
		...
		//if( self.inited===false ) {
			...
			if (	component_tools_container===null 
				...
				//console.log("modo ", page_globals.modo);
		...
		self.inited = true
		//console.log("self.inited B",self.inited);
	...
	*/	
	this.save_text_preview = function(editor_obj) {
		...
		var component_obj 	= editor_obj //component_common.get_wrapper_from_element(editor_obj)
		var save_arguments 	= null
		var ed 				= null

		// Ser load after save as false
		component_text_area.set_reload_on_save(false)

		return this.Save(component_obj, save_arguments, ed)
	...
	*/
	this.Save = function(component_obj, save_arguments, ed) {
		...
		let is_wrapper = component_obj.classList.contains('wrap_component');
		if (is_wrapper===true) {
			// Search text_preview div
			var text_preview_div = wrapper.querySelector('#text_preview')
			if (text_preview_div) {
		...
		let dato_clean = this.preprocess_text_to_save(container)
			if (dato_clean) {
		...
		if(SHOW_DEBUG===true) {
			const time 	= new Date().getTime() - clean_start
		...
		let wrapper = component_common.get_wrapper_from_element(component_obj)
		// SAVE COMPONENT_COMMON . Exec general save
		let jsPromise = component_common.Save(component_obj, save_arguments).then(function(response) {
			if(SHOW_DEBUG===true) {
	...
	*/
	this.preprocess_text_to_save = function(container) {
		...
		let section_elements = cloned_text.getElementsByTagName('section')
		let ar_section_id = []
		let ar_section_id_duplicates = []
		if (section_elements) {
		...
		let reference_elements = cloned_text.getElementsByTagName('reference')
		...
		let image_elements = cloned_text.getElementsByTagName('img')
		if (image_elements) {
			...
			for (let i = image_elements_len - 1; i >= 0; i--) {
				...
				let final_string = this.build_dedalo_tag(image_elements[i].dataset.type, image_elements[i].dataset.tag_id, image_elements[i].dataset.state, image_elements[i].dataset.label, image_elements[i].dataset.data) 
				if (final_string) {
					// Replaces tag content string with new created
	...
	*/
	this.load_tr = function(wrapper_obj, ed) {
		...
		let is_tiny = component_text_area.is_tiny(ed)
		...
		let js_promise = common.get_json_data(this.url_trigger, trigger_vars).then(function(response){
			...
			if (!response) {
				...
				if (response.result!==false) {				
					...
					let updated_received_data = response.result				
					...
					*/
					if (is_tiny===true) {
						...
						if (response.result!==false) {
							...
							let component_obj = wrapper_obj.querySelector('#text_preview')
							// Inject contents
	...
	*/
	this.build_dedalo_tag = function(type, tag_id, state, label, data) {
		...
		const valid_types = ["indexIn","indexOut","structIn","structOut","tc","tc2","svg","geo","page","person","note","referenceIn","referenceOut"]
	...
	*/
	this.get_tags = function (related_tipo, tag_type) {
		...
		let component_text_area 	= document.querySelector('textarea[data-tipo="'+related_tipo+'"]')
		...
		let component_text_area_id 	= component_text_area.id

		// ED . Select text editor
		let ed = tinymce.get(component_text_area_id)

		// Select nodes of type tag_type
		let ar_tags = ed.dom.select('[data-type="'+tag_type+'"]')
	...
	*/
	this.get_last_tag_id = function(container, tag_type) {
		...
		// LAST ID
		let last_tag_id = Math.max.apply(null, ar_id_final);
			if(SHOW_DEBUG===true) {
	...
	*/
	this.change_tag_state = function (select_obj) {
		...
		*/
		...
		let tag_obj = component_text_area.tag_obj
		let save 	= true
		...
		let	new_state = select_obj.value
		...
		let tag_data = component_text_area.selected_tag_data
	...
	*/
	this.update_tag = function(tag_data, new_data_obj, save) {
		...
		let related_tipo = tag_data.component_tipo
		...
		let len = current_elements.length
	...
	*/
	this.av_editor_key_up = function(e) {
		...
		try{
			...
			if(DEBUG) console.log("[component_text_area.av_editor_key_up] ", e)
		}
	...
	*/
	this.goto_time = function (timecode) {
		...
		let iframe_obj = document.getElementById('videoFrame')
...
alert("get_tag_label");
	...
	*/
	this.load_tags_person = function(button_obj, hide) {
		...
		var wrap_div = find_ancestor(button_obj, 'wrap_component')
			if (wrap_div === null ) {
				if(DEBUG) console.log("[component_text_area.load_tags_person] button_obj", button_obj);
	...
	this.show_reference_info = function( ed, evt, text_area_component ) {
		...
		let js_promise 	 = common.get_json_data(component_text_area.url_trigger, trigger_vars).then(function(response) {
			...
			if (response===null) {
				...
				let reference_dialog = component_text_area.build_reference_dialog({
					"evt" 	  	 : evt,
					"response"   : response,
					"tag_id" 	 : tag_id,
					"ed" 		 : ed,
				...
				$(reference_dialog).modal({
					...
					let input_search = reference_dialog.querySelector('input.css_autocomplete_hi_search_field')
					if (input_search) {
						input_search.focus()
					}
				}).on('hidden.bs.modal', function (e) {
					// Focus first input on show dialog		
					let input_search = reference_dialog.querySelector('input.css_autocomplete_hi_search_field')
					let wrapper 	 = component_common.get_wrapper_from_element(input_search) // css_wrap_autocomplete_hi							
					// Update lock_components state (BLUR)
					if(typeof lock_components!='undefined') {
						// Unlock all components of current section on close note dialog
						lock_components.delete_user_section_locks({
				...
				$('#div_reference_wrapper').modal('show').on('shown.bs.modal', function (e) {
					// Focus first input on show dialog		
					var input_search = reference_dialog.querySelector('input.css_autocomplete_hi_search_field')
					if (input_search) {
						input_search.focus()
					}				
				})*/
	...
	*/
	this.build_reference_dialog = function( options ) {
		...
		var wrapper_id = "div_reference_wrapper"
		var older_div_wrapper = document.getElementById(wrapper_id)
		...
		var body = document.createElement("div")				
			// component_autocomplete_hi element
			var component_autocomplete_hi = document.createElement("div")
				component_autocomplete_hi.classList.add('reference_container')
				component_autocomplete_hi.innerHTML = options.response.component_autocomplete_hi_html
				body = component_autocomplete_hi
				//exec_scripts_inside(component_autocomplete_hi)
	...
	*/
	this.update_reference = function(button_obj, options) {
		...
		// Get actual value
		var div_reference_wrapper = document.getElementById('div_reference_wrapper')
		...
		var input = div_reference_wrapper.querySelector('input.css_autocomplete_hi_dato_hidden');
			if (!input) {
	...
	this.show_structuration_info = function(ed, evt, text_area_component) {
		...
		var js_promise = common.get_json_data(this.url_trigger, trigger_vars).then(function(response) {
			...
			// Build structuration_info
				var structuration_info = component_text_area.build_structuration_info({
	...
	*/
	this.build_structuration_info = function( options ) {
		...
		let wrapper_id = "div_structuration_wrapper"
		let older_div_wrapper = document.getElementById(wrapper_id)
			if (older_div_wrapper) {
				older_div_wrapper.parentNode.removeChild(older_div_wrapper)
			}		
		// note wrapper
	...
	*/
	this.create_tag_image = function(img_object) {
		...
		let label = img_object.dataset.label
		let type  = img_object.dataset.type
		...
		let data = this.render_tag_image(label, type);
		img_object.src = data
	...
	/* IS_CONTENT_EDITABLE. Manage html5 content editable for current element
	--------------------------------------------------------------------------- */
