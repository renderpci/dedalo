...
var component_text_area = new function() {
	...
	*/
	this.update_tag = function(tag_data, new_data_obj, save) {
		...
		// tag_data values			
			const related_lang	= tag_data.lang // is optional
			const type			= tag_data.type
			const tag_id		= tag_data.tag_id
		...
		// text area container / wrapper
			const container = this.get_related_editor(related_tipo, related_lang)
			if (!container) {
		...
		// selection_pattern. DOM Selection pattern
			let selection_pattern
			if (type.indexOf('In')!==-1 || type.indexOf('Out')!==-1) {
				const type_name = type.replace(/In|Out/, '');
				// Selects elements with data start with 'type_name' like 'indexIn'
				selection_pattern = '[data-type^="'+type_name+'"][data-tag_id="'+tag_id+'"]'
			}else{
				selection_pattern = '[data-type="'+type+'"][data-tag_id="'+tag_id+'"]'
			}

		// editor check (is tinyMCE?)
		...
		// Select current tag in dom
			...
			if(SHOW_DEBUG===true) {
				// console.warn('current_elements', is_tiny, current_elements); return;
			}
			...
			if (is_tiny===true) {
				...
				if (typeof save!=="undefined" && save===true) {					
					...
					// text_area element and ed share the same id. 
						const current_component_text_area = document.getElementById(ed.id)
					
					// Save modified content
						const result = component_text_area.Save( current_component_text_area, null, ed );
	...
	/**
	* GET_RELATED_EDITOR
	* @return DOM node related_editor
	*/
	this.get_related_editor = function(related_tipo, related_lang) {
		...
		// wrapper
			const wrapper = (typeof related_lang==="undefined" || !related_lang)
				? document.querySelector('.wrap_component[data-tipo="'+related_tipo+'"]')
				: document.querySelector('.wrap_component[data-tipo="'+related_tipo+'"][data-lang="'+related_lang+'"]')
			// check is valid node
			if (!wrapper) {
				console.error(`[component_text_area.get_related_editor] ERROR. No wrapper found with related_tipo: ${related_tipo},  related_lang: ${related_lang}`);
				return false
			}
	...
	*/
	this.load_tags_person = function(button_obj, hide) {
		...
		let people_overlay = document.getElementById('people_overlay')
			if (!people_overlay) {
					people_overlay = document.createElement('div')
					people_overlay.id = 'people_overlay'
					people_overlay.style.display = ''
			}else{
				if (people_overlay.style.display==='none') {
					people_overlay.style.display = '';
				}else{
					people_overlay.style.display = 'none';
				}
				return false
			}
		...
		const js_promise = common.get_json_data(this.url_trigger, trigger_vars).then(function(response) {
			...
			editor_panel.appendChild(people_overlay)

			if (response===null) {
				people_overlay.innerHTML = "<div>null value was received</div>"
			}else{
				var parse_tags_person = component_text_area.parse_tags_person(response.result)
				people_overlay.appendChild(parse_tags_person)
			}
			...
			const button_close = document.createElement('div')
				...
				button_close.addEventListener('click', function() {
					...
					//people_overlay.remove()
					people_overlay.style.display = 'none';
				});
				people_overlay.appendChild(button_close)
	...
	*/
	this.parse_tags_person = function(data) {
		...
		var t  = document.createTextNode('People') // get_label.personas
