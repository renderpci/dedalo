...
var component_input_text = new function() {
	...
	*/
	this.Save = function(component_obj) {
		...
		if (self.checking_value===true) {
			setTimeout(function(){
				console.warn("Delayed Save command until checking_value false");
				self.Save(component_obj)
			}, 1000)
		...
		// check_patern_tld
			...
			if (tipo==='hierarchy6' || tipo==='hierarchy7') {
				const valid = self.check_patern_tld(component_obj)
				if (!valid) {
					alert("Bad format! Only chars [a-z] are allowed for new TLD \n(max. 16 char without spaces or numbers).\nDATA IS NOT SAVED!");
					
					// restore original value
						const value_original = component_obj.dataset.value_original
						component_obj.value = value_original
						component_obj.focus()
		...
		// check for the unique value (onkey event fixes dataset values previously using function self.is_unique)
		...
		if(unique===true){
			...
			if(is_saveable===false){
				alert("Bad value! This value already exists!.\nDATA IS NOT SAVED!");
	...
	*/
	this.is_unique = function(component_input_node){
		...
		// reset
			...
			// status checking_value
				self.checking_value = true
			// reset state
		...
		.then(function(result){				
				...
				if(result.length === 0){
					...
					const key = result && typeof result[0]!=="undefined" && result[0].key
						? result[0].key
						: false

					const found_section_id		= JSON.parse(key).section_id
					const current_section_id	= wrap_div.dataset.parent

					if (found_section_id && found_section_id!=current_section_id) {
						
						// warning node
							unique_warning = unique_warning || common.create_dom_element({
								element_type 	: "div",
	...
	* @return bool
	*/
	this.check_patern_tld = function(element, event) {
		
		const value = element.value

		function checkPattern(value) {

		    // Allow a-z. Min 1 char.
		    const re = /^[a-z]+$/i;

		    return re.test(value);
		}

		if ((value && value.length>0) && !checkPattern(value) || value.length>16) {

			// const source_value = element.dataset.source_value
			// const edited_value = value.slice(0, -1)

			// restore original value
			// element.value = source_value 

			return false
		}
		
		return true
	};//end check_patern_tld
