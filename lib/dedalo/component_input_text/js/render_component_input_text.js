...
const content_data_edit = async function(self) {
	...
	// tools
		...
		for (let i = 0; i < tools_length; i++) {
			if(tools[i].show.component){
				const requeriments = Object.keys(tools[i].requeriments)
				let check = {}
				for (let i = requeriments.length - 1; i >= 0; i--) {
					const req = requeriments[i]

					check.req = self.context[req]
				}
				const validate = Object.values(check)
					.every(item => item === true)

				if(validate){
					add_tools(tools[i], buttons_container, self)
				}
...
* @return dom element li
*/
const add_tools = function(tool_object, buttons_container, self) {
	...
	// Events
		...
		// change event, for every change the value in the imputs of the component
			tool_button.addEventListener('mouseup', (e) => {
				...
				common.prototype.load_tool(tool_object, self)
