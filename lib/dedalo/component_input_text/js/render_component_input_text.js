...
render_component_input_text.prototype.edit = async function(options={render_level:'full'}) {
	...
	// click event [keyup]
		wrapper.addEventListener("keyup", async (e) => {
			...
			if (self.context.properties.unique) {
				//const duplicates = await check_duplicates(self, e.target.value, false)
				const duplicates = await self.is_unique(e.target.value)
				//if (duplicates!==false) {
				if (typeof duplicates!=="undefined") {
					ui.component.show_message(wrapper,
						`Warning. Duplicated value '${e.target.value}' in id: ` + self.duplicates.section_id)
				}
			}
		})
...
* CHECK_DUPLICATES
*/
const check_duplicates = async (self, value, show_alert=true) => {

	if (value.length>0) {

		const record = await self.is_unique(value)

		if (typeof record!=="undefined" && record.section_id!==self.section_id) {

			if (show_alert===true) {

				alert(`Sorry. Value '${value}' already exists! \nIn record section_id: ${record.section_id} \nNothing will be saved!`);
			}
