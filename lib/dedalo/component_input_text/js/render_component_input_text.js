...
const content_data_edit = async function(self) {
	...
	// tools
		...
		for (let i = 0; i < tools_length; i++) {
			add_tools(tools[i], buttons_container)
		}
...
* @return dom element li
*/
const add_tools = (current_tool, buttons_container) => {

	// button tool 
		const tool_button = ui.create_dom_element({
			element_type	: 'span',
			class_name 		: 'button tool',
			style 			: {"background-image": "url('" +current_tool.icon +"')" },
			parent 			: buttons_container,
			dataset			: { tool :current_tool.name }
		})
