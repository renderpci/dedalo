...
var login = new function() {
	...
	*/
	this.Login = function () {
		...
		const username				= input_username.value //	login.wrap_login.find('#login_username').val();
		const password				= input_password.value // login.wrap_login.find('#login_password').val();			
		const tipo_login			= wrap_login.querySelector("#tipo_login").value // login.wrap_login.find('#tipo_login').val();
		const tipo_username			= wrap_login.querySelector("#tipo_username").value // login.wrap_login.find('#tipo_username').val();
		const tipo_password			= wrap_login.querySelector("#tipo_password").value // login.wrap_login.find('#tipo_password').val();
		const tipo_active_account	= wrap_login.querySelector("#tipo_active_account").value // login.wrap_login.find('#tipo_active_account').val();
		...
		const js_promise = common.get_json_data(this.url_trigger, trigger_vars).then(function(response) {
				...
				if (response===null) {
					...
					let msg  = document.createTextNode(server_null_msg)
					...
					if (response.result===true) {						
						...
						let msg  = document.createTextNode(response.msg)
						let span = document.createElement("span")
				...
				// Show trigger error messag
					let span = document.createElement("span")
						span.appendChild( document.createTextNode("Error on Login (trigger request error)" + error) )
						span.classList.add("error")
