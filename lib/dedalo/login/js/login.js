/*global html_page, DEDALO_LIB_BASE_URL, page_globals, common, SHOW_DEBUG, clear_some_local_storage, set_localStorage, component_password */
			...
			if (response===null) {
				...
				if (response.result.login===true) {
					...
					setTimeout(function() {
						//span.innerHTML = "Going to main page.."
						// Reload without force update browser cache
						if(response.result.default_section){
							window.location.href = '?t=' + response.result.default_section
						}else{
							window.location.reload(false)
						}
					},100)

					// event to parent window (on iframe cases)
						if (window.parent) {
