...
var component_select = new function() {
	...
	* @return 
	*/
	this.exec_component_triggers = function(wrap_div, propiedades, dato) {

		let controller_data = null
		if(propiedades.controller === true){
			const controller = wrap_div.querySelector(".components_controller")
		...
		if(SHOW_DEBUG===true) {
			console.log("[component_select:Save] triggered_items:",triggered_items)
		}
	...
	*/
	this.open_section = function(button_obj) {
		...
		let section_tipo = wrap_div.dataset.referenced_section_tipo
		...
		wrap_div = find_ancestor(button_obj, 'wrap_component')
			if (wrap_div === null ) {
				if(SHOW_DEBUG===true) {
