...
var button_new = new function() {	
	...
	this.New = function (button_obj) {
		...
		// Section tipo
		let section_tipo = button_obj.dataset.tipo
		...
		if(SHOW_DEBUG===true) {
			//console.log("[button_new.New] trigger_vars: " , trigger_vars)
		}		

		// SECTION_WRAP_DIV
		var section_wrap_div = document.getElementsByClassName('css_section_wrap')[0]
		...
		common.get_json_data(this.trigger_url, trigger_vars).then(function(response) {
			...
			var msg = "<span class='error'>Error on create new record<br>Nothing is created!</span>";				
