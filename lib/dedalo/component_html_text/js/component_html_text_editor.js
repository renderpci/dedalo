...
var component_html_text_editor = new function() {
	...
	*/
	this.init = function (html_text_id, modo, propiedades) {
		...
		let text_area_node = document.getElementById(html_text_id)
			if(!component_html_text) {
				if(SHOW_DEBUG) console.log("Ops.. component_html_text not found. html_text_id: "+html_text_id)
				return false;
			}

		let component_wrapper 	= component_common.get_wrapper_from_element(text_area_node)
		...
		tinymce.init({
				...
				toolbar2  			: "link image | fontsizeselect | print preview fullscreen | code | upload | button_reference",
				...
				// des
					// // IMAGES : Avoid user resize images
	 				...
	 				// valid_elements : "strong/b,em/i,div[class],span[class],img[id|src|class],br,p,apertium-notrans,reference", //"strong/b,em/i,div[class],span[class],img[id|src|class],br,p",
	 				valid_elements : '*[*]', // allow all tags including 'reference'
				...
				setup : function(ed) {
						...
						ed.addButton('button_reference', {
							...
							onclick: function(evt) {
								component_html_text.create_new_reference(ed, evt, text_area_node)
							}
						});
						...
						ed.on('focus', function(evt) {
							...
							text_area_node = document.getElementById( ed.id );
							component_wrapper	= component_common.get_wrapper_from_element(text_area_node)
						...
						ed.on('blur', function(evt) {
							...
							if(ed.isDirty()) {
								component_html_text_editor.save_command(ed, evt, text_area_node);
							}
						});// end blur event
						...
						ed.on('click', function(evt) {
							// Select current wrap
							const current_wrap = component_common.get_wrapper_from_element(text_area_node) //$(component_html_text).parents('.wrap_component').first()
							component_common.select_wrap(current_wrap);


							// REFERENCE : CLICK ON 
							if( evt.target.nodeName==='REFERENCE' ) {

								// Fix text area selection values
								component_html_text.tag_obj = evt.target								

								// SELECTED_TAG_DATA
								// Fix selected_tag_data var in class component_text_area
								component_html_text.selected_tag_data = {
									id				: evt.target.id,
									...
									component_tipo	: current_wrap.dataset.tipo,
									lang			: current_wrap.dataset.lang
								}
								...
								switch(evt.target.className) {									
									case 'reference':
										if(evt.altKey===true){
											...
											// Show reference info
											component_html_text.show_reference_info( ed, evt, text_area_node )
										}
										break;
									default:
	...
	*/
	this.save_command = function(ed, evt, obj_html_text) {
		...
		if(obj_html_text) {
			...
			const text_area_id = obj_html_text.id;
			...
			if(SHOW_DEBUG===true) {
				console.log("-> trigger Save from tinyMCE " + text_area_id);
			}
