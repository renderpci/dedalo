	...
	switch($modo) {			
		...
		case 'edit':
					...
					if (empty($current_tipo_section)) {
						$msg  = "Error Processing Request: current_tipo_section is not defined! id:$id, tipo:$tipo ". $called_class ;
					...
					$section_obj 	= new section(NULL, $current_tipo_section, 'relation');
					...
					$section_name	= RecordObj_ts::get_termino_by_tipo($current_tipo_section,DEDALO_APPLICATION_LANG);
		...
		case 'tool_time_machine'	:
					...
					if (empty($current_tipo_section)) {
						$msg = "TM Error Processing Request: current_tipo_section is not defined! id:$id, tipo:$tipo ". $called_class ;
						error_log($msg);	
						throw new Exception($msg, 1);												
					}
					# Creamos una nueva sección con el tipo de sección recibido ($current_tipo_section) y el modo actual (relation)
