...
# set vars
	$vars = array('mode','video_id','quality','source_quality','target_quality','timecode','tc_in','tc_out','watermark','tag_id','parent','id_matrix');
...
if($mode=='generate_version') {
	...
	# Ffmpeg
	$Ffmpeg			= new Ffmpeg();
	$setting_name	= $Ffmpeg->get_setting_name_from_quality($AVObj, $quality);		#dump($setting_name,'setting_name'); #die($setting_name);
...
if($mode=='delete_version') {
	...
	if(file_exists($file)) {
		...
		try{
			...
			# move / rename file
			$rename 		= rename($file, $folder_path_del . "/$reelID" . '_deleted_' . date("Y-m-d") . '.' . DEDALO_AV_EXTENSION );
