	...
	*/
	this.load_rows = function(options, button_obj) {
		...
		var $button_obj = $(button_obj),
			...
			if ($target.length!=1 ) {
				return alert("Sorry: $target dom element not found: "+ $target.text() );
			}
			if ($wrap_div.length!=1 ) {
				return alert("Sorry: $wrap_div dom element not found: "+ $wrap_div.text() );
			}
			if ( $container.length!=1 ) {
				return alert("Sorry. $container dom element not found: "+ $container.text() );
			}
