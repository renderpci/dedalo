...
var search = new function() {
	...
	*/
	this.load_rows = function(options, button_obj) {
		...
		//var target = document.querySelectorAll("[data-role='section_list_rows_content']")[0]
		...
		let trigger_vars = {
			...
			//return console.log("[search.load_rows] trigger_vars", trigger_vars);

		let url_trigger = DEDALO_LIB_BASE_URL + '/section_records/trigger.section_records.php'
		...
		var js_promise = common.get_json_data(url_trigger, trigger_vars).then(function(response){			
			if (SHOW_DEBUG===true) {
				if (response) {
					console.log("[search.load_rows] response "+response.msg +" "+ response.debug.exec_time, response);
				}
			}
			...
			if (!response) {
				...
				let content_obj = el.querySelector("div[data-rol='section_records']")
	...
	*/
	this.reload_rows_list = function (call_uid) {
		...
		let caller_obj = document.getElementById(call_uid)
			if (!caller_obj) {
		...
		let button_obj 			= caller_obj.querySelector('.css_wrap_rows_paginator')
		let options 	 		= button_obj.dataset.options
