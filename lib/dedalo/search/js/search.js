...
var search = new function() {
	...
	*/
	this.toggle_search_operators = function(){
		...
		var i=0;
		var len = operator_elements.length;
		//	console.log(len);
	...
	*/
	...
	/**
	* LOAD_ROWS_JSON : Ajax loads records
	* @return promise js_promise
	*//*
	this.load_rows_json = function(options, button_obj) {
		...
		if ( typeof target==="undefined" || target===null ) {			
			console.warn("load_rows_json: Error. Sorry: target dom element not found. ("+page_globals.modo+")")
			console.log("button_obj",button_obj);
			return false;
		}		
		...
		const wrap_div = target.querySelector('.css_section_list_wrap')
			if (wrap_div === null ) {
				return alert("load_rows_json: Sorry: wrap_div dom element not found");
			}

		// Options fallback to default
		//if (typeof options==="undefined" || options===null) {
		//	var options = $(button_obj).parents('.css_section_list_wrap').first().data('options');
		//	console.log("Using default options on load_rows_json");
		//}
		...
		const trigger_vars = {
			...
			//console.log("[search.load_rows_json] trigger_vars", trigger_vars)

		// PROMISE JSON XMLHttpRequest
		const js_promise = common.get_json_data(url_trigger, trigger_vars).then(function(response){
			if (SHOW_DEBUG===true) {
				console.log("[search.load_rows_json] response:",response);
				if (response) {
					console.log("[search.load_rows_json] response "+response.msg +" "+ response.debug.exec_time);
				}
			}

			if (!response) {
				// Notify to log messages in top of page
				console.error("[search.load_rows_json] Error. response is null", response);
				...
				if(options.modo==="edit") {
					content_obj = el.querySelector("div[data-rol='section_records']")
					if(typeof content_obj === 'undefined') {
						return alert('[search.load_rows_json] Error on parse received data: \n' + response.result)
					}
				}				
				...
				// target_obj
					const target_obj = target.querySelector("div[data-rol='section_records']")
					if(typeof target_obj === 'undefined') {
						return alert('[search.load_rows_json] Error on place received data. Target DOM obj (data-rol="section_records") not found')
					}
				...
				// Launch event load_rows_json is done
					const event_detail 	  = {}
					const load_rows_json_event = new CustomEvent('load_rows_json', {detail:event_detail})
					// launch custom event				
					window.dispatchEvent(load_rows_json_event)
			...
			// Debug
				if (SHOW_DEBUG===true) {
					//console.log( "[component_common.load_component_by_wrapper_id]-> loaded wrapper: " + wrapper_id + " tipo:"+tipo+ " modo:"+modo )				
					var end = new Date().getTime(); var time = end - start
					//console.log("[search.load_rows_json] -> execution time: " +time+' ms' )
					if (response && response.debug) {
						response.debug.ajax_time = time+' ms'
					}				
				}
			...
			// log
			console.log("[search.load_rows_json] Error.", error)
			html_page.loading_content( wrap_div, 0 )
		})
