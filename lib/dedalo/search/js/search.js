...
export const search = function() {
	...
	// render
	this.render = render_search.prototype.render
		...
		// events subscription
			...
			self.events_tokens.push(
				event_manager.subscribe('render_'+self.instance_caller.id , async (instance_wrapper) => {
					...
					// fix
					self.parent_node		= wrapper.querySelector('.filter')								
					// render
					const current_filter 	= await self.render()
					// dom add
					self.parent_node.appendChild(current_filter)
				})
			)//end events push
	...
	this.build = async function(){
		...
		// load data
			...
			const api_response 			= await current_data_manager.request({
				...
				body : {
					action 	 	 	: "filter_get_components",
					ar_section_tipo : [self.section_tipo]
				}				
			})
	...
	*/
	this.load_components_from_section = async function(options) {
		...
		// Wrap div			
			const wrap_div 		= options.target_div			
		...
		// load data
			...
			// Dom create html
				self.build_components_list(api_response.result, wrap_div, options.path)
		...
		for (let i = 0; i < len; i++) {
			const component = ar_components[i]
				...
				if(component.section_tipo!==last_section_tipo){
					// section title bar
					let section_bar = ui.create_dom_element({
						element_type : 'div',
						parent 		 : top_parent,
	...
	* @return promise
	*/
	this.get_component_presets = async function(options) {
	...
	* @return bool
	*/
	this.build_component_presets_list = async function(ar_elements, permissions, target_section_tipo) {
