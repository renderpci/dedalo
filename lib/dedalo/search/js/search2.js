				...
				switch(self.modo) {
					...
					case "json":
						// Exec database search an get the result rows 
							...
							// get_json_data from trigger request
								const current_js_promise = common.get_json_data(url_trigger, trigger_vars).then(function(response){
									if (SHOW_DEBUG===true) {
										//console.log("+++++ [search2.search_rows] response:",response);
										console.log("+++++ [search2.search_rows] CONTEXT:",response.result.context);
										console.log("+++++ [search2.search_rows] DATA:",response.result.data);
										if (response) {
											console.log("[search2.search_rows] response msg "+response.msg +" "+ response.debug.exec_time);
										}
									}
									...
									if(self.search_callback !== null){
											console.log("self.search_callback:",self.search_callback);
