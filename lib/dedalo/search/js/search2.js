...
var search2 = new function() {
	...
	*/
	this.hide_search_panel = function() {
		...
		let global_container = document.getElementById("search2_global_container")
	...
	*/
	this.init = function(options) {
		...
		let section_tipo 	 	= options.section_tipo
		let global_container 	= document.getElementById("search2_global_container")
		let search_wrap	 	 	= document.getElementById("search2_container_selector")
		let container_selection = document.getElementById("search2_container_selection")
		let container_presets   = document.getElementById("search2_container_selection_presets")
		...
		let component_presets_promise = self.get_component_presets({target_section_tipo : section_tipo}).then(function(){
				// Show hidden div container		
				global_container.style.display = "flex"
			})
		...
		if (temp_filter_string.length>10) {
			...
			if(SHOW_DEBUG===true) {
				console.log("Loaded from previous filter_obj:",filter_obj)
			}			
		}else{
			...
			component_presets_promise.then(function(){
				...
				let loaded_default_preset = self.load_default_preset()
				if(SHOW_DEBUG===true) {
					if (loaded_default_preset===true) {
						console.log("Loaded default preset. ",loaded_default_preset);
					}else{
						console.log("Failed load default preset. No default preset found. ",loaded_default_preset);
					}					
				}			
				...
				if (loaded_default_preset===false) {
					...
					//if (current_cookie_track===false) {
						...
						if(SHOW_DEBUG===true) {
							console.log("Created fallback root search_group (no previous options or presets found)");
						}	
					//}
				}
			})//end promise then					
		...
		let cookie_name_panel 	= "search_presets_open_panel"
		let cookie_value_panel 	= readCookie(cookie_name_panel) || "false"
		if (cookie_value_panel=="true") {
			container_presets.style.display = "block"
		}

		// Read cookie to auto open fields_open_panel
		let cookie_name_fields 	= "fields_open_panel"
		let cookie_value_fields = readCookie(cookie_name_fields) || "false"
	...
	*/
	this.update_state = function(options) {
		...
		let editing_section_id = search2_container_selection_presets.dataset.section_id
		//let editing_section_id = self.loaded_preset ? self.loaded_preset.section_id : null
	...
	*/
	this.load_components_from_section = function(options) {
		...
		const trigger_vars = {
			...
			//console.log("[search2.load_components_from_section] trigger_vars", trigger_vars);
	...
	*/
	this.build_components_list = function(ar_components, target_div, path) {
		...
		const section_tipo  = ar_components[0].section_tipo
		const section_label = ar_components[0].section_label			

		// Div container
		let top_parent = common.create_dom_element({
			element_type	: 'div',
		...
		let target_container = common.create_dom_element({
			element_type	: 'div',
	...
	*/
	this.get_component_presets = function(options) {
		...
		let wrap_div = document.getElementById("component_presets_list") 
	...
	*/
	this.build_component_presets_list = function(ar_elements, wrap_div, permissions, target_section_tipo) {
		...
		let cookie_name 				= "search_presets"
		...
		for (let i = 0; i < len; i++) {
			...
			// Builds li element
				let li_element = common.create_dom_element({
					element_type 	: 'li',
					parent 		 	: wrap_div,
				...
				// Button load preset (<)
				let icon_load = common.create_dom_element({
					element_type 			: 'span',
					parent 		 			: li_element,
				...
				// Span label name
				let span_name = common.create_dom_element({
					element_type 			: 'span',
					parent 		 			: li_element,
				...
				if (permissions>=2) {
				let icon_delete = common.create_dom_element({
					element_type 			: 'span',
					parent 		 			: li_element,
				...
				// DIV edit
				let div_edit = common.create_dom_element({
					element_type 			: 'div',
					parent 		 			: li_element,
					class_name 	 			: "div_edit"
				})
	...
	*/
	this.load_search_preset = function(button_obj) {
		...
		let search2_container_selection_presets = document.getElementById("search2_container_selection_presets")
		let section_tipo 						= search2_container_selection_presets.dataset.section_tipo
	...
	*/
	this.build_search_group = function(parent_div, options) {
		...
		// search_group
		let search_group = common.create_dom_element({
			element_type 			: 'div',
			parent 		 			: parent_div,
		...
		// Add operator
		let search_group_operator = common.create_dom_element({
			element_type 			: 'div',
			parent 		 			: search_group,
		...
		let search_group_button_close = common.create_dom_element({
			element_type 			: 'span',
			parent 		 			: search_group,
			//text_content 			: "X",
		...
		// Add button + group
		let search_group_button_plus = common.create_dom_element({
			element_type 			: 'span',
			parent 		 			: search_group,
			//text_content 			: "X",
		...
		if (options.is_root===true) {
			...
			let search_group_container = document.getElementById("search_group_container")
				...
				// max group
				let max_group = common.create_dom_element({
					element_type 			: "div",
					parent 		 			: search_group_container,					
					class_name 	 			: "max_group"
				})
				// max label
				let max_input_label = common.create_dom_element({
					element_type 			: "span",
					parent 		 			: max_group,
				...
				// max input
				let max_input = common.create_dom_element({
					element_type 			: "input",
					parent 		 			: max_group,
				...
				// Reset button
				let reset_button = common.create_dom_element({
					element_type 			: "button",
					parent 		 			: search_group_container,
				...
				// Submit button
				let submit_button = common.create_dom_element({
					id 						: "button_submit",
					element_type 			: "button",
					parent 		 			: search_group_container,
	...
	* @return dom object
	*/
	this.build_search_component = function(parent_div, path_plain, current_value) {
		
		let self = this

		let path 		= JSON.parse(path_plain)
		let last_item 	= path[path.length-1]

		// Create dom element before load html from trigger
		// search_component
		let search_component = common.create_dom_element({
			element_type 			: 'div',
			parent 		 			: parent_div,
		...
		let js_promise	= self.get_component_html({
				...
				// Add button close
				let search_component_button_close = common.create_dom_element({
					element_type 			: 'span',
					parent 		 			: search_component,
					//text_content 			: "X",
					class_name 	 			: "button_close"
	...
	*/
	this.calculate_search_group_counter = function() {
		...
		let all_search_groups = document.getElementById("search_group_container").querySelectorAll(".search_group")
		let total  			  = all_search_groups.length 		
		let counter 		  = total + 1
	...
	*/
	this.recursive_groups = function(group_dom_obj, add_arguments, mode) {
		...
		let operator = self.get_search_group_operator(group_dom_obj)
		...
		let ar_elements = group_dom_obj.children
	...
	*/
	this.get_search_group_operator = function(search_group) {
		...
		let childrens = search_group.children
	...
	*/
	this.parse_json_query_obj_to_dom = function(button_obj, json_object, options) {
		...
		let clean_q 		 = (options && options.clean_q) ? options.clean_q : false
		let allow_duplicates = (options && options.allow_duplicates) ? options.allow_duplicates : false
		
		// Reset resolved
		this.ar_resolved_elements = []
	
		let global_group = self.build_dom_group(json_object, search_group_container, {
			is_root 		 : true,
			clean_q 		 : clean_q,
		...
		for (let key in json_object) {
			...
			if (key==='path') {
				...
				if (self.ar_resolved_elements.indexOf(resolved_string)===-1) {
					...
					self.build_search_component( dom_element, JSON.stringify(json_object.path), current_value )
	...
	*/
	this.new_preset = function(button_obj) {
		...
		let new_preset_div = document.getElementById("new_preset_div")
		...
		let search2_container_selection_presets = document.getElementById("search2_container_selection_presets")
		let all_div_edit = search2_container_selection_presets.querySelectorAll('.div_edit')
		...
		let js_promise = common.get_json_data(self.url_trigger, trigger_vars).then(function(response){
			...
			if (response) {
				...
				let button_label = (get_label["crear"] ? get_label["crear"] : "Create") + " " + (get_label["nuevo"] ? get_label["nuevo"] : "new")

				let button_new = common.create_dom_element({
					element_type 		: "button",
	...
	*/
	this.save_new_preset = function(button_obj) {
		...
		let input_preset_name = button_obj.parentNode.querySelector('input[type="text"]')		
	...
	*/
	this.save_preset = function(button_obj) {
		...
		let wrap_div = search2_container_selection_presets // document.getElementById("component_presets_list") 
	...
	*/
	this.edit_preset = function(button) {
		...
		let div_edit 	= li.querySelector('.div_edit')
		let json_preset = {}
		...
		let js_promise = common.get_json_data(self.url_trigger, trigger_vars).then(function(response){
			...
			if (response) {
				...
				let wrapper_dd648  = div_edit.querySelector("div.wrap_component_dd648")
				let radio_buttons  = wrapper_dd648.querySelectorAll(".css_radio_button")
	...
	*/
	this.toggle_fields = function(button) {
		...
		let search2_container_selector = document.getElementById("search2_container_selector")

		// Read cookie to track state
		let cookie_name = "fields_open_panel"
	...
	*/
	this.toggle_presets = function(button, mode) {
		...
		// Read cookie to track state
		let cookie_name = "search_presets_open_panel"		
	...
	*/
	this.clean_all_inputs = function() {
		...
		let search_query_object = self.parse_dom_to_json_filter({})
	...
	*/
	this.search = function(button_obj) {
		...
		const search_json_object = self.get_search_json_object()
			console.log(JSON.stringify(search_json_object));
	...
	*/
	this.cookie_track = function(cookie_name) {
		...
		if (search2_container_selection_presets) {
			...
			let cookie_track = cookie_value[section_tipo] || false
