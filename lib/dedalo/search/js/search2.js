...
var search2 = new function() {
			...
			const js_promise = self.parse_html(wrapper_node).then(async function(response){
				...
				*/
				...
				// Is NOT mandatory init search with options.temp_filter (!)
					if (typeof options.temp_filter==="undefined" || options.temp_filter===null) {

						// Try to get from db/session
							self.load_temp_filter(section_tipo).then(function(temp_filter){
								// Parse filter in DOM
									self.parse_json_query_obj_to_dom(null, temp_filter, {allow_duplicates:true})
							})
					}else{

						if (options.temp_filter.length>10) {

							// Read received tem_filter and decode / parse
								const temp_filter_string = decodeURIComponent(options.temp_filter);
								const temp_filter 		 = JSON.parse(temp_filter_string)
							...
							// debug
								if(SHOW_DEBUG===true) {
									console.log("[search2.init] Loaded from previous temp_filter:",temp_filter)
								}
						}else{
						...
						}//end if (options.temp_filter.length>10)

					}//end if (typeof options.temp_filter==="undefined" || options.temp_filter===null)
			...
			if (main_object.modo==='thesaurus'){
				...
				const thesaurus_search_selector = common.create_dom_element({
					...
					const thesaurus_typology_selector = common.create_dom_element({
						...
						//add the Even onchage to the select, when it change the section selected will be loaded
	...
	*/
	...
	/**
	* NEW_PRESET
	* Creates a temp presets section to collect fields data and save a new preset
	* @return promise
	*/
	this.new_preset = function(button_obj) {
		...
		const trigger_vars = {
				...
				components 	: [
				{
					...
					clean 			: true // clean possible dato in temp sections
				},
				{
