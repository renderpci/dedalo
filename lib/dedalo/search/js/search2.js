/* global get_label, readCookie, SHOW_DEBUG, DEDALO_LIB_BASE_URL, common, html_page */
...
var search2 = new function() {
	...
	*/
		...
		// layout_map of the select for processing the columns of the result of the search
		this.ar_list_map
		// get the components in the result_query_object with a concrete mode, by default parse_mode = list
		this.parse_mode
	...
	* 		position of the script / node into the dom
	*
	* @return promise
	*/
	this.init = function(options, current_node) {
		...
		// options
			...
			const ar_real_section_tipo	= options.ar_real_section_tipo
			const search_callback		= options.search_callback || null
			const ar_list_map			= options.ar_list_map || null
			const parse_mode			= options.parse_mode || 'list'
			const modo					= options.modo || 'list'
			const temp_filter			= options.temp_filter || '{"$and":[]}'


		// debug
			if(SHOW_DEBUG===true) {
				//console.log("!search2 initied with options:" ,options)
				//console.log("temp_filter:", JSON.parse(decodeURIComponent(options.temp_filter)));
			}

		// set main self vars
			self.search_callback	= search_callback
			self.ar_list_map		= ar_list_map
			self.parse_mode			= parse_mode
			self.modo				= modo
			self.main_object		= options // full options object set
		...
		// parse_html
			const js_promise = self.parse_html(wrapper_node).then(async function(response){
				...
				// LEFT SIDE. Get initial list of components and load into search_wrap
					await self.load_components_from_section({
						target_div				: search_wrap,
						section_tipo			: section_tipo,
						ar_real_section_tipo	: ar_real_section_tipo,
						path					: []
					})
					//.then(function(){
				...
				// RIGHT SIDE. Load user presets list
					const component_presets_promise = self.get_component_presets({
					...
					// search_panel cookie state track
						if(search2.cookie_track("search_panel")===true) {
							// Open search panel
							//global_container.style.display = "flex"
							self.toggle_search_panel() // toggle to open from default state close
						}

				// Set initial state as unchanged
					self.update_state({state:'unchanged'})

				// temp_filter_string. Check if exists previous user search options stored
				// Is NOT mandatory init search with options.temp_filter (!)
					if (!temp_filter) {

						// Try to get from db/session
							self.load_temp_filter(section_tipo)
							.then(function(new_temp_filter){
								// Parse filter in DOM
								self.parse_json_query_obj_to_dom(null, new_temp_filter, {allow_duplicates:true})
							})
					}else{

						if (temp_filter.length>10) {

							// Read received tem_filter and decode / parse
								const temp_filter_string	= decodeURIComponent(temp_filter);
								const current_temp_filter	= JSON.parse(temp_filter_string)

							// Parse filter in DOM
								self.parse_json_query_obj_to_dom(null, current_temp_filter, {allow_duplicates:true})

							// debug
								if(SHOW_DEBUG===true) {
									console.log("[search2.init] Loaded from previous current_temp_filter:", current_temp_filter)
								}
						}else{
							...
							})//end component_presets_promise then
						}//end if (temp_filter.length>10)

					}//end if (typeof temp_filter==="undefined" || temp_filter===null)
	...
	/**
	* PARSE_HTML
	* Process the JSON received to create DOM nodes
	* @param DOM node wrapper_node
	* @return promise
	* 	Resolve bool
	*/
	this.parse_html = function(wrapper_node){
		...
		return new Promise(function(resolve) {
			...
			//create the button of the filter
				...
				//add the Even click to the select, when it change the section selected will be loaded
			...
			// build one select and one checkbox
			...
			// create the global_container
				...
				// search2_container_selector. create the search2_container_selector
	...
	* Create the left list of components to show in components selector
	* User can drag and drop components from this list to search canvas (at center of the page)
	* @param object options
	* 	string section_tipo
	* 		sample: 'hierarchy20'
	*	array ar_real_section_tipo
	* 		sample: ['hierarchy20', 'rsc75']
	*	DOM node target_div
	*	array path
	* @return promise
	*/
	this.load_components_from_section = function(options) {
		...
		// options
			...
			const target_div			= options.target_div || document.getElementById('search2_container_selector')


		// current_section_tipo. Use ar_real_section_tipo with section_tipo fallback
			const current_section_tipo = ar_real_section_tipo || section_tipo
		...
		return new Promise(function(resolve, reject){
			...
			.then(function(response){
				...
				if (!response) {
					...
					// Dom create html
					self.build_components_list(response.result, target_div, options.path)
				}//end if (!response)
				...
				console.error("[search2.load_components_from_section] Error.", error);
				// Spinner loading off
				html_page.loading_content( target_div, 0 );
				reject(false)
			})
	...
	*/
	this.build_components_list = function(ar_components, target_div, path) {
		...
		for (let i = 0; i < len; i++) {
			...
			// children recursive info
				if (component.modelo_name==="component_relation_children") {
					self.is_thesaurus = true
				}
			...
			// SECTION GROUP (build only on changes)
				if (component.section_group_tipo!==last_section_group_tipo && component.section_group_model!=='section_group_div') {
	...
	this.build_component_presets_list = function(ar_elements, wrap_div, permissions, target_section_tipo) {
		...
		for (let i = 0; i < len; i++) {
			...
			// Builds li element
				...
				// Span label name
				const preset_label = common.create_dom_element({
					element_type 			: 'span',
					text_content 			: element.name,
					class_name 	 			: "css_span_dato preset_label",
					data_set 				: {
				...
				if (permissions>=2) {
					preset_label.addEventListener("click",function(e){
						self.edit_preset(this)
					},false)
				}
	...
	*/
	...
	/**
	* LOAD_SEARCH_PRESET
	* On click arrow button in search presets list, load jquery preset from db and apply to current canvas
	* @return true
	*/
	...
	this.build_search_group = function(parent_div, options) {
		...
		// Add operator
			const search_group_operator = common.create_dom_element({
				...
				class_name 	 			: "operator search_group_operator" + (options.operator ? " "+options.operator.substring(1) : " and")
			})
		...
		if (options.is_root===true) {
			...
			// Add Send and reset buttons
				...
				const is_thesaurus = (typeof self.is_thesaurus!=="undefined" && self.is_thesaurus===true)
				...
				// max input
					const max_default = (is_thesaurus===true) ? 100 : 10
					const class_name_add = (is_thesaurus===true) ? ' is_thesaurus' : ''
					const max_input = common.create_dom_element({
						element_type 			: "input",
						value 					: max_default, // default 10
						class_name 	 			: "max_input" + class_name_add, // btn css_max_rows
						parent 		 			: max_group
					})
				// recursive children
					if (is_thesaurus===true) {
						const recursive_group = common.create_dom_element({
							element_type 	: "div",
							class_name 	 	: "recursive_group",
							parent 		 	: max_group
						})
						const recursive_label = common.create_dom_element({
							element_type 	: "span",
							text_content 	: get_label["children_recursive"] || "Children",
							class_name 	 	: "children_recursive_label",
							parent 		 	: recursive_group
						})
						const recursive_children = common.create_dom_element({
							element_type 	: "input",
							type 			: "checkbox",
							value 			: "",
	...
	*/
	this.build_search_component = function(parent_div, path_plain, current_value, q_operator) {
		...
		.then(function(response){
			...
			// check_components_link
				const button_operator = search_component.parentNode.firstChild
				if (button_operator.classList.contains("operator")) {
					self.check_components_link(button_operator)
				}
		})
	...
	*/
	this.get_component_html = function(options) {
		...
		return new Promise(function(resolve, reject) {
			...
			common.get_json_data(self.url_trigger, trigger_vars)
			.then(function(response){

				const component_html = (response && response.result)
					? response.result
					: "Error on load component. tipo: " + options.component_tipo
					...
					const wrapper 			= div_childrens[0]
						if (wrapper && typeof(wrapper.dataset.dato)!=="undefined" ) {
							...
							try {
								...
								console.warn("error on parse wrapper.dataset.dato:",error);
								console.warn("wrapper.dataset:",wrapper.dataset);
							}
	...
	*/
	this.toggle_operator_value = function(element) {
		...
		// clean styles
		element.classList.remove("and","or","or_link")
		...
		}else if (element.dataset.value==="$or") {

			// Replace dataset value
			element.dataset.value = "$or_link";

			// Inject new html value
			element.innerHTML = self.localize_operator(element.dataset.value) + " " + number

			element.classList.add("or_link")
		...
		// check_components_link
		const check = self.check_components_link(element)


		// reset wrappers styles
			if (check===false) {
				const parent_container = element.parentNode
				const wrappers = parent_container.querySelectorAll("div.wrap_component")
				for (var i = wrappers.length - 1; i >= 0; i--) {
					wrappers[i].classList.remove("duplicate_tipo")
				}
			}
	...
	/**
	* CHECK_COMPONENTS_LINK
	* check all search components have same tipo
	*/
	this.check_components_link = (element)=>{

		let errors = false


		if(element.dataset.value!=='$or_link') {
			return errors
		}

		const get_siblings = function (elem, class_name) {
			// Setup siblings array and get the first sibling
			const siblings = [];
			let sibling = elem.parentNode.firstChild;
			// Loop through each sibling and push to the array
			while (sibling) {
				if (sibling.nodeType===1 && sibling!==elem && sibling.classList.contains(class_name)) {
		...
		const siblings 	 = get_siblings(element, 'search_component')
		var current_tipo
		for (let i = 0; i < siblings.length; i++) {

			const sibling = siblings[i]
			const wrap_component = sibling.querySelector("div.wrap_component")

			if (wrap_component) {

				if (current_tipo && wrap_component.dataset.tipo!==current_tipo) {
					wrap_component.classList.add("duplicate_tipo")

					//return false
					errors = true
				}
				current_tipo = wrap_component.dataset.tipo
			}
		}

		if (errors===true) {
			alert("WARNING! \n\nOnly components with same 'tipo' are valid with link operator");
		}

		return errors
	};//end check_components_link
	...
	*/
	this.search = function(button_obj, search_json_object) {
		...
		// children_recursive checkbox
			if (button_obj) {
				const children_recursive = button_obj.parentNode.querySelector(".children_recursive")
				if (children_recursive) {
					// modify search_json_object
					if (children_recursive.checked===true) {
						search_json_object.children_recursive = true
					}else{
						search_json_object.children_recursive = false
					}
				}
			}
		...
		// promise
			...
			// debug
				// if(SHOW_DEBUG===true) {
				// 	self.static_search(search_json_object, self.ar_list_map, null)
				// }
	...
	*/
	this.localize_operator = function(operator) {
		...
		if (operator==="and") {
			...
			name = "o"
			label = get_label[name]
		}else if(operator==="or_link"){
			name = "link"
			label = get_label[name] || "link"
		}
	...
	this.load_temp_filter = function(section_tipo) {
		...
		return new Promise(function(resolve, reject){
			...
			.then(function(response){
				...
				// debug
					if (SHOW_DEBUG===true) {
						if (response) {
							...
							//console.log("[search2.load_temp_filter] response "+response.msg +" "+ response.debug.exec_time, response);
						}
					}
				...
				console.error("[search2.load_temp_filter] Error.", error);
				reject(false)
			})
		})
