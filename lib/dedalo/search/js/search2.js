...
var search2 = new function() {
	...
	*/
	this.parse_html = function(section_tipo){
		...
		const js_promise = new Promise(function(resolve, reject) {
			...
			const content_html = document.getElementById('wrap_rows_search')
	...
	this.build_search_group = function(parent_div, options) {
		...
		if (options.is_root===true) {
			...
			const search_group_container = document.getElementById("search_group_container")
				...
				submit_button.addEventListener("click",function search(e){
					self.search(this)
				},false)
		}
	...
	this.search = function(button_obj,search_json_object) {
		...
		// modo switcher determines call
			switch(self.modo) {
				...
				case "json":
					// Load rows with current seach options
					search.load_rows_json(search_options, button_obj).then(function(response){
						if(SHOW_DEBUG===true) {
							console.log("[search2.search] end load_rows_json response: ", self.modo, response);
						}
						})
					break;
