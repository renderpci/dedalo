...
* @return bool
*/
render_search.prototype.render_components_list = function(ar_elements, target_div, path) {
		...
		//let section_tipo  = ar_elements[0].section_tipo
		...
		for (let i = 0; i < len; i++) {
			...
			switch (true) {
				...
				default:
					...
					let has_subquery_event_function = [ 
						 {'type':'dragstart','name':'search.on_dragstart'}
						,{'type':'dragend','name':'search.on_drag_end'}
						,{'type':'drop','name':'search.on_drop'}
					]
					if (element.model==="component_autocomplete") {
						...
						has_subquery_event_function = null
					}
...
const build_search_group = function(self, parent_div, options) {
	...
	const event_function 	  = [{'type':'dragstart','name':'search.on_dragstart'}
								,{'type':'dragend','name':'search.on_drag_end'}
								,{'type':'drop','name':'search.on_drop'}
								,{'type':'dragenter','name':'search.on_dragenter'}
								,{'type':'dragover','name':'search.on_dragover'}
								,{'type':'dragleave','name':'search.on_dragleave'}]								  
