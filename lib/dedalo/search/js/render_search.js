...
* @return bool
*/
render_search.prototype.render_components_list = function(ar_components, target_div, path) {
		...
		// Fisrts item check
			if (typeof ar_components[0]==="undefined") {
				console.error("Error on render_components_list. Empty ar_components",ar_components);
				return false
			}
...
render_search.prototype.render_user_preset_list = async function(ar_elements, permissions, target_section_tipo) {
		...
		// first item check
			if (typeof ar_elements[0]==="undefined") {
				...
				//console.warn("[search2.render_user_preset_list] Warning. Empty ar_elements received",ar_elements);
				return false
			}
