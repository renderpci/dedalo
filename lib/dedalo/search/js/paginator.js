...
export const paginator = function() {
	...
	this.init = async function(options) {
		...
		self.sqo 				= self.sqo_context.find(element => element.typo === 'sqo')
		self.count_rows			= options.count_rows
		self.events_tokens		= []
		self.node 				= []
		self.parent_node 		= options.parent_node

		// events subscription
		// event active (when user focus in dom)
		self.events_tokens.push(
			event_manager.subscribe('section_rendered'+self.instance_caller.id , async (active_section) => {			
				
				const ar_node 			= self.node
				self.node 				= []
				const current_paginator = await self.render()
				const parent_node 		= await self.parent_node
				//const parent_node 		= active_section.node[0]
				
				if (typeof ar_node !== 'undefined' && ar_node.length > 0){
					...
					for (let i = 0; i < ar_node.length; i++) {
						...
						// remove the all child nodes of the section
							while (current_node.firstChild) {
								current_node.removeChild(current_node.firstChild)
							}
						// replace the node with the new render
							parent_node.replaceChild(current_paginator, current_node)
		 			}
		 			self.node.push(current_paginator)

				}else{
					parent_node.insertBefore(current_paginator, parent_node.firstChild);
					self.node.push(current_paginator)
				}
	...
	*/
	this.render = async function() {
		...
		const offset 			= self.sqo.offset || 0
		const limit 			= self.sqo.limit  || 10
		const count 			= await self.count_rows
		...
		//self.node.push(wrap_rows_paginator)
	};//end render
	...
	*/
	this.search_paginated = async function(offset) {
		...
		// Get current search_query_object
			const search_query_object = self.sqo 
		// Update value offset
			search_query_object.offset = offset
		// Save search_query_object
			const instance_caller = self.instance_caller
			//console.log("self.instance_caller:",instance_caller);
			instance_caller.render_paginator()
