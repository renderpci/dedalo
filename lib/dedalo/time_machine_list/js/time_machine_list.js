    ...
    */
    this.build_grid_html = function(context, columns, data, count_data, CSS_style_sheet){
      ...
      const header  = common.create_dom_element({
                      element_type      : 'div',
                      parent            : grid,
                      class_name        : 'time_machine_list_header',
      ...
      const header_count  = common.create_dom_element({
                      element_type      : 'span',
                      parent            : header,
                      class_name        : 'time_machine_list_header time_machine_list_count',
      ...
      const data_header  = common.create_dom_element({
                element_type      : 'ul',
                parent            : grid,
                class_name        : css_selector + ' time_machine_list_data_header'
                })
      ...
      columns.forEach(function(column){
        ...
        if(column.component_label === 'id'){
           class_name = 'time_machine_list_data_row_center'
        }
     ...
     /* 3 Create the rows with the data */    
      ...
      data.forEach(function(current_data){
        ...
        if(curent_section_id !== current_data.section_id){
          ...
          const event_function    = [{'type':'click','name':'time_machine_list.edit_relation'}];
              data_row_header  = common.create_dom_element({
                  element_type            : 'ul',
                  parent                  : grid,
                  class_name              : css_selector + ' time_machine_list_data_row',
    ...
    */
    this.edit_relation = function(object){
      ...
      if (typeof section_id=="undefined") {
       return console.error("[time_machine_list.edit] Error on find section_id", object);
      }

      if (typeof section_tipo=="undefined") {
        return console.error("[time_machine_list.edit] Error on find section_tipo", object);
      }
