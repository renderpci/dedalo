		...
		if( intval($current_value)===0 ) {
			...
			// update already existing counter data
				...
				if ($result===false) {
					...
					debug_log(__METHOD__." Updated counter with value: dato:$counter_dato_updated, tipo:$tipo (".str_replace(array('$1','$2'), array($counter_dato_updated,$tipo), $strQuery).')', logger::DEBUG);
				}
		}
		...
		// TEST IF COUNTER EXISTS BEFORE SET
			...
			if ($counter_dato===false) {
				...
				// counter already exists. Update value
					...
					if($result===false) throw new Exception("Error Processing Request. DB error on update counter value", 1);
					if(SHOW_DEBUG===true) {
