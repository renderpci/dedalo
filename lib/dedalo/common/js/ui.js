...
export const ui = {
	...
	*/
	place_element : function(options) {
		...
		if (!target_instance) {
			console.log("Error on get target instance:", options);
			return false
		}
		...
		if (target_instance.status==="rendered") {
			...
			for (let i = 0; i < target_instance.node.length; i++) {

				const target_node = target_instance.node[i].querySelector(target_selector)
				if (target_node) {
			...
			// target_instance node not exists case
			//section.events_tokens.push(
