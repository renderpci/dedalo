...
var get_instance = function(object_data) {
	...
	if(current_instance == false){
		//current_instance = Object.create(name,{ id: { value: id_wrapper } })
			var current_instance = Object.create(window[name],{ id: { value: id_wrapper } })
...
var common = new function() {
	...
	*/
	this.jump_select_lang = function(select_obj) {
		...
		//return console.log("request_data",request_data,this.trigger_url);
	...
	*/
	this.stop_all_videos = function() {
		...
		let ar_html5_videos = document.getElementsByTagName("video")
		if (ar_html5_videos) {
	...
	* @return Promise
	*/
	this.get_json_data = function(trigger_url, trigger_vars, async) {
		
		let	url = trigger_url;	//?mode=get_childrens_data';
	...
	*/
	this.create_dom_element = function(element_options){
		...
		let element_type			= element_options.element_type
		let parent					= element_options.parent
		let class_name				= element_options.class_name
		let style					= element_options.style
		...
		let custom_function_events	= element_options.custom_function_events
		let title_label				= element_options.title_label
		let text_node				= element_options.text_node
		let text_content			= element_options.text_content
		let inner_html				= element_options.inner_html
		let id 						= element_options.id
		let draggable				= element_options.draggable
		let value					= element_options.value
	...
	*/
	this.export_str = function(button_obj, wrap_obj) {
		...
		if (!wrap_obj) {
			var wrap_obj = document.getElementById('log_messages')
	...
	/**
	* IMPORT_STR
	*//* NO USAR . USAR LA DE TOOL_ADMINISTRATION
	this.import_str = function(button_obj, wrap_obj) {
		...
		var trigger = DEDALO_LIB_BASE_URL + "/backup/trigger.db_utils.php"
		var trigger_vars = {
				mode : 'import_str'
		}
		...
		// HTTPX Request
		var js_promise = common.get_json_data(trigger, trigger_vars).then(function(response) {
			if(SHOW_DEBUG===true) {
				console.log("[common.import_str] response: "+trigger_vars.mode, response);
			}
			...
			if (response && response.result) {
				if (wrap_obj) {
					...
					console.log("[common.import_str] ", response.msg);
					alert(response.msg)
				}
			}else{
				console.log("[common.import_str] Error. null response");
			}
	...
	*/
	this.load_script = function(url, callback) {
		...
		if(SHOW_DEBUG===true) {
			//console.log("[common.load_script] is_loaded:", url, is_loaded);
		}		
		if (true===is_loaded) {
			return false;
		}		
		...
		if(SHOW_DEBUG===true) {
			//console.log("[common.load_script] loaded url: ", url)
		}
	...
	* @return bool true
	*/
	this.load_style = function(url, callback) {
		...
		if(SHOW_DEBUG===true) {
			//console.log("[common.load_style] loaded url: ", url)
		}
