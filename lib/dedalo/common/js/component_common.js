	...
	// queue overlaped calls to avoid server concurrence issues
		if(self.status==='changing') {
			...
			return new Promise(function(resolve) {
				//setTimeout(function() {
					resolve( change_value_queue(self, options) );
				//}, 150);
			})
		}
...
const change_value_queue = function(self, options) {
	...
	const fun = wrap_function(self.change_value, self, [options]);

	self.change_value_pool.push( fun )


	return fun
}//end change_value_queue
