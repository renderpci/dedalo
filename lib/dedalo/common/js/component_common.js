...
*/
component_common.prototype.save = async function(current_changed_data) {

	const self = this

	const changed_data = JSON.parse(JSON.stringify(current_changed_data))
...
component_common.prototype.update_datum = async function(api_response) {
	...
	// remove the component old data in general datum (from down to top array items)
		...
		if(SHOW_DEBUG===true) {
			...
			//console.log(" [component_common.update_datum] api_response.result.data:", api_response.result.data);
		}
...
component_common.prototype.change_value = async function(options) {
	...
	if (self.status === 'changing') {
		console.log("Stop execution: changing");
		return false
	}
	...
	*/
	...
	const value 		= options.value // set null for remove all values
	...
	// update the data in the instance previous to save
		...
		//self.data.changed_data = JSON.parse(JSON.stringify(changed_data))
