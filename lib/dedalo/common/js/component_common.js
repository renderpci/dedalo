...
component_common.prototype.update_datum = async function(api_response) {
		...
		console.log("+++[update_datum] api_response:",api_response);
...
* @return promise
*/
component_common.prototype.deep_render = async function(options={}) {

	const self = this

	self.status = 'rendering'

	const records_mode 	= options.records_mode || "list"
		...
		for (let i = 0; i < value_length; i++) {
			...
			// section_record instance
				const current_section_record = await instances.get_instance({
					...
					data			: current_data,
					datum 			: self.datum,
					paginated_key 	: locator.paginated_key || null // used by autocomplete / portal
				})
	...
	// render using external proptotype
