...
component_common.prototype.init = async function(options) {
	...
	// events subscription
		...
		// event save (when user change component value) every component is looking if the own the instance was changed. 
		event_manager.subscribe('component_save_'+self.id, (saved_component) => {
			// call component
...
component_common.prototype.save = async function(saved_component) {
	...
	// update the data of the instance with the data changed
...
component_common.prototype.update_data_value = function(){
	...
	const self = this
	const data_key 		= self.changed_data.key
	const changed_data 	= self.changed_data.value

	if (changed_data !== null) {
		self.data.value[data_key] = changed_data
	}else{
		self.data.value.splice(0,data_key)
	...
	// render using external proptotypes of 'render_component_input_text'
		...
		switch (mode){
			case 'list':
				...
				// set
				const assigned_list = (assign===true) ? self.node.push(list_node) : false
				node = list_node
				break
			...
			default :				
				...
				const assigned_edit = (assign===true) ? self.node.push(edit_node) : false
				node = edit_node
