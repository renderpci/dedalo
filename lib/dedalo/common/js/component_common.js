...
component_common.prototype.update_data_value = function(){
	...
	const changed_data 	= self.data.changed_data.value
...
component_common.prototype.rebuild_nodes = async function() {
	...
	for (var i = 0; i < ar_nodes_length; i++) {
	// render
		const new_node = await self.render(false)

		const parent_node = ar_nodes[i].parentNode
		parent_node.replaceChild(new_node, ar_nodes[i]);
