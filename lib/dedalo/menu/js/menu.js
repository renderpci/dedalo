...
var menu = new function() {
	...
	window.addEventListener("load", function (event) {
		...
		if (general_menu) {
			...
			self.mainmenus 			= general_menu.getElementsByTagName('li');
			const mainmenus_length  = self.mainmenus.length
			for (let i = mainmenus_length - 1; i >= 0; i--) {

			 	const li_item = self.mainmenus[i]
			...
			if (menu_wrapper) {
				...
				// do global click action of the menu items
	...
	*/
	this.close_all_drop_menu = function() {
		...
		if (typeof self.mainmenus!=="undefined") {
			
			const len = self.mainmenus.length
			for (let i = len - 1; i >= 0; i--) {

				const li = self.mainmenus[i]
