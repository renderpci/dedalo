	...
	*/
	...
	this.init = function(options) {
		...
		if (typeof this.inited[init_uid]==="undefined" || this.inited[init_uid]!==true) {
			...
			if(custom){
				const url = DEDALO_LIB_BASE_URL +"/extras"+custom.file
				let ar_paths = custom.process.split('.')
				common.load_script(url).then(function(){})
			...
			// Save component event. Triggerred on save component with tipo inckuded in formula
				window.addEventListener('component_save', function(event){
					...
					if(SHOW_DEBUG===true) {
						console.log("component_calculation init event component_save data tipo: ", saved_tipo, data);
					}
