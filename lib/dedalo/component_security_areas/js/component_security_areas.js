    ...
    case 'edit' :	//var security_access_iframe = document.getElementById('security_access_iframe');
                ...
                $('.css_component_security_areas').filter('input[type="checkbox"]').each(function() {
                          ...
                          $(this).bind("change", function(event) {
                                ...
                                /*  
                                // Reload iframe security_access_iframe if is present
                                // Only in section 'Usuarios'
                                // if (DEBUG) console.log(security_access_iframe)
                                if( $(security_access_iframe).length > 0)
                                  security_access_iframe.src = security_access_iframe.src;

                                if( $(filter_master_iframe).length > 0)
                                  filter_master_iframe.src = filter_master_iframe.src;
                                */
            							});
      						});


                  test_all_component_security_areas_admin();  
...
var component_security_areas = new function() {
  ...
  this.Save = function(component_obj) {
    ...
    //return alert("Saving component_security_areas")

    test_all_component_security_areas_admin();

    // Lock component_filter_master & component_security_access   
...
*/ 
function test_all_component_security_areas_admin() {
