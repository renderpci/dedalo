...
class Ffmpeg {
	...
	*/
	public function create_av_alternate(AVObj $AVObj, $setting) {
		...
		#$extension 		= strval($force);	# definido en los settings
	...
	*/
	public function create_posterframe(AVObj $AVObj, $timecode) {
		...
		$ffmpeg_installed_path 			= $this->get_ffmpeg_installed_path();
		if(!file_exists($ffmpeg_installed_path))
			throw new Exception("Option not installed (ffmpeg)", 1);		
			...
			if($actualPerms < $wantedPerms) {
				...
				if(!$chmod) die(" Sorry. Error on set valid permissions to directory for \"posterframe\".  ") ;
			}			
		...
		# paso 1 sólo video			
		#$command	.= "$ffmpeg_installed_path -itsoffset -$timecode -i $src_file -y -vframes 1 -f rawvideo -an -vcodec mjpeg $target_file > /dev/null  ";	#if(SHOW_DEBUG) echo "Debug:<br>$command<br>";
		$command	.= "$ffmpeg_installed_path -ss $timecode -i $src_file -y -vframes 1 -f rawvideo -an -vcodec mjpeg $target_file ";
			#dump( $command );

		#if(SHOW_DEBUG) dump($command, "Admin Debug command for ".__METHOD__."<div class=\"notas\">sudo -u _www $command </div><hr>");
		
		#exec("unset DYLD_LIBRARY_PATH; ", $o, $r);

		# EXEC COMMAND									
		$posterFrame_command_exc = exec_::exec_command($command);

		/*
		# GUARDAMOS EL POSTERFRAME PARA LA CAPTACIÓN SI NO EXISTE YA
		require_once( DEDALO_ROOT . '/reels/class.RecordObj_reels.php');
		$RecordObj_reels		= new RecordObj_reels($reelID);
		$captacionID			= $RecordObj_reels->get_captacionID();
		global $patImages;
		$patImages_abs			= DEDALO_ROOT . substr( $patImages, 2 ); 				# like ../media/media_foto
		$target_file			= $patImages_abs . $captacionID . '.jpg';
		if( !file_exists($target_file) && !empty($captacionID) ) {
			
			# COMMANDS SHELL
			#$command = "$ffmpeg_installed_path -itsoffset -$timecode -i $src_file -y -vframes 1 -f rawvideo -an -vcodec mjpeg $target_file > /dev/null ";	#if(SHOW_DEBUG) echo "Debug:<br>$command<br>";
			$command = "$ffmpeg_installed_path -ss $timecode -i $src_file -y -vframes 1 -f rawvideo -an -vcodec mjpeg $target_file ";	#if(SHOW_DEBUG) echo "Debug:<br>$command<br>";

			# EXEC COMMAND			
			$captacion_command_exc = exec_::exec_command($command);
		}
		*/
	...
	*/
	public function build_fragment(AVObj $AVObj, $tcin, $tcout, $target_filename, $watermark=0) {
		...
		$ffmpeg_installed_path  	= $this->get_ffmpeg_installed_path();	#dump($ffmpeg_installed_path,'$ffmpeg_installed_path');
		$reelID 					= $AVObj->get_reelID();
		...
		if ($watermark==1) {
			...
			$command = "nice -n 15 $ffmpeg_installed_path -ss $tcin -t $duracion -i $source_file -vcodec copy -acodec copy -y $target_filename_path_temp" ;

			$command .= " && nice -n 15 $ffmpeg_installed_path -i $target_filename_path_temp -vf 'movie=$watermark_file [watermark]; [in][watermark] overlay=main_w-overlay_w-10:10 [out]' -y $target_filename_path";
	...
	*/
	public function conform_header(AVObj $AVObj) {
		...
		try {
			...
			/* DESACTIVO : YA SE VERIFICA AL INICIAR SESIÓN 
			if(!file_exists($ffmpeg_installed_path))
				die('Option not installed (ffmpeg)');
