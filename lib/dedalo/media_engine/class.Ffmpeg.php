	...
	*/
	public function create_av_alternate(AVObj $AVObj, $setting) {
		...
		*/
		...
		// check object value
			if (is_object($media_streams)) {
				$media_streams = $media_streams->streams ?? [];
			}
			debug_log(__METHOD__." media_streams ".to_string($media_streams), logger::DEBUG);

		foreach ( (array)$media_streams as $stream_obj ) {
