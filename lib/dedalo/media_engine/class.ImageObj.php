...
class ImageObj extends MediaObj {
	...
	function __construct($image_id, $quality=false, $aditional_path=false) {
	...
	public function get_media_path() {
		return DEDALO_MEDIA_BASE_URL . DEDALO_IMAGE_FOLDER . '/' . $this->quality . $this->aditional_path . '/';
	}
	public function get_media_path_abs() {
		return DEDALO_MEDIA_BASE_PATH . DEDALO_IMAGE_FOLDER. '/' . $this->quality . $this->aditional_path . '/';
	}
	...
	*/
	...
	public function get_ar_quality_with_file() {
		...
		if(is_array($ar_quality)) foreach($ar_quality as $quality) {
			...
			$obj = new ImageObj($this->image_id, $quality, $this->get_aditional_path);
	...
	*/
	public function get_thumb_url($maxWidht, $maxHeight, $fx=null, $p=null, $prop=null) {
		...
		$thumb_url = DEDALO_LIB_BASE_URL . '/media_engine/img.php?m=' .$m. '&quality=' .$quality. '&aditional_path=' .$aditional_path. '&SID=' .$SID. '&w=' .$w. '&h=' .$h. '&fx=' .$fx. '&p=' .$p. '&prop=' .$prop  ;  	
