...
function player_goto_timecode(videoObj, timecode, timecode_in_seconds) {
	...
	try {
		switch( get_modo() ) {
			...
			// JWPLAYER
			case "jwplayer"	:	videoObj.seek( Math.round(seconds) );		console.log('-> Saltado a '+  Math.round(seconds) + ' [player_goto_timecode jwplayer]');
								break;
			
			// HTML5 STANDAR
			case "html5"	:	videoObj.currentTime = seconds ;			console.log('-> Saltado a '+  seconds + ' [player_goto_timecode html5]' );
								break;
...
// TC 2 SECONDS . CONVERT TC LIKE 00:12:19 TO TOTAL SECONDS LIKE 139	
function tc2secs(tc) {	
	...
	var total_segundos 		= parseFloat( (horas * 3600) + (minutos * 60) + segundos ) ;
