...
window.addEventListener("load", function (event) {
	...
	// Load subtitles
	try {
	    if( typeof top.tool_subtitles.add_subtitle_track_to_video != 'undefined' ) {
			top.tool_subtitles.add_subtitle_track_to_video()
		}
	}
	catch(err) {
	}
	...
	*/
	this.fix_keys = function() {
		...
		if (av_media_player_play_pause_key==null) {
			av_media_player_play_pause_key = default_av_media_player_play_pause_key;	if(SHOW_DEBUG===true) console.log("->fix_keys fixed play_pause: "+av_media_player_play_pause_key) ;
		};
		if (av_media_player_insert_tc_key==null) {
			av_media_player_insert_tc_key = default_av_media_player_insert_tc_key;		if(SHOW_DEBUG===true) console.log("->fix_keys fixed insert_tc: "+av_media_player_insert_tc_key) ;
		};
...
function seconds_to_TIMECODE( secs ) {
    ...
    //var mseconds = (secs % 1).toFixed(3).substring(2);
    var mseconds =(Math.floor(1000 * (secs % 1)) / 1000) * 1000;


    if (hours   < 10) {hours   = "0"+hours;}
    if (minutes < 10) {minutes = "0"+minutes;}
    if (seconds < 10) {seconds = "0"+seconds;}
    if (mseconds < 1) {mseconds = "000";}else
    if (mseconds < 10) {mseconds = "00"+mseconds;}else
    ...
    //console.log("secs",secs+" - "+time); //console.log(last_secs);
    return time;
}
