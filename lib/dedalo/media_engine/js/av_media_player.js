...
var av_media_player_play_pause_key 	= get_localStorage('av_media_player_play_pause_key');
var av_media_player_insert_tc_key 	= get_localStorage('av_media_player_insert_tc_key');
...
var av_media_player = new function() {
	...
	*/
	this.fix_keys = function() {
		...
		$('input[name=play_pause]').keyup(function(event) {		   
			...
			$('span.play_pause_name').first().text( keycode.getKeyCodeValue(av_media_player_play_pause_key) );
			// store cookie
			set_localStorage('av_media_player_play_pause_key',av_media_player_play_pause_key);
		...
		$('input[name=insert_tc]').keyup(function(event) {		   
			...
			$('span.insert_tc_name').first().text( keycode.getKeyCodeValue(av_media_player_insert_tc_key) );
			// store cookie
			set_localStorage('av_media_player_insert_tc_key',av_media_player_insert_tc_key);		
		})
	...
	*/
	...
	this.toggle_config_player_content = function() {
		$(".config_player_content").fadeToggle("fast");
	}
...
function load_video_controls() {
	...
	if(DEBUG) console.log("-> load_video_controls completed");		
}
...
function get_modo() {
	...
	if(!modo || modo==null) {
		$('#debugMovie').append("DEBUG: get_modo: modo if not defined! (" + modo + ') Set default ('+modo_default+')<br>');
		return modo_default;
	}
	return modo ;
}
...
function write_current_tc() {
	...
	try{			
		...
		$('#debugMovie').append("DEBUG: Error in write_current_tc:  " + err +'<br>');
	}
}
