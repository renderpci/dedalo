...
var component_autocomplete_hi = new function() {
	...
	this.wrapper_id;
	this.save_arguments = {}
	this.trigger_url 	= DEDALO_LIB_BASE_URL + '/component_autocomplete_hi/trigger.component_autocomplete_hi.php';
	...
	* @return 
	*/
	this.init = function(wrapper_id, cookie_name) {
		...
		var wrapper = document.getElementById(wrapper_id)
			if (wrapper===null) {
				console.log("Error. Wrapper not found. "+wrapper_id);
				return false;
			}
		
		//var autocomplete_input = wrapper.querySelector('.css_autocomplete_hi_search_field')

		var toponymy_list_button_options = wrapper.querySelector('.toponymy_list_button_options')
			toponymy_list_button_options.addEventListener("click",function(event){
				//event.preventDefault();
				//console.log(this)

				var toponymy_list_ul = this.parentNode.querySelector('ul.toponymy_list') 
		...
		// set_hierarchy_sections . Read from cooki if exists and update ul list
		this.set_hierarchy_sections(wrapper, cookie_name)
	...
	this.activate = function( input_obj ) {	
		...
		$(input_obj).autocomplete({
			...
			source: function( request, response ) {
				...
				// Ajax call
				common.get_json_data(component_autocomplete_hi.trigger_url, trigger_vars).then(function(response_data) {
							if(SHOW_DEBUG===true) {
								//console.log(response_data);
								var end  = new Date().getTime();
								var time = end - start;
								console.log("Time for "+term+": "+time);
							}
							...
							*/
							var label_value = component_autocomplete_hi.convert_data_to_label_value(response_data)
