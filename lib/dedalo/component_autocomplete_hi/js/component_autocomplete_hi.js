	...
	*/
	this.build_grid_images = function(options) {
		...
		let search_query_object = JSON.parse(self.search_query_object)
			...
			const reference_item = JSON.parse(JSON.stringify(search_query_object.filter["$and"][0])) // Clone old sentence to re-use
			// Delete current filter sentence
			delete search_query_object.filter["$and"]
			// Create new one
			...
			for (let i = 0; i < len; i++) {
				...
				let filter_item   = JSON.parse(JSON.stringify(reference_item))
					filter_item.q = q
				search_query_object.filter["$and"].push(filter_item)
			}								
