...
var component_autocomplete_hi = new function() {
	...
	*/
	this.Save = function(component_obj) {
		...
		if(page_globals.modo!=='edit' && page_globals.modo!=='tool_cataloging') {
			console.warn("Only edit/tool_cataloging modes can save. Stop save command here! page_globals.modo: ",page_globals.modo);
			return false
		}
	...
	*/
	this.select_component = function(obj_wrap) {
		...
		if(component_options_container){
			...
			//component_options_container.classList.remove("hide")
			// Focus search field
	...
	*/
	this.open_element = function(button) {
		...
		const window_url  = '?t=' + section_tipo + '&id=' + section_id + '&m=' + modo + '&menu=no'
		const window_name = 'Edit element ' + section_tipo // + ' ' + section_id

		const float_window_features = "menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=1200,height=860"
