...
var component_common = new function() {
	...
	*/
	this.Save = function (component_obj, save_arguments) {		
		...
		//Inicio - DCA 2015/03/12
		// Validar si el registro de la ficha PCI esta introducida con el formato correcto
		if (typeof name != 'undefined'){
			if (name.substring(0, 5) == 'dd376'){

				if (dato.trim() == '') return alert("El número de registro no puede estar vacío.");

				var expreg = /^31-[0-9]{6}$/;
				if ( dato.match(expreg) == null ) {			
	    			return alert("El número de registro debe empezar por 31 seguido de '-' y 6 digitos.");
				}

				//if (dato.substring(0, 2) != '31'){
				//	$(component_obj).val("");
				//	return alert("El número de registro debe empezar por 31 seguido de '-' y un número correlativo.");
				//} 
			}
		}
		//Fin - DCA 2015/03/12
