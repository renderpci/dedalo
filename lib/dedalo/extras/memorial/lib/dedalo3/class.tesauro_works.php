	...
	public static function walk_alphabetic_tesauro($alphabetic_tesauro) {
		...
		foreach ($alphabetic_tesauro as $key => $ar_value) {
			...
			$esdescriptor 	= $RecordObj_ts->get_esdescriptor();
				#dump($RecordObj_ts,'$RecordObj_ts'); break;

			# TG Padre
			$tg_id 	= $RecordObj_ts->get_parent();
			...
			# TE Hijos
			$te_children  = $RecordObj_ts->get_ar_childrens_of_this('si', 'no', $order_by=null );
			$ar_te=array();
			...
			$nd_pre 		= $RecordObj_ts->get_ar_childrens_of_this('no', 'no', $order_by=null );
			$ar_nd=array();
