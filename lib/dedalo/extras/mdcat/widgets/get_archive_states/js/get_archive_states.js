...
* get_archive_states
*/
var get_archive_states = new function() {
	...
	this.activate_listener_value = function(component_target_tipo) {
		...
		$(function() {
			...
			if (reference_input) {	
				...
				reference_input.removeEventListener('change', get_archive_states.update_state)				
				
				// Add listner change
				reference_input.addEventListener("change", get_archive_states.update_state, false);				
			}
		});	
	...
	*/
	this.update_state = function(e) {
		...
		var get_archive_states = document.getElementById('get_archive_states')
		
		// Update info (reload component info)
		var js_promise = component_info.update_info(get_archive_states)
