var expressos = new function() {
	...
	this.calculate_period = function(options){
		...
		const check_months = total_days / 30.42
		const rest_months = check_months - total_months
		
		if(rest_months > 0.99){total_months = total_months + 1}
	...
	this.calculate_import_minor = function(options){
		...
		if(rest_months > 0.99){total_months +1}
