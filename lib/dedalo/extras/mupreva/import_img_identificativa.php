...
# Set vars
	$vars = array('mode','process','variante');
...
if ($mode=='create_records') {
	...
	for ($i=1; $i <= $total; $i++) {
		...
		$component_dato 		= (string)$section->get_section_id();
		$component_modelo_name 	= RecordObj_ts::get_modelo_name_by_tipo($component_tipo);
...
if ($mode=='process_folder') {
	...
	if ($process!=1) {
		...
		# PREVIEW
		$html .= "<h2 class=\"h2_title\"> PREVIEW (MUPREVA IMAGEN IDENTIFICATIVA) </h2><div class=\"process_right_info\">[type:$type - variante:$variante]</div><HR>";
		...
		echo "<h2 class=\"h2_title\"> PROCESS (MUPREVA IMAGEN IDENTIFICATIVA) </h2><HR>";
		#echo "PROCESS <HR>";
		...
		foreach ($ar_image_group as $number => $ar_group_value) {
			...
			foreach ($ar_group_value as $key => $ar_data) {
				...
				/**/
				# VERIFICAMOS SI EXISTE EL REGISTRO EN 'IMÁGENES' PARA EVITAR DUPLICIDADES
				$arguments=array();
				$arguments['strPrimaryKeyName']	= 'parent';
				$arguments['tipo']				= 'dd1115';	# Código
				$arguments['dato:json_exact']	= $file_name_dd; # tipo "1-1"
				$matrix_table 					= common::get_matrix_table_from_tipo($arguments['tipo']);
				$RecordObj_matrix				= new RecordObj_matrix($matrix_table,NULL);
				$ar_result						= $RecordObj_matrix->search($arguments);
				if(empty($ar_result)) {

					$msg = " <pre> - Warning: Este registro con código $file_name_dd - (number:$number) (tipo:dd1115) (file_name_dd:$file_name_dd) no existe en matrix. Será creado ahora </pre>";
					...
					#
						...
						$current_component->set_dato($file_name_dd);
						$current_component->Save();
				...
				}#end create record if(empty($ar_result))
					...
					if( strtolower($extension) != DEDALO_IMAGE_EXTENSION ) {
						...
						if (!copy($ar_data['full_path'], $path_copia)) {
						    throw new Exception("<pre>ERROR al copiar ".$ar_data['full_path']." a ".$path_copia."</pre>");
						}
						# JPG : la convertimos a jpg
						ImageMagick::convert($path_copia, $target_dir .'/'. $file_name_dd .'.'. DEDALO_IMAGE_EXTENSION );

						echo "<pre>La imagen de origen NO es jpg ($extension). Copiado el fichero y convertido a jpg en destino</pre>";

					}else{
						# Lo copiamos asegurándonos que la extensión queda en minúsculas
						$path_copia = $target_dir .'/'. $file_name_dd.'.'.DEDALO_IMAGE_EXTENSION;
						if (!copy($ar_data['full_path'], $path_copia)) {
						    throw new Exception( "<pre>ERROR al copiar ".$ar_data['full_path']." a ".$path_copia."</pre>" );
						}
						echo "<pre>La imagen de origen ya es jpg y no necesita conversión. Copiado el fichero en destino</pre>";
					}
					...
					if($make_thumb===true) {
						...
						$source_quality = $quality ; if($quality!='identificativa') throw new Exception("Error. Source quality debe ser identificativa", 1);
							#dump($source_quality,"source_quality ");
						...
						if (file_exists($target_image)) {
							echo "<pre class=\"warning\">Reemplazada la miniatura existente de calidad por defecto (".DEDALO_IMAGE_QUALITY_DEFAULT.") por la miniatura creada desde '$source_quality'</pre>";
						}
			...
			#break;
				...
				#
					...
					# La sección de Inventario debe estar creada obligatoriamente. Sólo se re-crearán los registros de los portales asociados,
					# pero nunca el registro 'padre' de Inventario que debe existir previamente.
				...
				if($disparo_int == 1 && !empty($section_id) ) {
					...
					echo "<pre>Añadido locator $rel_locator a portal tipo $component_tipo para Imagen identificativa (disparo: $disparo_int)</pre>";
					...
					*/
					...
					echo "<pre>Añadido locator $rel_locator a portal tipo $component_tipo para Imagen asociada (disparo: $disparo_int)</pre>";

				}

				if(empty($section_id)) {
					echo "<pre>Ya existe el registro para esta imagen. Se utilizó el mismo, se añadió la imagen de calidad actual ('identificativa') y se reemplazó la miniatura actual (1.5MB) con una miniatura generada a partir de ella ('identificativa') </pre>";
				}
...
function get_file_data($dir, $value, $type) {
	...
	switch ($type) {
		...
		case 'catalogo':
			...
			$quality 	= 'identificativa';	# QUALITY 	 original | identificativa
			break;
	...
	*/
	...
	#$ar_data['image'] 			= "Preview from $extension ";#<a href=\"$img_url\" target=\"_blank\"><img src=\"".$img_url."\" valign=\"middle\" style=\"max-height:260px;max-width:400px;margin-right:10px\" /></a>";	#float: right;position: relative;top: -260px;right: 50px;
	$ar_data['image'] 			= "Source: <a href=\"$img_url\" target=\"_blank\"><img src=\"".$img_url."\" valign=\"middle\" style=\"max-height:260px;max-width:400px;margin-right:10px;\" /></a> ";
	...
	$ar_data['image'] .= "Thumb:<a href=\"$img_url_preview\" target=\"_blank\"><img src=\"".$img_url_preview."\" valign=\"middle\" style=\"max-height:260px;max-width:400px;margin-left:10px;\" /></a>";

	# ACTUAL IMAGE DEFAULT PREVIEW
	$ImageObj				= new ImageObj($ar_data['file_name_dd'], DEDALO_IMAGE_QUALITY_DEFAULT, $ar_data['target_dir_name']);
	$local_full_path 		= $ImageObj->get_local_full_path();
		#dump($local_full_path,'$local_full_path');
	$img_url_default_actual = 'foto_preview.php?f='.$local_full_path ;
	if(file_exists($local_full_path)) {
		$ar_data['image'] .= " Actual default:<a href=\"$img_url_default_actual\" target=\"_blank\"><img src=\"".$img_url_default_actual."\" valign=\"middle\" style=\"max-height:260px;max-width:400px;margin-left:10px;\" /></a> ";
	}else{
		#$ar_data['image'] .= " <span title=\"$local_full_path\">Actual default:none</span>";
		$ar_data['image'] .= " Actual default:<img src=\"".DEDALO_LIB_BASE_URL."/themes/default/0.jpg\" valign=\"middle\" style=\"max-height:260px;max-width:260px;margin-left:10px;\" title=\"$local_full_path\" />";
		...
		$colorspace_info  = shell_exec( MAGICK_PATH . "identify -format '%[colorspace]' " .$full_path. "[0]" );
		...
		$command = MAGICK_PATH . "convert \"$full_path\" $flags \"/target_file\" ";	# -negate -profile Profiles/sRGB.icc -colorspace sRGB -colorspace sRGB 
			#if(SHOW_DEBUG) dump($command,'ImageMagic

	$ar_data['colorspace_info'] = $colorspace_info;
	$ar_data['command'] 		= $command;
