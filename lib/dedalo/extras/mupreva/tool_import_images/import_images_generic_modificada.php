			...
			# IMAGE COPY FILE
				...
				if (!in_array($target_dir, $ar_verified_paths)) {
					...
					if (!chmod($target_dir, 0777)) {
						if (SHOW_DEBUG) {
							trigger_error("Permission error in:" . $target_dir);
							echo 'Current script owner: ' . get_current_user();
							echo 'Current script owner: ' . trim(shell_exec('whoami'));
						}
					}
					#dump($target_dir);
				}
				if( strtolower($extension) != DEDALO_IMAGE_EXTENSION ) {
					...
					//$html .= "<div class=\"info_line alert_icon\">La imagen de origen NO es jpg ($extension). Será ignorada </div>";
					...
					if (!chmod($orginal_jpg_path, 0777)) {
						if (SHOW_DEBUG) {
							trigger_error("Permission error in:" . $orginal_jpg_path);
							echo 'Current script owner: ' . get_current_user();
							echo 'Current script owner: ' . trim(shell_exec('whoami'));
						}
					}
				...
				$image_default_path 	= $ImageObj->get_local_full_path();
					...
					if (!in_array($target_dir, $ar_verified_paths)) {
						...
						if (!chmod($target_dir, 0777)) {
							if (SHOW_DEBUG) {
								trigger_error("Permission error in:" . $target_dir);
								echo 'MANOLOOOOO: ' . get_current_user();
								echo 'Current script owner: ' . trim(shell_exec('whoami'));
							}
						}
					}
					...
					if (!chmod($source_image, 0777)) {
						if (SHOW_DEBUG) {
							trigger_error("Permission error in:" . $source_image);
							echo 'MANOLOOOOO: ' . get_current_user();
							echo 'Current script owner: ' . trim(shell_exec('whoami'));
						}
					}
					if (!chmod($target_image, 0777)) {
						if (SHOW_DEBUG) {
							trigger_error("Permission error in:" . $target_image);
							echo 'MANOLOOOOO: ' . get_current_user();
							echo 'Current script owner: ' . trim(shell_exec('whoami'));
						}
					}
