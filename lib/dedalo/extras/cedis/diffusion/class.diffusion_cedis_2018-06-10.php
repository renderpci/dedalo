...
class diffusion_cedis {
	...
	*/
	public static function save_segment_traslations( $request_options ) {
		...
		foreach (array('lg-ell','lg-deu') as $lang) {
	...
	*/
	public static function save_tape( $request_options ) {
		...
		###################### 4-2-2018 IN ################################################################
		# contributors
		# $ar_contributors_tipo = array('rsc61','rsc62','rsc63','rsc376','rsc48','rsc400','rsc401','rsc402','rsc51');
		$ar_contributors_tipo = array(	'rsc51', # Camera recorder # ADDED 15-03-2018 !!
	...
	*/
	public static function build_segments( $request_options ) {
		...
		foreach ($fragments_text_by_tc as $key => $value) {
			...
			# Translation
			$segments_options = new stdClass();
				$segments_options->tc_in 	= $tc_in;
				$segments_options->tc_out 	= $tc_out;
				$segments_options->lang 	= 'lg-deu';
				$segments_options->component_text_area 	= $component_text_area;

			$translation = self::get_segment_translation($segments_options);
				...
				if (!empty($matches[0])) {
					...
					if ($mainheading_val!==null) {
						...
						$header_component = component_common::get_instance($modelo_name,
																		   ...
																		   $mainheading_val->section_tipo,
																		   false);
						$header_value['lg-deu'] = $header_component->get_valor('lg-deu');
					}
					...
					if ($subheading_val!==null) { 
						...
						$subheader_component 	 = component_common::get_instance($modelo_name,
																		   	  	  ...
																		   	  	  $subheading_val->section_tipo,
																		   	  	  false);
						$subheader_value['lg-deu'] = $subheader_component->get_valor('lg-deu');
					}

				}//end if (!empty($matches[0])) {			
				$segment->mainheading['lg-ell'] = !empty($header_value['lg-ell']) ? $header_value['lg-ell'] : '';
				$segment->mainheading['lg-deu'] = !empty($header_value['lg-deu']) ? $header_value['lg-deu'] : '';
				$segment->subheading['lg-ell']  = !empty($subheader_value['lg-ell']) ? $subheader_value['lg-ell'] : '';
				$segment->subheading['lg-deu']  = !empty($subheader_value['lg-deu']) ? $subheader_value['lg-deu'] : '';			
