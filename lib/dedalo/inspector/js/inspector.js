...
var inspector = new function() {
		...
		window.ready(function(){
			...
			switch(page_globals.modo) {
				...
				case 'edit' :
					...
					self.maintain_inspector_position()
	...
	*/
	this.maintain_inspector_position = function(){
		...
		if (sidebar) {
			let fix_top = function()	{
				let window_height 	 = window.innerHeight
				let	window_width 	 = window.innerWidth
				let	inspector_height = sidebar.clientHeight
			...
			if(!isiPad || window.innerWidth > inspector.max_screen_width) {
				// Add window event listener scroll
	...
	};//end maintain_inspector_position
	...
	*/
	...
	this.update_inspector_info = function (obj_warp) {
		...
		var label = "";
		var label_objs = obj_warp.getElementsByTagName('label')
		...
		if (target_obj) {
			...
			var label_div = common.create_dom_element({ element_type : 'div',
														parent 		 : target_obj,
														class_name 	 : "key capitalize",
														text_node 	 : get_label.seleccionado
													 })			
			var value_div = common.create_dom_element({ element_type : 'div',
														parent 		 : target_obj,
		...
		try {
			...
			if (len > 0) {				
				for (let i = 0; i < len; i++) {
					...
					let tool_name 	= ar_tools_name[i]
					let	label 	  	= get_label[tool_name]
					let	title 		= get_label.abrir + ' ' + get_label.herramienta
					...
					// HTML TO INSPECTOR
					// Container tool div					 
					let tool_button = inspector.build_tool_button({	'tool_name'		: tool_name,
																	'label' 		: label,
																	'title' 		: title,
																	'tipo' 			: tipo,
					...
					let tool_inspector_div = document.createElement("div")
	...
	*/
	this.build_tool_button = function( data ) {
		...
		let tool_button = document.createElement("div")
	...
	*/
	this.open_data_link = function(button) {
		...
		let url = DEDALO_LIB_BASE_URL + "/json/" + page_globals.section_tipo + "/" + page_globals._parent
		window.open(url);
