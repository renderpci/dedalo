...
var inspector = new function() {
	...
	* @return bool true
	*/
	this.show_component_tm_history_status
	this.show_component_tm_history = function(options) {

		const self = this

		if (self.show_component_tm_history_status==='loading') {
			console.warn("// show_component_tm_history stopped!", options.tipo);
			return
		}
		...
		if (wrap_component_history && wrap_info_line_tm) {
			...
			.then(function(response){				
				self.show_component_tm_history_status = 'loaded'							
			})
		}
	...
	*/
	...
	this.update_inspector_info = function (obj_warp) {
		...
		const tipo				= obj_warp.dataset.tipo
		const current_parent	= obj_warp.dataset.parent || obj_warp.dataset.section_id
		const lang				= obj_warp.dataset.lang
		const dato				= obj_warp.dataset.dato || ''
		const component_name 	= obj_warp.dataset.component_name || null
		...
		if( typeof current_parent==="undefined" || !current_parent ) {
			console.log('Error: current_parent not defined! (update_inspector_info) ', obj_warp)
			return false
		}
		...
		if (inspector_info) {
			...
			const is_section = (obj_warp.dataset.section_info) ? true : false;
			switch(is_section) {
				case true:
					// Section info
					const section_info = JSON.parse(obj_warp.dataset.section_info)
				...
				default:
					...
					let component_label		 = "";
					const label_objs = obj_warp.getElementsByTagName('label')
	...
	*/
	...
	this.load_inspector_tools = function (obj_warp) {
		...
		const tipo					= obj_warp.dataset.tipo
		const current_parent		= obj_warp.dataset.parent
		const lang					= obj_warp.dataset.lang
		const context_name			= obj_warp.dataset.component_name
		const current_tipo_section	= obj_warp.dataset.current_tipo_section	//return alert('load_inspector_tools \nid:'+id_matrix + " \ntipo:"+tipo + " \ncurrent_tipo_section:"+current_tipo_section)
		const section_tipo			= obj_warp.dataset.section_tipo
		...
		try {
			...
			const component_info	= JSON.parse(obj_warp.dataset.component_info)
