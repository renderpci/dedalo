...
var inspector = new function() {
	...
	*/
	this.open_content = function(content_div_id) {
		...
		const section_content_selector = (typeof content_div_id !== 'undefined')
			? document.getElementById(content_div_id)
			: this.css_section_content
		...
		try {
			...
			if (len > 0) {
				for (let i = 0; i < len; i++) {
					...
					// tool_docu case
						if (tool_name==='tool_docu') {
							const tool_button2 = inspector.build_tool_button({
								tool_name		: tool_name,
								label			: label + ' Online',
								title			: title,
								tipo			: tipo,
							...
							tool_inspector_div.appendChild(tool_button2)
						}
	...
	*/
	this.open_data_link = function(button) {
		...
		const url = DEDALO_LIB_BASE_URL + "/json/json_display.php?url_locator=" + page_globals.section_tipo + "/" + page_globals._parent
		window.open(url);
