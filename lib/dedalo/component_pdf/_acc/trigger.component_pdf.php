...
# set vars
	$vars = array('mode','pdf_id','quality','source_quality','target_quality','timecode');
...
if($mode=='generate_version') {
	...
	if (empty($pdf_id) || strlen($pdf_id)<4) {
		return "Error: pdf_id is not defined!";
	}

	$quality 	= $target_quality ;
			
	# ImageObj
	$ImageObj			= new ImageObj($pdf_id, $quality);
	...
	logger::$obj['activity']->log_message(
		...
		$tipo,
		NULL,
		array("msg"=>"Generating av file $pdf_id - quality $quality from $source_quality")
	);
...
if($mode=='delete_version') {
	...
	$file				= $folder_path . $pdf_id . '.' . $ImageObj->get_extension();
	...
	if(file_exists($file)) {
		...
		try{
			...
			# move / rename file
			$rename 		= rename($file, $folder_path_del . "/$pdf_id" . '_deleted_' . date("Y-m-d") . '.' . $ImageObj->get_extension() );
			if(!$rename) 	throw new Exception(" Error on move files to folder \"deleted\" . Permission denied . The files are not deleted");
			
			# DELETE TEMP SH FILE
			$tmp_file		= DEDALO_MEDIA_BASE_PATH . DEDALO_IMAGE_FOLDER . "/tmp/".$quality.'_'.$pdf_id.'.sh';	
			...
			logger::$obj['activity']->log_message(
				...
				$tipo,
				NULL,
				array("msg"=>"Deleted image file $pdf_id - quality $quality (file is renamed and moved to delete folder)")
			);
			
			echo "File ". $pdf_id . '.' . $ImageObj->get_extension() . " deleted ! "  ;
...
if($mode=='file_exists') {
	...
	$file_size = 0;
	$file_name = DEDALO_MEDIA_BASE_PATH . DEDALO_IMAGE_FOLDER . '/' . $quality . '/' . $pdf_id . '.' . DEDALO_IMAGE_EXTENSION;	
...
if($mode=='download_file') {	
	...
	logger::$obj['activity']->log_message(
		...
		$tipo,
		NULL,
		array("msg"=>"Download image file $pdf_id - quality $quality")
	);
