...
var tool_time_machine = new function() {
	...
	this.selected_wrap // component wrapper node selected (inspector)

	this.tm_notes_component_tipo	= 'rsc329'
	this.tm_notes_section_tipo		= 'rsc832'
	this.tm_tm_notes_permissions	= 0
	...
	this.init = function(options={}) {
		...
		// component_save event
			window.addEventListener('component_save', function(e){
				if (self.tm_notes_component_tipo===e.detail.dataset.tipo) {
					// refresh rows list
	...
	* Used in list mode sections only
	* @return 
	*/
	...
	*/
	this.render_list = function(options) {
		...
		// options
			...
			const load_all					= options.load_all || false
	...
	this.render_rows_list = function(rows_data, target_wrapper, append, context) {
		...
		return new Promise(function(resolve, reject) {
			...
			for (let i = 0; i < rows_data_length; i++) {
				...
				// sample_data
					const sample_data_string = row_obj.dato_string
						? row_obj.dato_string.replace(/(\r\n|\n|\r|\<br\>)/gm, " ")
						: ''
			...
			// tm notes
				...
				.then(function(notes_rows){
					...
					for (let i = tm_notes_nodes_length - 1; i >= 0; i--) {
						...
						if (found_row) {
							// NOTE EXIST
							if (self.tm_notes_permissions>0) {
							...
							}//end if (self.tm_notes_permissions>0)
							...
							// NOTE NOT EXIST
							if (permissions>1 && self.tm_notes_permissions>0) {
								const icon_sticky = common.create_dom_element({
									element_type	: 'img',
										...
										if (note_section_id) {
											...
											.then(function(component_html){
												if (component_html) {
													...
													self.render_list({
														limit	: self.total_loaded, // self.limit,
														offset	: self.offset_previous,
														append	: false
													})
												}
	...
	this.get_tm_notes = function(options) {
		...
		return new Promise(function(resolve){
			...
			.then(function(response){
				...
				self.tm_notes_permissions = response.tm_notes_permissions || 0
	...
	*/
	this.load_tm_note_component = function(section_id, permissions) {
		...
		const component_tipo	= self.tm_notes_component_tipo
		const section_tipo		= self.tm_notes_section_tipo

		return new Promise(function(resolve){

			// component text area js and css files
				const ar_promises = []

				const component_text_area_css = DEDALO_LIB_BASE_URL + '/component_text_area/css/component_text_area.css';
				ar_promises.push( common.load_style(component_text_area_css) )

				const mce_editor_url = DEDALO_LIB_BASE_URL + '/component_text_area/js/mce_editor.js';
				ar_promises.push( common.load_script(mce_editor_url) )

				const text_editor_url = DEDALO_LIB_BASE_URL + '/component_text_area/js/text_editor.js';
				ar_promises.push( common.load_script(text_editor_url) )
				
				const text_area_url = DEDALO_LIB_BASE_URL + '/component_text_area/js/component_text_area.js';
				ar_promises.push( common.load_script(text_area_url) )

				const button_delete_url = DEDALO_LIB_BASE_URL + '/button_delete/js/button_delete.js';
				ar_promises.push( common.load_script(button_delete_url) )
		...
		// Open dialog Bootstrap modal
			$(modal_dialog).modal({
				...
				if (self.selected_wrap) {
					// component_common.select_component(self.selected_wrap)
					component_common.select_wrap(self.selected_wrap)
					return
				}

				// refresh rows list. Used when inspector call method 'load_component_time_machine_list'
				// whithout tool init
				// (!) note: When caller is inspector (no tool linit), is worthy to reload list 
				// because the component loses focus when dialog is opened
				if (self.initied!==true) {
					const new_limit = self.total_loaded
					self.render_list({
						limit	: new_limit, // self.limit,
	...
	*/
	this.load_component_time_machine_list = function(options) {
		...
		// options
			...
			const selected_wrap				= options.selected_wrap
		...
		// fix initials
			...
			self.selected_wrap			= selected_wrap // used to refocus on close dialog note
