...
var tool_time_machine = new function() {
	...
	*/
	this.load_preview_component = function ( tipo, parent, lang, id_time_machine, current_tipo_section) {
		...
		var target_div				= $('#tm_component_time_machine');	
			...
			//console.log(parent +","+ tipo +","+ lang +","+ id_time_machine)
	...
	*/
	...
	/**
	* SECTION_LIST_RECOVER_SECTION
	*/
	this.section_list_recover_section = function ( button_obj ) {
		...
		.done(function(received_data) {
			...
			if(error_response) {
				...
				section_list.reload_rows_list(call_uid);
					...
					if (current_options.filter_by_id.length>1) {
						$.each( current_options.filter_by_id, function( index, item ) {
							...
							if (current_key==id_time_machine) {
								...
								return section_list.reload_rows_list(call_uid);
							}
						});
					}else{
	...
	*//*
	...
	this.section_list_recover_section = function ( button_obj ) {
		...
		var trigger_url	= DEDALO_LIB_BASE_URL + '/tools/tool_time_machine/trigger.tool_time_machine.php' ;
