...
* Paths vars are defined on invoke class tool_import_files_dcnav
*/
require_once( dirname(dirname(dirname(dirname(__FILE__)))) .'/config/config4.php');
require_once( DEDALO_LIB_BASE_PATH.'/tools/tool_import_files_dcnav/class.tool_import_files_dcnav.php');
require_once( DEDALO_UPLOADER_DIR.'/server/php/UploadHandler.php');

#
# TOOL_IMPORT_FILES Class create setup path vars
$tool_import_files = new tool_import_files_dcnav(null);

#
# BASIC VARS ( set up on call tool_import_files class)
   
    # UPLOAD_DIR
    # Path to folder where files are stored / uploaded (until next script moves to final loaction)
    $upload_dir = TOOL_IMPORT_FILES_UPLOAD_DIR;
   
    # UPLOAD_URL
    # URL path where files are uploaded and browser get files
    $upload_url = TOOL_IMPORT_FILES_UPLOAD_URL;

    # SCRIPT_URL
    # URL of server side file handler ( is current scritp :-P url )
    # This url contain var tipo like: DEDALO_LIB_BASE_URL . '/tools/tool_import_files_dcnav/inc/files_handler.php?t='.$tipo
...
}//end CustomUploadHandler



#
# CUSTOMUPLOADHANDLER CLASS INIT
