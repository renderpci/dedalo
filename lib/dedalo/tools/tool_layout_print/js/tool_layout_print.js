...
var tool_layout_print = new function() {
	...
	this.get_data = function( url, mydata ) {
		...
		var jsPromise = Promise.resolve(
			...
			.done(function(received_data) {
				...
				//if(SHOW_DEBUG===true) console.log("->Save response: "+received_data );
			})
			// FAIL ERROR 
	...
	* @return 
	*/
	this.go_home = function(button_obj) {
		
		const current_url = window.location.href
		const new_url 	  = change_url_variable(current_url, "context_name", "list")
	...
	*/
	this.get_ar_components = function(ar_div_components){
		...
		var components = []
		var c_len = ar_div_components.length
	...
	*/
	this.get_ar_freeText = function(ar_div_freeText){
		...
		var ar_free_text = []
		var d_len 		 = ar_div_freeText.length
		for (var i = 0; i < d_len; i++) {
			
			var element 	= ar_div_freeText[i]
			var typology	= element.dataset.typology
			var	id 			= element.id
	...
	*/
	this.build_templates_list = function(options) {
	
		const self = this		

		const list_title 			= options.list_title
		const layout_section_tipo 	= options.layout_section_tipo
		const ar_templates 			= options.ar_templates		
		const list_type 			= options.list_type // public | private
		const list_wrapper_id 		= options.list_wrapper_id
		const list_wrapper 			= document.getElementById(list_wrapper_id)
		...
		const js_promise = new Promise(function(resolve) {
			...
			const title = common.create_dom_element({
				element_type 	: 'h2',
				class_name 	 	: "",
				text_content 	: list_title,
				parent 			: list_wrapper,
			})

			const ul = common.create_dom_element({
				element_type 	: 'ul',
				class_name 	 	: "text_unselectable text_shadow_inset",				
				parent 			: list_wrapper,
			})
	
			// Li list
			const ar_templates_length = ar_templates.length;
			for (let i = 0; i < ar_templates_length; i++) {
				
				const item 		 	= ar_templates[i]
				const section_id 	= item.section_id
				const section_tipo 	= item.section_tipo // template_tipo
				const label 		= item.label
				const template_key 	= section_tipo + "_" + section_id
				...
				const li = common.create_dom_element({
					...
					class_name 	 	: "",
					dataset 		: {
						template_id   : item.section_id,
						template_tipo : item.section_tipo
					},
					parent 			: ul,
				})
				/* NO se usa.. 
				const icon_template_more = common.create_dom_element({
					element_type 	: 'div',
					class_name 	 	: "t_icon icon_template_mini icon_template_more",
					parent 			: li,
				})
				*/
				if (list_type==="public") {
					const icon_template_minus = common.create_dom_element({
						element_type 	: 'div',
						class_name 	 	: "t_icon icon_template_mini icon_template_minus",
						parent 			: li,
					})
					icon_template_minus.addEventListener("click", function(){
						self.delete_template(this)
					})
				}

				const icon_template_public = common.create_dom_element({
					element_type 	: 'div',
					class_name 	 	: "t_icon icon_template icon_template_public",
					parent 			: li,
				})
				icon_template_public.addEventListener("click", function(){
					self.goto_edit(this)
				})

				const span_label = common.create_dom_element({
					element_type 	: 'span',
					class_name 	 	: "template_label",
					text_content 	: label,
					parent 			: li,
				})

				if(SHOW_DEBUG===true) {
					const label_debug = common.create_dom_element({
						element_type 	: 'span',
						class_name 	 	: "note",
						text_content 	: template_key,
						parent 			: span_label,
					})
				}
			}//end for (var i = 0; i < ar_templates.length; i++)

			// Fixed li on public list_type
					const fixed_li = common.create_dom_element({
						element_type 	: 'li',
						class_name 	 	: "",
						dataset 		: {
							template_id   : "",
							template_tipo : layout_section_tipo
						},
						parent 			: ul,
					})					

					const icon_add_template_css = (list_type==="public")
						? 'icon_template_public'
						: 'icon_template_private'

					const icon_add_template = common.create_dom_element({
						element_type 	: 'div',
						class_name 	 	: "t_icon icon_template "+icon_add_template_css+" icon_add_template",
						parent 			: fixed_li,
					})
					icon_add_template.addEventListener("click", function(){
						self.goto_edit(this)
					})
					...
					const span_label = common.create_dom_element({
						element_type 	: 'span',
						class_name 	 	: "template_label",
						text_content 	: get_label["nueva_plantilla_vacia"] + ((list_type==="private") ? ' (all instalations)' : ''),
						parent 			: fixed_li,
					})
