	...
	*/
	this.parse_hierarchy_html_terms = function(ar_terms, components_to_parse, parent, recursion) {
		...
		for (let i = 0; i< ar_terms_length ; i++) {
			...
			if (ar_related.length>0) {
				...
				ar_related.forEach( (current_related_term) => {
					...
					if (typeof ar_rel_terms[0]!=="undefined") {
						const rel_term_data = ar_rel_terms[0].data
						const ar_rel_term 	= rel_term_data.filter(term => term.from_component_tipo === components_to_parse.term && term.lang === data_lang)
						//console.log("rel_term_data:",ar_rel_term);
			...
			const scope_note 	= typeof ar_scope_note[0]!=="undefined" ? ar_scope_note[0] : undefined
