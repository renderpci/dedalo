	...
	this.fix_height = function() {
    	...
    	if(tinyMCE.activeEditor) {
	    	...
	    	tinyMCE.activeEditor.theme.resizeTo(
		        null,
		        document.querySelector('div.indexation_page_text').clientHeight
		    );
    	}	   
	};//end fix_height
	...
	this.new_index_data_record = function(button_ob, evt) {
		...
		var js_promise 	 = common.get_json_data(tool_indexation.url_trigger, trigger_vars).then(function(response) {
			...
			if (response===null) {
				...
				// Update data in tag
				component_text_area.update_tag(tool_indexation.tag_obj, new_data_obj, tool_indexation.component_tipo, true)
