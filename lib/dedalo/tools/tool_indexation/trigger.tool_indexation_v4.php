...
function indexations_list() {
	...
	$response->result = $ar_indexations;
...
function delete_tag() {
	...
	$vars = array('section_tipo', 'section_id', 'component_tipo', 'tag_id', 'locator', 'lang');
		foreach($vars as $name) {
			$$name = common::setVar($name);
			if (empty($$name)) {
				$response->msg = 'Error. Empty mandatory var '.$name;
	...
	#
	# INDEX
	# Delete all references to current tag
	...
	$component_text_area = component_common::get_instance('component_text_area',
														  ...
														  $lang,
														  $section_tipo);		
	$response->result = $component_text_area->delete_tag_from_all_langs($tag_id);

	if($response->result===true) {
		$response->msg = "Removed tag \"$tag_id\" from all langs component_text_area $component_tipo [$section_tipo - $section_id]";
	}
