...
var tool_import_dedalo_csv = new function() {
	...
	*/
	this.get_selected_files = function() {
		...
		let read_files_response_div = tool_import_dedalo_csv.dir_files_element
			if (!read_files_response_div) {
				console.log("[get_selected_files] Error on read 'read_files_response_div' ")
				return false;
			}
		
		let checkboxes 			= read_files_response_div.querySelectorAll('input[type="checkbox"]');
		let selected_files  	= []
		let input_section_tipo 	= read_files_response_div.querySelectorAll('input[type="text"]');
		let ar_section_tipo 	= []
	...
	*/
	this.rename_files = function( button_obj ) {
		...
		*/
		...
		const csv_file_path = files_path + '/' + selected_files[0].file
