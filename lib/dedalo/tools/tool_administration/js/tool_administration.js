/*global get_label, SHOW_DEBUG, DEDALO_LIB_BASE_URL, html_page */
...
var tool_administration = new function() {
	...
	* UPDATE_DEDALO_CODE
	*/
	this.update_dedalo_code = function(obj, event) {
		if( !confirm(' WARNING !\nAre you sure to update DEDALO CODE ? ') ){
			return false;
		}

		// is_preview
		const update_dedalo_code_check = obj.parentNode.querySelector("#update_dedalo_code_check")
		obj.dataset.is_preview = update_dedalo_code_check.checked
	...
	*/
	...
	this.toggle_content = function(panel_heading_obj) {
		$(panel_heading_obj).next('.panel-body').toggle(10);
	};
	...
	* DELETE_SECTIONS
	*/
	this.delete_sections = function(button_node, event) {
		//console.log(navigator.userAgent.indexOf('AppleWebKit'));
		...
		// config button_node dataset with input values

		// section_tipo
		const input_section_tipo	= button_node.parentNode.querySelector('.input_section_tipo')
		const section_tipo			= input_section_tipo.value
		if (!section_tipo || section_tipo.length<3) {
			alert("Error. Empty section_tipo");
			input_section_tipo.focus()
			return false
		}

		// ar_section_id
		const input_section_id		= button_node.parentNode.querySelector('.input_section_id')
		const section_id			= input_section_id.value
		if (!section_id || section_id.length<1) {
			alert("Error. Empty section_id");
			input_section_id.focus()
			return false
		}

		// Inject button_node dataset vars
		button_node.dataset.section_tipo	= section_tipo
		button_node.dataset.section_id		= section_id


		return this.trigger_call(button_node, event)
	}//end delete_sections
	...
	*/
	...
	this.trigger_call = function(obj, event) {
		...
		// HTTPX Request
		const js_promise = common.get_json_data(tool_administration.url_trigger, trigger_vars).then(function(response) {
			if(SHOW_DEBUG===true) {
	...
	* @return
	*/
	this.export_structure_to_json = function(obj, event) {

		const ar_tld = document.getElementById('ar_tld').value
		if (!ar_tld || ar_tld.length<1) {
			alert("Empty field ar_tld");
			return false
		}

		if( !confirm('!!! WARNING !!! \nAre you sure to EXPORT and to overwrite current structure JSON file "structure.json" ?\n') ) {
			return false;
		}

		// Inject obj dataset vars
			obj.dataset.tld_list = ar_tld

			console.log("export_structure_to_json obj.dataset:",obj.dataset);
	...
	* @return
	*/
	this.import_structure_from_json = function(obj, event) {

		const ar_tld = document.getElementById('import_ar_tld').value

		const tld_list = (!ar_tld || ar_tld.length<1) ? 'All' : ar_tld

		if( !confirm('!!! WARNING !!! \nAre you sure to IMPORT structure data form JSON file and to overwrite DATABASE data of selected tld list? \n\n'+tld_list+'\n') ) {
			return false;
		}
		...
		// Inject obj dataset vars
			...
			console.log("import_structure_from_json obj.dataset:",obj.dataset);
	...
	* @return promise
	*/
	this.build_install_version = function(obj, event) {

		if( !confirm('!!! WARNING !!! \nAre you sure to export current database and build a install version?\n') ) {
			return false;
		}
	...
	* @return promise
	*/
	this.update_json_ontology_items = function(obj, event) {

		if( !confirm('!!! WARNING !!! \nAre you sure to update all JSON ontology items?\n') ) {
			return false;
		}
