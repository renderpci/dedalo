...
var tool_administration = new function() {
	...
	this.trigger_call = function(obj, event) {
		...
		var mydata = {};
			...
			if (typeof mydata.mode==='undefined') {
				console.log("Error on get data vars. Mode is not defined in element dataset");
				return false
			}
		...
		var trigger_vars = mydata
		var js_promise 	 = common.get_json_data(tool_administration.url_trigger, trigger_vars).then(function(response) {
			console.log(response);

			if (response===null) {
				wrap_div.innerHTML = "<div>Null value was received. <br>Is possible that server timeout is done and the update process continues on server side, or what an error has occurred (See console for details about)</div>"
			}else{
	...
	*/
	this.move_component_data = function(obj, event) {
		...
		/*
		var wrap_div = document.getElementById('move_component_data');
		...
		.fail(function(error_data) {
			...
			var msg = "<span class='error'>ERROR: on move_component_data: " + error_data.responseText + " (Ajax error)</span>";
			alert(msg);
