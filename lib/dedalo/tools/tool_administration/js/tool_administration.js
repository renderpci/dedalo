...
var tool_administration = new function() {
	...
	*/
	this.update_version = function( update_version ) {
		...
		var wrap_div = document.getElementById('update_version_response');
			wrap_div.innerHTML = "<span class=\"css_spinner blink\">Processing. Please wait..</span>";
			//wrap_div.innerHTML = "<span class=\"css_spinner_DES blink\">Processing. Please wait..</span>";
			html_page.loading_content( wrap_div, 1 );
		...
		.done(function(received_data) {
			try {
				...
				if (received_data_obj) {
					...
					wrap_div.innerHTML = "<div>"+received_data_obj+"</div>";
				}
			} catch (e) {
				console.log(e);
				if (DEBUG) console.log(received_data);
	...
	*/
	this.skip_publication_state_check = function( check_box_obj ) {

		var mydata = {	'mode' : 'skip_publication_state_check',
						'value': check_box_obj.checked,
					 };
					//return console.log(mydata)		

		var wrap_div = document.getElementById('skip_publication_state_check_response');
			wrap_div.innerHTML = "<span class=\"css_spinner blink\">Processing. Please wait..</span>";
			html_page.loading_content( wrap_div, 1 );
