...
var dedalo_upload = new function() {
	...
	*/
	this.file_selected = function(input_file, event) {
		if(SHOW_DEBUG===true) console.log(event);
		...
		var valid_file = this.valid_file(input_file)
			if(valid_file!==true) return false;


		if (this.target_file_name===null || typeof this.target_file_name==="undefined") {
			// Use input file name whe no name is passed
	...
	*/
	this.upload_complete = function(e) {
		...
		if(SHOW_DEBUG===true) console.log("upload_complete")
	...
	*/
	this.upload_canceled = function() {
		if(SHOW_DEBUG===true) console.log("upload_canceled")
