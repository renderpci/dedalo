...
var tool_common = new function() {
	...
	*/
	this.open_tool_transcription = function ( button_obj ) {
		...
		// On close window, update component if changed
		transcription_window.onbeforeunload = function (e) {
		    
			var related_component_wrapper 	= document.querySelectorAll(".wrap_component[data-tipo='"+ transcription_window.component_related_obj_tipo +"']")[0]
			if (related_component_wrapper) {				
				// Update component text area	
				component_common.load_component_by_wrapper_id( related_component_wrapper.id )
				if (DEBUG) {
					console.log("--> Updating component "+ranscription_window.component_related_obj_tipo);
				};
			}else{
				console.log("WARNING: Unable update related component "+ transcription_window.component_related_obj_tipo + ". No found wrapper in DOM");
			}

			return 'Leaving tool trancriptions page..';
		}
