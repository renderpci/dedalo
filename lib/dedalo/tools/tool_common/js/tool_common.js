...
var tool_common = new function() {
	...
	this.open_tool_pdf_versions = function ( button_obj ) {
		...
		top.$("#dialog_page_iframe").dialog({
			...
			close: function(event, ui) {
	            ...
	            const component_related_obj = top.$('.css_wrap_pdf[data-parent='+parent+'][data-tipo='+tipo+']');
				...
				const matches = $('.pdf_in_list[data-parent='+parent+'][data-tipo='+tipo+']')//alert("Search for id_matrix: "+parent_matrix+ " matches: "+matches.length)
				if (matches.length>0) {
		...
		// open_tool_pdf_versions. Carga la url del iframe y abre el modal box (el iframe 'dialog_page_iframe' está al final de la página principal)		
	...
	*/
	this.open_tool_lang = function ( button_obj ) {		
		...
		const target_modo 		= 'tool_lang'
		const iframe_src		= DEDALO_LIB_BASE_URL + '/main/?m='+target_modo+'&t='+tipo+'&parent='+parent+'&section_tipo='+section_tipo+'&lang='+lang +tool_common.add_url_globals 	//return alert(iframe_src)	
