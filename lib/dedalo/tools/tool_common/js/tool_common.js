...
var tool_common = new function() {
	...
	* OPEN_TOOL_DOCU
	*/
	this.tool_docu_window = null
	this.open_tool_docu = function ( button_obj ) {		
		...
		// direct open window
			if (context_name==='online') {

				// case online documentation window https://dedalo.dev/ontology

				// const url = 'https://dedalo.dev/ontology/' + tipo
				const url = 'https://127.0.0.1:8443/web_dedalo/ontology/' + tipo
				if (this.tool_docu_window && !this.tool_docu_window .closed) {
					this.tool_docu_window.location = url
					this.tool_docu_window.focus()
				}else{
					const window_width		= 920
					const screen_width		= window.screen.width
					const screen_height		= window.screen.height
					this.tool_docu_window	= window.open(url, 'tool_docu_window', `left=${screen_width-window_width},top=0,width=${window_width},height=${screen_height}`)
				}

			}else{

				// case default. iframe to ontology section record inside this Dédalo instalation
				...
				const url			= DEDALO_LIB_BASE_URL + '/main/?' + build_url_arguments_from_vars(url_vars); // + tool_common.add_url_globals
				const dialog_id		= tipo + "_" + "dialog"
				const dialog_height	= ((window.innerHeight * 90) / 100) -32 -56 +26 + "px"
				...
				// dialog build
					const dialog = (function(){
						...
						const h4 = document.createElement("h4")
							...
							// Add
							h4.appendChild(document.createTextNode(get_label.tool_docu))
							header.appendChild(h4)

						const iframe = document.createElement("iframe")
							  iframe.src			= url
							  iframe.style.width	= "100%"
							  iframe.style.height	= dialog_height
	...
	*/
	this.open_tool_add_component_data = function(button_obj) {
		...
		const window_url		= DEDALO_LIB_BASE_URL + '/main/?m='+modo+'&t='+tipo+'&parent='+parent +'&section_tipo='+section_tipo + tool_common.add_url_globals;
		const window_name		= get_label.tool_add_component_data
		const window_options	= null
		...
		// var current_window = window.open(window_url, window_name, window_options)
		// 	console.log("current_window:",current_window);

		// current_window.onbeforeunload = function () {
		//     alert("123 current_window");
		// };

		// current_window.addEventListener("onclose", function(){
	...
	* OPEN TOOL_QR
	*/
	this.open_tool_qr = function ( button_obj ) {

		const url_vars = {
			m			: button_obj.dataset.tool_name || "tool_qr",
			t			: button_obj.dataset.section_tipo,
			button_tipo	: button_obj.dataset.tipo
		}
		
		// URL
		const window_url = DEDALO_LIB_BASE_URL + '/main/?' + build_url_arguments_from_vars(url_vars)
		
		// Window name
		const window_name		= "Tool " + button_obj.dataset.tool_name
		const window_options	= null
		// OPEN_TOOL_MAIN_WINDOW
		tool_common.open_tool_main_window(window_url, window_name, window_options)
	...
	* OPEN TOOL_METADATA
	*/
	this.open_tool_metadata = function ( button_obj ) {

		const url_vars = {
			m			: button_obj.dataset.tool_name || "tool_metadata",
			t			: button_obj.dataset.section_tipo,
			button_tipo	: button_obj.dataset.tipo
		}
	...
	* OPEN TOOL_QR
	*/
	this.open_tool_import_rdf = function ( button_obj ) {

		const url_vars = {
			m				: button_obj.dataset.tool_name || "tool_import_rdf",
			t				: button_obj.dataset.section_tipo,
