...
var tool_common = new function() {
	...
	*/
	this.open_tool_image_versions = function ( button_obj ) {
		...
		const iframe = document.createElement("iframe")
			iframe.src = iframe_src
			iframe.style.width = "100%"
			iframe.style.height = 880 + "px" // (screen.height -80) + "px"
	...
	*/
	...
	/**
	* OPEN TOOL CATALOGUE (OPEN DIALOG WINDOW)
	* Open TOOL CATALOGUE dialog window (from TOOL CATALOGUE button in inspector)
	*/
	//this.transcription_window = null
	this.open_tool_catalogue = function ( button_obj ) {
		
		const url_vars = {
				m 			 : button_obj.dataset.tool_name || "tool_catalogue",
				t			 : button_obj.dataset.section_tipo,
				tool_tipo 	 : button_obj.dataset.tool_tipo,
				id	 		 : button_obj.dataset.section_id,
				//context_name : button_obj.dataset.context_name || null,				
