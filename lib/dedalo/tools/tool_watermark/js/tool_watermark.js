...
// tool_watermark CLASS
var tool_watermark = new function() {
	...
	this.path
	this.url_trigger = DEDALO_LIB_BASE_URL + '/tools/tool_watermark/trigger.tool_watermark.php';
	...
	*/
	this.init = function(options) {
		...
		// debug
			if(SHOW_DEBUG===true) {
				console.log("->tool_watermark init options:",options);
			}
	...
	this.count_files = function(options) {
		...
		return new Promise(function(resolve){
			...
			common.get_json_data(url_trigger, trigger_vars).then(function(response) {
				if(SHOW_DEBUG===true) {
					console.log("[tool_watermark.count_files] response",response)
				}

				resolve(response.result)
			}, function(error) {
				console.log("[tool_watermark.count_files] error", error)
			})
		})
	};//end count_files
	...
	*/
	this.process_files = function(options) {
		...
		return new Promise(function(resolve){
			...
			common.get_json_data(url_trigger, trigger_vars).then(function(response) {
				if(SHOW_DEBUG===true) {
					console.log("[tool_watermark.process_files] response",response)
				}

				resolve(response.result)
			}, function(error) {
				console.log("[tool_watermark.process_files] error", error)
			})
		})
	};//end process_files
