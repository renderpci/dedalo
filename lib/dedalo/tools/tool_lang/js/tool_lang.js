...
var tool_lang = new function() {
		...
		if (self.inited!==true) {
			...
			window.ready(function(){
				...
				if (typeof last_target_lang !== 'undefined') {
					...
					const selector_obj 		= document.getElementsByClassName('tool_lang_selector_target')[0]; //$('.tool_lang_selector_target').first();
			...
			document.addEventListener("visibilitychange", function(event) {
				...
				const wrap_tool_lang_component_source 	= document.getElementById('wrap_tool_lang_component_source')
				const wrap_tool_lang_source 			= wrap_tool_lang_component_source.querySelector(".wrap_component")
				const source_lang 						= wrap_tool_lang_source.dataset.lang
				const locator = {
			...
			// bulk_translate
				const bulk_translate_container = document.getElementById("bulk_translate_container")
				self.render_bulk_translate_otions(bulk_translate_container)
	...
	*/
	this.render_component = function(selector_obj) {
		...
		// CHECK_EQUAL_LANGS
		const equal = this.check_equal_langs(selector_obj)
		if (equal===true) {
		...
		switch(role) {
			case 'selector_source':
					wrap_div = document.getElementById('wrap_tool_lang_component_source')
					break;
			case 'selector_target':
					wrap_div = document.getElementById('wrap_tool_lang_component_target')
					break;
		}
	...
	* GET_CURRENT_CONFIG
	*/
	this.get_current_config = function() {

		const self = this

		// wrap_tool_lang_source
			const wrap_tool_lang_component_source	= document.getElementById('wrap_tool_lang_component_source')
			const wrap_tool_lang_source				= wrap_tool_lang_component_source.querySelector(".wrap_component")

		// wrap_tool_lang_target
			const wrap_tool_lang_component_target	= document.getElementById('wrap_tool_lang_component_target')
			const wrap_tool_lang_target				= wrap_tool_lang_component_target.querySelector(".wrap_component")
			if( !wrap_tool_lang_target ) {
				document.querySelector(".tool_lang_selector_target").focus();
				alert( get_label.seleccione_un_idioma_de_destino || "Select a valid target lang!" );
				return false
			}
		...
		// source_lang
			...
			if (source_lang.length<5){
				alert("Error: source_lang lang is not valid: "+source_lang);
				return false
			}
		...
		// target_lang
			...
			if (target_lang.length<5){
				alert("Error: target lang is not valid: "+target_lang);
				return false
			}

		// component_tipo
			const component_tipo = wrap_tool_lang_target.dataset.tipo
			if (component_tipo.length<3){
				alert("Error: component_tipo (dataset.tipo) is not valid: "+component_tipo);
				return false
			}

		// component_tipo
			const section_tipo = wrap_tool_lang_target.dataset.section_tipo || 'oh1'
			if (section_tipo.length<3){
				alert("Error: section_tipo (dataset.section_tipo) is not valid: "+section_tipo);
				return false
			}

		// section_id
			const section_id = wrap_tool_lang_target.dataset.parent ;
			if ( typeof section_id==='undefined' || !section_id || section_id.length<1 ) {				
				return alert("Error: section_id (dataset.parent) is not valid: "+section_id);
				return false
			}
		...
		// config object
			const current_config = {
				...
				wrap_tool_lang_target	: wrap_tool_lang_target
			}
	...
	*/
	this.automatic_translation = function(button_obj) {		
		...
		const current_config = self.get_current_config()
		if (!current_config) {
			console.error("Error. Invalid current_config:", current_config);
			return false
		}

		const source_lang			= current_config.source_lang
		const target_lang			= current_config.target_lang
		const component_tipo		= current_config.component_tipo
		const section_id			= current_config.section_id
		const wrap_tool_lang_target	= current_config.wrap_tool_lang_target
		...
		const textarea	= wrap_tool_lang_target.querySelector("textarea")
		const input		= wrap_tool_lang_target.querySelector("input")
			if (textarea) {
		...
		const wrap_div		= wrap_tool_lang_target
		html_page.loading_content( wrap_div, 1 );
	...
	*/
	this.copy_source_text_to_target = function(btn_obj) {
		...
		// SOURCE
		var wrap_tool_lang_source 	= null
		...
		switch(true) {
			...
			case (wrap_tool_lang_component_source.querySelector(".text_area_content")!==null) :
				wrap_tool_lang_source 	= wrap_tool_lang_component_source.querySelector(".text_area_content")
				source_text 			= wrap_tool_lang_source.innerHTML
				break;
		}
	...
	this.fix_height_of_texteditor = function() {
		...
		for (var i = 0; i < len; i++) {
			if (warp_height>100 && tinyMCE.editors[i].theme) {
				...
				console.log("resixed editor to : ", warp_height);
			}
		}
	...
	*/
	...
	/**
	* RENDER_BULK_TRANSLATE_OTIONS
	* @return 
	*/
	this.render_bulk_translate_otions = function(bulk_translate_container) {
		
		const self = this				

		const button = common.create_dom_element({
			element_type	: 'div',
			class_name		: 'button css_button_generic ' + (self.total_records<1 ? ' unactive' : ''),
			inner_html		: 'Bulk translate [' + self.total_records +' records]',
			parent			: bulk_translate_container
		})
		button.addEventListener("click", exec_bulk_translate)
		function exec_bulk_translate(){

			// zero records selected
				if (self.total_records<1) {
					alert("Total records is zero! \nPlease select a valid range of records before make a bulk traslation");
					return
				}

			// warning. massive translation
				const msg = "WARNING!\nThis process could take a long time, depends of the amount of records you have filtered.\nAre you sure to bulk translate all selected records? " + self.total_records
				if (!confirm(msg)) {
					return false
				}
				if (self.total_records>10) {
					const msg = "DANGER!\nYou have selected "+self.total_records+" records to translate.\nAre you totally sure to continue? "
					if (!confirm(msg)) {
						return false
					}
				}

			// current_config . Current user langs selection
				const current_config = self.get_current_config()
				if (!current_config) {
					console.error("Error. Invalid current_config:", current_config);
					return false
			...
			const section_tipo		= current_config.section_tipo
			const component_tipo	= current_config.component_tipo
			const source_lang		= current_config.source_lang
			const target_lang		= current_config.target_lang
			...
			// verfy langs
				if (!source_lang || !target_lang || source_lang===target_lang) {
					console.error("Error. Invalid source_lang/target_lang:", source_lang, target_lang);
					return false
				}
			...
			// trigger call
				...
				const js_promise = common.get_json_data(self.trigger_url, trigger_vars)
				.then(function(trigger_response) {
	...
	};//end render_bulk_translate_otions
