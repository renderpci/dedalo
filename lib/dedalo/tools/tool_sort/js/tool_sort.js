					...
					for (let i = rows_length - 1; i >= 0; i--) {
						...
						// columns original duplicate
							const column_original_duplicate  = common.create_dom_element({
 							...
 							const current_data = result_query_object.result.data
 							
 							const discard = current_data.find(item => item.section_id === current_section_id && item.tipo === 'numisdata157')
 				
 							const discard_value = typeof discard.value[0] !=='undefined'
 								? discard.value[0].section_id 
 								: 0
	 						...
	 						if (discard_value === "1") {
	 							checkbox_original_label.classList.add('original');
	 						}
	 						...
	 						if (discard_value === "2") {
	 							checkbox_duplicate_label.classList.add('copy');
	 						}
	 						//const button_original_duplicate  = common.create_dom_element({
	 						//		element_type	: 'input',
	 						//		type 			: 'button',
