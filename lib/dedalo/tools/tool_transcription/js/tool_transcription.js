document.write('<scr'+'ipt src="'+DEDALO_LIB_BASE_URL+'/tools/tool_lang/js/tool_lang.js" type="text/javascript"></scr'+'ipt>');
/*
	TRANSCRIPTION
*/
...
//window.onbeforeunload = function(){
	/*
	var save_arguments = {"callback" : tool_transcription.update_opener_component} 

	var current_text_area = $('.css_text_area').first();
	if ( $(current_text_area).length!=1 ) {		
		return alert("Error on save text. Nothing is saved! (component not found)")
	};			 
	window.component_text_area.Save( current_text_area, save_arguments );	
	*/
	var component_related_obj = window.opener.$(".css_text_area[data-tipo='"+component_related_obj_tipo+"']");
		//window.opener.console.log(component_related_obj);
	var wrapper_obj = window.opener.$(component_related_obj).parents('.wrap_component').first();
		//window.opener.console.log(wrapper_obj);
	var wrapper_id  = $(wrapper_obj).attr('id');
		//window.opener.console.log("Called component_common.load_component_by_wrapper_id with wrapper_id: "+wrapper_id);

	if( $(wrapper_obj).length == 1 ) {		
		// Update component text area	
		window.opener.component_common.load_component_by_wrapper_id( wrapper_id )
	}
