...
var tool_transcription = new function() {
	...
	*/
	this.pdf_automatic_transcription = function(button_obj) {
		...
		// short vars
			const source_tipo	= button_obj.dataset.source_tipo
			const target_tipo	= button_obj.dataset.target_tipo
			const section_id	= button_obj.dataset.section_id
			const section_tipo	= button_obj.dataset.section_tipo

		// target_component_wrap. Locate target component wrap on page for reload on finish
			const target_component_wrap = $('.wrap_component[data-parent='+page_globals._parent+'][data-tipo='+target_tipo+'][data-lang='+page_globals.dedalo_data_lang+']');
			if ( $(target_component_wrap).length<1) {
				return alert("Error on pdf_automatic_transcription: target_component_wrap not found.");
			}
			const target_wrapper_id = $(target_component_wrap).attr('id');
		...
		const trigger_url = this.trigger_tool_transcription_url

		const js_promise = common.get_json_data(trigger_url, trigger_vars).then(function(response) {
			if(SHOW_DEBUG===true) {
				console.log("[tool_transcription.pdf_automatic_transcription] response", response)
			}
			...
			if (response && response.result!==false) {
				...
				const msg = response.msg || "Error. null response is received" 
				alert("[tool_transcription.pdf_automatic_transcription] " + msg)
			}
			//html_page.loading_content( button_obj, 0 )
		}, function(error) {
			console.error("[tool_transcription.pdf_automatic_transcription] Failed get_json!", error)
			inspector.show_log_msg(" <span class='error'>Error: on pdf_automatic_transcription trigger response !</span> " + error);
			html_page.loading_content( button_obj, 0 );
		})//end js_promise
		...
		// 	inspector.show_log_msg(" <span class='error'>Error: on pdf_automatic_transcription trigger response !</span> "+error_data);
		// 	console.error("error_data:",error_data);
		// })
		// // ALWAYS
		// .always(function() {
		// 	// Spinner off
