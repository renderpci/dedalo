...
var tool_cataloging = new function() {
	...
	*/
	...
	this.on_dragstart = function(obj, event) {
		console.log("tool_cataloging on_dragstart !");
		obj.ondrop = null;
		...
		if (handle) {
			...
			// data_obj
				const data_obj = {
						mode 	: "tool_cataloging",
						locator : { section_id 	 : obj.dataset.section_id,
								 	section_tipo : obj.dataset.section_tipo
								  }
					}				
			event.dataTransfer.setData('application/json', JSON.stringify(data_obj));
			//event.dataTransfer.setData('text/plain', obj.innerHTML);			
		} else {
			event.preventDefault();
		}
	...
	* @return 
	*/
	this.set_new_thesaurus_value = function(response) {
		//console.log("+++ [tool_cataloging.set_new_thesaurus_value] response:",response)

		const self = this
	 	
	 	// set autocomplete value as locator object
	 		const section_id 	 = response.result // from trigger ts_object
	 		const dato			 = [response.locator]
	 		const section_tipo 	 = self.new_thesaurus_value.section_tipo
		...
		// set_new_thesaurus_value trigger vars
			const trigger_url  = self.trigger_tool_cataloging_url
			const trigger_vars = {
				...
				//console.log("[tool_cataloging.set_new_thesaurus_value] trigger_vars",trigger_vars); return

		// json get call
			// Return a promise of XMLHttpRequest
			const js_promise = common.get_json_data(trigger_url, trigger_vars).then(function(trigger_response) {
					if(SHOW_DEBUG===true) {
						console.log("[tool_cataloging.set_new_thesaurus_value] trigger_response:",trigger_response)
					}
					...
					// ok
						...
						// children_element. list_thesaurus_element of current wrapper
							...
							if(!children_element) {
								console.warn("[tool_cataloging.set_new_thesaurus_value] Error on find children_element 'link_childrens'");
								return false
							}
						...
						// refresh childrens container 
							ts_object.get_childrens(children_element).then(function(){
								...
								// open thesaurus portal to show (for example numisdata679)
									setTimeout(function(){
										...
										if (button_show) {
											...
											console.error("[tool_cataloging.set_new_thesaurus_value] Unable locate button_show tipo: ",icon_tipo );
										}
									},10)
							})
				...
				console.error("[tool_cataloging.set_new_thesaurus_value] error:",error)			
			})
