...
var tool_cataloging = new function() {
	...
	*/
	...
	this.init = function(options) {
		...
		if(SHOW_DEBUG===true) {
			console.log("[tool_cataloging.init] options_obj:",options_obj);
		}
	...
	*/
	this.load_section = function(section_tipo){
		...
		// First search with previous user search options
			search2.search(null, search2.get_search_query_object()).then((a)=>{
				console.log("a:",a);
	...
	*/
	this.load_section_records = function( result_query_object ){
		//console.log("******************** result_query_object:",JSON.stringify( result_query_object.result) );
		...
		// debug
			if(SHOW_DEBUG===true) {
				console.log("[tool_cataloging.load_section_records] result_query_object: ",result_query_object);
			}
	...
	this.check_used = function(row) {
		...
		relation_list.load_relation_list_data({
			...
			if(SHOW_DEBUG===true) {
				...
				//console.log("[tool_cataloging.check_used] response:",response);
			}
	...
	*/
	...
	this.on_dragstart = function(obj, event) {
		if(SHOW_DEBUG===true) {
			//console.log("tool_cataloging on_dragstart ! , obj",obj);
		}
					...
					// ok
						// link_childrens_element. list_thesaurus_element of current wrapper
							...
							if(!link_childrens_element) {
								console.warn("[tool_cataloging.set_new_thesaurus_value] Error on find link_childrens_element 'link_childrens'");
								return false
							}
								...
								// open thesaurus portal to show (for example numisdata679 - coins)
									setTimeout(function(){
										...
										if (button_show) {
											...
											console.warn("[tool_cataloging.set_new_thesaurus_value] Unable locate button_show tipo: ",icon_tipo, " section_tipo:",section_tipo, " section_id:", section_id );
										}
									},100)
							})
