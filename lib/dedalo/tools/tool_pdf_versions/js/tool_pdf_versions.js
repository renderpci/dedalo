/* global get_label, page_globals, SHOW_DEBUG, build_url_arguments_from_vars, DEDALO_LIB_BASE_URL, html_page, Promise */
...
var tool_pdf_versions = new function() {
	...
	*/
	this.download_file = function(button_obj) {
		...
		// short vars
			const pdf_id				= button_obj.dataset.pdf_id
			...
			// const extension			= button_obj.dataset.extension

		// confirm dialog
			if(!confirm( get_label.descargar + ' ' + pdf_id + ' ['+quality+']' )) {
				return false;
			}
		...
		// const url = this.url_trigger + '?mode=download_file&pdf_id='+pdf_id +'&quality=' + quality +'&aditional_path=' + aditional_path + '&initial_media_path=' + initial_media_path + '&parent=' + parent +'&top_tipo='+page_globals.top_tipo;
		const url = this.url_trigger + '?' + build_url_arguments_from_vars(url_vars)
		// console.log('url:', url); return

		// new window
		const new_window = window.open(url, get_label.descargar);

		if(SHOW_DEBUG===true) {
			console.log("->dowload_file vars:" +pdf_id+' '+quality);
		}
	...
	*/
	this.delete_version = function(button_obj) {
		...
		const wrap_div = $(button_obj).parents('.wrap_tool:first');
		html_page.loading_content( wrap_div, 1 );
