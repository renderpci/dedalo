...
var tool_upload = new function() {
	...
	this.uploadFile = function() {
		...
		*/
		...
		try {
			...
			// hide button submit
			if(!DEBUG) document.getElementById('btn_upload').style.display = 'none';
	...
	this.uploadComplete = function(evt) {
		...
		if (typeof response.update_components != 'undefined') {
			...
			for (var i = 0; i < response.update_components.length; i++) {
		...
		if ( typeof SID!=="undefined" ) {
			...
			jQuery(document).ready(function(){		
				...
				if(media_type=='image') {
					...
					window.opener.location.reload();					
					//window.opener.console.log("opener 77777");
