					...
					// try {
						...
						// check response
							if (!evt.target.response) {
								console.error("Error in XMLHttpRequest load response. Invalid response is received [1]");
								console.log('evt.target.response', evt.target.response);
								alert('Error in XMLHttpRequest load response. Invalid response is received [1]')
								return false
							}

						// try_parse
							try {
								const try_parse = JSON.parse(evt.target.response);
							} catch (error) {
								console.error(error)
								alert('Error parsing evt.target.response: ' + error)
								return false
							}
						...
						// trigger response
							...
							if (!api_response) {
								console.error("Error in XMLHttpRequest load response. Invalid response is received [2]");
								alert('Error in XMLHttpRequest load response. Invalid response is received [2]')
								return false
							}
