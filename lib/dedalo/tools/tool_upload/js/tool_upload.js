...
var tool_upload = new function() {
	...
	*/
	this.uploadFile = function() {
						...
						if(file_data.chunked){
							...
							if(count_uploaded.length === total_chunks){
								...
								tool_upload.join_chunked_files({
									...
									tool_upload.uploadProgress(api_response)
		...
		// process_file(file)
		// send()
console.log("DEDALO_UPLOAD_SERVICE_CHUNK_FILES:",DEDALO_UPLOAD_SERVICE_CHUNK_FILES);
		switch (true) {
	...
	*/
	this.join_chunked_files = function(options) {
		...
		// options
			...
			const url_trigger = tool_upload.url_trigger + '&mode=join_chunked_files'
		// rqo
			const trigger_vars = {
				// mode 			: 'join_chunked_files',
				file_data		: file_data,
				files_chunked	: files_chunked,
				tipo			: tipo,
				parent			: parent,
		...
		// call to the API, fetch data and get response
			return new Promise(function(resolve){
				...
				common.get_json_data(url_trigger, trigger_vars)
				.then(function(response){
					if(SHOW_DEVELOPER===true) {
						console.log("-> get_system_info API response:",'DEBUG',response);
					}
					resolve(response)
				})
			})
