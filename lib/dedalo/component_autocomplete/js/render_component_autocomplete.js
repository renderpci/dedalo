...
render_component_autocomplete.prototype.edit = async function(ar_section_record) {
	...
	// input 
		...
		content_data.addEventListener("click", function(e){
			...
			if(content_data.dataset.active === "false"){
				const autocomplete = new service_autocomplete()
				const options = {
					caller			: self,
					wrapper 		: wrapper
				}
				autocomplete.init(options)
				content_data.dataset.active = true
			}
...
//		});
	...
	// add all nodes
		...
		for (let i = 0; i < length; i++) {
			...
			//const child = ar_section_record[i].node
			const child = await ar_section_record[i].render()
			content_data.appendChild(child)
		}
