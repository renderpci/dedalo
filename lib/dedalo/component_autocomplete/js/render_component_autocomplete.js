...
*/
render_component_autocomplete.prototype.edit = async function(options={
		render_level : 'full'
	}) {
	...
	wrapper.addEventListener("click", function(e){
		...
		// remove
			if (e.target.matches('.button.remove')) {
				...
				self.remove_value(e.target, e.target.previousElementSibling.textContent, false)
				.then(()=>{
					...
					// update offset
						self.pagination.offset = self.get_last_offset()
		...
		// change_mode
			else if (e.target.matches('.button.change_mode')) {
					alert(`change_mode clicked `);
				//self.change_mode()
		...
		// activate service. Enable the service_autocomplete when the user do click
			else if(self.autocomplete_active===false){
...
const content_data_edit = async function(self) {
	...
	//console.log("autocomplete ar_section_record:",ar_section_record);
	// content_data
	...
	// build values (add all nodes from the rendered_section_record)
		const length = ar_section_record.length
		for (let i = 0; i < length; i++) {
			await input_element(ar_section_record[i], inputs_container)
