...
var component_autocomplete = new function() {	
	...
	*/
	this.new_element = function(button_obj, id_wrapper) {
		...
		if( wrap_div.querySelectorAll('[data-type="component_autocomplete_new_element"]').length>0 ) {		
			...
			for (var i = ar_ajax_container.length - 1; i >= 0; i--) {
	...
	this.submit_new_element = function(button_obj) {
		...
		var component_autocomplete  = this,
			...
			hidden_input 			= wrap_div.querySelectorAll('[data-type="autocomplete_dato_hidden"]')[0],	//$(wrap_div).find('.css_autocomplete_dato_hidden');
 			ar_inputs 				= ajax_container.querySelectorAll('input[type=text]')		
		...
		var ar_data  = {},
			is_empty = false;		
		for (var i = ar_inputs.length - 1; i >= 0; i--) {
	...
	*/
	...
	this.activate = function(input_obj) {
		...
		obj_warp = document.getElementById(input_obj.dataset.id_wrapper);    	
		...
		$(input_obj).each(function() {
			...
			var tipo					= $(this).data('tipo'),
				ar_target_section_tipo	= $(this).data('ar_target_section_tipo'),
				...
				div_valor 				= $(this).parent().find('div.css_autocomplete_valor');					
					...
					//console.log(ar_target_section_tipo); return;	
			...
			.autocomplete({
				...
				source: function( request, response ) {
			        ...
			        $.ajax({
			          ...
			          data: {
			            'mode' 					 : "autocomplete",
			            'tipo'					 : tipo,			            
			            'ar_target_section_tipo' : JSON.stringify(ar_target_section_tipo),
			    ...
			    select: function( event, ui ) {			    	
					...
					switch(page_globals.modo){
						...
						case 'edit':
							// Edit Save value 
							component_autocomplete.Save(hidden_input[0]);	// PASAR A JAVASCRIPT PURO
	...
	*/
	this.open_element = function(button_obj) {
		...
		var window_url 		= '?t='+tipo+'&id='+id+'&m='+modo+'&menu='+menu,
			window_name 	= 'Edit element '+tipo+' '+id,
			w_width			= screen.width,	
			w_height		= screen.height
			//return 	console.log(window_url);

		var edit_elemet_window = window.open(window_url,window_name,'status=yes,scrollbars=yes,resizable=yes,width='+w_width+',height='+w_height);
