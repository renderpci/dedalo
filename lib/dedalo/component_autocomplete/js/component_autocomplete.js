...
* @return promise
*//*
component_autocomplete.prototype.render__DES = async function() {
	...
	// iterate section records
		...
		for(const current_section_tipo in grouped_sections) {
			...
			const current_context 	= self.datum.context.filter(element => element.section_tipo===current_section_tipo)
				//console.log("---autocomplete rows value:",current_value);
			...
			const process_locators = async function (current_value) {
				...
				for (const item of current_value) {
					...
					const current_section_record = await instances.get_instance({
							...
							key_suffix 			: 'autocomplete_' + self.tipo +'_'+ self.section_id, // note this value affects the instance id
		...
		//console.log("+++autocomplete ar_section_record:",ar_section_record, self.section_id);
	...
	// render using external proptotypes of 'render_component_autocomplete'
		...
		switch (mode){
			case 'list':
				...
				// debug
				if (self.node) {
					console.error("XXX WARN: Already exists node in autocomplete mode list!", self.node, list_node);
				}
				//if (self.node) {
				//	// replace old node
...
* @return promise
*//*
component_autocomplete.prototype.render_OLD = async function() {
