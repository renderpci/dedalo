...
component_autocomplete.prototype.build  = async function(autoload=false){
	...
	// debug
		if(SHOW_DEBUG===true) {
			console.log("+ component_autocomplete. Time to build:",performance.now()-t0);
		}	
...
component_autocomplete.prototype.add_value = async function(value) {
	...
	// rebuild and save the component
		const js_promise = self.save().then(async api_response => {
			...
			// offset_last
				...
				self.pagination.offset = offset_last
