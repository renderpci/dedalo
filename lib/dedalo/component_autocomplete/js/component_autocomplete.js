...
var component_autocomplete = new function() {
	...
	*/
	this.init_filter_by_list = function(options) {
		...
		this.set_filter_sections(options.wrapper, options.cookie_name)
	...
	*/
	this.select_component = function(obj_wrap) {
		...
		const search_field 					= component_options_container.querySelector(".autocomplete_wrapper")
		if(buttons_and_fields) {
			buttons_and_fields.style.display = "block";
		}
		if(component_options_container){
			component_options_container.classList.add("component_options_container_active")
			const input_search_field = search_field.querySelector("input")
	...
	*/
	this.save_filter_sections = function(input_obj, cookie_name) {
		...
		const cookie_create = createCookie(cookie_name, filter_sections, 365);
