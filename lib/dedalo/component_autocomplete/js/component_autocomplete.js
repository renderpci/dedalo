...
var component_autocomplete = new function() {	
	...
	*/
	this.Save = function(component_obj) {
		...
		component_autocomplete.wrapper_id = $(component_obj).parents('.css_wrap_autocomplete').first().attr('id');
			//console.log(component_autocomplete.wrapper_id);	
		...
		jsPromise.then(function(response) {
		  	...
		  	component_autocomplete.refresh_component(null)
		}, function(xhrObj) {
		  	console.log(xhrObj);
		});
	...
	*/
	this.submit_new_element = function(button_obj) {
		...
		var ajax_container  		= this.ajax_container,
			...
			hidden_input 			= $(wrap_div).find('.css_autocomplete_dato_hidden');
		...
		.done(function(received_data) {
			...
			if (received_data.indexOf("Error")!=-1 || received_data.indexOf("error")!=-1 || received_data.indexOf("Failed")!=-1) {
				...
				div_valor.html( div_valor.html() + "<li><div class=\"icon_bs link css_autocomplete_button_delete\" \
					data-current_value='"+received_data+"' onclick=\"component_autocomplete.delete(this)\"></div>"+ label + "</li>" )
				*/
	...
	*/
	this.delete = function(btn_obj) {
		...
		var input_text_hide = $(btn_obj).parents('.content_data').first().find('.css_autocomplete_dato_hidden:input');
