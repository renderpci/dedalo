...
var component_autocomplete = new function() {
	...
	*/
	this.Save = function(component_obj, request_options) {
		...
		if(wrap_div.dataset.modo!=='edit' && wrap_div.dataset.modo!=='portal_list' && wrap_div.dataset.modo!=='tool_add_component_data') {
		...
		const js_promise = component_common.Save(component_obj, self.save_arguments).then(function(response) {
				...
				if (save_options.reload===true) {
					...
					// Exec mandatory test
					component_autocomplete.mandatory(wrap_div.id)
				}
	...
	*/
	this.open_element = function(button_obj) {
		...
		let window_url  = '?t=' + tipo + '&id=' + id + '&m=' + modo + '&menu=0'
		const window_name = 'Edit element ' + tipo + ' ' + id

		// let float_window_features = page_globals.float_window_features.small
		...
		if(typeof external_data !== 'undefined'){
			...
			if (external_engine) {
				...
				// float_window_features		= 'menubar=no,location=no,resizable=yes,scrollbars=yes,status=no,width=1200,height=860'
			}
		}else{
	...
	*/
	this.mandatory = function(id_wrapper) {
		...
		const component_ul = wrap_div.querySelector('ul.css_autocomplete_valor')

		if(!component_ul) return false;
		// Component dataset mandatory info
		const mandatory = JSON.parse(component_ul.dataset.mandatory)
		if (!mandatory || mandatory!==true) return false;

		if (this.is_empty_value(wrap_div)===true) {
			component_ul.classList.add('mandatory')
			
		}else{
			component_ul.classList.remove('mandatory')
		}
