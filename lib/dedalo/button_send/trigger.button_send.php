...
throw new Exception("Error Processing Request: EN PROCESO DE CONVERSIÓN A POSTGRESQL JSON", 1);
...
# NEW	
	...
	if(count($ar_result)>0) {
		...
		$section_obj 		= section::get_instance($id_matrix, $tipo);
	...
	$RecordObj_dd		= new RecordObj_dd($section);
	$hijos 				= $RecordObj_dd->get_ar_recursive_childrens_of_this($section);	#var_dump($hijos);	
	...
	if (is_array($hijos)) foreach ($hijos as $terminoID) {
		...
		switch($modelo) {
			...
			case 'filter_master' :
						...
						$user_actual 			= navigator::get_user_id();			
						$ar_ts_relacionados		= RecordObj_dd::get_ar_terminos_relacionados($terminoID);
						...
						if( is_array ($ar_ts_relacionados) ) foreach($ar_ts_relacionados as $ar_termino_relacionadoID) {
							...
							foreach($ar_termino_relacionadoID as $modeloID => $terminoID) {
								...
								if ($modelo =='filter_key'){
									...
									$ar_ts_relacionados_key		= RecordObj_dd::get_ar_terminos_relacionados($terminoID);
									if( is_array ($ar_ts_relacionados_key) ) foreach($ar_ts_relacionados_key as $ar_termino_relacionadoID_key){
										foreach($ar_termino_relacionadoID_key as $modelo_keyID => $termino_keyID) {
											$modelo_key			= RecordObj_dd::get_termino_by_tipo($modelo_keyID); 
