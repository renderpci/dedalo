...
var component_image_read = new function() {
	...
	*/
	this.init = function(options) {
		...
		let wrapper_id 			= options.id_wrapper
		let image_url  			= options.image_url
		let original_img_height = options.original_img_height
	...
	*/
	this.raster_canvas = function(options) {
		...
		let wrapper_obj  	= document.getElementById(options.wrapper_id)
		let loading_image 	= wrapper_obj.querySelector('.loading_image')
		
		// Hide canvas while is gererated
		canvas_obj.style.visibility = 'hidden'

		// Set canvas initial size from image dimensions
		canvas_obj.height = options.image.naturalHeight
		canvas_obj.width  = options.image.naturalWidth
		...
		let current_paper 	 = ar_Papers[canvas_id]
			current_paper.setup(canvas_obj)

			// Set paper ratio
			current_paper.ratio	= canvas_obj.height / options.original_img_height
			
			// Reset ar_tag_loaded
			current_paper.ar_tag_loaded = new Array();		
		
		// raster image
	    let raster = new current_paper.Raster({
	        source   : options.image_url,
	        position : current_paper.view.center    
	    });	  

		raster.onLoad = function(e) {

			let height  	 = current_paper.view.size._height
  			let image_height = raster.height
  			let ratio 		 = height / image_height
