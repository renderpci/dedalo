...
class rows_paginator {

	public $section_list_obj;
	public $modo;

	function __construct(section_list $section_list_obj, $modo) {
	...
	public function get_html() {
		...
		$html =  ob_get_clean();
		$this->section_list_obj->rows_obj->generated_time['rows_paginator'] = round(microtime(1)-$start_time,6);
...
class rows_paginator_OLD extends common {
	...
	public static function get_maxRows() {
		...
		if(isset($_REQUEST["max_rows"]) && $_REQUEST["max_rows"]>0) {
			...
			$_SESSION['dedalo4']['config']['max_rows'] = $_REQUEST["max_rows"];
			return intval($_REQUEST["max_rows"]);			
		}
		$max_rows = $_SESSION['dedalo4']['config']['max_rows'];
		return $max_rows;
	}
