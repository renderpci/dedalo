...
class rows_search extends common {
	...
	function __construct( $section_obj, $modo="search" ) {
	...
	*/
	public function get_html() { // Aprox 100 ms
		...
		# CACHE	
		#unset($_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['html']);
		if(!SHOW_DEBUG && isset($_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['html'])) {
			return $_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['html'];
		}
		...
		# CACHE
		$_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['html'] = $html;
	...
	*/
	protected function get_ar_search_fields() {
		...
		# CACHE	
		#unset($_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['ar_search_fields']);
		#if(isset($_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['ar_search_fields'])) {
		#	return unserialize($_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['ar_search_fields']);
		#}
		...
		foreach ($ar_terminos_relacionados as $current_element_tipo) {
			...
			if( strpos($current_modelo_name, 'component_')!==false ) {
				...
				$ar_search_fields[$current_element_tipo] = component_common::get_instance($current_modelo_name, $current_element_tipo, NULL, 'search', DEDALO_DATA_LANG);	#($tipo=NULL, $parent=NULL, $modo='edit', $lang=DEDALO_DATA_LANG)
		...
		# CACHE
		#$_SESSION['dedalo4']['config']['rows_search'][$this->section_tipo]['ar_search_fields'] = serialize($ar_search_fields);
	...
	*/
	public function get_ar_tools_search() {
		...
		#if(isset($_SESSION['dedalo4']['config']['rows_search'][$section_tipo]['ar_tools_search'])) {
			#return unserialize($_SESSION['dedalo4']['config']['rows_search'][$section_tipo]['ar_tools_search']);
		#}
		...
		foreach ($ar_childrens as $current_element_tipo) {
			...
			switch (true) {
				...
				case ( strpos($current_modelo_name, 'component_')!==false ) :
					$current_element 	 = component_common::get_instance($current_modelo_name, $current_element_tipo, NULL, 'search');
					break;
			}
		...
		# CACHE
		#$_SESSION['dedalo4']['config']['rows_search'][$section_tipo]['ar_tools_search'] = serialize($ar_tools_search);
