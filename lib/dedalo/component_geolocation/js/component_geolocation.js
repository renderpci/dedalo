...
var component_geolocation = new function() {
	...
	this.Save = function() {
		...
		this.save_arguments = {
			'dato' : {
				'lat' : this.field_lat.val(),
				'lon' : this.field_lon.val(),
				'zoom' : this.field_zoom.val()
			}			
		} 
		//this.save_arguments.dato = JSON.stringify(this.save_arguments);
		//return console.log( this.save_arguments );
		//return;
		//console.log( this.field_lat.val() );
		//alert("under construction")
		
		// Exec general save
		component_common.Save(this.field_lat, this.save_arguments);
		/*
	...
	*/
	...
	this.init_map = function( div_container_id ) {		
		...
		$(function() {
			...
			//var container document.getElementById(div_container_id).parentNode.getElementsByName("lat")
			var field_lat = $('#'+div_container_id).parent().find("input[name=\"lat\"]");//content_data.getElementsByName("lat");
			var field_lon = $('#'+div_container_id).parent().find("input[name=\"lon\"]");
			var field_zoom = $('#'+div_container_id).parent().find("input[name=\"zoom\"]");
			//var field_lat = container.getElementsByTagName("lat");
			//console.log(field_lon);

			geolocation_obj.field_lat = field_lat;
			geolocation_obj.field_lon = field_lon;
			geolocation_obj.field_zoom = field_zoom;

			//console.log(geolocation_obj);
			...
			map.on('dragend', function(){
				field_lat.val(map.getCenter().lat);
				field_lon.val(map.getCenter().lng);
			...
			field_lat.on('change',function(){
				map.panTo(new L.LatLng(field_lat.val(), field_lon.val()));				
			})

			field_lon.on('change',function(){
				map.panTo(new L.LatLng(field_lat.val(), field_lon.val()));
			})

			field_zoom.on('change',function(){
				map.setZoom(field_zoom.val());
			})
