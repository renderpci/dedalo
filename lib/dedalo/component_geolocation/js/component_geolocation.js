...
var component_geolocation = new function() {	
	...
	*/
	this.Save = function() {
		...
		// Exec general save
		var jsPromise = component_common.Save(component_geolocation.field_lat, component_geolocation.save_arguments);
	...
	this.save_draw_data = function() {
		...
		var current_draw_data = JSON.stringify(draw_data.toGeoJSON());
			if(DEBUG) {	
				...
				console.log(draw_data.toGeoJSON());
			}

		var tag_obj 		= ar_tag_loaded[current_editable_FeatureGroup_id]
		var	parts_of_tag 	= component_geolocation.get_parts_of_tag(tag_obj)
		var	tag_id 			= parts_of_tag.capaId
		var	tagState 		= parts_of_tag.tagState
			//data 			= current_draw_data;
		var	current_draw_data = replaceAll('"', '\'', current_draw_data)
			new_data_obj = {
				data : current_draw_data
			}
				console.log(current_draw_data);
		...
		//component_text_area.update_geo_tag(tag_id, tagState, data, related_tipo);
		component_text_area.update_tag( tag_obj, new_data_obj, related_tipo, true);
	...
	*/
	...
	/**
	* RENDER_TAGS
	* load all tags information into the map
	*/
	this.render_tags = function(){		

		var ar_related_tipo = JSON.parse(this.related_tipo)
		if(ar_related_tipo.length <= 0 ){
			return false
		}

		var ar_tags = component_text_area.get_tags(ar_related_tipo[0],'geo');
		if(ar_tags){
			var len = ar_tags.length
