...
var component_geolocation = new function() {
	...
	*/
	...
	this.init_map = function( div_container_id ) {
		...
		// vissible observer. 
		// When map is inside section tab and current tab is hidden, map needs to refresh when tab is activated by the user
			if (wrapper) {
	...
	*/
	...
	/**
	* LOAD_GEO_EDITOR
	* Load all data information of the current selected tag. Init the editor if it is not loaded.
	...
	*/
	...
	this.load_geo_editor = function(tag, all_tags) {
		...
		// OVERLAY : Lo añadimos al map overlay (Adds an overlay (checkbox entry) with the given name to the control)
	...
	* @return
	*/
	this.create_point = function(input) {
		...
		if(SHOW_DEBUG===true) {
		 	console.log("create_point: lat",lat," lon:",lon, " zoom:",zoom);
		}
		...
		// build and save the point into the text_area tag
		const text_area_node = document.querySelector('textarea[data-tipo="'+ ar_related_tipo[0]+'"]')

		const editor = tinymce.get(text_area_node.id);

		const geojson = "{'type':'FeatureCollection','features':[{'type':'Feature','properties':{},'geometry':{'type':'Point','coordinates':["+lon+","+lat+"]}}]}"

		const type		= 'geo'
		const tag_id	= component_text_area.get_last_tag_id(editor, type) + 1
		const state		= 'n'
		const label		= ''
		const data		= geojson

		const tag = component_text_area.build_dom_element_from_data(type, tag_id, state, label, data)

		// Insert html on editor
			editor.selection.setContent( tag.outerHTML, {format:'raw'} )

			editor.setDirty(true); // Set editor content as changed
			// editor.save(); // updates this instance's textarea
			// editor.isNotDirty = false; // Force not dirty state

			// const text_area_wrapper = document.querySelector('.css_wrap_text_area[data-tipo="'+ ar_related_tipo[0]+'"]')

			component_text_area.Save(text_area_node, null, editor)
