...
page.prototype.init = async function() {
	...
	const page_items = self.options.page_items
			...
			switch(item.model) {
				...
				case 'section':
					...
					// paginator
						...
						const paginator				= current_data_manager.count(sqo)
					...
					// item instance
						...
						current_instance.count = paginator
					// render item
						const item_node = await current_instance.render()
	...
	switch (this.model){
		case 'section':	
			...
			// data_manager. load_section_data. start load in parallel
				...
				// const total_obj 				= current_data_manager.count(sqo)
