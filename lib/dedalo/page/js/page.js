...
*/
const load_section = async function(self, delete_previous=false) {
	...
	if (delete_previous===true) {
		...
		//const section_to_remove = await instances.get_instance({
		//	model 			: 'section',
		//	tipo 			: self.section_tipo,
		//	section_tipo	: self.section_tipo,
		//	section_id		: self.section_id,
		...
		//	lang			: self.lang			
		//})
		//instances.remove_me(section_to_remove)
	}		
...
page.prototype.load_section_data_debug = async function(load_section_data_promise) {
	...
	.addEventListener("click", (e) => {
		self.context[0].offset = (self.context[0].offset>0) ? parseInt(self.context[0].offset) - 2 : 0
		console.log("self.context[0].offset:",self.context[0].offset);
