...
export const page = function (options) {
	...
	self.context = [
		...
		{ // component_input_text test159
			typo			: "ddo",
			tipo 			: 'test149', // potal test149,  autocomplete: test153
			section_tipo 	: 'test65',
			mode 			: this.mode ,
			lang 			: self.lang,
			parent			: 'test65',				
...
page.prototype.init = async function() {
	...
	switch (this.model){
		case 'section':				
			...
			// load_section_data start in parallel
			...
			import('../../section/js/section.js').then(function(result){
					 		 	
	 		 	// render section on load data
		 		 	load_section_data_promise.then(function(api_response){
		 		 		...
		 		 		// inject section datum
		 		 			const datum = api_response.result
		 		 			//console.log("[page.init] current_section:",current_section);

		 		 		// init with already loaded datum
		 		 			current_section.init(datum)
						
						// render section
							current_section.render()
							.then( section_node => {
				 		 		const test_container = document.getElementById("test_container")
				 		 		test_container.appendChild(section_node)
