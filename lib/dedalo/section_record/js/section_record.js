...
section_record.prototype.render = async function(){
	...
	// get the items inside the section of the record to render it
		const items		= (mode==="list") ? 
							...
							self.context.filter(element => element.section_tipo===section_tipo && (element.type==='grouper' || element.type==='component'))
	...
	// render using external proptotypes		
		switch (mode){
			case 'list':
				...
				// debug
				if (self.node) {
					console.warn("XXX WARN: Already exists node in section record mode list!",self.node,list_node);
				}
				// set
				self.node = list_node
				break
