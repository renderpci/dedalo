...
section_record.prototype.render = async function(){
	...
	//return document.createElement("div")
		console.log("--section_record render:",self);
	...
	// render all components and groupers
		...
		for (let i = 0; i < items_length; i++) {
			...
			// current_instance. create the instance options for build it, the instance is reflect of the context and section_id
				...
				// render_promises.push(current_instance)	
				const element = await current_instance.render()
				render_promises.push(element)

		}//end for


	// all render finished. when the all instaces are rendered we can create the nodes hierarchy
		//return Promise.all(render_promises).then( async function(ar_instances){

			const ar_instances = render_promises
				...
				switch (mode){
					case 'list':
						...
						if (self.node) {
							...
							//self.node.appendChild(list_node)
						}else{
							// set
							self.node = list_node
						}						
					...
					default :
						...
						if (self.node && self.node.parentNode) {
							...
							//self.node.appendChild(edit_node)
						}else{
							// set
							self.node = edit_node
						}						
