...
section_record.prototype.init = function(options) {
	...
	self.key_suffix 	 = options.key_suffix
			...
			for (const item of items) {
				...
				// current_instance. create the instance options for build it, the instance is reflect of the context and section_id
					const current_instance = await instances.get_instance({
						...
						data 			: current_data,
						datum 			: self.datum,
						key_suffix 		: self.key_suffix // note this value affects the instance id
					})
	...
	// render using external proptotypes		
		switch (mode){
			case 'list':
				...
				// debug
				if (self.node) {
					console.warn("XXX SECTION_RECORD WARN: Already exists node in section record mode list!",self.node, list_node);
				}
				// set
				self.node = list_node
				break
			...
			default :
				...
				// debug
				if (self.node) {
					console.warn("XXX SECTION_RECORD WARN: Already exists node in section record mode list!",self.node, edit_node);
				}
				// set
				self.node = edit_node
				break
