	...
	*/
	...
	this.Save = function(component_obj) {
		...
		var wrap_div = find_ancestor(component_obj, 'wrap_component')
			if (wrap_div === null ) {
				if(SHOW_DEBUG) console.log(component_obj);
	...
	*/
	this.delete = function(btn_obj) {
		...
		var wrap_div = find_ancestor(btn_obj, 'wrap_component')
			if (wrap_div === null ) {
				if(SHOW_DEBUG) console.log(btn_obj);
	...
	this.open_ts_window = function(button_obj) {
		...
		this.selected_wrap_div = find_ancestor(button_obj, 'wrap_component')
			if (this.selected_wrap_div === null ) {
				if(SHOW_DEBUG) console.log(button_obj);
	...
	*/
	...
	this.fire_tree_resolution_DES = function(ar_tipo_to_search) {
		...
		var mydata		= { 'mode': mode,
						...
						//if(SHOW_DEBUG) console.log(JSON.stringify(mydata))
		...
		if ( typeof(current_cookie)!='undefined' && current_cookie ) {			
			$(wrapper_id).html( current_cookie );
				//if(SHOW_DEBUG) console.log('->fire_tree_resolution. '+tipo_to_search+' cookie exists ('+cookie_name+')' );
			return null;
		};

		if(SHOW_DEBUG) console.log("->fire_tree_resolution calculating: "+tipo_to_search)
		...
		.done(function(received_data) {
			...
			if(SHOW_DEBUG) console.log('->fire_tree_resolution. '+tipo_to_search+' finish ' + received_data );
