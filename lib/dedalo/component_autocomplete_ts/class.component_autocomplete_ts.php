...
class component_autocomplete_ts extends component_common {
	...
	*/
	...
	public function set_dato($dato) {
		...
		if (!empty($dato)) {
			...
			if( empty($output_array) ) {
				throw new Exception("Error Processing Request. Invalid dato: '$dato' ", 1);			
			}
		}
	...
	public function get_ar_referenced_tipo() {
		...
		#dump($this->RecordObj_ts,'$this->RecordObj_ts');
		$tipo 			= $this->get_tipo();
		$RecordObj_ts 	= new RecordObj_ts($tipo);
		$ts_propiedades = $RecordObj_ts->get_propiedades();
		...
		if (!empty($propiedades->jer_tipo) && $propiedades->jer_tipo==2) {
			...
			$ar_parents = $RecordObj_ts->get_ar_parents_of_this($dato); ksort($ar_parents);
				...
				foreach ($ar_parents as $current_terminoID) {
					$current_termino = RecordObj_ts::get_termino_by_tipo($current_terminoID, DEDALO_DATA_LANG);
	...
	*/
	public static function get_tree_resolution($tipo) {
		...
		# No calculate tre for root tipo
		if($is_root==true) return null;

		#unset($_SESSION['config4']['ar_recursive_childrens'][$tipo]);		
		
		if(isset($_SESSION['config4']['ar_recursive_childrens'][$tipo])) {

			$ar_recursive_childrens = $_SESSION['config4']['ar_recursive_childrens'][$tipo];
			...
			$_SESSION['config4']['ar_recursive_childrens'][$tipo] = $ar_recursive_childrens;
		}
		
		#dump($_SESSION['config4']['ar_recursive_childrens'][$tipo]);
	...
	*/
	public static function autocomplete_ts_search($ar_referenced_tipo, $string_to_search, $max_results=30, $show_modelo_name=true) {
		...
		$matrix_table					= RecordObj_descriptors::get_matrix_table_from_tipo($ar_referenced_tipo[0]);
		...
		# ESDESCRIPTOR : Removome non desctiptors
		...
		*/
		...
		foreach ($ar_records as $current_terminoID) {
			...
			$arguments['tipo']				= 'termino';
			#$arguments['lang']				= DEDALO_DATA_LANG;
			$matrix_table					= RecordObj_descriptors::get_matrix_table_from_tipo($current_terminoID);	
			$RecordObj_descriptors			= new RecordObj_descriptors($matrix_table, NULL);
			...
			$modelo_name = NULL;
			if($show_modelo_name)
				$modelo_name = ' - '.RecordObj_ts::get_modelo_name_by_tipo($current_terminoID);
	...
	public static function autocomplete_ts_search_old($tipo, $string_to_search, $max_results=30) {		
		...
		if(!isset($ar_records)) {
			...
			foreach ($ar_recursive_childrens as $terminoID) {
				...
				$ar_records[$terminoID] = RecordObj_ts::get_termino_by_tipo($terminoID, DEDALO_DATA_LANG);
			}
