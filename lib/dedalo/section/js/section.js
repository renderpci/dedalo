...
var section = new function() {
		...
		const js_promise = new Promise(function(resolve, reject) {
			...
			// Header
				if (build_header===true) {
					// Get header_columns from context
						const header_columns = context.filter(item => item.type==='column_info')
			...
			// Rows
				// Group data by section_id
					const grouped_rows = common.group_by(rows_data, "section_id")

				// Iterate grouped rows
					for (const i in grouped_rows) {
						// Add row
							self.build_row(grouped_rows[i], container, edit_column)						
					}
	...
	* @return 
	*/
	this.build_row = function(grouped_row, container, edit_column) {
