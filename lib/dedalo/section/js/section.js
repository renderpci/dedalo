...
var section = new function() {
	...
	*/
	this.update_inspector_info = function () {
		...
		let it = document.getElementById('inspector_tools')
			if(it) it.innerHTML=''
		let id = document.getElementById('inspector_debug')
			if(id) id.innerHTML=''

		// Reset some content
		let ii = document.getElementById('inspector_indexations')
			if(ii) ii.innerHTML=''
		let irl = document.getElementById('inspector_relation_list_sections')
			if (irl) irl.innerHTML=''

		let target_obj = document.getElementById('inspector_info')	//$('#inspector_info'),			
					
			if (target_obj && wrap_section_obj.dataset.section_info!=="undefined") {				

				let section_info  = JSON.parse(wrap_section_obj.dataset.section_info)	//wrap_section_obj.data('section_info')	
	
				let label 					= typeof section_info.label != undefined ? unescape(decodeURI(section_info.label)) : null
				let section_id 				= section_info.section_id
				let created_date 			= section_info.created_date
				let created_by_user_name 	= section_info.created_by_user_name
				let modified_date 			= section_info.modified_date
				let modified_by_user_name 	= section_info.modified_by_user_name
				...
				let csidl = document.getElementById('current_section_id_label')
	...
	*/
	this.render_component_html = function( json_build_options ) { 
		//console.log(json_build_options);
		...
		// Component instance
		let component_obj_name = window[json_build_options.model_name]
		let component_instance = Object.create(component_obj_name) //json_build_options[model_name]			
		...
		let js_promise = component_instance.render_html()
...
/*
var component_test = new function() {
...
section.render_component_html({
	model_name 	 : 'component_test',
	section_tipo : 'oh1',
	section_id 	 : 1,
	lang 		 : 'lg-eng',
})
