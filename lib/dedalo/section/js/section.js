...
var section = new function() {
	...
	*/
	this.init = function() {
		...
		// Ready events
		//$(function() { // Revoved JQuery init 19-04-2019
				...
				case 'edit' :
					...
					// BODY CLICK RESET ALL SELECTED WRAPS
						...
						document.addEventListener("click", function(e){
							...
							// avoid trigger when in calendar date picker header
							const ancestor = find_ancestor(e.target, 'ui-widget-header')
							if (ancestor) {
	...
	this.parse_json_rows = function(options) {
		...
		const js_promise = new Promise(function(resolve, reject) {
			...
			// Header
				...
				//		const header_columns = context.filter(item => item.type==='component')
				//
				//	// Add row header
			...
			// Rows
				// Get array of existing section_tipo from context
					const ar_section_info = context.filter(current_item => current_item.type==="section")
	...
	*/
	this.build_row = function(section_ar_columns, container, context, data) {
		...
		if(SHOW_DEBUG===true) {
			//console.log("build_row:",section_ar_columns,container,context)
		}

		// Filter context. get section tipo to use in css selctor
			//const section_info = context.filter(item => item.type==="section")[0]
			//const section_tipo = section_info.section_tipo
		...
		// Row container
			const row_container = common.create_dom_element({
				...
				parent 			: container,
				dataset 		: { section_id 		: section_ar_columns[0].section_id,
									section_tipo	: section_ar_columns[0].section_tipo}
			})
		...
		// Iterate columns of current row
			...
			for (let j = 0; j < ar_columns_length; j++) {
				...
				// Component context / data
					const component_context = context.filter(item => item.type==="component" && item.tipo===column.tipo)[0]
					const component_data 	= column

				// Render value
					const model  = component_context.model
					if(model ==='component_select') continue;

					// Check available component function
						if( typeof window["render_" + model]!=="object" ) {
							console.error("[section.build_row] Ignored unavailable component script for ", "render_" + model)
							continue;
						}
					...
					// component function render call
			...
			}//end for (let j = 0; j < ar_columns_length; j++)
