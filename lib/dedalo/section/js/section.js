...
section.prototype.init = async function(options) {
	...
	// set ar_section_id
		...
		self.offset 		= section_data.offset
		self.limit 			= section_data.limit
		self.total 			= section_data.total
...
section.prototype.render = async function(){
	...
	// iterate records
		...
		for (let i = 0; i < ar_section_id_length; i++) {
			...
			// section_record
			const current_section_record = instances.get_instance({
	...
	// promise all 
		return Promise.all(ar_instances).then( async function(ar_section_record){
			...
			// event publish
				event_manager.publish('section_rendered'+self.id, self)
...
* @return 
*/
section.prototype.destroy = async function (){
	...
	//const current_instance = await instances.delete_instance({
	//	model 			: self.model,
	//	tipo 			: self.tipo,
	//	section_tipo 	: self.section_tipo,
...
section.prototype.render_paginator = async function() {
	...
	// desstroy the own instance for build the new one	
		self.destroy();

	//change the section with the new data
		const section_init	= await self.init(options)

	// render section
		const section 		= await self.render()
	...
	for (let i = 0; i < ar_node.length; i++) {
		...
		// replace the node with the new render
			parent_node.replaceChild(section, current_node)
	 		parent_node.classList.remove("loading")
	 		parent_node.classList.remove("hide")
 	}
