...
section.prototype.load_data = function() {
	...
	const self = this
		...
		const context = [search_query_object,dd_object_section_id,dd_object]//,dd_object2,dd_object3]
		//number
		//const context = [search_query_object2,dd_object,dd_object8,dd_object9,dd_object10]
		//email
	...
	// data_loader
		...
		const loaded = current_data_loader.load().then(function(response){
			if(SHOW_DEBUG===true) {
				console.log("[section.load_data] response:",response)
