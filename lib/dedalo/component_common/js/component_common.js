...
var component_common = new function() {
	...
	*/
	this.Save = function (component_obj, save_arguments, event) {
		...
		var event_detail = {dataset: cloneDeep( component_obj.dataset )};
			//console.log("[Save] event_detail (cloneDeep):",event_detail);
		//let detail_component_obj = component_obj.cloneNode(true);
		var save_event = new CustomEvent('component_save', {detail:event_detail})
		...
		if (typeof dato=="object" || typeof dato=="array") {			
			if(SHOW_DEBUG===true) {
				console.warn("[component_common:Save] dato is a object or array. Will be stringifyed, but please stringify objects before save")
			}
		...
		var js_promise = common.get_json_data(this.url_trigger, trigger_vars, this.save_async).then(function(response){
			if (SHOW_DEBUG===true) {
	...
	*/
	this.get_component_json_data = function(component_obj) {		
		...
		//console.log("[component_common:get_component_json_data] trigger_vars:",trigger_vars);
