			...
			for (var key in current_obj) {
				...
				if(key=='trigger') {
					...
					var trigger_functions = function(current_value) {
						...
						for (var i=0; i<ar_test_values.length; i++) {
							...
							var current_test_value = ar_test_values[i];
								//console.log(current_test_value==current_value)
							...
							if(current_test_value==current_value) {
								...
								for (var i = actions_len - 1; i >= 0; i--) {									
									...
									eval(current_action);
									if (DEBUG) {
										console.log("TRIGGERED ACTION (EVAL) IN parse_propiedades_js: "+current_action);
									}
					...
					wrapper_obj.addEventListener('change',function(event){
						// DATO : New dato
						var new_dato = JSON.parse(event.target.value);
						//console.log('new_dato '+new_dato.section_id)

						// EXEC FUNCTIONS
						trigger_functions(new_dato.section_id)
