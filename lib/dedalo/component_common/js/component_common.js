...
var component_common = new function() {
	...
	this.Save = function (component_obj, save_arguments, event) {
		...
		let js_promise = common.get_json_data(trigger_url, trigger_vars, this.save_async).then(function(response){
			...
			if (response && response.result!==false) {				
				...
				var msg = "<span class='error'>" + get_label.fail_to_save + "<br>" +response.msg+ " - " + get_label.field + " "+ label + "</span>";
				inspector.show_log_msg(msg);
				if(SHOW_DEBUG===true) {
	...
	*/
	this.select_component = function(obj_wrap) {
		...
		if(typeof(window[current_component_name].select_component)){
	...
	*/
	this.unselect_component = function(obj_wrap) {
		...
		const current_component_name = obj_wrap.dataset.component_name
		if(typeof(window[current_component_name].unselect_component)){
