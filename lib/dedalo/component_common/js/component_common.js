...
var component_common = new function() {
	...
	*/
	this.Save = function (component_obj, save_arguments) {	
		...
		if( $target_div.length != 1 ) {
			//return alert("Component Common save Error: $target_div not defined!");
		}
	...
	// UPDATE_COMPONENT_BY_PARENT_TIPO_LANG	(Alias of 'load_component_by_wrapper_id')
	...
	*/
	this.load_component_by_wrapper_id = function (wrapper_id, arguments, callback) {
		//console.log("load_component_by_wrapper_id:"+ wrapper_id)
		...
		// Wrap data modo test
		if (!wrapper_obj_data.modo) return alert("Error/Warning: wrapper_obj has no data modo");

		// VARS ON WRAPPER		

		 //id			= $(wrapper_obj).data('id_matrix'), // Optional
		var	tipo			= wrapper_obj_data.tipo,		// Required
			modo			= wrapper_obj_data.modo,
			parent			= wrapper_obj_data.parent,
			section_tipo	= wrapper_obj_data.section_tipo,
			lang			= wrapper_obj_data.lang,
			context_name	= wrapper_obj_data.context_name;
		...
		var current_tipo_section = wrapper_obj_data.current_tipo_section;	// Usado al actualizar relation list
		...
		var $target = wrapper_obj.find('.content_data').first();
			if($target.length!=1) return alert("load_component_by_wrapper_id Error: (content_data not found inside!) : " +wrapper_id);
		...
		var jsPromise = Promise.resolve(
			...
			.done(function(received_data) {
				...
				var component_name 	= wrapper_obj_data.component_name;
