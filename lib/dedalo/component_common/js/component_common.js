...
var component_common = new function() {
	...
	*/
	this.Save = function (component_obj, save_arguments, event) {
		...
		if( save_arguments && typeof save_arguments.dato!=="undefined" ) {
			...
			dato = this.common_get_dato(component_obj);
		}
			...
			if (response && response.result) {
				...
				const new_component = tempDiv.querySelector(".wrap_component");
					...
					if (!new_component) { // && component_name!=='component_portal'
						...
						// re-select wrapper
						const current_wrapper	= document.getElementById(wrapper_id)
						const parent_node		= current_wrapper.parentNode;
						if (!parent_node) {
							console.warn("[component_common.load_component_by_wrapper_id] Error. wrapper_obj.parentNode not found in received data: ", current_wrapper);
						}else{
							const js_promise_saved = new Promise(function(resolve, reject) {

								// Replace wrapper
								if ( parent_node.replaceChild(new_component, current_wrapper) ) {
									resolve("DOM updated!");
								}else{
							...
							//console.log(new_component,current_wrapper);
							//parent_node.replaceChild(new_component, current_wrapper);
							//exec_scripts_inside(new_component);
							//insertAndExecute(current_wrapper, tempDiv)
						}
	...
	*/
	this.resize_iframe = function (iframe_obj, height_adjustment) {
		...
		try {
			// Iframe container
			//var iframe_id		= 'security_access_iframe' ;
	...
	*/
	this.select_wrap = function(obj_wrap, id_wrapper) {
		...
		// fix component wrapper as selected
		this.selected_component = obj_wrap
	...
	*/
	this.select_component = function(obj_wrap) {
		...
		if (obj_wrap.dataset.modo==='edit_note') {
			return false
		}

		const current_component_name = obj_wrap.dataset.component_name
		if(current_component_name && typeof(window[current_component_name].select_component) !== 'undefined'){
	...
	*/
	this.unselect_component = function(obj_wrap) {
		...
		// blur active element (moved to lock_component)
			// const el = document.querySelector(':focus') || document.activeElement;
			// if(el) {
			// 	el.blur()
			// 	console.warn("unselect_component blured element:", el)
			// }
		...
		// Call to component custom unselect_component function
			const current_component_name = obj_wrap.dataset.component_name
			if(current_component_name && typeof(window[current_component_name].unselect_component) !== 'undefined'){
	...
	*/
	this.show_section_group = function(ar_tipo, previous_dato, call_back) {

		const self = this

		self.set_section_group_visibility(true, ar_tipo)
	...
	*/
	this.hide_section_group = function(ar_tipo, previous_dato, call_back) {
		...
		if(typeof call_back!=='undefined'){
			...
			// 	self.remove_server_dato_of_hidden_components(ar_tipo)
			// 	.then(function(){
			...
			// user confirm dialog if its necessary
				const require_confirm = ( previous_dato && previous_dato.length>0 && previous_dato[0] )
				if (require_confirm) {
					if ( !confirm( get_label.remove_hidden_data || 'Remove hidden data?' ) ) {
						call_back(false)
						return false
					}
				}
	...
	this.remove_server_dato_of_hidden_components = function(ar_tipo) {
		...
		return new Promise(function(resolve, reject){
			...
			.catch((err) => {
				...
				const msg = "ERROR[remove_server_dato_of_hidden_components] err: " + err
	...
	*/
	this.remove_client_dato_of_hidden_components = function(ar_tipo) {
		...
		if (ar_tipo && Array.isArray(ar_tipo)) {
			ar_tipo.forEach(function(current_group_tipo) {

				// let current_group = document.getElementById(current_group_tipo)
				// if(typeof current_group==='undefined' || current_group ===null) {
				// 	current_group = document.getElementById('section_tab_content_' + current_group_tipo)
				// }

				// current_group node
					const current_group = document.getElementById(current_group_tipo) ||
										  document.getElementById('section_tab_content_' + current_group_tipo)

				// ar_components nodes
					const ar_components = current_group
						? current_group.querySelectorAll('div.wrap_component')
						: []

				// reload each component found
					ar_components.forEach(function(current_component) {
						component_common.load_component_by_wrapper_id(current_component.id)
					})
			})
		}else{
			console.warn('[remove_client_dato_of_hidden_components] Ignored invalid ar_tipo:', ar_tipo, typeof ar_tipo)
		}
	...
	*/
	this.set_section_group_visibility = function(visibility, ar_tipo) {
		...
		if(typeof ar_tipo!=='object') {
			...
			if(visibility) {
				console.warn("->set_section_group_visibility: show_section_group: Error on parse ar_tipo. Current element is not an valid array object", ar_tipo)
		    } else {
				console.warn("->set_section_group_visibility: hide_section_group: Error on parse ar_tipo. Current element is not an valid array object", ar_tipo)
			}
			return false
		}
		...
		for (let i = 0; i < len; i++) {
			...
			const element = document.getElementById(tipo)
				...
				if (element) {
					...
					if(!visibility) {
						 // (!) removed 23-09-2022 to prevent infinite loop on create new record and pagination
	...
	* @return bool
	*/
	this.select_tab = function(ar_tipo, previous_dato, call_back) {
		const self = this

		const set_section_tab_visibility = function(ar_tipo){
			const element = document.getElementById('section_tab_' + ar_tipo)
			...
			if (element) {
				...
				console.warn("[component_common.select_tab] section_tab not found by id: ",'section_tab_' + ar_tipo);
			}
		...
		if(typeof call_back !== 'undefined'){
			...
			if(confirmation){
				...
				remove_dato.then(function(){
					set_section_tab_visibility(ar_tipo)
					self.remove_client_dato_of_hidden_components(ar_tipo)
					call_back(true);
				})
			}else{
	...
	* @return bool
	*/
	this.hide_ar_tabs = function(ar_tipo, previous_dato, call_back) {

		const self = this

		const set_section_tab_visibility = function(ar_tipo){

			const ar_tipo_length = ar_tipo.length;
	...
	*/
	...
	this.open_ts_window = function(button_obj) {
		...
		const url = DEDALO_LIB_BASE_URL + '/main/?' + build_url_arguments_from_vars(url_vars)
