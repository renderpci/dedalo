...
var component_common = new function() {
	...
	* SELECT WRAP
	*/
	this.select_wrap = function (obj_warp, id_wrapper) {

		if (id_wrapper) {
			obj_warp = document.getElementById(id_wrapper);
		}
		...
		if( obj_warp.classList.contains('selected_wrap')!=false) {
			return false;
		}
		...
		obj_warp.classList.add("selected_wrap");
	...
	*/
	this.reset_all_selected_wraps = function () {
		...
		for (var i = elements.length - 1; i >= 0; i--) {
			elements[i].classList.remove("selected_wrap");
		};
		/*
		$('.wrap_component').each(function() {
			$(this).removeClass('selected_wrap');
