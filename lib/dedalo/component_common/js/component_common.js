...
var component_common = new function() {
	...
	*/
	this.hide_section_group = function(ar_tipo, parent) {
		return this.set_section_group_visibility(false, ar_tipo, parent)
	...
	*/
	this.set_section_group_visibility = function(visibility, ar_tipo, parent) {
		...
		for (let i = 0; i < len; i++) {
			...
			//const ar_wrppers = self.get_all_wrappers_from_element(element)

			//	console.log("ar_wrppers:",ar_wrppers);
				if (element) {			
					if(visibility) {
	...
	this.exec_component_triggers = function(properties_js, dato, controller_data) {
		...
		for (let i = 0; i < triggers_len; i++) {
			...
			switch( true ) {
				...
				case (typeof controller_data!=="undefined" && controller_data.length > 0):
