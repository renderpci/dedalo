...
var component_common = new function() {
	...
	*/
	this.Save = function (component_obj, save_arguments, event) {
		...
		let js_promise = common.get_json_data(trigger_url, trigger_vars, this.save_async).then(function(response){
			if (SHOW_DEBUG===true) {
		...
		*/
		...
		const js_promise = common.get_json_data(trigger_url, trigger_vars).then(function(response){
			...
			if (response && response.result) {
				...
				let msg = "<span class='error'>ERROR: on Load component</span>";
				inspector.show_log_msg(msg);
	...
	this.select_tab = function(tipo) {
		...
		if (element) {
			...
			// Reset
				...
				for (let i = 0; i < ar_childrens.length; i++) {
			...
			// Hide others
				...
				//const len 			= ar_childrens.length
				for (let i = 0; i < ar_childrens.length; i++) {
					if( ar_childrens[i].classList.contains("section_tab_label") && ar_childrens[i].id!==element.id ) {
						ar_childrens[i].style.display = "none";
					}
				}
	...
	* @return bool
	*/
	this.hide_ar_tabs = function(tipo) {

		for (var i = tipo.length - 1; i >= 0; i--) {
			tipo[i]
		
			const element = document.getElementById('section_tab_' + tipo[i])	
			if (element) {
				...
				console.warn("[component_common.hide_ar_tabs] section_tab not found by id: ",'section_tab_' + tipo[i]);
			}
		}
	...
	*/
	this.exec_component_triggers = function(properties_js, dato, controller_data) {
		...
		for (let i = 0; i < triggers_len; i++) {
			...
			switch( true ) {
				...
				case (typeof controller_data!=="undefined"):
					...
					if(js_item.controller_value==='copy_dato'){
						...
						ar_controller_item.forEach(function(controller_item) {
							...
							// Trigger function
							common.execute_function_by_name(js_item.action, window, js_item.arguments)
						...
						ar_controller_item.forEach(function(controller_item) {
							...
							if (typeof(controller_item.dato[0])!=="undefined" && true===is_obj_equal(js_item.controller_value, controller_item.dato[0])){
					...
					/*
					controller_data.forEach(function(controller_item) {
						
						if(item.controller_value ==='copy_dato'){

							const copy_dato = {dato:controller_item.dato}
							item.arguments.push(copy_dato)

							// Trigger function
							common.execute_function_by_name(item.action, window, item.arguments)

							// Add to executed_items array
							executed_items.push(item)

						}else{
							console.log(controller_item.dato)
	...
	this.read_only = function(request_options){
		...
		for (let i = 0; i < len_request; i++) {
			...
			for (let j = len_ar_tipo - 1; j >= 0; j--) {
				...
				if (wrap_element) {			
					// set the warp element of the component with the read_only css (dissable the pointer click in the component)
					wrap_element.classList.add("read_only");
					//get the inputs nodes of the taget component
					const inputs_elements = wrap_element.getElementsByTagName('input')

					//loop the inputs nodes of the taget component
					const len_inputs_elements = inputs_elements.length
					for (var k = len_inputs_elements - 1; k >= 0; k--) {
						//set the current input node of the taget component to disable (the user can't change the component / disable input and selection by keyboard)
						inputs_elements[k].disabled = true;
					}
				}	
			}			
		}
	...
	* 
	*/
	this.remove_read_only = function(request_options){

		const self = this 

		if( typeof request_options !== 'object' ) {
			return console.log("->remove_read_only: Error on parse request_options. Current element is not an valid array object")
		}
		// loop of the array of the request_options
		const len_request = request_options.length
		for (let i = 0; i < len_request; i++) {
			...
			for (let j = len_ar_tipo - 1; j >= 0; j--) {
				...
				if (wrap_element) {			
					// remove of the warp element of the component the read_only css (active the pointer click in the component)
					wrap_element.classList.remove("read_only");
					//get the inputs nodes of the taget component
					const inputs_elements = wrap_element.getElementsByTagName('input')
					...
					const len_inputs_elements = inputs_elements.length
					for (var k = len_inputs_elements - 1; k >= 0; k--) {
						//remove the disable property of the current input node of the taget component (the user will can change the component / active input and selection by keyboard)
						inputs_elements[k].disabled = false;
					}
				}	
			}			
		}
