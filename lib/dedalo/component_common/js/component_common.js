	...
	*/
	this.reset_all_selected_wraps = function(exec_update_lock_components_state) {
		...
		for (let i = len - 1; i >= 0; i--) {
			...
			const unique_warning = obj_wrap.querySelector('.unique_warning')
			if(unique_warning){
				unique_warning.remove()
				const icon_prohibited = obj_wrap.querySelector('.icon_prohibited')
				icon_prohibited.classList.remove("icon_prohibited");
			}
			//Remove icon_validated
			const icon_validated = obj_wrap.querySelector('.icon_validated')
			if (icon_validated) {
				icon_validated.classList.remove("icon_validated");
			}			
		...
		for (let i = 0; i < triggers_len; i++) {
			...
			dato.forEach(function(dato_element) {
				if (true===is_obj_equal(item.value, dato_element)) {
					//alert("OK "+JSON.stringify(item.value));
					// Trigger function
