	...
	// send_data
		const send_data = async () => {
			try {
				...
				// debug
					if(SHOW_DEBUG===true) {
						console.log(`[component_common.save ${changed_data.action} ${self.context.lang}: ${changed_data.key},${changed_data.value}] api_response value:`, api_response.result.data[0].value);
					}
