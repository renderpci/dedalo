...
<body>
	...
	<?php foreach ($data->result as $section_tipo => $ar_nodes) { ?>
		<?php foreach ($ar_nodes as $key => $ts_node_obj) {	?>		
			<div><strong>Term:</strong> <?php echo $ts_node_obj->term ?> <em> [<?php echo $ts_node_obj->term_id ?>]</em> 
			<?php if (!empty($ts_node_obj->ar_childrens)) { ?>
				<a href="javascript:void(0)" onclick='load_childrens(this,"<?php echo "ts_container_".$key ?>",<?php echo json_encode($ts_node_obj->ar_childrens) ?>)'>Load childrens</a>
				<div id="<?php echo "ts_container_".$key ?>" style="padding-left:20px;">
					<!-- ajax load html here -->
				</div>
			<?php } ?>
			<?php 
			
			$ar_indexation = json_decode($ts_node_obj->indexation);
			if (!empty($ar_indexation) && count($ar_indexation)>0 ) { ?>
				<a href="javascript:void(0)" onclick='load_indexations(this,"<?php echo "index_container_".$key ?>",<?php echo json_encode($ar_indexation) ?>)'>Load indexations <?php echo count($ar_indexation) ?></a>
				<div id="<?php echo "index_container_".$key ?>" style="padding-left:20px;">
					<!-- ajax load html here -->
				</div>
			<?php } ?>
			</div>			
