...
**This API is completely isolated from the Dédalo engine.** It reads the MariaDB database(s) produced
by the diffusion process and nothing else — it imports no engine code, holds no credentials to the
work database, and can run on a different machine entirely (typically the web server's, next to the
published data). Talking to MariaDB uses Bun's native `Bun.sql` (`mariadb` adapter): no third-party
