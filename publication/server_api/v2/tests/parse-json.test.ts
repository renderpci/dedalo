...
describe('parseJsonStrings', () => {
  ...
  test('leaves prose that merely opens with a bracket alone', () => {
    // Real transcriptions start like this. They are text, not JSON candidates.
    const row = {
      id: 1,
      transcription: '[sic] the witness described the events of 1936',
      note: '{ handwritten in the margin',
    } as Record<string, unknown>;

    const result = parseJsonStrings(row);

    expect(result.transcription).toBe('[sic] the witness described the events of 1936');
    expect(result.note).toBe('{ handwritten in the margin');
  });

  test('still parses genuine JSON columns', () => {
    const row = {
      relations: '[{"section_id":1}]',
      properties: '{"lang":"lg-spa"}',
    } as Record<string, unknown>;

    const result = parseJsonStrings(row);

    expect(result.relations).toEqual([{ section_id: 1 }]);
    expect(result.properties).toEqual({ lang: 'lg-spa' });
  });
});